...
function main(): void {
  ...
  if (!options.output) fail("--output is required so the signed receipt is never printed to the terminal");
  if (existsSync(options.output)) fail("--output must name a new file");
  ...
  if (packageJson.name !== PACKAGE_NAME) fail(`the release commit package.json must declare ${PACKAGE_NAME}`);
  ...
  if (packageJson.publishConfig?.registry !== PUBLIC_NPM_REGISTRY) {
    fail("the release commit must target the public npm registry");
  }
  ...
  const payload: NpmReleaseAgentReviewPayload = {
    ...
    tag: `${TAG_PREFIX}${packageJson.version}`,
  ...
  writeFileSync(options.output, JSON.stringify(receipt), { encoding: "utf8", flag: "wx", mode: 0o600 });
  console.log("Signed npm release agent-review receipt written to the requested mode-0600 file.");
}
