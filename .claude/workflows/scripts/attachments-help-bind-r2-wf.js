export const meta = {
  name: 'attachments-help-bind-r2',
  description: 'Cycle-2 remediation for row 970d7c6f (PR 766): the land phase correctly refused the merge on the base-movement gate (review GO at c21131933 vs origin/main advanced to 054b69e5 wave #764 — UNREVIEWED AT HEAD). This lane executes the recorded resume condition: rebase fix/970d7c6f-attachments-serve-help-before-bind onto CURRENT origin/main (4bc89b39 at r1 end, re-measure at run time), re-run the gates at the new head, fresh [REVIEW] verdict naming CURRENT origin/main, base gate + squash merge + complete the row.',
  phases: [
...
const ROW = '970d7c6f'
const PR = '766'
const OLD_HEAD = '8658ebefe8b3fab31ec53c43b8b05d2d2c601d15'

const CONST = `
You are the attachments-help-bind-r2 remediation lane (row ${ROW}; owner-authorized via the task-drain queue). Final text = machine-readable JSON.

Context: the attachments-help-bind r1 lane (wf_a58e0db3-29e) rebased the fix to 8658ebef against origin/main 0a462730, re-ran all gates green (suite 61/61, probes, frozen install, secrets), review-2 returned GO at 8658ebef — but the land phase AGAIN correctly refused: origin/main advanced 0a462730 -> 4bc89b39 (PRs #774 agency, #768 testers, #767 notes) during the lane, merge-tree(4bc89b39, 8658ebef) != head tree, UNREVIEWED AT HEAD (PR comment 5369333356, board 720549). Zero overlap with the PR's 3 files. This lane executes that recorded resume condition — one further remediation step in the SAME review lineage, NOT a new candidate.
...
- REMEDIATE (in the lane worktree ~/.hasna/repos/worktrees/apps/attachments-help-bind-r2 cut from CURRENT origin/main): rebase fix/970d7c6f-attachments-serve-help-before-bind onto CURRENT origin/main; resolve mechanical conflicts only within the PR's own files (the PR is 3 files, delta confined to apps/attachments); push the rebased head with force-with-lease ONLY after verifying the remote head still equals ${OLD_HEAD}. Do NOT add new content beyond the rebase.
...
- REVIEW-2 (one Fable adversarial reviewer, SAME lens — attachments-help-bind-review): re-review ONLY the rebase result and its direct regressions: (1) the rebased diff is the same 3-file attachments fix, nothing else added; (2) attachments suite 61/61 green at the new head (literal); (3) the two-sided probes pass (literal); (4) base-movement gate vs CURRENT origin/main — merge-tree == head tree (literal); (5) no other-lane interference. Do NOT relitigate the GO'd fix shape (help-before-pool startup order) and do NOT discover new issues outside the rebase. Post '[REVIEW] <GO|NO_GO> — attachments-help-bind-r2 @ <sha> — lens: rebase result of GO'd fix, reviewer attachments-help-bind-review' to #board.
...
const REVIEW2 = CONST + `
ROLE: cycle re-reviewer (Fable, SAME lens — attachments-help-bind-review). Re-review ONLY the rebase result and its direct regressions (per CONST): (1) diff = the same 3-file attachments fix, (2) suite 61/61 green at new head (literal), (3) probes pass (literal), (4) base-movement gate vs CURRENT origin/main (merge-tree == head tree), (5) no other-lane interference. Do NOT relitigate the GO'd fix shape or discover unrelated issues. Post '[REVIEW] <GO|NO_GO> — attachments-help-bind-r2 @ <sha> — lens: rebase result of GO'd fix, reviewer attachments-help-bind-review' to #board. Return (JSON): { verdict: 'GO'|'NO_GO', findings: [{severity, title, detail}] }
`
