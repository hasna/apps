export const meta = {
  name: 'browser-publish-guard-r1',
  description: 'Row 0cbbd621 cycle-1 remediation: hasna/apps publish-guard gate red at EVERY PR head and on main — apps/browser prepack `bun run build` exits 2 with TS2307 Cannot find module @hasna/conversations at src/lib/coordination.ts(13,30). Lane: reproduce at CURRENT main (TDD red), root fix in apps/browser deps/imports, publish-guard green, one Fable review, PR, merge, complete the row.',
  phases: [
...
const CONST = `
You are the browser-publish-guard-r1 lane (row ${ROW}; owner-authorized via the task-drain queue). Final text = machine-readable JSON.

Context (cycle-1 remediation, 2026-08-21): the base lane fixed the browser TS2307 via the import-specifier correction (PR 771, head 139825ff — variable-bound dynamic import specifier, probe-verified at tsc 5.9.3; browser slice publish-guard PASSED). The cycle-1 Fable review returned NO_GO with two P1s: (1) publish-guard job red at the head on NON-browser residual classes only (recordings version:check, @hasna/contracts@0.13.2 unpublished for todos/machines/contracts, gates self-test — all identical at CURRENT origin/main); (2) head stale vs CURRENT origin/main (base 054b69e5 -> 4bc89b39+) — no CI on the merge result. P2: the browser fix itself is correct and fully verified. This lane remediates the named P1s: rebase PR 771 onto CURRENT origin/main, re-run the gates at the new head (classify residual reds against CURRENT main state — the contracts 0.13.2 class resolves when publish-all publishes; recordings class owned b335a922), fresh same-lens review at the new head, land.
...
- ${MONOREPO} is READ/context only. Sync first (git -C ${MONOREPO} fetch origin main -q; never discard local work). Resolve CURRENT origin/main from FETCH_HEAD and verify FETCH_HEAD == gh api repos/hasna/apps/commits/heads/main --jq .sha. File mutation in YOUR OWN task worktree ~/.hasna/repos/worktrees/apps/browser-publish-guard cut from CURRENT origin/main. NEW BRANCH fix/browser-publish-guard. PR-first; never push to main. Commits end with 'Agent: browser-publish-guard-r1-<role>' (the ONLY attribution line; never Co-Authored-By). Commit identity MUST be the canonical fleet identity (Andrei Hasna <andrei@hasna.com>).
