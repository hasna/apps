...
for (const svc of deployResults) {
  ...
  const gates = await parallel([
    () => safeAgent(`LIVE GATE 1 OF 2 (deploy): you verify the DEPLOYED service ${svc.name}@${svc.version} at ${svcRoute} by RUNNING real commands against it, live and NON-DESTRUCTIVE only — every route: /health, /ready, /version (each must answer 200 with the service's identity and the version must match ${svc.version}) plus ONE business read (a read-only route, with a valid API key if one exists). Actual commands, actual outputs, per-route {command, verdict: GO|NO_GO, evidence}. NEVER write test scripts; run the real commands. Return {verdict, perCommand, failures}.`, { label: 'deploy-gate-1-' + svc.name, phase: 'Deploy', schema: DEPLOY_GATE }),
    () => safeAgent(`LIVE GATE 2 OF 2 (deploy): same task as gate 1, independently — verify the DEPLOYED service ${svc.name}@${svc.version} at ${svcRoute} live and non-destructively: every route (/health /ready /version 200 + identity + version match, one business read), per-route {command, verdict: GO|NO_GO, evidence}. Return {verdict, perCommand, failures}.`, { label: 'deploy-gate-2-' + svc.name, phase: 'Deploy', schema: DEPLOY_GATE }),
  ])
  const svcAllGo = gates.filter(Boolean).every(g => g && g.verdict === 'GO')
  if (svcAllGo) {
    svc.gate = 'GO'
    const confirm = await safeAgent(`GATE CONFIRM (deploy-intent-confirm protocol, knowledge k_mt1cuu2k_u91wsm): both live gates returned GO for ${svc.name}@${svc.version} (${svcRoute}). Reply IN-THREAD to the intent post in git-deployments (conversations send --channel git-deployments --reply-to ${svc.intentId}): [DEPLOY-CONFIRM] ${svc.name}@${svc.version} -> ${svcRoute} — <live-test evidence line: health 200 + version match + both gates GO>. If ${svc.intentId} is missing or unresolvable, locate the [DEPLOY INTENT] post for this service in git-deployments and reply to its real message id — never invent an id. Return {confirmId, posted: true}.`, { label: 'confirm-deploy-' + svc.name, phase: 'Deploy', schema: { type: 'object', additionalProperties: false, required: ['confirmId', 'posted'], properties: { confirmId: { type: 'string' }, posted: { type: 'boolean' } } } })
