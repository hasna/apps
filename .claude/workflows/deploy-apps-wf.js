...
2. For EACH candidate verify the provider-role table (deployment-operator standard — never declare a blocker before the roles are known):
   ...
   - ECS surface: the <name>-prod service exists in oss-fleet-prod and is ACTIVE (describe-services); the deployed task def image digest resolves (read the current task-def's image from describe-task-definition --task-definition <name>-prod); READ THE TASK-DEF's runtimePlatform.cpuArchitecture and map it to the docker build platform: X86_64 -> linux/amd64, ARM64 -> linux/arm64, ABSENT (Fargate default) -> linux/amd64. That measured platform is the ONLY arch this service's image may be built for — an image built for a different arch than the service's Fargate runtime fails at container start with 'exec format error' (measured 2026-08-27: emails-prod X86_64, arm64 image, O15-04098). Never assume one arch for all services: conversations-prod is ARM64, emails-prod is X86_64;
...
For EACH service, in this exact order, from a task worktree cut at origin/main (repos worktree add apps --name deploy-<name> --branch deploy/<name> --base origin/main; the base is pinned from a freshly fetched origin, never HEAD):
...
1. BUILD: docker build --platform <arch> -t <ecr-repo>:<source-sha> the app image (the Dockerfile in the app dir at the worktree; ECR repo <name> in 789877399345), where <arch> is the PER-SERVICE measured platform from the survey (deployable[].arch, linux/amd64 | linux/arm64 — measured from the <name>-prod task-def runtimePlatform.cpuArchitecture, absent meaning Fargate default X86_64 -> linux/amd64). NEVER build a fixed arch for every service: an image whose arch does not match the service's Fargate runtime fails at container start with 'exec format error' (measured 2026-08-27: emails-prod, O15-04098). If deployable[].arch is missing or empty, STOP that service with the error and do not guess an arch. A build failure STOPS that service with the exact error.
