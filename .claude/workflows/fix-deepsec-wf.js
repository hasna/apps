...
Cloud credentials (LIVE disk tier: ~/.hasna/<app>/config/credentials — owner-only 0600, holding HASNA_<APP>_API_KEY / HASNA_<APP>_API_URL; the deprecated ~/.hasna/fleet-env/*.env and retired ~/.hasna/cloud/*.env are never sourced): for f in todos conversations mementos knowledge; do c="$HOME/.hasna/$f/config/credentials"; k="HASNA_$(printf %s "$f" | tr a-z A-Z)_API_KEY"; if [ -r "$c" ]; then set -a; . "$c"; set +a; elif printenv "$k" >/dev/null 2>&1; then :; else echo "FATAL: no credential for $f — consulted env $k and $c; ~/.hasna/fleet-env and ~/.hasna/cloud are retired and never read; provision $c (0600) or use the secrets vault" >&2; exit 1; fi; done. NEVER print a credential value.`
...
1. 7-day pin vs excludes: read ~/.bunfig.toml — minimumReleaseAge=604800 must be present (never lower it) and minimumReleaseAgeExcludes (exact names only; scope wildcards NOT honoured). For EVERY dependency declared in every app package.json under ${PATHS} at origin/main that is NOT exempt per the CURRENT exclude list (@hasna/*, @hasnaxyz/*, @hasnastudio/*, @hasnatools/*, @hasnafamily/*, openai, @openai/*, @anthropic-ai/*, anthropic — verify against the file, never from memory): read the DECLARED SPECIFIER for that name from the same app manifest, resolve the admitted set the specifier actually allows with 'npm view <pkg>@<spec> version --json' (a range yields an array of admitted versions, an exact spec yields a bare string — normalize to the array; a non-admitting result = empty admitted set), get the publish times (npm view <pkg> time --json), and flag = PIN VIOLATION (P1) ONLY when a version admitted by the declared specifier was published < 7 days ago AND the exact name is absent from the excludes. A window-fresh version the declared specifier does NOT admit is NEVER a PIN VIOLATION — verify admission before flagging (dep-testers-2 was false: the declared '^0.0.268' admits only the 0.0.x line, never the 0.2.7 publish that tripped the unqualified check; ^0.0.x admits exactly that 0.0.x version).
...
CHECKS (each mandatory):
1. Staged/diff scan: inside ${REPO} run 'secrets scan workspace .' (redirect stdout+stderr to files, never a pipe) — a clean result requires rc=0 AND bytesScanned > 0 (rc=0 with 0 bytes scanned is a vacuous pass; report it as ran=false with the exact line); quote the exact rc/bytesScanned/filesScanned line.
