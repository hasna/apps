...
function runPrompt(row, prep) {
  return 'RUN phase — execute the QA suite at the prepared box (boxId ' + prep.boxId + ', kind ' + prep.boxKind + ', appDir ' + prep.appDir + '). Start each line by capturing raw output TO FILES; never report a run from memory.\n\n'
    ...
    + '2. APP SUITES: for EVERY suite in the manifest ' + JSON.stringify(prep.suites) + ', run the real command (cd <appDir>; <invocation>) and tee the raw output to a file: sandboxes exec <id> bash -c "cd <appDir> && <invocation> 2>&1 | tee /tmp/logs/<suite>.log" (that /tmp is INSIDE the credential-zero box and dies with it — it is NOT station scratch; station scratch is $HOME/Workspace/scratch/verify-apps-qa/, never /tmp, never an app home). Record per suite the exact command, the exit code, and outcome (pass = exit 0 AND the suite\'s success markers in the log; fail = exit non-zero or expected-marker absence; error = the suite did not run or the box failed).\n'
