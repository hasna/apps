...
1. \`todos list --project ${APPS} --status pending --json\` (redirect to a file, never pipe). Select rows whose title starts with "BUG" OR rows whose title starts with "RELEASE UNVERIFIED:", "SHIP UNVERIFIED:", "DEPLOY UNVERIFIED:" (the live-gate NO_GO rows filed by the publish/deploy/ship lanes) OR "RELEASE CONFIRM MISSING:" (the gate-record rows filed by the publish lane when both gates returned GO but the [PUBLISH-CONFIRM] agent failed twice — O15-04231; the confirm was never posted and the app is already current on the registry, so ONLY this lane can retry it) — BOTH classes are drain candidates; a gate row is never drainable by the fix-lane alone. For each candidate record kind: "bug" for BUG rows, "unverified" for the UNVERIFIED gate rows, "confirm-missing" for the RELEASE CONFIRM MISSING rows.
...
1. Parse the artifact from the title/description: gate class (RELEASE | SHIP | DEPLOY), the package/service@version (or wave #N), and the gate evidence recorded in the description. For a RELEASE CONFIRM MISSING row the two gates already returned GO at publish time — the missing piece is the in-thread [PUBLISH-CONFIRM]; re-verification confirms the artifact is still good, then posts the missing confirm.
