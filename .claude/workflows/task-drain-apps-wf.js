...
4. ANY NO_GO: comment the row with the exact per-command/per-route failing evidence and the verdict — "NO_GO at head — <artifact> still fails the gate". Route the underlying defect to ONE deduped BUG row: check for an existing row for this exact defect class (todos list --project 3bbc22e0 --status pending --json, redirect to a file, never pipe; and gh pr list --repo hasna/apps --state open) — reuse it if it exists (add the evidence as a comment), otherwise file ONE BUG row in project 3bbc22e0 carrying the exact failing evidence. Comment the gate row: "NO_GO ROUTED — fix routed to <BUG shortId or id>". Complete the gate row with that verdict (fail-closed preserved: no confirm was ever posted; the routed BUG row carries the repair). Outcome 'skipped' with reason naming the routed row.
...
Return the schema.`, { label: 'exec-gate:' + row.shortId, phase: 'Execute', schema: EXEC_SCHEMA })
    ...
    return safeAgent(`Execute ONE hasna/apps BUG row via the fix-lane discipline. Row: ${JSON.stringify(row)}. You are ONE OF ${Math.min(MAX_CONCURRENT, wave.length)} CONCURRENT fix agents — each works its OWN row in its OWN worktree; never touch another agent's worktree, never the shared checkout.
...
NEVER publish to npm (publish-all owns publishing).`, { label: 'exec-row:' + row.shortId, phase: 'Execute', schema: EXEC_SCHEMA })
  ...
  log('task-drain-apps: pass ' + pass + ' done — ' + execs.length + ' rows executed, ' + (census ? census.queueSize : 0) + ' unowned BUG/UNVERIFIED rows remain — next pass re-censuses (infinite loop)')
}
