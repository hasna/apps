export const meta = {
  ...
  phases: [
    ...
    { title: 'Release', detail: 'waves of 4 lanes, one app per lane: adversarial release review -> intent -> publish -> two-sided verify; then a 2-agent live gate (publish-gate-1/publish-gate-2, both must return GO) before the gated [PUBLISH-CONFIRM] -> live install + CLI smoke' },
...
- The release review is the ADVERSARIAL GATE (the npm-release rule: an independent agent verdict bound to repo+sha+package+version+registry): dispatch the repo's READ-ONLY adversarial reviewer agent (.claude/agents/reviewer.md) and never publish without its GO. A failed/timed-out review run retried once, then SKIP the app (never publish unreviewed).
...
const RELEASE = CONST + `
RELEASE-GATE NOTE 2026-08-20T09:2xZ: the prior pass skipped ${'${APP}'} on TWO gate failures (a usage-limit pre-reset, then a provider model-capacity error). GATE RULE (standing): publish ONLY on a [REVIEW] GO bound to the reviewed sha; a failed/timed-out review is retried once, then the app is SKIPPED — never publish unreviewed. Do NOT replay the prior skips.
ROLE: release lane for ${'${APP}'} (repo ${'${REPO_VER}'} vs registry ${'${REG_VER}'}, bins ${'${BINS}'}). You own THIS ONE APP. Do:
...
2. Re-verify at lane start: the repo version is still ahead of the registry (negative control: npm view @hasna/${'${TSHORT}'} version must NOT already show the repo version — if it does, someone else published: STOP, record, and return skipped with 'already-published'). If a changeset is pending for the app ('bunx changeset status' or the .changeset files), run 'bunx changeset version' in a worktree, commit the version+changelog changes ('Agent: publish-all-${'${TSHORT}'}'), push via a PR (title 'release(${'${APP}'}): version <v>'), and let it merge BEFORE publishing (the PR needs a review — post the release review on that PR instead; the review covers the release candidate).
3. RELEASE REVIEW (the adversarial gate): dispatch the repo's READ-ONLY adversarial reviewer agent (.claude/agents/reviewer.md) with the release brief. Write release-brief-${'${TSHORT}'}.md: 'Adversarially review the release candidate for @hasna/${'${TSHORT}'}@<version>: repo hasna/apps, head <sha>, the diff since the last published version (git log + git diff), repo laws (AGENTS.md + .claude/rules), the npm release rule (independent agent verdict). Check: secrets/internal-infra strings in the packed content, the changelog accuracy, the version bump correctness, regression risk. FIRST LINE exactly: [REVIEW] GO|NO_GO — @hasna/${'${TSHORT}'}@<version> @ <sha> — registry npmjs. Then ONLY concrete P0/P1 blocking findings.' Capture the reviewer's report to release-review-${'${TSHORT}'}.md (bounded 45 min; on timeout RETRY ONCE; second failure = SKIP — never publish unreviewed). NO_GO: remediate ONLY the named P0/P1 findings (via a PR), re-review — bounded 2 cycles; third NO_GO: SKIP with the findings recorded.
...
Categories:
- SKILLS: repeated procedures worth a skill (the per-app release-with-adversarial-review + live-install recipe — this is now a repeatable pattern)?
