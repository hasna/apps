...
4. FINDINGS: for each hit, record {commit (the introducing commit sha when determinable, else null), file, line, detector (the secrets detector name or the marker class), marker (the matched marker class, never the matched value)}. EVIDENCE RULE — binding: NEVER paste a credential value, a matched marker value, or any secret into the findings or your output. The detector/marker name + file + line IS the evidence. If a scan output would carry values, the scan tool redacts them by default — never render a value to your transcript. DEDUPE the findings array within this result: one finding per (repo, commit sha, detector) — if the same detector fires at the same commit in multiple files, keep ONE finding carrying the first file:line and note the extra count in a comment on that finding.
...
1. todos list --project 3bbc22e0-205f-4e3d-8c5a-d8ce8e99afd8 --status pending --json --limit 5000 > <scratch>/leak-dedupe-rows.json   (redirect to a file; NEVER pipe). <scratch> is the absolute directory ONE 'scratchpad path' call prints — make that call once, then use ordinary file I/O there. Scratch NEVER goes in /tmp; if the scratchpad CLI is not installed on this station, STOP and report the missing CLI rather than substituting /tmp.
