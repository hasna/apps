...
Return the census: yielded, specPresent, sweep (the sweep you are working), sweepStarted, population (all names read), populationComplete, reposTotal, remainingThisSweep, idle, queue (each with repo, mode, cursor). Read-only: never open, close, comment, file, or clone anything in this phase.`), { label: 'leak-census:' + pass, phase: 'Census', schema: CENSUS, model: 'sonnet' })
