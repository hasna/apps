...
3. DEDUPE: skip any PR already handled — check for an existing comment with the [WRONG-CLOSE] marker or an existing row whose title starts with "WRONG-CLOSE <klass>: <repo>#<prNumber>" (PREFIX match — the separator after the PR number can arrive in different forms across transport; never match byte-exact titles) before filing/commenting. NEVER re-comment or re-file a handled PR, and NEVER delete, cancel, or update any row or comment — this lane files and comments ONLY; a duplicate is skipped, never removed.
