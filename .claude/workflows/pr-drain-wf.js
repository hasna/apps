...
- No secrets: never print/capture/commit credential values; consume ONLY via 'secrets exec <key> --as VAR -- <cmd>'. Staged secrets scan (redirect + 'secrets scan input', rc 0 clean) before every commit/push. No internal-infra strings in artifacts. Capture path: redirect to files, never pipe large reads. Paste literal output lines. NEVER run bash -x / set -x (trace mode) in this environment — credential files are sourced here (~/.hasna/<app>/config/credentials, the live disk tier; the deprecated ~/.hasna/fleet-env/*.env and retired ~/.hasna/cloud/*.env are never sourced) and trace echoed the sourced KEY=value lines into a transcript (measured: pr-drain-census, #incidents 736502, 2026-08-25).
