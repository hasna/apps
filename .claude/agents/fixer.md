...
1. **Worktree only.** Create the task worktree with the sanctioned verb:
   ...
   (never `git worktree add` by hand), or check out the existing PR branch when
   fixing review findings. Never edit the shared checkout; never push `main` —
   the one bootstrap commit to main is already done and everything after is
   PR-first.
