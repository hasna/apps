...
if [ "$OBSERVED_TASK_DEFINITION" != "$CANDIDATE_TASK_DEFINITION" ] || [ "$PRIMARY_COUNT" != "1" ]; then
  jq -n \
    ...
    '{schema:"hasna.files.deployment_reconciliation_required.v1",status:"RECONCILIATION_REQUIRED",reason:"concurrent_service_change_before_rollback",source_sha:$source_sha,cluster:$cluster,service:$service,candidate:{task_definition:$candidate_task_definition,image:$candidate_image},previous:{task_definition:$previous_task_definition,image:$previous_image},observed:{task_definition:$observed_task_definition,deployments:$deployments},automatic_rollback_performed:false}' \
    > "$RECONCILIATION_OUT"
...
jq -n \
  ...
  --arg previous_task_definition "$PREVIOUS_TASK_DEFINITION" \
  --arg previous_image "$PREVIOUS_IMAGE" \
  '{schema:"hasna.files.rollback_receipt.v1",source_sha:$source_sha,cluster:$cluster,service:$service,candidate:{task_definition:$candidate_task_definition,image:$candidate_image},restored:{task_definition:$previous_task_definition,image:$previous_image},automatic_rollback_performed:true,cas_anchor_verified:true}' \
  > "$ROLLBACK_OUT"
