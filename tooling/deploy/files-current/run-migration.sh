...
capture_observed() {
  ...
  if [ "$describe_status" -eq 0 ] && jq -e '.tasks | length == 1' <<<"$described" >/dev/null 2>&1; then
    ...
    jq -n \
      ...
      --arg candidate_image "$CANDIDATE_IMAGE" \
      --arg task_arn "$TASK_ARN" \
      '{schema:"hasna.files.migration_observed_state.v1",source_sha:$source_sha,capture_reason:$reason,command_status:$command_status,requested:{task_definition:$requested_task_definition,image:$candidate_image},task_arn:$task_arn,describe_available:false,observed:null}' > "$OBSERVED_OUT"
  fi
...
RUN_RESULT="$(aws ecs run-task \
  ...
  --launch-type FARGATE \
  --count 1 \
  --started-by "gha-files-migrate-${GITHUB_RUN_ID:-local}" \
...
jq -n \
  ...
  --arg candidate_image "$CANDIDATE_IMAGE" \
  '{schema:"hasna.files.migration_launch.v1",source_sha:$source_sha,task_arn:$task_arn,requested_task_definition:$requested_task_definition,accepted_task_definition:$accepted_task_definition,candidate_image:$candidate_image}' > "$LAUNCH_OUT"
