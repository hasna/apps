...
jq -e '.schema == "hasna.files.migration_catalog.v1"
  ...
  and ([.migrations[].id] | length == (unique | length))' "$CATALOG" >/dev/null
CATALOG_ROWS="$(jq -c '.migrations | map({id,checksum})' "$CATALOG")"
...
PROBE_JS='const{Client}=require("pg");const{createHash}=require("node:crypto");const c=new Client({connectionString:process.env.HASNA_FILES_DATABASE_URL});await c.connect();const q=await c.query("SELECT id, checksum FROM schema_migrations ORDER BY id ASC");const rows=q.rows.map(r=>({id:String(r.id),checksum:String(r.checksum)}));const digest="sha256:"+createHash("sha256").update(JSON.stringify(rows)).digest("hex");console.log(JSON.stringify({schema:"hasna.files.migration_ledger_probe.v1",phase:process.env.FILES_LEDGER_PHASE,rows,ledger_digest:digest}));await c.end();'
OVERRIDES="$(jq -cn --arg c "$CONTAINER" --arg phase "$PHASE" --arg script "$PROBE_JS" '{containerOverrides:[{name:$c,command:["bun","-e",$script],environment:[{name:"FILES_LEDGER_PHASE",value:$phase}]}]}')"
RUN="$(aws ecs run-task \
  --cluster "$CLUSTER" \
  --task-definition "$TASK_DEFINITION" \
  --launch-type FARGATE \
  --count 1 \
  --started-by "gha-files-ledger-${PHASE}-${GITHUB_RUN_ID:-local}" \
...
for _ in $(seq 1 20); do
  ...
  PROBE="$(jq -c --arg phase "$PHASE" '[.[] | fromjson? | select(.schema == "hasna.files.migration_ledger_probe.v1" and .phase == $phase)] | if length == 1 then .[0] else empty end' <<<"$MESSAGES")"
  [ -n "$PROBE" ] && break
  sleep 3
done
...
# Every applied row must be known by this exact candidate and retain its exact checksum.
jq -e --argjson applied "$APPLIED" '
  (.migrations | map({key:.id,value:.checksum}) | from_entries) as $known
  | ($applied | all(.[]; ($known[.id] // null) == .checksum))
...
jq -n \
  ...
  '{schema:"hasna.files.migration_ledger_receipt.v1",phase:$phase,source_sha:$source_sha,probe:{task_arn:$task_arn,task_definition:$task_definition,image:$image,image_digest:$image_digest},catalog_digest:$catalog_digest,ledger_digest:$ledger_digest,applied:$applied,pending:$pending}' > "$OUT"
chmod 600 "$OUT"
