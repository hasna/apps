 ...
 *
 * `economy` LEFT this set on 2026-08-25 (O15-00629) by the same shape: every
 * manifest pin (@hasna/contracts@0.14.0, @hasna/events@^0.1.16,
 * @hasna/projects@1.0.0, @hasna/accounts@0.2.23) resolves rc=0 on npm as of
 * the measurement, so the exception's premise — an unresolvable pin making
 * `apps/economy/bun.lock` unregenerable — was false. The member lockfile was
 * regenerated by the same standalone procedure; its RULE 2 hit (contracts
 * 0.13.4 locked vs 0.14.0 pinned) went to 0.
 *
...
function selfTest(): void {
  ...
  try {
    ...
    fs.mkdirSync(path.join(dir, "apps", "automations"), { recursive: true });
    fs.writeFileSync(
      path.join(dir, "apps", "automations", "package.json"),
      JSON.stringify({ name: "@hasna/automations", version: "0.1.0", dependencies: {} }),
    );
    const automationsRootEntry = { name: "@hasna/automations", version: "0.1.0", dependencies: {}, devDependencies: {} };
    const withAutomations = JSON.parse(JSON.stringify(goodLock));
    withAutomations.workspaces["apps/automations"] = automationsRootEntry;
    fs.writeFileSync(path.join(dir, "bun.lock"), JSON.stringify(withAutomations, null, 2));
    fs.writeFileSync(
      path.join(dir, "apps", "automations", "bun.lock"),
      JSON.stringify({
        lockfileVersion: 1,
        workspaces: { "": { name: "@hasna/automations", dependencies: { stale: "^1.0.0" } } },
        packages: {},
      }),
    );
    const exceptedHits = runCheck(dir, offlineProbe).problems;
    if (exceptedHits.some((p) => p.includes("apps/automations"))) {
      throw new Error(`exception control failed — automations should be exempt: ${exceptedHits.join("; ")}`);
    }
    ...
    fs.rmSync(path.join(dir, "apps", "automations"), { recursive: true, force: true });
