...
`,{mode:0o700});
    const result=spawnSync("bash",[script, waiting?"https://api.hasna.com/switcher":"https://switcher.hasna.xyz","0.2.5",...(waiting?["--wait-for-route"]:[])], {
      encoding:"utf8",timeout:8000,env:{...process.env,PATH:`${dir}:${process.env.PATH}`,SWITCHER_PUBLIC_VERIFY_TIMEOUT_SECONDS:timeout,SWITCHER_PUBLIC_VERIFY_DELAY_SECONDS:"0"},
    });
...
test("accepts directly ready origin only after exact version and auth denial",()=>{
  ...
  expect(r.requests).toBe("https://switcher.hasna.xyz/ready\nhttps://switcher.hasna.xyz/version\nhttps://switcher.hasna.xyz/v1/providers\n");
});
...
for(const body of [{...ready,version:"0.2.4"},{...ready,backend:"sqlite"},{...ready,status:"ok"},[],{}]) test(`rejects wrong readiness ${JSON.stringify(body)}`,()=>{
...
test("workflow waits only after exact ECS and direct proof, and keeps rollback",()=>{
  const w=Bun.YAML.parse(readFileSync(join(root,".github/workflows/deploy-switcher.yml"),"utf8"));
...
for (const [name,responses] of [
  ...
  ["version", [accepted[0],[200,{raw:'{"version":"0.2.4"}\n{"version":"0.2.5"}'}],accepted[2]]],
