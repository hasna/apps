...
const workflow = Bun.YAML.parse(readFileSync(join(root, ".github/workflows/deploy-switcher.yml"), "utf8")) as { jobs: { deploy: { steps: { name?: string; run?: string }[] } } };
const step = workflow.jobs.deploy.steps.find(item => item.name === "Verify staged bootstrap image provenance and scan")!;
const source = "a".repeat(40);
const digest = `sha256:${"b".repeat(64)}`;
const ecr = "789877399345.dkr.ecr.us-east-1.amazonaws.com/switcher";

function runScan(responses: string[], tags = [`deploy-${source}-bootstrap`]) {
  const scratch = mkdtempSync(join(tmpdir(), "switcher-bootstrap-scan-"));
  try {
...
set -euo pipefail
case "$1 $2" in
  'ecr describe-images') cat "$SWITCHER_SCAN_FIXTURE/tags.json" ;;
  'ecr describe-image-scan-findings')
    count=0
    if [[ -f "$SWITCHER_SCAN_FIXTURE/count" ]]; then count="$(cat "$SWITCHER_SCAN_FIXTURE/count")"; fi
    printf '%s' "$((count + 1))" > "$SWITCHER_SCAN_FIXTURE/count"
    cat "$SWITCHER_SCAN_FIXTURE/$count.json"
    ;;
  *) exit 44 ;;
esac
`, { mode: 0o700 });
    ...
    const result = spawnSync("bash", ["-c", step.run!], {
      ...
      env: { ...process.env, PATH: `${scratch}:${process.env.PATH}`, SWITCHER_SCAN_FIXTURE: scratch,
        SOURCE_SHA: source, PREVIOUS_IMAGE: `${ecr}@${digest}`, ECR_URL: ecr,
        EXPECTED_ECR_REPOSITORY: "switcher", GITHUB_OUTPUT: output },
    });
