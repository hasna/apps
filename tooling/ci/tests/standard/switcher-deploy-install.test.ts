...
const workflow = Bun.YAML.parse(readFileSync(join(root, ".github/workflows/deploy-switcher.yml"), "utf8")) as { jobs: { deploy: { steps: Step[] } } };

test("Switcher deployment installs without unordered workspace hooks before building", () => {
  const steps = workflow.jobs.deploy.steps;
  const buildIndex = steps.findIndex(step => step.name === "Build the Switcher distribution");
  const step = steps[buildIndex];
  expect(step?.run).toBeString();
  const scratch = mkdtempSync(join(tmpdir(), "switcher-deploy-install-"));
  try {
...
`, { mode: 0o700 });
    ...
    const result = spawnSync("bash", ["-c", step!.run!], {
      cwd: scratch,
      env: { ...process.env, PATH: `${scratch}:${process.env.PATH}`, SWITCHER_INSTALL_TRACE: trace },
