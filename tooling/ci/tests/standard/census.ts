...
export const SERVE_EXCEPTIONS: Array<{ member: string; reason: string }> = [
  ...
  { member: "sandboxes", reason: "CLI/MCP-shaped member (sandboxes CLI + MCP server); no HTTP serve bin. Imported 2026-08-15; aggregate task (todos ee9fbb4d)." },
...
export const SDK_EXCEPTIONS: Array<{ member: string; reason: string }> = [
  ...
  { member: "sandboxes", reason: "SDK lane (c7ce8b75); no ./sdk export yet (manifest declares an sdk serviceSurface, package.json exports ./adapters/./managed/./postgres). Imported 2026-08-15." },
...
export const CONTRACTS_EXCEPTIONS: Array<{ member: string; cause: string; task: string }> = [
  ...
  {
    member: "sandboxes",
    cause: "surface_bindings (serviceSurfaces[0].exportSubpath targets missing dist files) + storage_capabilities (missing storage engines sqlite/postgres; storage.envPrefix and storage.pgTestGate required for the PostgreSQL DATABASE_URL contract). Validated at kitVersion 0.8.5 (no pinned dep). Imported 2026-08-15.",
    task: "todos ee9fbb4d (import row — sandboxes; modernize the manifest: dist subpath + storage engines/envPrefix/pgTestGate)",
  },
  {
    member: "prompts",
