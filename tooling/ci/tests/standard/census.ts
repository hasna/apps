...
export const SERVE_EXCEPTIONS: Array<{ member: string; reason: string }> = [
  ...
  { member: "bench", reason: "CLI/MCP-shaped member (bench CLI + MCP server); no HTTP serve bin. Imported by #169; aggregate task (todos ee9fbb4d)." },
...
export const MANIFEST_MISSING_EXCEPTIONS: Array<{ member: string; reason: string }> = [
  ...
  { member: "bench", reason: "No hasna.contract.json; imported by #169 after the original census; manifest lane (todos 41208cbe)." },
];
