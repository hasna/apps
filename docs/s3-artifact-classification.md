...
| 9 | attachments | B (live) | source moved to `hasna-internal/internal-apps` on 2026-09-17; production S3 uses keys `attachments/<yyyy-MM-dd>/<id>/<filename>`, versioning off, no lifecycle, grandfathered bucket | keep bucket (grandfathered), align layout + versioning + lifecycle | bucket ops in infra-live #41; no kit needed (non-versioned bytes) | #1650 |
...
| 20 | shortlinks | C | source moved to `hasna-internal/internal-apps` on 2026-09-17; rows only | — | — | — |
