...
export function validateNpmReleaseAgentReviewReceipt(
  ...
  addIf(failures, payload.schema !== NPM_RELEASE_AGENT_REVIEW_SCHEMA, "release-agent-review-payload-schema", `payload schema must be ${NPM_RELEASE_AGENT_REVIEW_SCHEMA}`);
  ...
  addIf(failures, payload.reviewer.agent !== expected.reviewerAgentId, "release-agent-review-reviewer", "reviewer.agent must equal RELEASE_REVIEWER_AGENT");
  addIf(failures, payload.repository !== expected.repository, "release-agent-review-repository", `repository must be ${expected.repository}`);
  addIf(failures, payload.commit !== expected.releaseCommit, "release-agent-review-commit", "commit must equal the exact release commit");
  addIf(failures, payload.package.name !== expected.packageName, "release-agent-review-package", `package.name must be ${expected.packageName}`);
  addIf(failures, payload.package.version !== expected.packageVersion, "release-agent-review-version", `package.version must be ${expected.packageVersion}`);
  addIf(failures, payload.tag !== expected.tag, "release-agent-review-tag", `tag must be ${expected.tag}`);
  addIf(failures, payload.workflow.path !== expected.workflowPath, "release-agent-review-workflow-path", `workflow.path must be ${expected.workflowPath}`);
  addIf(failures, payload.workflow.revision !== expected.workflowRevision, "release-agent-review-workflow-revision", "workflow.revision must equal the release commit workflow blob object");
  addIf(failures, payload.registry !== expected.registry, "release-agent-review-registry", `registry must be ${expected.registry}`);
  addIf(failures, payload.publisher.agent !== expected.publisherAgentId, "release-agent-review-publisher", "publisher.agent must equal the single Agent trailer on the annotated release tag");
