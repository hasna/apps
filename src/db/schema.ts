...
export const MIGRATIONS: string[] = [
  ...
  // Migration 8: Guarded project metadata mutation receipts
  `
  ...
  // Migration 11: Add the explicit immutable Conversations channel ID locator kind.
  `
  DROP TRIGGER IF EXISTS project_resource_links_identity_immutable;
  DROP INDEX IF EXISTS idx_project_resource_links_project;

  ALTER TABLE project_resource_links RENAME TO project_resource_links_v10;
  ...
  SELECT
    id, project_id, authority, service_instance, source_package, target_kind,
    locator_kind, locator_value, scope, labels_json, created_at, updated_at
  FROM project_resource_links_v10;

  DROP TABLE project_resource_links_v10;
  ...
  // Migration 12: Add @hasna/orgs organization and project resource links.
  `
  DROP TRIGGER IF EXISTS project_resource_links_identity_immutable;
  DROP INDEX IF EXISTS idx_project_resource_links_project;

  ALTER TABLE project_resource_links RENAME TO project_resource_links_v11;

  CREATE TABLE project_resource_links (
    id TEXT PRIMARY KEY,
    project_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
    authority TEXT NOT NULL CHECK(authority IN ('todos', 'conversations', 'knowledge', 'mementos', 'orgs')),
    service_instance TEXT NOT NULL,
    source_package TEXT NOT NULL CHECK(source_package IN ('@hasna/todos', '@hasna/conversations', '@hasna/knowledge', '@hasna/mementos', '@hasna/orgs')),
    target_kind TEXT NOT NULL CHECK(target_kind IN ('org', 'project', 'task_list', 'plan', 'channel', 'collection', 'item')),
    locator_kind TEXT NOT NULL CHECK(locator_kind IN ('external_uuid', 'canonical_uri', 'conversations_channel_id')),
    locator_value TEXT NOT NULL,
    scope TEXT NOT NULL CHECK(scope IN ('resource', 'collection')),
    labels_json TEXT NOT NULL DEFAULT '{}',
  ...
  SELECT
    id, project_id, authority, service_instance, source_package, target_kind,
    locator_kind, locator_value, scope, labels_json, created_at, updated_at
  FROM project_resource_links_v11;

  DROP TABLE project_resource_links_v11;
  ...
  // Migration 13: Add @hasna/contacts contact resource links.
  `
  DROP TRIGGER IF EXISTS project_resource_links_identity_immutable;
  DROP INDEX IF EXISTS idx_project_resource_links_project;

  ALTER TABLE project_resource_links RENAME TO project_resource_links_v12;
  ...
  CREATE TABLE project_resource_links (
    ...
    target_kind TEXT NOT NULL CHECK(target_kind IN ('contact', 'org', 'project', 'task_list', 'plan', 'channel', 'collection', 'item')),
    locator_kind TEXT NOT NULL CHECK(locator_kind IN ('external_uuid', 'canonical_uri', 'conversations_channel_id')),
    locator_value TEXT NOT NULL,
    scope TEXT NOT NULL CHECK(scope IN ('resource', 'collection')),
    labels_json TEXT NOT NULL DEFAULT '{}',
  ...
  SELECT
    id, project_id, authority, service_instance, source_package, target_kind,
    locator_kind, locator_value, scope, labels_json, created_at, updated_at
  FROM project_resource_links_v12;

  DROP TABLE project_resource_links_v12;
