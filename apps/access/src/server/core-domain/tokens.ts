...
export async function issueToken(input: IssueTokenInput, ctx?: AuthorizationContext): Promise<IssuedTokenResult> {
  ...
  const granted = new Set((await effectiveScopes(identity.id, ctx)));
  ...
  return { token, record: (await getToken(id, ctx)) };
}
...
export async function getToken(id: string, ctx?: AuthorizationContext): Promise<PublicIssuedToken> {
  const db = getDatabase();
  const row = (await db.query("SELECT * FROM issued_tokens WHERE id = ?").get(id)) as TokenRow | null;
...
export async function listTokens(filter: ListTokensFilter = {}, ctx?: AuthorizationContext): Promise<PublicIssuedToken[]> {
...
export async function revokeToken(id: string, reason: string, ctx?: AuthorizationContext): Promise<PublicIssuedToken> {
  const db = getDatabase();
  const existing = (await getToken(id, ctx));
  authorize("revoke", ctx, { entity_id: existing.entity_id, resource: "token" });
  (await db.query("UPDATE issued_tokens SET status = 'revoked', revoked_at = ? WHERE id = ?").run(now(), id));
  (await appendAuditEvent(db, {
