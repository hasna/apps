...
export async function grantScope(input: GrantScopeInput, ctx?: AuthorizationContext): Promise<Scope> {
  ...
  return (await getScope(id, ctx));
}

export async function getScope(id: string, ctx?: AuthorizationContext): Promise<Scope> {
  const db = getDatabase();
  const row = (await db.query("SELECT * FROM scopes WHERE id = ?").get(id)) as ScopeRow | null;
...
export async function listScopes(filter: ListScopesFilter = {}, ctx?: AuthorizationContext): Promise<Scope[]> {
...
export async function revokeScope(id: string, reason: string, ctx?: AuthorizationContext): Promise<Scope> {
  const db = getDatabase();
  const existing = (await getScope(id, ctx));
  authorize("revoke", ctx, { entity_id: existing.entity_id, resource: "scope" });
  (await db.query("UPDATE scopes SET status = 'revoked', revoked_at = ?, updated_at = ?, version = version + 1 WHERE id = ?").run(now(), now(), id));
  (await appendAuditEvent(db, {
...
export async function effectiveScopes(identityId: string, ctx?: AuthorizationContext): Promise<string[]> {
  const identity = (await getIdentity(identityId, ctx));
  authorize("read", ctx, { entity_id: identity.entity_id, resource: "scope" });
  const db = getDatabase();
  const permanent = (await db.query("SELECT scope FROM scopes WHERE identity_id = ? AND status = 'granted'").all(identityId)) as { scope: string }[];
  const nowTs = now();
