...
import { assertNoRawSecretValues, validateSecretRef } from "../../services/secret-boundary.js";
import {
...
export async function createAccessRequest(input: CreateAccessRequestInput, ctx?: AuthorizationContext): Promise<AccessRequest> {
  const requester = (await getIdentity(input.requested_by_identity_id, ctx));
  ...
  return (await getAccessRequest(id, ctx));
}

export async function getAccessRequest(id: string, ctx?: AuthorizationContext): Promise<AccessRequest> {
  const db = getDatabase();
  const row = (await db.query("SELECT * FROM access_requests WHERE id = ?").get(id)) as AccessRequestRow | null;
...
export async function listAccessRequests(filter: ListAccessRequestsFilter = {}, ctx?: AuthorizationContext): Promise<AccessRequest[]> {
...
export async function approveAccessRequest(id: string, input: ApproveAccessRequestInput = {}, ctx?: AuthorizationContext): Promise<AccessRequest> {
  const db = getDatabase();
  const existing = (await getAccessRequest(id, ctx));
...
export async function markAccessRequestProvisioned(
  id: string,
...
export async function failAccessRequest(id: string, input: FailAccessRequestInput, ctx?: AuthorizationContext): Promise<AccessRequest> {
...
export async function cancelAccessRequest(id: string, input: CancelAccessRequestInput = {}, ctx?: AuthorizationContext): Promise<AccessRequest> {
