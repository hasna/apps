...
export async function createIdentity(input: CreateIdentityInput, ctx?: AuthorizationContext): Promise<Identity> {
  ...
  return (await getIdentity(id, ctx));
}

export async function getIdentity(id: string, ctx?: AuthorizationContext): Promise<Identity> {
  const db = getDatabase();
  const row = (await db.query("SELECT * FROM identities WHERE id = ?").get(id)) as IdentityRow | null;
...
export async function listIdentities(filter: ListIdentitiesFilter = {}, ctx?: AuthorizationContext): Promise<Identity[]> {
...
export async function updateIdentity(id: string, patch: UpdateIdentityInput, ctx?: AuthorizationContext): Promise<Identity> {
  const db = getDatabase();
  const existing = (await getIdentity(id, ctx));
...
export async function setIdentityStatus(id: string, status: IdentityStatus, ctx?: AuthorizationContext): Promise<Identity> {
  const db = getDatabase();
  const existing = (await getIdentity(id, ctx));
  authorize("write", ctx, { entity_id: existing.entity_id, resource: "identity" });
  (await db.query("UPDATE identities SET status = ?, updated_at = ?, version = version + 1 WHERE id = ?").run(status, now(), id));
  (await appendAuditEvent(db, {
