...
export async function scheduleReview(input: ScheduleReviewInput, ctx?: AuthorizationContext): Promise<AccessReview> {
  ...
  return (await getReview(id, ctx));
}

export async function getReview(id: string, ctx?: AuthorizationContext): Promise<AccessReview> {
  const db = getDatabase();
  const row = (await db.query("SELECT * FROM access_reviews WHERE id = ?").get(id)) as ReviewRow | null;
...
export async function listReviews(filter: ListReviewsFilter = {}, ctx?: AuthorizationContext): Promise<AccessReview[]> {
...
export async function setReviewStatus(id: string, status: ReviewStatus, ctx?: AuthorizationContext, completedBy?: string): Promise<AccessReview> {
  const db = getDatabase();
  const existing = (await getReview(id, ctx));
