...
import type { CoreDatabase as Database } from "../core-store.js";
import { AUDIT_TABLE } from "../../db/schema.js";
...
async function lastRowHash(db: Database): Promise<string> {
  const row = (await db.query(`SELECT row_hash FROM ${AUDIT_TABLE} ORDER BY id DESC LIMIT 1`).get()) as { row_hash: string } | null;
  return row?.row_hash ?? GENESIS;
}

export async function appendAuditEvent(db: Database, input: AuditInput): Promise<AuditEvent> {
  const createdAt = new Date().toISOString();
  const entityId = input.entity_id ?? null;
  const actor = input.actor ?? null;
  const prevHash = (await lastRowHash(db));
  ...
  const id = ((await db.query(`SELECT currval(pg_get_serial_sequence('audit_log', 'id'))::integer AS id`).get()) as { id: number }).id;
...
export async function verifyAuditChain(db: Database): Promise<ChainVerification> {
  const rows = (await db.query(`SELECT * FROM ${AUDIT_TABLE} ORDER BY id ASC`).all()) as AuditRow[];
  let prev = GENESIS;
