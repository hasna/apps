...
describe("published @hasna/contracts client types", () => {
  ...
  it("the published declaration graph never re-exports @hasna/contracts values", async () => {
    // `bun build --target bun` inlines the resolver, but `tsc` declarations are
    // not bundled: a re-export in source lands in `dist/**/*.d.ts` as a live
    // `@hasna/contracts` import and breaks every TS consumer, which installs
    // this package's runtime dependencies and not its devDependencies
    // (hasna/apps#1782). The graph starts at `src/index.ts` (dist/index.d.ts),
    // ./sdk (dist/sdk/index.d.ts) and the lib subpaths; none of it may
    // re-export a contracts value. Type-only imports are fine — they are
    // spelled in ./client-types.ts and asserted identical above.
    const publishedRoots = ["../index.ts", "../sdk/index.ts", "../db/store.ts", "./domains-resolver.ts"];
    for (const root of publishedRoots) {
      const source = await Bun.file(new URL(root, import.meta.url)).text();
      expect(
        source.match(/export\s*\{[^}]*\}\s*from\s*["']@hasna\/contracts/g),
        `${root} re-exports @hasna/contracts values into the published declarations`,
      ).toBeNull();
    }
  });
});