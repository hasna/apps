/**
 * Regression tests for O15-00671, O15-00758 and O15-00762: the prod
 * `schema_migrations` ledger carries out-of-band rows
 * (`domains_apikeys_tenancy_0001`, `domains_apikeys_tenancy_0002`,
 * `domains_tenancy_0001`, `domains_tenancy_0002`) that no build in this repo's
 * history generates (verified across every published tarball 0.0.30-0.0.46 and
 * all git history — the rows date from the 2026-07 self-hosted cutover). The
 * storage kit's downgrade guard refused them, so `domains db migrate` failed
 * and the domains deploy lane was blocked. The app acknowledges the rows via
 ...
 * `domains_tenancy_0002` at `domains-prod-migrate:42` (O15-00762).
 *
...
describe("domains migration ledger legacy acknowledgment (O15-00671 / O15-00758 / O15-00762)", () => {
