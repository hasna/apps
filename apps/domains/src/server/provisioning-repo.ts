...
function rowToJob(row: ProvisioningRow): DomainProvisioningJob {
  return {
    ...
    target: row.target as "shortlinks",
    ...
    try {
      const row = await this.db.get<ProvisioningRow>(
        `INSERT INTO domain_provisioning_jobs (
           ...
           max_price_usd, years, auto_renew, registrar, dns_provider, target,
           worker_name, provider_state, attempts, error, lease_token, lease_until,
           created_at, updated_at
         ) VALUES ($1,$2,$3,$4,$5,'requested',$6,$7,$8,$9,$10,$11,$12,'{}',0,NULL,NULL,NULL,$13,$14)
         RETURNING *`,
        [
          id,
          domain.id,
