...
describe("Domain Owners — CRUD", () => {
  ...
  test("setup: create domain for owner tests", () => {
    const domain = createDomain({ name: "owner-test.com" });
    domainId = domain.id;
    expect(domainId).toBeTruthy();
  });

  test("create domain owner", () => {
  ...
  test("create domain owner with minimal fields", () => {
  ...
  test("update non-existent owner returns null", () => {
  ...
  test("delete non-existent owner returns false", () => {
...
describe("Domain Owners — List & Filter", () => {
  ...
  test("setup: create domains for list tests", () => {
    const d1 = createDomain({ name: "list-test1.com" });
    const d2 = createDomain({ name: "list-test2.com" });
    const d3 = createDomain({ name: "list-test3.com" });
    domainId1 = d1.id;
    domainId2 = d2.id;
    domainId3 = d3.id;
...
describe("Domain Owners — WHOIS Extraction", () => {
  test("extract owner from WHOIS creates new record", () => {
    const domain = createDomain({ name: "whois-extract-new.com" });
    const whoisRaw = `
...
`;
    ...
    const owner = extractOwnerFromWhois("whois-extract-new.com", whoisRaw);
  ...
  test("extract owner from WHOIS updates existing record", () => {
    const domain = createDomain({ name: "whois-extract-existing.com" });
    createDomainOwner({
      domain_id: domain.id,
      owner_name: "Old Name",
      source: "manual",
...
`;
    ...
    const owner = extractOwnerFromWhois("whois-extract-existing.com", whoisRaw);
    expect(owner).not.toBeNull();
  ...
  test("extract owner returns null for non-existent domain", () => {
    const result = extractOwnerFromWhois("nonexistent-whois.xyz", "some whois text");
    expect(result).toBeNull();
  });

  test("extract owner returns null when no registrant info found", () => {
    const domain = createDomain({ name: "whois-empty.com" });
    const whoisRaw = `
Domain Name: WHOIS-EMPTY.COM
Name Server: ns1.example.com
Status: ok
`;
    const result = extractOwnerFromWhois("whois-empty.com", whoisRaw);
    expect(result).toBeNull();
  });
});
...
describe("Domain Owners — List Domains With Owners", () => {
  ...
  test("setup: create premium domain and owned domain", () => {
    const premium = createDomain({
    ...
    const owned = createDomain({ name: "with-owner.com", status: "researching" });
  ...
  test("listDomainsWithOwners includes premium domains without owners", () => {
    const results = listDomainsWithOwners();
    expect(results.some((r) => r.domain_name === "premium-owner.com")).toBe(true);
  });

  test("listDomainsWithOwners includes domains with owners", () => {
    const results = listDomainsWithOwners();
  ...
  test("listDomainsWithOwners returns structured data", () => {
    const results = listDomainsWithOwners();
...
describe("Domain Owners — Link to Contacts", () => {
  ...
  test("returns null when no owner record exists", () => {
    const domain = createDomain({ name: "no-owner-link.com" });
    const result = linkOwnerToContacts(domain.id, {
  ...
  test("returns null when owner has no email", () => {
    const domain = createDomain({ name: "no-email-link.com" });
  ...
  test("creates new contact when none exists", () => {
    const domain = createDomain({ name: "new-contact-link.com" });
    createDomainOwner({
      domain_id: domain.id,
  ...
  test("uses existing contact when email matches", () => {
    const domain = createDomain({ name: "existing-contact-link.com" });
    createDomainOwner({
      domain_id: domain.id,
...
describe("Domain Owners — Source Types", () => {
  test("all source types are valid", () => {
    const domain = createDomain({ name: "source-types-test.com" });
