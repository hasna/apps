...
describe("Domain Reputation — CRUD", () => {
  ...
  test("setup: create domain for reputation tests", () => {
    const domain = createDomain({ name: "reputation-test.com" });
    domainId = domain.id;
    expect(domainId).toBeTruthy();
  });

  test("create reputation record", () => {
  ...
  test("get reputation by domain ID", () => {
    const rep = getDomainReputation(domainId);
    expect(rep).not.toBeNull();
    expect(rep!.domain_id).toBe(domainId);
  });

  test("get reputation by domain name", () => {
    const rep = getDomainReputationByName("reputation-test.com");
    expect(rep).not.toBeNull();
    expect(rep!.domain_id).toBe(domainId);
  });

  test("get reputation for non-existent domain ID returns null", () => {
    expect(getDomainReputation("nonexistent-id")).toBeNull();
  });

  test("get reputation by non-existent domain name returns null", () => {
  ...
  test("update non-existent reputation returns null", () => {
  ...
  test("delete reputation", () => {
    const cleanDomain = createDomain({ name: "delete-rep-test.com" });
  ...
  test("delete non-existent reputation returns false", () => {
...
describe("Domain Reputation — Blacklist & Threats", () => {
  ...
  test("setup: create domains with various reputation levels", () => {
    // Blacklisted domain
    const d1 = createDomain({ name: "blacklisted-test.com" });
    blacklistedId = d1.id;
    upsertDomainReputation({
    ...
    const d2 = createDomain({ name: "high-threat-test.com" });
    highThreatId = d2.id;
    upsertDomainReputation({
    ...
    // Clean domain
    const d3 = createDomain({ name: "clean-test.com" });
    cleanId = d3.id;
    upsertDomainReputation({
      domain_id: cleanId,
      threat_score: 5,
  ...
  test("list high threat domains with default threshold", () => {
  ...
  test("list high threat domains with custom threshold", () => {
  ...
  test("list high threat domains with very high threshold returns empty", () => {
...
describe("Domain Reputation — DNS Blacklist", () => {
  test("checkDnsBlacklist returns structured result", () => {
  ...
  test("checkDnsBlacklist zones include common DNSBL zones", () => {
...
describe("Domain Reputation — Full Check", () => {
  test("checkDomainReputation creates history entry", () => {
    const domain = createDomain({ name: "reputation-check-test.com" });
    ...
    const result = checkDomainReputation("reputation-check-test.com");
  ...
  test("checkDomainReputation throws for missing domain", () => {
    expect(() => checkDomainReputation("nonexistent-rep.xyz")).toThrow(
      "Domain 'nonexistent-rep.xyz' not found in database"
    );
  });

  test("checkDomainReputation updates last_checked_at", () => {
    const domain = createDomain({ name: "reputation-update-test.com" });
    upsertDomainReputation({
      domain_id: domain.id,
      threat_score: 20,
    });
    ...
    checkDomainReputation("reputation-update-test.com");
...
describe("Domain Reputation — Data Types", () => {
  test("boolean fields are properly converted", () => {
    const domain = createDomain({ name: "bool-convert-test.com" });
    upsertDomainReputation({
      domain_id: domain.id,
      is_blacklisted: true,
      malware_detected: true,
  ...
  test("default values for new reputation", () => {
    const domain = createDomain({ name: "default-rep-test.com" });
