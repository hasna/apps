...
describe("compact CLI output", () => {
  test("domain list is compact and bounded for human and JSON output with explicit full compatibility", async () => {
    ...
    expect(invalidStderr).toContain("--limit must be a positive integer");
  ...
  test("hosted JSON pages advance without overlap and reject a zero limit", async () => {
    const { dbPath } = await seedDomainDb(45);

    const readPage = (offset: number) => {
      const result = runDomains(["domain", "list", "--json", "--limit", "20", "--offset", String(offset)], dbPath);
      expect(result.exitCode).toBe(0);
      return JSON.parse(text(result.stdout)) as {
    ...
    const first = readPage(0);
    const second = readPage(first.next_offset!);
    const third = readPage(second.next_offset!);
    const ids = [...first.domains, ...second.domains, ...third.domains].map((domain) => domain.id);
    expect(new Set(ids).size).toBe(45);
    expect(first.next_offset).toBe(20);
    expect(second.next_offset).toBe(40);
    expect(third.next_offset).toBeNull();
    expect(third.has_more).toBe(false);

    const zero = runDomains(["domain", "list", "--json", "--limit", "0"], dbPath);
    expect(zero.exitCode).toBe(1);
    expect(text(zero.stderr)).toContain("--limit must be a positive integer");
  });
