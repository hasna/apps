  ...
  history
    ...
    .action(async (identifier: string, opts: { type?: string; limit?: string; json?: boolean }) => {
      ...
      for (const e of entries) {
        printLine(`  [${e.created_at}] ${e.snapshot_type} — ${e.registrant_name ?? e.registrant_email ?? "no owner info"}`);
        if (e.registrar) printLine(`    Registrar: ${e.registrar}`);
        if (e.notes) printLine(`    Notes: ${truncateText(e.notes, 120)}`);
      }
  ...
  history
    ...
    .action(async (opts: { limit?: string; all?: boolean; json?: boolean }) => {
      ...
      for (const r of page.items) {
        printLine(`  ${r.domain_name} — ${r.snapshot_count} snapshots, latest: ${r.latest_snapshot_type} at ${r.latest_snapshot_at}`);
      }
    ...
    .action(async (opts: { from: string; to: string; domain?: string; limit?: string; all?: boolean; verbose?: boolean; json?: boolean }) => {
      ...
      for (const e of page.items) {
        const extra = opts.verbose
          ...
          : "";
        printLine(`  [${e.created_at}] ${e.snapshot_type} — domain ${e.domain_id}${extra}`);
      }
