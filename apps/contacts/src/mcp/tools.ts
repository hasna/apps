...
const PREDICATE_SCHEMA = { type: "object", properties: { kind: { type: "string", enum: ["tag", "attribute", "group"] }, key: { type: "string", description: "Attribute key (required for attribute predicates)" }, op: { type: "string", enum: ["eq", "neq", "in", "not_in", "exists", "not_exists"] }, value: { description: "Value for eq/neq" }, values: { type: "array", description: "Values for in/not_in" } }, required: ["kind"] } as const;
export const TOOL_DEFINITIONS = [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          limit: { type: "number", description: "Max results (default 20 compact, 50 verbose; max 200)" },
    ...
    // ─── Distribution: audiences / consent / suppression (hasna.audience.v1) ──
    { name: "create_audience", description: "Create an audience segment (hasna.audience.v1): predicates over contact tags, attributes (columns or custom fields), and group membership. match: all|any. consent_policy: opt_in|opt_out|transactional|none.", inputSchema: { type: "object", properties: { audience_id: { type: "string", description: "Lowercase dashed slug, e.g. beta-testers" }, name: { type: "string" }, match: { type: "string", enum: ["all", "any"] }, predicates: { type: "array", items: PREDICATE_SCHEMA }, consent_policy: { type: "string", enum: ["opt_in", "opt_out", "transactional", "none"] } }, required: ["audience_id", "name", "predicates"] } },
    { name: "get_audience", description: "Get an audience by id or slug as its hasna.audience.v1 contract document.", inputSchema: { type: "object", properties: { id: { type: "string" } }, required: ["id"] } },
    { name: "list_audiences", description: "List all audience segments.", inputSchema: { type: "object", properties: {} } },
    { name: "update_audience", description: "Update an audience's name, match mode, predicates, or consent policy.", inputSchema: { type: "object", properties: { id: { type: "string" }, name: { type: "string" }, match: { type: "string", enum: ["all", "any"] }, predicates: { type: "array", items: PREDICATE_SCHEMA }, consent_policy: { type: "string", enum: ["opt_in", "opt_out", "transactional", "none"] } }, required: ["id"] } },
    { name: "delete_audience", description: "Delete an audience segment by id or slug.", inputSchema: { type: "object", properties: { id: { type: "string" } }, required: ["id"] } },
    { name: "resolve_audience", description: "Resolve an audience to per-channel recipients honoring consent policy, per-channel consent, suppressions, and do_not_contact. Returns recipients plus exclusions with reasons.", inputSchema: { type: "object", properties: { id: { type: "string" }, channel: { type: "string", enum: ["email", "telegram", "sms"] } }, required: ["id", "channel"] } },
    { name: "set_contact_consent", description: "Set a contact's subscription consent for a channel (opt_in, opt_out, unknown).", inputSchema: { type: "object", properties: { contact_id: { type: "string" }, channel: { type: "string", enum: ["email", "telegram", "sms"] }, status: { type: "string", enum: ["opt_in", "opt_out", "unknown"] }, source: { type: "string", description: "Where the consent signal came from" } }, required: ["contact_id", "channel", "status"] } },
    { name: "get_contact_consent", description: "Get a contact's consent status for all channels.", inputSchema: { type: "object", properties: { contact_id: { type: "string" } }, required: ["contact_id"] } },
    { name: "suppress_address", description: "Suppress an address on a channel (unsubscribe/bounce/complaint). Also records an opt-out for the linked contact when contact_id is provided.", inputSchema: { type: "object", properties: { channel: { type: "string", enum: ["email", "telegram", "sms"] }, address: { type: "string" }, contact_id: { type: "string" }, reason: { type: "string" } }, required: ["channel", "address"] } },
    { name: "unsuppress_address", description: "Remove an address from the suppression list.", inputSchema: { type: "object", properties: { channel: { type: "string", enum: ["email", "telegram", "sms"] }, address: { type: "string" } }, required: ["channel", "address"] } },
    { name: "list_suppressions", description: "List suppressed addresses, optionally filtered by channel or unsynced-only.", inputSchema: { type: "object", properties: { channel: { type: "string", enum: ["email", "telegram", "sms"] }, unsynced_only: { type: "boolean" } }, required: [] } },
