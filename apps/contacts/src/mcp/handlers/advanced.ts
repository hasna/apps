...
export const advancedHandlers: Record<string, ToolHandler> = {
  ...
  set_contact_photo: async (a) => {
    ...
    return json({ ok: true, contact_id, filename, avatar_url: `~/.hasna/contacts/images/${filename}` });
  },
  ...
  set_company_logo: async (a) => {
    ...
    return json({ ok: true, company_id, filename, logo_url: `~/.hasna/contacts/images/${filename}` });
  },
