...
const CONTACTS_DISCOVERY_DEFINITIONS = [
  ...TOOL_DEFINITIONS,
  {
...
export function registerContactsDiscoveryTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async (args: { names?: string[] }) => {
      ...
      const found = new Set<string>(items.map((item) => item.name));
      const missing = names.filter((name) => !found.has(name));
      return { content: [{ type: "text" as const, text: JSON.stringify({ items, count: items.length, requested: names.length, missing, complete: missing.length === 0, hint: "Restart with HASNA_CONTACTS_MCP_PROFILE=full for callable non-core tools." }) }] };
    },
  );
}
