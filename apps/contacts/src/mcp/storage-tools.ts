...
// bearer key). These tools are read-only transport/status plus local feedback.
...
export function registerContactsStorageTools(server: McpServer): void {
  server.tool(
    ...
    async () => {
      try {
        return ok({ transport: transportStatus(), local: getStorageStatus() });
      } catch (error) {
        return err(error);
      }
    }
  ...
  server.tool(
    ...
    async () => {
      try {
        return ok({ service: "contacts", transport: transportStatus(), storage: getStorageStatus() });
      } catch (error) {
        return err(error);
      }
    }
