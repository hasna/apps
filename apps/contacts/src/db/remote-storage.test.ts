...
function pgUrl(host: string, query?: string): string {
  return `postgres://user:pass@${host}:5432/contacts${query ? `?${query}` : ""}`;
}
...
describe("contacts PostgreSQL remote storage TLS", () => {
  ...
  test("rejects remote hosts that disable or weaken TLS", () => {
    expect(() => buildPgPoolConfig(pgUrl("db.example.com", ["sslmode", "disable"].join("=")))).toThrow("verified TLS");
    expect(() => buildPgPoolConfig(pgUrl("db.example.com", ["ssl", "false"].join("=")))).toThrow("verified TLS");
    expect(() => buildPgPoolConfig(pgUrl("db.example.com", "sslmode=prefer"))).toThrow("verified TLS");
    const duplicateSslMode = `${["sslmode", "verify-full"].join("=")}&${["sslmode", "disable"].join("=")}`;
    expect(() => buildPgPoolConfig(pgUrl("db.example.com", duplicateSslMode))).toThrow("verified TLS");
    expect(() => buildPgPoolConfig(pgUrl("db.example.com", `${["ssl", "true"].join("=")}&${["ssl", "0"].join("=")}`))).toThrow("verified TLS");
    expect(() =>
      buildPgPoolConfig(pgUrl("db.example.com", `${["sslmode", "require"].join("=")}&${["uselibpqcompat", "true"].join("=")}`))
