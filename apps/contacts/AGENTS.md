...
Every tool uses the single `Store` abstraction through `getStore()`. The
transport is automatic: a resolved authenticated HTTPS `/v1` authority — the
shared `@hasna/contracts` 1.0.2 client chain — selects the hosted API
transport; otherwise tools read and write the on-box SQLite store. A contacts
API key is resolved from, in order: deliberate pointers
...
`api-url` item / credentials file) overrides it. The storage-mode axis is
retired: `*_MODE`, DB-path selector, and DSN variables never gate a tool, and
every tool works in either transport.

Use `contacts_connection_status` to inspect value-free configuration details.
For SQLite files from retired releases, use `contacts legacy inspect` and
`contacts legacy preserve`; these commands only ever copy, never adopt.
