/**
 * @hasna/logs — local agent-lifecycle registry (inlined from @hasna/agent-registry)
 ...
 * (a dedicated `agent-registry.db` beside logs' own data store, so the
 * registry survives restarts and is shared by every MCP process on the box).
 *
...
import { getLogsDataDir } from "../db/index.ts";

/**
 * Structural subset of @hasna/events' EventsClient that the registry emits
 * lifecycle events through (logs mirrors them into its own durable event
 * store). Kept identical to the deleted package's `AgentEventsClient`.
 */
 ...
 * feedback tool (logs' stays local with its own category enum).
 */
