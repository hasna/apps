...
export class ApiStore implements Store {
  ...
  async getEvent(
    ...
    // Raw event bodies live in local segment files; the hosted tier has none,
    // so `raw` is always null here regardless of the includeRaw flag.
  ...
  async pushEvent(
    ...
    // deterministic machine/repo/app IDs travel on the event and the hosted
