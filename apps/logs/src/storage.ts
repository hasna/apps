 ...
 * ApiStore (HTTP /v1 + bearer key — the hosted API) behind ONE interface,
 * resolved from the environment. Every CLI command and MCP tool routes log
 * reads/writes through this. There is NO DSN-on-client path: the raw RDS DSN
 * is never distributed to machines (CLAUDE.md §2).
 */
