...
/**
 * Hosted sink — ships captured telemetry to the shared `/v1` API via the
 * resolved {@link Store}. Writes are queued onto a serial chain so hosted
 * persistence preserves the capture order; `flush()` awaits them and surfaces
 * the first failure. The server derives process/run rows and artifact/
