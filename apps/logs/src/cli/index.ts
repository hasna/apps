...
program
  ...
  .action(async (opts) => {
    ...
    // Normalize exactly once and use that same bounded value for both the
    // request and the label. This prevents `--days 2.5` from requesting two
    // days while claiming 2.5, and `--days 999` from claiming 999 while the
    // server clamps to 366.
    const windowDays = statsWindowDays(Number(opts.days));
    ...
    if (days.length) {
      ...
      console.log(`\n${C.bold}Last ${windowDays} Days:${C.reset}`);
