...
/** Answers the single snapshot-consistent aggregate `statsSummary` issues. */
    ...
    return [
      {
        ...
        by_level: count(data.map((row) => row.level)),
        by_service: count(data.map((row) => row.service ?? "-")),
        by_day: count(
          times
...
function hosted(home: string): Record<string, string | undefined> {
  return {
    HOME: home,
    // Credential/config authority is the app base. The client appends /v1.
...
describe("logs stats (hosted)", () => {
  ...
  test.each([
    ...
    ["2.5", "2"],
    ["999", "366"],
  ])("--days %s is normalized once for request and display", async (input, expected) => {
    const home = mkdtempSync(join(tmpdir(), "logs-stats-home-"));
    try {
      requests.length = 0;
      const result = await runCli(["stats", "--days", input], hosted(home));
      expect(result.status, result.stderr).toBe(0);
      expect(requests).toEqual([`GET /v1/logs/stats?days=${expected}`]);
      expect(result.stdout).toContain(`Last ${expected} Days:`);
      expect(dbFiles(home)).toEqual([]);
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
