...
service previews, exact log IDs for `logs get <id>`, and a 32 KiB output
ceiling. When more rows remain, the footer reports a resumable `next_offset`;
pass it back with `--offset`. Use `--limit` and `--max-bytes` to tune the compact
page, `--format table` for the legacy terminal columns, or `--format json` for
explicit full records.
