/**
 * servers data-root resolution — thin app wrapper over the single paths
 * resolver in `@hasna/contracts` (ruling hasna/apps#1668). The resolver owns
 * platform placement (`~/.hasna/servers` on macOS, XDG data root on Linux)
 * and the `HASNA_{CONFIG,DATA,STATE,CACHE}_HOME` kind overrides; this module
 * layers the servers-specific exact-app override on top.
 */
...
/**
 * The resolver servers data root: kind overrides honored,
 * `~/.hasna/servers` on macOS, `~/.local/share/hasna/servers` on Linux.
 */
export function getResolverDataRoot(env: NodeJS.ProcessEnv = process.env): string {
  return resolverDataDir({ app: "servers", home: getHomeDir(env), env, });
}

/**
 * The pre-ruling legacy root (`~/.hasna/servers`). On macOS this equals the
...
export function getLegacyDataRoot(env: NodeJS.ProcessEnv = process.env): string {
  return join(getHomeDir(env), ".hasna", "servers");
}
...
/**
 * The effective servers data root (ruling #1668 — the resolver root IS
