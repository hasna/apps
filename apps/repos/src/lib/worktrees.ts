...
/** Hard upper bound for a configured worktree git process: 30 minutes. */
export const MAX_GIT_TIMEOUT_MS = 30 * 60_000;
 ...
 *
 * An unusable value — blank, non-integer, non-numeric, zero, negative,
 * `Infinity`, or above 30 minutes — falls back to the default instead of
 * widening or removing the bound. This is the whole point of the guard:
 * `execFileSync({ timeout: 0 })` means NO timeout, so a config typo would
 * otherwise silently convert the ceiling into an unbounded git process. A
 * ceiling that a typo can delete or overflow is not a ceiling.
 */
export function gitTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0 || parsed > MAX_GIT_TIMEOUT_MS) {
