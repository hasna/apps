...
| `repos registry register <path>` | Plans one existing standalone checkout registration. Requires `--expected-remote`, `--expected-head`, `--expected-branch`; `--apply` additionally requires `--expected-database` and `--expected-plan-hash`; optional `--dry-run`, `--json`. Uses the existing normally resolved registry without bootstrap, migrations, hooks, history scanning, remote sync, or duplicate merging. |
