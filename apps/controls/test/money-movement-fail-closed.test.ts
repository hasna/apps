...
describe("money-movement contract: fail-closed execution mode", () => {
  it("denies a malformed execution_mode instead of treating it as non-live", () => {
    ...
    for (const mode of ["production", "LIVE", "prod", "", "sandboxx", null]) {
      ...
      const gate = result.decisions.find((d) => d.control === "live_mode_gate");
      expect(gate?.ok).toBe(false);
      expect(gate?.reason).toMatch(/execution_mode/);
    }
  });
