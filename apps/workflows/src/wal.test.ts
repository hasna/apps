...
import { SessionWAL, type WalOp, type WalReplay } from "./wal.js";
...
describe("SessionWAL", () => {
  ...
  test("detects a torn tail (partial line) and truncates it", () => {
    ...
    writeFileSync(join(dir, "session.wal"), readFileSync(join(dir, "session.wal")) + '{"seq":2,"at":"2026-01-01T00:00:00.000Z"', "utf8");
  ...
  test("detects a checksum mismatch (corrupted entry) and truncates from it", () => {
    ...
    const file = join(dir, "session.wal");
