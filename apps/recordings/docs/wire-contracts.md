...
Inputs reject unknown fields. Recording input has a UUID, title, transcript and duration in milliseconds; caller input cannot supply trusted provenance. Titles are trimmed. Paste input uses UTC timestamps ending in `Z`; output timestamps may have offsets. A paste receipt's `client_reported` evidence does not mean the server observed the target. Pagination requires `before` and `beforeId` together, or neither; either cursor component alone is invalid. Recording, account and stream responses preserve extra JSON fields, including nested fields, while still checking known fields. Provider catalog responses project only public fields and omit unknown fields at every level.
...
`session.start` accepts optional `provider` and `model` selections. Provider IDs
are lowercase slugs up to 64 characters; model IDs are 1–100 characters. The
server owns defaults and validates configured membership. The caller sends
`provider` only after the server advertises the optional `provider-selection`
capability; it is not added to the four required capabilities. Legacy starts and
servers without this advertisement remain valid. `providersResponseParser`
accepts legacy catalogs without default/name/model details and preserves their
absence. New catalogs may describe defaults and realtime or segmented
transcription. See [Hosted SDK](./hosted-sdk.md#transcription-provider-catalog)
for the catalog fields and read surfaces.
