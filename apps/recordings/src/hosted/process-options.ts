...
export interface HostedProcessOptions { apiBase: string; credentialEnv?: string; host?: string; port?: number }
...
export function parseHostedProcessOptions(args: string[], surface: "mcp" | "serve"): HostedProcessOptions {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (key === "--hosted" || (surface === "mcp" && key === "--stdio")) {
  ...
  if (surface === "mcp") {
    const options = { apiBase, credentialEnv: values["--credential-env"] };
  ...
  return { apiBase, host, port: Number(rawPort) };
}
