/**
 * `recordings hosted` must reach the hosted `/v1` plane through the ONE fleet
 * resolver, with no `--api-base` / `--credential-env` handed in by the
 * operator, and must fail closed when nothing resolves.
 *
...
test("an explicit --api-base with its credential-env still uses only that named authority", async () => {
  const written: string[] = [];
  const status = await runHostedCLI([
    "--api-base", `http://127.0.0.1:${server.port}/v1/`, "--credential-env", "SELECTED_SESSION",
    "list", "--limit", "1",
  ], {
    env: { HASNA_STATION: "no-such-station", SELECTED_SESSION: credential,
      // A resolvable fleet authority sits right there and must NOT be consulted.
      HASNA_RECORDINGS_API_URL: "https://fleet.invalid.example", HASNA_RECORDINGS_API_KEY: "fleet-fictional" },
    write: (value: string) => { written.push(value); },
  });
  expect(status).toBe(0);
  expect(JSON.parse(written.join("")).recordings).toHaveLength(1);
});
