...
describe("macOS finalized artifact installer", () => {
  ...
  testOnNonDarwin("does not accept command-only candidate launch evidence from another executable", async () => {
    const fixture = createInstallerFixture();
    const impostor = Bun.spawn(["sleep", "300"], { stdout: "ignore", stderr: "ignore" });
    try {
      const result = await runInstaller(fixture, ["--launch", "--launch-timeout", "1"], {
...
`,
    ...
    // The smoke gives up after SMOKE_MAX_ATTEMPTS; the budget must cover a full
    // give-up cycle under load, where a single attempt can take more than a second.
    const timeout = Bun.sleep(6_000).then(() => "timeout" as const);
