...
describe("prepublishOnly", () => {
  test("runs the gated suite, never a bare `bun test`", () => {
    const prepublishOnly = script("prepublishOnly");
    // The regression: prepublishOnly used to be `bun run typecheck && bun run test`, which ran
    // the full suite under Bun's default 5000ms timeout with no partition check. A standalone
    // `bun run test` token (not `test:gated`) means the gate silently stopped covering what CI
    // covers. The token is matched as a whole word so `test:gated` cannot satisfy it.
    expect(prepublishOnly).not.toMatch(/(?:^|[&| ])bun run test(?:$|[ &|])/);
    expect(prepublishOnly).toContain("test:gated");
  });

  test("keeps typecheck ahead of the suite", () => {
    const prepublishOnly = script("prepublishOnly");
    const typecheckIndex = prepublishOnly.indexOf("typecheck");
    const gatedIndex = prepublishOnly.indexOf("test:gated");
    ...
    expect(gatedIndex, "test:gated must still be part of the publish gate").toBeGreaterThanOrEqual(0);
    expect(typecheckIndex).toBeLessThan(gatedIndex);
  });
});
