  ...
  async run(sql: string, ...params: unknown[]) {
    ...
    if (/insert\s+into\s+recording_idempotency/i.test(sql)) {
      ...
      recordingRows.push({
        id: params[0],
        audio_path: params[1],
        audio_object_key: params[2],
        audio_sha256: params[3],
        audio_bytes: params[4],
        raw_text: params[5],
        processed_text: params[6],
        processing_mode: params[7],
        model_used: params[8],
        enhancement_model: params[9],
        duration_ms: params[10],
        language: params[11],
        tags: params[12],
        agent_id: params[13],
        project_id: params[14],
        session_id: params[15],
        goal: params[16],
        role: params[17],
        task_list_id: params[18],
        machine_id: params[19],
        metadata: params[20],
        created_at: "2026-01-01T00:00:00.000Z",
      });
