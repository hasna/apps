...
function getClient(config: RecordingsConfig): OpenAI {
  if (!config.openai_api_key) {
    throw new TranscriptionError(
      "OpenAI API key not configured. Set OPENAI_API_KEY (e.g. `secrets exec <key> --as OPENAI_API_KEY -- recordings …`)"
    );
  }
  ...
  if (!apiKey) {
    return {
      ...
      message:
        ...
        "Set OPENAI_API_KEY (e.g. `secrets exec <key> --as OPENAI_API_KEY -- recordings …`)",
    };
  }
