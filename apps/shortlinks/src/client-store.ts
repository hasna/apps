...
/** Options shared by client resolution and public redirect-server helpers. */
export interface ExplicitLocalStoreOptions {
  /** Optional database path; the explicit local opt-in is still required. */
  dbPath?: string;
...
/**
 * Open the on-box store only after the caller has explicitly selected it.
 *
 * Public server helpers use this seam when no store is injected. It deliberately
 * does not inspect hosted credentials: hosted callers resolve their transport
 * through {@link resolveStore} and inject that resolved store. This prevents a
 * library helper from silently choosing either a fleet authority or SQLite.
 */
export async function openExplicitLocalStore(
  env: Env = process.env,
  options: ExplicitLocalStoreOptions = {},
): Promise<Store> {
  if (!isLocalOptIn(env)) {
    throw new Error(
      `A Shortlinks store must be injected, or ${LOCAL_OPT_IN_ENV_KEY}=1 ` +
      `(alias SHORTLINKS_LOCAL=1) must explicitly authorize the on-box SQLite store.`,
    );
  }
  const target = getDatabasePath(options.dbPath, env);
  const reason = options.dbPath !== undefined
    ? `${LOCAL_OPT_IN_ENV_KEY}=1, --db ${target}`
    : `${LOCAL_OPT_IN_ENV_KEY}=1`;
  announceLocalBackend(reason, target, options.notice);
...
export async function resolveStore(
  ...
  if (optedIn) {
    return openExplicitLocalStore(env, options);
  }
