...
export function resolveMcpHttpPort(argv: string[] = process.argv, env: NodeJS.ProcessEnv = process.env): number {
  ...
  if (portIdx !== -1 && argv[portIdx + 1]) return parsePort(argv[portIdx + 1]!, "--port");
  if (env.MCP_HTTP_PORT) return parsePort(env.MCP_HTTP_PORT, "MCP_HTTP_PORT");
...
export type McpHttpServerHandle = { port: number; host: string; close: () => Promise<void> };
