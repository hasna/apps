...
describe("redactContent — json", () => {
  test("redacts token fields", () => {
    const r = redactContent('  "token": "npm' + '_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"', "json");
  ...
  test("catches npm tokens in json values", () => {
    const r = redactContent('  "authToken": "npm' + '_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"', "json");
...
describe("redactContent — toml", () => {
  test("redacts token = value", () => {
    const r = redactContent('token = "npm' + '_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"', "toml");
    expect(r.content).toContain("{{TOKEN}}");
    expect(r.redacted).toHaveLength(1);
  });
...
describe("redactContent — ini/.npmrc", () => {
  test("redacts npmrc auth value to npm env reference", () => {
    const r = redactContent("//registry.npmjs.org/:_authToken=npm" + "_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890", "ini");
...
describe("redactContent — generic/markdown", () => {
  test("catches known token patterns in markdown", () => {
    const r = redactContent("Use token: npm" + "_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890", "markdown");
    expect(r.redacted).toHaveLength(1);
  });
...
describe("hasSecrets", () => {
  test("returns true when secrets present", () => {
    expect(hasSecrets("export NPM_TOKEN=npm" + "_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890", "shell")).toBe(true);
  });
...
describe("redactContent — edge cases", () => {
  ...
  test("redacts AWS access keys in any format", () => {
    const r = redactContent("key = AKIA" + "IOSFODNN7EXAMPLE", "toml");
    expect(r.redacted.length).toBeGreaterThan(0);
  });
