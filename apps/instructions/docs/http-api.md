...
| GET | `/v1/profiles/resolve` | Query: `hostname`, `os`, `arch`, `limit`; returns `{ profile, scanned, total, batch_limit, source_bounded, complete, truncated }`; `profile` is `null` when no machine matches. |
...
The served OpenAPI document and the generated `InstructionsV1Client` cover:
config CRUD and snapshots, profiles (list, create, get, update, delete,
resolve-by-machine), per-profile configs, config and asset bindings, and
stats. The remaining runtime routes listed above (`/v1/machines`,
`/v1/machines/applied`, `/v1/feedback`) are implemented but are not yet
represented in that document; use direct HTTP for those three.
