...
Token rotation requires an explicit idempotency key so plan and apply are identical. Apply atomically reserves a plan: concurrent apply and identical replanning while an apply is in flight are rejected, while an identical still-pending plan is safely reused without extending its expiry. Success or a validated non-application response (authentication, permission, validation, not-found, conflict, precondition, request-size, or rate-limit status) consumes the reservation. Network, TLS, timeout, redirect, 5xx, interrupted-response, malformed-response, oversized-response, and unclassified remote failures are ambiguous after a request may have reached the server, so they remain in flight for explicit reconciliation and are never automatically retried. Local settlement retries are short and bounded; if a successful remote mutation cannot be recorded after those retries, the CLI returns exit 9 with the request ID and must not be automatically rerun.
