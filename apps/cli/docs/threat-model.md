...
| Concurrent apply or replay after state change | Digest binds operation, target, request, profile, API origin, org, provider/API revision. Atomic in-flight reservations cannot be reset by replanning, reject concurrent apply, and consume on success or validated non-application status. Network/TLS/timeout, redirect, 5xx, and incomplete/invalid/unclassified remote outcomes remain closed for operator reconciliation across expiry and process death; bounded lock retries fail closed. An unrecorded successful remote settlement reports partial without automatically replaying. Server idempotency and version preconditions cover API races. |
