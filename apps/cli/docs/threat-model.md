...
| API impersonation, SSRF, or downgrade | HTTPS by default; explicit loopback opt-in; no redirects; credentialed private/link-local/metadata address rejection after DNS resolution; OpenAPI title/version/operation validation. |
...
| Plan replay after state change | Digest binds operation, target, request, profile, API origin, org, provider/API revision; pending markers expire and are consumed before execution. Server idempotency and version preconditions cover API races. |
