...
| Concurrent apply or replay after state change | Digest binds operation, target, request, profile, API origin, org, provider/API revision. Atomic in-flight reservations cannot be reset by replanning, reject concurrent apply, release only on observed transient transport failure, and consume on success/definitive response. Ambiguous in-flight state remains closed across expiry and process death; bounded lock retries fail closed. An unrecorded successful remote settlement reports partial without automatically replaying. Server idempotency and version preconditions cover API races. |
