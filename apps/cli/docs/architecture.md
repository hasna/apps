...
The parser applies route-specific flag allowlists before runtime construction. High-impact operations persist a digest-scoped ten-minute plan, atomically reserve it during apply, reject concurrent apply, release it only on transient transport failure, and consume it on success or definitive response. Ordinary config mutations and plan transitions share one atomic lock/update discipline with dead-owner, age-bounded stale-lock recovery.
