...
import { z } from "zod";
import { createIntakeClient, validateBinding, validateRequest, object, boundedText } from "../intake/client.js";
...
export function createIntakeMcpServer(env:Record<string,string|undefined>=process.env){
  ...
  const common={tenant_id:z.string().min(1).max(256),sink_id:z.string().uuid(),producer_id:z.string().uuid(),corpus_id:z.string().uuid(),source_authority_id:z.string().uuid()};
