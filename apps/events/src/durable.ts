...
const SCHEMA_V1_COLUMNS: Record<keyof typeof SCHEMA_V1_TABLE_SQL, SchemaColumnShape[]> = {
  channels: [
...
interface SchemaForeignKeyShape {
  table: string;
  from: string;
  to: string;
  onUpdate: string;
...
const EVENT_FOREIGN_KEY: SchemaForeignKeyShape = {
  table: "events",
  from: "event_id",
  to: "id",
  onUpdate: "NO ACTION",
...
const SCHEMA_V1_FOREIGN_KEYS: Record<keyof typeof SCHEMA_V1_TABLE_SQL, SchemaForeignKeyShape[]> = {
  channels: [],
  events: [],
...
const SCHEMA_V1_INDEXES: Record<keyof typeof SCHEMA_V1_TABLE_SQL, SchemaIndexShape[]> = {
  channels: [
...
interface SchemaIndexColumnRow {
  seqno: number;
  name: string;
}
...
export class DurableEventsBroker {
  ...
  private createSchemaV1(): void {
    for (const sql of Object.values(SCHEMA_V1_TABLE_SQL)) this.db.exec(`${sql};`);
    for (const sql of Object.values(SCHEMA_V1_INDEX_SQL)) this.db.exec(`${sql};`);
  }
  ...
  private assertSchemaV1(): void {
    ...
    assertSchemaShape(
      "application objects",
      objects.map(({ type, name, table }) => ({ type, name, table })),
      expectedObjects.map(({ type, name, table }) => ({ type, name, table })),
    );

    for (const table of Object.keys(SCHEMA_V1_TABLE_SQL) as Array<keyof typeof SCHEMA_V1_TABLE_SQL>) {
      const columns = (this.db.query(`PRAGMA table_info(${schemaIdentifier(table)})`).all() as SchemaColumnRow[])
        .map((column) => ({
          name: column.name,
          type: column.type,
      ...
      const foreignKeys = (this.db.query(`PRAGMA foreign_key_list(${schemaIdentifier(table)})`).all() as SchemaForeignKeyRow[])
        .map((foreignKey) => ({
      ...
      const indexes = (this.db.query(`PRAGMA index_list(${schemaIdentifier(table)})`).all() as SchemaIndexRow[])
        .map((index) => ({
          name: index.name,
          unique: Number(index.unique),
          origin: index.origin,
          partial: Number(index.partial),
          columns: (this.db.query(`PRAGMA index_info(${schemaIdentifier(index.name)})`).all() as SchemaIndexColumnRow[])
  ...
  private applicationSchemaObjects(): SchemaObject[] {
    return (this.db.query(`
      SELECT type, name, tbl_name, sql
      FROM sqlite_master
      WHERE substr(name, 1, 7) <> 'sqlite_'
      ORDER BY type, name
    `).all() as SchemaObjectRow[]).map((row) => ({
      type: row.type,
      name: row.name,
      table: row.tbl_name,
      sql: row.sql,
  ...
  private assertEmptyApplicationSchema(): void {
...
function schemaIndexTable(name: string): string {
  if (name === "events_dedupe_key_unique" || name === "events_source_type_idx") return "events";
  if (name === "outbox_due_idx") return "outbox";
  throw new Error(`Unknown durable schema index: ${name}`);
}

function schemaIdentifier(value: string): string {
  if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(value)) throw new Error("Invalid durable schema identifier");
  return value;
}

function compareSchemaObjects(
  left: Pick<SchemaObject, "type" | "name">,
  right: Pick<SchemaObject, "type" | "name">,
): number {
...
function assertSchemaShape(label: string, actual: unknown, expected: unknown): void {
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw incompatibleSchema(label);
}

function incompatibleSchema(detail: string): Error {
