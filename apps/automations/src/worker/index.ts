...
function requireAutomationForReference(
  ...
  let exact: AutomationRecord | undefined;
  let exactLookupError: unknown;
  try {
    exact = store.requireAutomation(slug);
  } catch (error) {
    exactLookupError = error;
  }
  if (exact?.spec.version === version) return exact;
