...
import type {
  ...
  MaterializedWebhookRequest, QueueLeaseOptions, QueuedAction, WebhookRequestInput,
  WebhookRoute, WebhookRouteStatus,
} from "../types.js";
import type { AutomationsStore, CreateWebhookRouteInput, AdmitActionInput } from "../lib/store.js";
...
export interface LeaseMutationOptions { actionId: string; runnerId: string; fencingToken: number; now?: string | Date }
...
export interface LeasedQueuedAction extends QueuedAction { fencingToken: number }
...
export interface ServerAutomationsStore {
  ...
  admitAction(input: AdmitActionInput): Promise<QueuedAction>;
  requireQueueEntry(id: string): Promise<QueuedAction>;
  listQueueEntries(options?: ListPageOptions): Promise<QueuedAction[]>;
  listDeadLetterActions(options?: ListPageOptions): Promise<QueuedAction[]>;
  leaseNextAction(options: QueueLeaseOptions): Promise<LeasedQueuedAction | undefined>;
  ...
  readmitDeadAction(id: string, options?: { now?: string | Date; requestedBy?: string; reason?: string }): Promise<QueuedAction>;
