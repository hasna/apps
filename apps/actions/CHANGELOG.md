...
- 9e6ed7003: fix(actions): a delayed breaker's lock-owner restore is now atomic and non-clobbering — it can never replace a successor's owner file in a recreated lock directory (release + reacquire while the breaker was delayed), so a stale owner with a dead pid can never sit on a live writer's lock (release-review P1, cycle-2 remediation)
