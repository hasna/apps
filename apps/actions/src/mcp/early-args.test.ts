 ...
 *
 * actions-mcp --version/--help previously fell straight into the stdio
 * transport and printed nothing (silent-empty family). Both must answer rc=0
 * before the transport connect.
 *
 * The probes are two-sided: --version/--help must answer rc=0 with version or
 * usage on stdout (positive), and plain actions-mcp must STILL reach the stdio
 * path — with stdin closed it ends, printing neither version nor usage
 * (negative).
 */

const ACTIONS_ROOT = join(import.meta.dir, "../..");
...
describe("actions-mcp answers --version/--help before any transport (row 7e5f8f3d)", () => {
  test("--version prints the package version and exits", async () => {
    const result = await runMcp(["--version"]);
    expect(result.timedOut).toBe(false);
    expect(result.exitCode).toBe(0);
    expect(result.stdout.trim()).toBe(ACTIONS_VERSION);
  ...
  test("--help prints usage and exits", async () => {
    ...
    expect(result.stdout).toContain("actions-mcp");
  });

  test("plain actions-mcp still reaches the stdio path (negative probe)", async () => {
    ...
    expect(result.timedOut).toBe(false);
    expect(result.stdout.trim()).not.toBe(ACTIONS_VERSION);
