...
function boundedJsonRows<T>(rows: T[], options: { limit: number; cursor?: string }) {
...
program
  ...
  .action(async (opts: { by?: string; since?: string; limit?: string; cursor?: string; verbose?: boolean; full?: boolean; json?: boolean }) => {
    ...
    if (by === 'project') {
      ...
      if (opts.json) {
        console.log(opts.full ? JSON.stringify(rows, null, 2) : JSON.stringify({ by, since: since ?? null, ...boundedJsonRows(rows, { limit, cursor: opts.cursor }) }))
        return
      }
...
program
  ...
  .action(async (periodArg: string | undefined, opts: { limit?: string; cursor?: string; verbose?: boolean; full?: boolean; json?: boolean }) => {
    ...
    if (opts.json) {
      ...
      console.log(opts.full ? JSON.stringify(rows, null, 2) : JSON.stringify(boundedJsonRows(rows, { limit, cursor: opts.cursor })))
      return
    }
