 ...
 * never writes a SQLite file under the app home (hasna/apps#1720).
 * `HASNA_AGENT_REGISTRY_DB_PATH` is a local-lane path override only; hosted
 * transport ignores it and always uses the process-local in-memory registry.
 *
...
/**
 * Where the registry lives. The storage seam decides first: a hosted client
 * (a credential resolved through the @hasna/contracts chain) always gets the
 * in-memory registry, even when `HASNA_AGENT_REGISTRY_DB_PATH` is present.
 * Only the explicit local opt-in may honor that path override or persist the
 * default `agent-registry.db` beside economy's store. With neither, the seam
 * throws its fail-closed error (the same one the server's own store raised at
 * startup).
 */
export function resolveRegistryDbPath(): string {
  // Resolve the authority lane before consulting any filesystem path. A stale
  // or injected registry path must never turn a hosted MCP process into a
  // persistent local writer. `economyCloudStorage()` returns inactive only for
  // the explicit HASNA_ECONOMY_LOCAL=1 lane; every unconfigured or malformed
  // hosted setup refuses before this function can choose a file.
  if (economyCloudStorage().active) return MEMORY_REGISTRY_PATH
  const explicit = process.env.HASNA_AGENT_REGISTRY_DB_PATH?.trim()
  return explicit || join(getDataDir(), 'agent-registry.db')
}
