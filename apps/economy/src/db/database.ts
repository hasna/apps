...
/**
 * The resolver economy data root: kind overrides honored,
 * `~/.hasna/economy` on macOS, XDG data root on Linux.
 */
export function getResolverDataRoot(): string {
  return resolverDataDir({ app: 'economy', home: getHomeDir() })
}

/**
 * The pre-ruling legacy root (`~/.hasna/economy`). On macOS this equals the
 ...
 * the resolver data root (ruling #1668 — the resolver root IS the convention
 * on every platform). The store path (`HASNA_ECONOMY_DB_PATH` / `ECONOMY_DB`)
 * is layered on top of this by `getDbPath`, so an explicit store path always
 * wins regardless. The legacy `~/.economy` files are auto-migrated into the
