...
export const openApiSpec: Record<string, unknown> = {
  ...
  "info": {
    ...
    "version": "0.0.0",
    "description": "AI coding cost tracker — deployed economy control plane. The server data backend is sqlite | postgresql, selected by the presence of a database URL; with postgresql the serve process reads AND writes PostgreSQL directly with API-key auth via @hasna/contracts. All /v1 routes require a valid `economy:*` scoped API key (x-api-key or Authorization: Bearer). The foundation probes /health, /ready, /version are open."
  },
  "servers": [
    {
      "url": "http://localhost:3456",
      "description": "local default; set your own URL when deploying"
    }
  ],
  "security": [
    {
