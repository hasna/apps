...
//
// This is the hosted-API storage client for a CLI/MCP/SDK. It sits on top of
...
//
// An app's storage resolver selects this client when the env contract resolves
// to `cloud-http` (API_URL + API_KEY set), and falls through to the local store
// otherwise. See `resolveClientTransport` / `createClientTransport` in
// ./transport.ts.
//
...
/**
 * The one call an app's storage resolver makes. Reads the env contract for
 * `name`; when it resolves to `cloud-http` (API_URL + API_KEY set), returns a
 * ready {@link HasnaStorageClient}. Otherwise returns
 * `{ transport: 'local', client: null }` so the app uses its local store.
 * Throws if the hosted client was requested but is misconfigured (API URL set
 * without a key), so callers never silently read the wrong dataset.
 */
