...
waiting=false
if [[ "$#" == 3 ]]; then
  [[ "$3" == --wait-for-route && "$base" == https://api.hasna.com/trash ]] || exit 2
  waiting=true
fi
[[ "$base" =~ ^https://[a-z0-9.-]+(/[a-z0-9-]+)?$ && "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || exit 2
max_seconds="${TRASH_PUBLIC_VERIFY_TIMEOUT_SECONDS:-1200}"
delay="${TRASH_PUBLIC_VERIFY_DELAY_SECONDS:-10}"
...
while true; do
  ...
  case "$status" in
    200)
      jq -es --arg version "$version" 'length == 1 and (.[0] | type == "object" and .status == "ready" and .storage == "postgresql" and .version == $version)' "$body" >/dev/null 2>&1 || {
