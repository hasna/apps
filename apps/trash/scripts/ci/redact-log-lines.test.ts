...
describe("ECS log evidence redaction", () => {
  test("redacts credential-shaped values and preserves migration evidence", () => {
    const result = redactLogText([
      "DATABASE_URL=postgresql://service:password@example.test/trash?sslmode=require",
      "Authorization: Bearer header.payload.signature",
      "HASNA_TRASH_API_SIGNING_KEY=top-secret-value",
      "trash-serve fatal: Applied migration 'trash:0002_profiles' (checksum 'sha256:abc123') is not recognized.",
    ].join("\n"));
    ...
    expect(result).toContain("trash:0002_profiles");
