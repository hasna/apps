...
export const REQUIRED_SUITES = Object.freeze({
  ...
  "src/server/events-intake-delivery.pg.test.ts": 17,
...
export function validateTestDatabaseUrl(value: string | undefined): string {
  const refuse = () => { throw new Error("CONVERSATIONS_TEST_DATABASE_URL must name the disposable conversations_test user/database on literal 127.0.0.1 with an explicit port and no URL options"); };
  if (!value || /[\s\x00-\x1f\x7f?#]/.test(value)) return refuse();
  let url: URL;
  try { url = new URL(value); } catch { return refuse(); }
  if (!["postgres:", "postgresql:"].includes(url.protocol)
    || url.hostname !== "127.0.0.1" || url.username !== "conversations_test"
    || !["", "conversations_test"].includes(url.password) || url.pathname !== "/conversations_test"
...
export function buildTestEnv(source: NodeJS.ProcessEnv, home: string): Record<string, string> {
  return {
    ...
    CONVERSATIONS_TEST_DATABASE_URL: validateTestDatabaseUrl(source.CONVERSATIONS_TEST_DATABASE_URL),
  };
}
...
export function discoverSuites(root: string): string[] {
  ...
  const walk = (directory: string) => {
    for (const item of readdirSync(directory, { withFileTypes: true })) {
      ...
      else if (item.name.endsWith(".test.ts") && readFileSync(path, "utf8").includes("process.env.CONVERSATIONS_TEST_DATABASE_URL")) paths.push(relative(root, path));
    }
  };
  walk(join(root, "src"));
  return paths.sort();
...
export function assertSuiteInventory(actual: string[]): void {
  if (JSON.stringify([...actual].sort()) !== JSON.stringify(Object.keys(REQUIRED_SUITES).sort())) {
    throw new Error("Required Conversations PostgreSQL suite inventory changed; update and review the executed-case census");
  }
}
...
export function runLivePostgresTests(source = process.env): void {
  ...
  validateTestDatabaseUrl(source.CONVERSATIONS_TEST_DATABASE_URL);
  const root = resolve(import.meta.dir, "..");
  assertSuiteInventory(discoverSuites(root));
  const home = mkdtempSync(join(tmpdir(), "conversations-postgres-gate-"));
  let failed = false;
  try {
    ...
    if (failed) throw new Error("Conversations PostgreSQL gate failed: every required case must execute without skips");
...
if (import.meta.main) {
  try { runLivePostgresTests(); } catch (error) {
    console.error(error instanceof Error ? error.message : "Conversations PostgreSQL gate failed");
    process.exitCode = 1;
  }
}
