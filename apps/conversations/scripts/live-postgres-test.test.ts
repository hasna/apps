...
const url = "postgresql://conversations_test@127.0.0.1:5432/conversations_test";
const suite = "src/cli/project-registration.e2e.test.ts";
const success = { status: 0, signal: null, stdout: "", stderr: "(pass) credentials boundary\n(pass) durable registration\n\n 2 pass\n 0 fail\nRan 2 tests across 1 file. [1.00s]\n" };
...
test("only the explicitly disposable loopback test target is accepted without echoing rejected values", () => {
  ...
  for (const invalid of [undefined, "", url.replace("127.0.0.1", "localhost"), url.replace("127.0.0.1", "db.example.invalid"), url.replace("conversations_test@", "other@"), url + "?options=-csearch_path=public", url.replace(":5432", ""), url.replace("@", ":synthetic-private-value@")]) {
    let message = "";
    try { validateTestDatabaseUrl(invalid); } catch (error) { message = String(error); }
    expect(message).toContain("CONVERSATIONS_TEST_DATABASE_URL");
...
test("the gate drops ambient credentials, local database paths and client profile selectors", () => {
  const env = buildTestEnv({ PATH: "/usr/bin", CONVERSATIONS_TEST_DATABASE_URL: url, DATABASE_URL: "synthetic-private-value", PGHOST: "foreign", HASNA_CONVERSATIONS_API_KEY: "synthetic-private-value", CONVERSATIONS_DB_PATH: "/tmp/legacy.db", HASNA_PROFILE: "foreign", HASNA_STATION: "real-station" }, "/tmp/synthetic-gate-home");
  expect(Object.keys(env).sort()).toEqual(["PATH", "HOME", "USERPROFILE", "TMPDIR", "NO_COLOR", "FORCE_COLOR", "HASNA_STATION", "CONVERSATIONS_TEST_DATABASE_URL"].sort());
...
test("required suite inventory includes every real PG file and rejects removals/additions/duplicates", () => {
  ...
  expect(Object.values(REQUIRED_SUITES).reduce((total, count) => total + count, 0)).toBe(32);
...
test("a crash, skip, partial output or stale passing census never certifies PostgreSQL", () => {
  ...
  for (const result of [
    ...
    { ...success, stderr: success.stderr.replace("2 pass", "1 pass") },
    { ...success, stderr: success.stderr.replace("(pass) durable registration", "(skip) durable registration").replace("2 pass", "1 pass\n 1 skip") },
    { ...success, stderr: success.stderr.replace("0 fail", "1 fail") },
    { ...success, stderr: success.stderr.replace("1 file", "2 files") },
    { ...success, stderr: success.stderr.replace("(pass) durable registration\n", "") },
...
test("the required workflow uses read-only credentials and executes the no-skip runner", () => {
  const workflow = readFileSync(resolve(import.meta.dir, "../../../.github/workflows/conversations-live-postgres.yml"), "utf8");
  expect(workflow).toContain("contents: read");
  expect(workflow).toContain("persist-credentials: false");
  expect(workflow).not.toContain("continue-on-error");
  expect(workflow).not.toContain("secrets.");
  expect(workflow).toContain("bun run test:postgres");
  expect(workflow).toContain("CONVERSATIONS_TEST_DATABASE_URL: postgres://conversations_test@127.0.0.1:5432/conversations_test");
