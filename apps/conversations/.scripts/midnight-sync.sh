...
if ! git diff --quiet || ! git diff --cached --quiet || [ -n "$(git ls-files --others --exclude-standard)" ]; then
    ...
    if git diff --cached --diff-filter=ACM -- '*.env' '*.json' '*.toml' '*.ts' '*.js' '*.md' '*.sh' '*.yml' '*.yaml' | grep -qiE 'sk-ant-|sk-proj-|npm_[a-zA-Z]|gho_|ghp_|secret-token:|ctx7sk-|xai-|AIza[a-zA-Z0-9]|AKIA[A-Z0-9]'; then
