...
const MAX_COMPLETE_COLLECTION_PAGES = 10_000;
const MAX_COLLECTION_RESTARTS = 3;
...
function isCollectionChanged(error: unknown): boolean {
  ...
  ) || (error instanceof Error && /collection changed/i.test(error.message));
}

function exactResponseBytes(page: ProjectChannelCollectionPage): number {
  let responseBytes = 0;
  for (let index = 0; index < 16; index++) {
    const next = Buffer.byteLength(
      JSON.stringify({ ...page, response_bytes: responseBytes }),
      "utf8",
    );
    if (next === responseBytes) return next;
    responseBytes = next;
  }
  throw new Error("complete project channel collection response_bytes did not converge.");
}

async function listAllProjectChannels(input: {
  project_id: string;
  page_size: number;
  ...
  for (let restart = 0; restart <= MAX_COLLECTION_RESTARTS; restart++) {
    let cursor: string | undefined;
    let collectionRevision: string | undefined;
    let first: ProjectChannelCollectionPage | null = null;
    const items: ProjectChannelCollectionItem[] = [];
    const seenIds = new Set<string>();
    const seenCursors = new Set<string>();
    try {
      for (let pageNumber = 1; pageNumber <= MAX_COMPLETE_COLLECTION_PAGES; pageNumber++) {
        const page = await store.listProjectChannelRegistrationPage({
        ...
        first ??= page;
        collectionRevision ??= page.collection_revision;
        if (page.collection_revision !== collectionRevision) {
          throw Object.assign(
            new Error("project channel collection changed during complete traversal"),
            { code: PROJECT_CHANNEL_COLLECTION_CHANGED },
          );
        }
        for (const item of page.items) {
          if (seenIds.has(item.target_id)) {
            throw new Error(`project channel collection repeated stable id ${item.target_id}.`);
          }
        ...
        if (!page.has_more) {
          if (page.next_cursor !== null) {
            throw new Error("project channel collection returned a cursor while claiming no more results.");
          }
          const result: ProjectChannelCollectionPage = {
            ...first,
            items,
            cursor: null,
            next_cursor: null,
          ...
          result.response_bytes = exactResponseBytes(result);
          if (result.response_bytes > input.response_byte_limit) {
            throw new Error(
              `complete project channel collection exceeded response_byte_limit (${result.response_bytes} > ${input.response_byte_limit}).`,
            );
          }
          return result;
        }
        if (!page.next_cursor) {
          throw new Error("project channel collection claimed more results without a cursor.");
        }
        if (seenCursors.has(page.next_cursor)) {
          throw new Error("project channel collection repeated a continuation cursor.");
        }
        seenCursors.add(page.next_cursor);
        cursor = page.next_cursor;
      }
      throw new Error(
        `project channel collection exceeded ${MAX_COMPLETE_COLLECTION_PAGES} pages.`,
      );
    } catch (error) {
      if (!isCollectionChanged(error) || restart === MAX_COLLECTION_RESTARTS) {
        throw error;
      }
    }
  }
  throw new Error("project channel collection restart budget was exhausted.");
}
...
export function registerProjectRegistrationCommands(program: Command): void {
  ...
  registration
    ...
    .option("--collection-revision <sha256>", "Collection revision returned by the first page; required with --cursor")
    .option("--all", "Traverse all pages with bounded mutation restarts")
    ...
    .action(async (opts) => {
      try {
        const maxItems = pageLimit(opts.limit, opts);
        const bounds = collectionBounds(opts);
        if (opts.all && (opts.cursor || opts.collectionRevision)) {
          emitCliError("--all cannot be combined with --cursor or --collection-revision.", opts);
        }
        printJson(opts.all
          ? await listAllProjectChannels({
            project_id: opts.project,
            page_size: maxItems,
            ...bounds,
          })
          : await getStore().listProjectChannelRegistrationPage({
            project_id: opts.project,
            cursor: opts.cursor,
            collection_revision: opts.collectionRevision,
            max_items: maxItems,
            ...bounds,
          }));
      } finally {
