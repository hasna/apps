...
function serializeRecords(records: Array<Record<string, unknown>>, format: ExportFormat): string {
...
export function serializeMessageExport(
  ...
  options: { format: ExportFormat; detail: "preview"; maxBytes: number; hasMore?: boolean },
): SerializedMessageExport {
  const included: Array<Record<string, unknown>> = [];
  let skippedCount = 0;
  for (const record of records) {
    const candidate = serializeRecords([...included, record], options.format);
  ...
  const payload = serializeRecords(included, options.format);
