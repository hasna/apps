...
describe("bounded message collection projections", () => {
  ...
  test("default list returns a byte-capped redacted projection without full content or read mutations", () => {
    ...
    const sent = sendMessage({ from: "alice", to: "bob", content: "legacy body placeholder", reply_to: undefined });
    getDb().prepare("UPDATE messages SET content = ? WHERE id = ?").run(fullBody, sent.id);
    ...
    expect(getMessageById(sent.id)?.content).toContain("[REDACTED:BEARER_TOKEN]");
    expect(getMessageById(sent.id)?.content).not.toContain(syntheticBearer);
  ...
  test("incident and security collection projections never expose a body or body-derived snippet", () => {
    ...
    const sent = sendMessage({
      ...
      content: "legacy restricted placeholder",
      blocking: true,
    });
  ...
  test("search and unread blocker collections return projections and remain non-mutating", () => {
    ...
    const secret = sendMessage({ from: "alice", to: "bob", content: "needle legacy secret placeholder" });
    getDb().prepare("UPDATE messages SET content = ? WHERE id = ?").run(`needle secret ${syntheticBearer}`, secret.id);
