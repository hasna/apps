// LocalStore — the on-box SQLite store. TEST-ONLY as of the API-only client
// cutover: no shipped bin (conversations, conversations-mcp, conversations-hook,
// conversations-serve) and no public export (`@hasna/conversations`, `./sdk`)
// imports this module, so `bun:sqlite` never reaches a client bundle
// (`src/client-bundles-sqlite-free.test.ts` proves it). The SQLite domain
// helpers it delegates to remain in-tree only for their own unit tests until
// they are deleted with the legacy corpus tooling.
...
export class LocalStore implements ConversationsStore {
  ...
  readChannelNotifications: ConversationsStore["readChannelNotifications"] = async (opts) =>
    notificationsLib.readChannelNotifications(opts);
  ...
  searchMessagePreviews: ConversationsStore["searchMessagePreviews"] = async (opts) =>
    messagesLib.searchMessagePreviews(opts);
  ...
  exportMessages: ConversationsStore["exportMessages"] = async (opts = {}) =>
    messagesLib.createMessageExport(opts);
  ...
  getUnreadBlockerPreviews: ConversationsStore["getUnreadBlockerPreviews"] = async (agent, opts = {}) =>
    messagesLib.getUnreadBlockerPreviews(agent, opts);
  readMentionPreviews: ConversationsStore["readMentionPreviews"] = async (agent, opts = {}) =>
    messagesLib.readMentionPreviews(agent, opts);
