...
describe("parseStoredWriteTime — stored write-time anchors on both backends", () => {
  test("parses the SQLite UTC wall-clock string form", () => {
    const ms = parseStoredWriteTime("2026-08-17T11:59:00.123");
    expect(ms).toBe(Date.parse("2026-08-17T11:59:00.123Z"));
  });

  test("parses a zone-suffixed string form unchanged", () => {
    expect(parseStoredWriteTime("2026-08-17T11:59:00.000Z")).toBe(
      Date.parse("2026-08-17T11:59:00.000Z"),
    );
    expect(parseStoredWriteTime("2026-08-17T13:59:00.000+02:00")).toBe(
      Date.parse("2026-08-17T13:59:00.000+02:00"),
    );
  });

  test("parses a Date object exactly as the Postgres driver returns timestamptz", () => {
    // P1 regression (successor of hasna/apps#252): the pg driver returns
    // timestamptz columns as Date objects. parseStoredWriteTime treated every
    // non-string as 0, so the write-time window check (Math.abs(now - 0) >
    // window) ALWAYS passed on the real backend and the stored-row dedupe was
    // silently disabled — the fake test client returned ISO strings and masked
    // the gap.
    const stored = new Date(Date.parse("2026-08-17T11:59:00.000Z"));
    expect(parseStoredWriteTime(stored)).toBe(Date.parse("2026-08-17T11:59:00.000Z"));
  });

  test("returns 0 for an invalid or absent value", () => {
    expect(parseStoredWriteTime(null)).toBe(0);
    expect(parseStoredWriteTime(undefined)).toBe(0);
    expect(parseStoredWriteTime("not-a-time")).toBe(0);
  });
});
...
describe("assertNotDuplicateWorkStatusTransition — one event per real transition", () => {
  ...
  test("rejects a duplicate when the previous stored write time is a Date object", () => {
    // P1 regression (successor of hasna/apps#252): on Postgres the stored
    // created_at arrives as a Date object. Before the fix, the Date parsed to
    // 0, so Math.abs(now - 0) always exceeded the window and the dedupe never
    // fired on the real backend. The window must be measured from the Date's
    // epoch ms exactly as from a string form.
    const first = parseWorkStatusEvent(validEvent("START", { at: NOW }))!;
    const second = parseWorkStatusEvent(validEvent("START", { at: NOW }))!;
    const storedAsDate = new Date(Date.parse("2026-08-17T11:59:30.000Z"));
    expect(() =>
      assertNotDuplicateWorkStatusTransition(first, second, parseStoredWriteTime(storedAsDate), NOW_MS),
    ).toThrow(/duplicate START event/);
  });
