...
describe("G4 — MCP collection reads do not mutate by default", () => {
  ...
  test("default compact read_channel leaves matching channel notifications unread", async () => {
    ...
    expect((after.notifications as Array<{ channel: string }>).map((n) => n.channel).sort()).toEqual([
      "peek-chan",
      "peek-chan-other",
    ]);
  });
  ...
  test("default verbose read_channel leaves matching channel notifications unread", async () => {
    ...
    await call("subscribe_channel_notifications", { from: "peek-watcher-verbose", channel: "peek-chan-verbose" });
    await call("send_to_channel", { channel: "peek-chan-verbose", from: "peek-writer", content: "channel body verbose" });

    expect((await call("read_channel_notifications", { from: "peek-watcher-verbose" })).count).toBe(1);
    await call("read_channel", {
      channel: "peek-chan-verbose",
      from: "peek-watcher-verbose",
      limit: 10,
      verbose: true,
    });
    expect((await call("read_channel_notifications", { from: "peek-watcher-verbose" })).count).toBe(1);
