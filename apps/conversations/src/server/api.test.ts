...
import { makeFakeClient } from "../lib/store/test-support/api-query-fixture.js";
...
describe("H3 /v1/messages/read mention acknowledgements", () => {
  ...
  test("mention_ids WITHOUT mentions_only is refused 400 — the discriminator is required", async () => {
    ...
    // The former broken ApiStore wire shape omitted this discriminator.
    // Reject it explicitly and leave the mention untouched.
    const res = await fetch(`${base}/v1/messages/read`, {
      method: "POST",
      headers: { "x-api-key": rwKey, "content-type": "application/json" },
    ...
    expect(res.status).toBe(400);
    expect(await res.json()).toEqual({ error: "mention_ids requires mentions_only=true." });
