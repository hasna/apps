 ...
 * A request carrying `mention_ids` WITHOUT `mentions_only` falls through every
 * branch and reports `{marked: 0}` — the silent no-op a weak happy-path test
 * never sees. The ApiStore wire shape (markMentionsReadByIds) is exactly that
 * shape, so the discriminator is a load-bearing contract, not an option.
 */
