# Hasna Notes Sync — Client Design and Conflict Policy

The sync client (`sync/`) connects the canonical local markdown store
(`~/.hasna/apps/notes/notes/`) to a Hasna Notes server over `POST
/api/v1/sync`. It is **one dialect with two backends**: the hosted service
(`http://127.0.0.1:8788`, the default) or any self-hosted server that
implements the same `/api/v1` protocol. The base URL is the only switch:
`HASNA_NOTES_API_URL`, config `apiUrl`, or the default. Device-code auth
(`notes auth device`) works identically against either.

```
notes sync [--dry-run] [--json] [--api-url https://...]
notes sync --watch [--interval 5m] [--log-file path]
notes sync status [--json]
notes sync --install-service [--dry-run] | --uninstall-service
```
...
**Daemon** — `notes sync --watch` runs two triggers:

1. an interval poll — `syncIntervalMinutes` from the client config
   (`~/.config/hasna-notes/config.json`), `--interval`, or
   `HASNA_NOTES_SYNC_INTERVAL_MINUTES`; default **5 minutes**, floor **1
...
`notes sync status` prints it; the app shows it in Settings →
...
**Logs** — `~/Library/Logs/HasnaNotes/sync.log` on macOS,
`$XDG_STATE_HOME/hasna-notes/sync.log` (default
`~/.local/state/hasna-notes/sync.log`) on Linux; one line per run, rotated
once at 512 KiB (`sync.log.1`).

**Install story** — `notes sync --install-service` writes the
user-level service that keeps the daemon alive and prints the enable command:

- macOS: `~/Library/LaunchAgents/com.hasna.notes.sync.plist` (launchd,
  RunAtLoad + KeepAlive), then `launchctl load <plist>`.
- Linux: `~/.config/systemd/user/notes-sync.service`, then
  `systemctl --user enable --now notes-sync`. Linux is a first-class
...
(`SyncScheduler` in `HasnaNotesApp/main.swift`, its own serial queue,
never the main thread) that spawns the bundled CLI (`Resources/bin/
notes.mjs sync --json`) on the same interval setting, then rebuilds
