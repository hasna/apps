# Hasna Notes — App Icon & Menu-Bar Visual System

The visual system for the Hasna Notes app icon and the macOS menu-bar surface:
...
the build**: `scripts/build_notes.sh` assembles `AppIcon.icns` from the
...
Master: [`assets/brand/notes-icon.svg`](../assets/brand/notes-icon.svg)
...
The status item lives in `Sources/HasnaNotesApp/main.swift` (`showStatusItem`)
...
- Masters:
  [`assets/brand/notes-menubar.svg`](../assets/brand/notes-menubar.svg)
  (idle card) and
  [`assets/brand/notes-menubar-rec.svg`](../assets/brand/notes-menubar-rec.svg)
...
| Asset | Consumed by |
|-------|-------------|
| `notes-icon.svg` → `assets/brand/AppIcon.iconset/` → `AppIcon.icns` | `tools/render-appicon.mjs` rasterizes the checked-in iconset; `scripts/build_notes.sh` assembles the `.icns` with `iconutil` at build time (graceful skip off-macOS) and sets `CFBundleIconFile` only when the icns exists. |
| `notes-menubar*.svg` | Bundled to `Resources/brand/` by `scripts/build_notes.sh`; `showStatusItem`/`refreshStatusTitle` in `Sources/HasnaNotesApp/main.swift` render them as template images. |
