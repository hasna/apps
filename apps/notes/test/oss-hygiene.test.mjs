...
// The UI-surface and HTML-branding guards were removed with the macOS desktop app
// (`Sources/`, `web/`) — this package is headless (CLI, MCP server, SDK, HTTP
// server) and ships no browser UI, so both guards would pass vacuously over an
// empty set of HTML files. The desktop app and its UI guards now live in
