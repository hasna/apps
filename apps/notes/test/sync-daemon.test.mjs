...
async function tempRoot(t) {
  const root = await mkdtemp(join(tmpdir(), 'notes-daemon-'));
...
test('serviceDefinition: launchd plist points at the CLI daemon', () => {
  const def = serviceDefinition({
    ...
    scriptPath: '/opt/notes/bin/notes.mjs',
    home: '/Users/casey',
    env: { HASNA_NOTES_API_URL: 'http://linux-box.local:8788' },
  });
  assert.equal(def.path, '/Users/casey/Library/LaunchAgents/com.hasna.notes.sync.plist');
  assert.match(def.content, /<string>\/usr\/local\/bin\/node<\/string>/);
  assert.match(def.content, /<string>\/opt\/notes\/bin\/notes\.mjs<\/string>/);
  assert.match(def.content, /<string>sync<\/string>\s*<string>--watch<\/string>/);
  assert.match(def.content, /<key>KeepAlive<\/key><true\/>/);
  assert.match(def.content, /HASNA_NOTES_API_URL/);
  assert.match(def.content, /Library\/Logs\/HasnaNotes\/sync-launchd\.log/);
...
test('serviceDefinition: systemd user unit points at the CLI daemon (Linux parity)', () => {
  const def = serviceDefinition({
    ...
    scriptPath: '/opt/notes/bin/notes.mjs',
    home: '/home/casey',
    env: { HASNA_NOTES_ROOT: '/home/casey/.hasna/apps/notes' },
  });
  assert.equal(def.path, '/home/casey/.config/systemd/user/notes-sync.service');
  assert.match(def.content, /ExecStart=\/usr\/bin\/node \/opt\/notes\/bin\/notes\.mjs sync --watch/);
  assert.match(def.content, /Restart=on-failure/);
  assert.match(def.content, /Environment=HASNA_NOTES_ROOT=\/home\/casey\/\.hasna\/apps\/notes/);
  ...
  assert.ok(def.instructions.some(step => step.includes('systemctl --user enable --now notes-sync')));

  // XDG_CONFIG_HOME is honored.
  const xdg = serviceDefinition({
    platform: 'linux',
    scriptPath: '/x/bin/notes.mjs',
    home: '/home/casey',
    env: { XDG_CONFIG_HOME: '/home/casey/cfg' },
  });
  assert.equal(xdg.path, '/home/casey/cfg/systemd/user/notes-sync.service');
  ...
  const spaced = serviceDefinition({
    ...
    scriptPath: '/opt/notes/bin/notes.mjs',
    home: '/home/casey',
    env: { HASNA_NOTES_ROOT: '/home/casey/My Notes' },
  });
  assert.match(spaced.content, /ExecStart="\/opt\/tools with space\/node" \/opt\/notes\/bin\/notes\.mjs sync --watch/);
  assert.match(spaced.content, /Environment="HASNA_NOTES_ROOT=\/home\/casey\/My Notes"/);
});
...
test('installService/uninstallService: write and remove the unit file', async (t) => {
  ...
  assert.match(await readFile(installed.path, 'utf8'), /notes\.mjs sync --watch/);
...
test('syncLogPath: macOS Library/Logs vs Linux XDG state home', () => {
  assert.equal(
    ...
    '/Users/casey/Library/Logs/HasnaNotes/sync.log',
  );
  assert.equal(
    syncLogPath({ platform: 'linux', home: '/home/casey', env: {} }),
    '/home/casey/.local/state/hasna-notes/sync.log',
  );
  assert.equal(
    syncLogPath({ platform: 'linux', home: '/home/casey', env: { XDG_STATE_HOME: '/var/state' } }),
    '/var/state/hasna-notes/sync.log',
  );
});
...
test('CLI: notes sync status reports never/ok/error honestly', async (t) => {
  const root = await tempRoot(t);
  const env = { HASNA_NOTES_ROOT: root, HASNA_NOTES_CONFIG: join(root, 'config.json') };
...
test('CLI: sync --install-service writes the platform service file', async (t) => {
  ...
  const env = {
    ...
    HASNA_NOTES_ROOT: join(home, 'notes-root'),
  };
  ...
  if (out.platform === 'linux') {
    assert.ok(out.path.endsWith('notes-sync.service'));
    assert.match(body, /Environment=HASNA_NOTES_ROOT=/);
  } else {
    assert.ok(out.path.endsWith('com.hasna.notes.sync.plist'));
  }
...
test('CLI: one-shot sync skips (exit 0) when another run holds the lock', async (t) => {
  ...
  const res = await runNode(['sync', '--json'], {
    HASNA_NOTES_ROOT: root,
    HASNA_NOTES_CONFIG: join(root, 'config.json'),
    HASNA_NOTES_API_KEY: 'pn_test_0000000000000000000000000000',
  });
  assert.equal(res.code, 0, res.stderr);
  const out = JSON.parse(res.stdout);
