...
test('web chat composer: an Enter that only commits an IME candidate must not send', async () => {
  ...
  windowTarget.HasnaNotes.hydrate({ thisMachine: 'studio-mac', notes: [], machines: [{ id: 'studio-mac' }] });
...
test('web inline markdown placeholder tokens cannot be spoofed by NUL bytes', async () => {
  ...
  const html = windowTarget.HasnaNotes.markdown.render('`safe` ' + NUL + '0' + NUL + ' [link](https://example.com/a) ' + NUL + '1' + NUL);
...
test('web sidebar defaults to the latest 10 notes and keeps pagination across hydrates', async () => {
  ...
  windowTarget.HasnaNotes.hydrate({ thisMachine: 'studio-mac', notes, machines: [{ id: 'studio-mac' }], listDefaults: { limit: 6 } });
...
test('recording and realtime transcription contracts are exposed to UI/native host', async () => {
  ...
  const buildScript = await readFile(join(repoRoot, 'scripts', 'build_notes.sh'), 'utf8');
