...
import { NotesClient } from '../sync/client.mjs';
import {
  loadSyncState,
  runSync,
  saveSyncState,
...
// Synthetic mock key (non-matching sentinel so the credential scanner stays silent).
const apiKey = 'mock-api-key-0000000000000000000000000000';

async function tempRoot(t) {
  const root = await mkdtemp(join(tmpdir(), 'notes-sync-'));
...
async function createMockServer(t, { pageSize = 100, feed = 'hosted' } = {}) {
  ...
  function handleSync(req, raw, res) {
    if (req.headers.authorization !== `Bearer ${apiKey}`) return json(res, 401, { error: { code: 'unauthorized', message: 'bad key' } });
    ...
    req.on('end', () => {
      try {
        ...
        if (req.method === 'POST' && req.url === '/api/v1/auth/device/token') {
          ...
          return json(res, 200, { status: 'approved', approved: true, apiKey });
        }
  ...
  return {
    ...
    client: () => new NotesClient({ apiUrl: url, apiKey }),
  };
}
...
test('device auth flow works against any server speaking the dialect', async (t) => {
  const mock = await createMockServer(t);
  const client = new NotesClient({ apiUrl: mock.url });
  ...
  assert.equal(approved.apiKey, apiKey);
});

test('CLI: notes sync pushes/pulls with --json and --dry-run', async (t) => {
  const mock = await createMockServer(t);
  const root = await tempRoot(t);
  const note = await saveNote(fixtureNote({ title: 'CLI note' }), root);
  const env = {
    HASNA_NOTES_ROOT: root,
    HASNA_NOTES_API_URL: mock.url,
    HASNA_NOTES_API_KEY: apiKey,
    HASNA_NOTES_CONFIG: join(root, 'client-config.json'),
  };
  ...
  const anon = await runNode(binPath, ['sync'], { ...env, HASNA_NOTES_API_KEY: '' });
