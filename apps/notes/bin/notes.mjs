...
Usage:
  notes auth login --email you@example.com [--json]
  notes auth verify --email you@example.com --code 123456 [--json]
  notes auth device [--json]
  notes auth device-token --device-code dc_... [--json]
  notes auth device-exchange --exchange-token dt_... [--json]
  notes auth device-approve --user-code XXXX-XXXX [--json]
  notes auth whoami [--json]
  notes auth logout
  notes sync [--dry-run] [--json] [--api-url https://...]
  notes sync --watch [--interval 5m] [--log-file path]
  notes sync status [--json]
  notes sync --install-service [--dry-run] [--json]     (launchd on macOS, systemd --user on Linux)
  notes sync --uninstall-service [--json]
  notes cloud status [--json]
  notes cloud list [--limit 10] [--json]
  notes cloud create --title title --body markdown [--json]
  notes cloud sync [--dry-run] [--json]   (alias of: notes sync)
  notes billing status [--json]
  notes billing checkout [--json]
...
http://127.0.0.1:8788 by default; point HASNA_NOTES_API_URL (or config
...
~/Library/Logs/HasnaNotes (macOS) or ~/.local/state/hasna-notes (Linux).
...
async function commandSyncStatus(opts) {
  ...
  if (!status) {
    ...
    return line('Never synced. Run: notes sync');
  }
  ...
  if (!uninstall) {
    ...
    if (lnp.blocked) {
      ...
      if (lnp.safeApiUrl) {
        ...
        // The service file embeds HASNA_NOTES_API_URL when set — make sure
        // it embeds the safe URL, not the LAN one the config no longer uses.
        if (hasnaEnv('API_URL')) env = { ...process.env, HASNA_NOTES_API_URL: lnp.safeApiUrl };
      }
    }
  }
  if (dryRun) {
...
async function handleHosted(cmd, args, opts) {
  ...
  if (cmd === 'cloud') {
    ...
    if (action === 'sync') return commandSync(opts, []); // deprecated alias of `notes sync`
  }
  if (cmd === 'billing') {
    const action = args[0];
