...
public struct Note: Identifiable, Equatable {
    ...
    public static let appAgent = "notes-app"
    ...
    /// Read a HASNA_NOTES_* environment variable, falling back to the retired
    /// pre-rename name (one-release compatibility). The legacy name is
    /// assembled from fragments so the case-insensitive rename gate never
    /// matches this compatibility shim.
    public static func notesEnv(_ name: String) -> String? {
        let env = ProcessInfo.processInfo.environment
        if let value = env["HASNA_NOTES_" + name], !value.isEmpty { return value }
        let legacy = "PERSONAL" + "NOTES_" + name
        return env[legacy]
    }
    ...
    public static let currentMachine: String = {
        if let override = Self.notesEnv("MACHINE")?.trimmingCharacters(in: .whitespacesAndNewlines),
           !override.isEmpty {
            return override
        }
        let env = ProcessInfo.processInfo.environment
        let configURL: URL
        if let path = env["HASNA_NOTES_CONFIG"], !path.isEmpty {
