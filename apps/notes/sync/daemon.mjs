...
// Sync scheduling: `notes sync --watch` daemon mode plus the pieces the
...
//   surfaces (Settings row, `notes sync status`) can never render an
...
/// Platform log home: `~/Library/Logs/HasnaNotes/sync.log` on macOS,
/// `$XDG_STATE_HOME/hasna-notes/sync.log` (default `~/.local/state/...`)
/// on Linux and everything else.
export function syncLogPath({ platform = process.platform, env = process.env, home = homedir() } = {}) {
  if (platform === 'darwin') return join(home, 'Library', 'Logs', 'HasnaNotes', 'sync.log');
  const stateHome = env.XDG_STATE_HOME || join(home, '.local', 'state');
  return join(stateHome, 'hasna-notes', 'sync.log');
}
...
export const SERVICE_LABEL = 'com.hasna.notes.sync';
export const SYSTEMD_UNIT_NAME = 'notes-sync.service';
...
export function serviceDefinition({
  ...
  const passthrough = {};
  for (const key of ['HASNA_NOTES_ROOT', 'HASNA_NOTES_CONFIG', 'HASNA_NOTES_API_URL']) {
    if (env[key]) passthrough[key] = env[key];
  }
  if (platform === 'darwin') {
    const path = join(home, 'Library', 'LaunchAgents', `${SERVICE_LABEL}.plist`);
    const launchdLog = join(home, 'Library', 'Logs', 'HasnaNotes', 'sync-launchd.log');
...
`;
  ...
  const envLines = Object.entries(passthrough)
    ...
    .join('\n');
  const content = `[Unit]
Description=Hasna Notes sync daemon (notes sync --watch)
...
/// `notes sync --watch`. Returns a controller `{ stop }`; the pending
...
export async function watchSync(options = {}) {
  ...
  async function intervalMinutes() {
    ...
    return resolveSyncInterval(
      options.interval
      ?? hasnaEnv('SYNC_INTERVAL_MINUTES')
