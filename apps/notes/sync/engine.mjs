...
import { createClient, NotesApiError } from './client.mjs';
...
async function sendWithRetry(client, body, key, { retryDelayMs, attempts = 4 }) {
  for (let attempt = 1; ; attempt += 1) {
    try {
      ...
      const retryable = err instanceof NotesApiError && (err.status === 409 || err.status === 429);
...
function isPayloadTooLarge(err) {
  return err instanceof NotesApiError && err.status === 413;
}
...
export async function runSync(options = {}) {
  ...
  if (!client.config.apiKey && !client.config.token) {
    throw new NotesApiError('Not signed in. Run: notes auth device', { status: 401, code: 'unauthorized' });
  }
