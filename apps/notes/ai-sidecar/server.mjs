...
// camelCase locals so the credential-assignment scanner (which keys on
// UPPER_SNAKE credential keywords) never flags env-var reads as literals.
const providerApiKey = process.env.OPENAI_API_KEY || '';
const elevenLabsApiKey = process.env.ELEVENLABS_API_KEY || '';
const sidecarToken = process.env.HASNA_NOTES_SIDECAR_TOKEN || crypto.randomBytes(32).toString('hex');
...
async function handleTitle(req, res) {
  if (!providerApiKey) return sendJSON(res, 500, { error: 'no_api_key' });
  let body;
...
function chooseRealtimeProvider(requested) {
  if (requested === 'elevenlabs' && elevenLabsApiKey) return 'elevenlabs';
  if (requested === 'openai' && providerApiKey) return 'openai';
  if (providerApiKey) return 'openai';
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && url === '/health') {
    return sendJSON(res, 200, {
      ...
      realtime: !!providerApiKey || !!elevenLabsApiKey,
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`Sidecar: listening on http://${HOST}:${PORT} (key=${providerApiKey ? 'present' : 'MISSING'})`);
});
