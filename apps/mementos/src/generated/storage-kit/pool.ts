...
export function createServerPoolFromEnv(
  ...
  // LOCAL DEVIATION (see backend.ts header): the stock 1.0.2 generator's
  // backend never returns null (postgres-only), so this guard was dropped
  // upstream; mementos's dual-backend resolver returns null when no DSN
  // selects the postgresql backend, and building a pool for it must throw.
  if (!connectionString) {
    throw new Error(
