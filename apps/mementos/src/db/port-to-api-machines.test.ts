...
describe("PORT-TO-API — machine registry reaches /v1", () => {
  ...
  test("rename_machine → PATCH /v1/machines/:id by stable id", async () => {
    const lines = await runScenario("rename_machine");
    const r = routes(lines);
    expect(r).toEqual(["PATCH /v1/machines/machine-1"]);
  ...
  test("credential rotation on one authority re-registers instead of reusing another account cache", async () => {
    const lines = await runScenario("machine-cache-credential-change");
    expect(routes(lines).filter((route) => route === "POST /v1/machines")).toHaveLength(2);
  });

  test("deleting the cached identity forces the next lookup to register again", async () => {
    const lines = await runScenario("machine-cache-delete");
  ...
  test("memory_save refuses hosted machine identity failure before writing a machine-null memory", async () => {
    const lines = await runScenario("memory-save-machine-failure", "malformed-machine-register");
    expect(routes(lines)).toContain("POST /v1/machines");
    expect(routes(lines)).not.toContain("POST /v1/memories");
  });

  test.each([
    "malformed-machine-register",
    "malformed-machine-register-binding",
    "malformed-machine-list",
    "malformed-machine-rename",
    "malformed-machine-primary",
    "malformed-machine-get",
    "malformed-machine-touch",
    "malformed-machine-delete",
  ])("%s refuses malformed hosted 2xx responses", async (scenario) => {
    const lines = await runScenario(scenario, scenario);
