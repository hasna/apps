...
describe("Mementos project registration schema parity", () => {
  test("migration 41 retains registration and guarded-update receipt contracts", () => {
  ...
  test("SQLite and PostgreSQL migration tails carry equivalent receipt and binding contracts", () => {
    ...
    expect(MIGRATIONS.at(-4)).toContain(sqlite);
    expect(PG_MIGRATIONS.at(-4)).toContain(postgres);
    expect(MIGRATIONS.at(-3)).toContain(sqliteUpdate);
    expect(PG_MIGRATIONS.at(-3)).toContain(postgresUpdate);
