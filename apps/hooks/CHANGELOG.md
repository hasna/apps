...
- ecbc577: Add the `trash-guard` `PreToolUse` hook: rewritable `rm` calls are redirected to `trash guard`, and anything the hook cannot verify is refused rather than run — redirect degrades to BLOCK, never to allow, so a missing `trash` binary denies the command instead of letting an unrecoverable delete through. `HookMeta.rewritesInput` declares input-rewriting hooks; install refuses a second overlapping `PreToolUse` rewriter and `hooks doctor` checks the overlap. Payloads whose `command` cannot be read are refused instead of allowed (hasna/apps#2152).

### Patch Changes

- e83b1d7: `hooks doctor` detects and repairs stale hook registrations — a settings entry
