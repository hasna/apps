...
- The REST API now requires `Authorization: Bearer <PROMPTS_API_TOKEN>` on every `/api/*` data request and fails closed when no token is configured (503). The former `Access-Control-Allow-Origin: *` wildcard is gone. OPTIONS preflights from a loopback origin (the bundled React dashboard's Vite dev server) — or from an exact origin listed in the new `PROMPTS_API_CORS_ORIGIN` — receive restricted CORS headers without a bearer, so the dashboard workflow keeps working; every actual data request still requires the token, and no other browser origin can preflight or read the API. (finding code-prompts-1)
