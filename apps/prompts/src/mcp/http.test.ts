...
describe("prompts MCP HTTP transport", () => {
  ...
  test("MCP exposes storage status without configured values", async () => {
    ...
    try {
      ...
      expect(diagnostics.client.transport).toBe("sqlite");
      expect(diagnostics.server.backend).toBe("sqlite");
      expect(diagnostics.body_store.type).toBe("local");
      // No configured values may leak into the report.
