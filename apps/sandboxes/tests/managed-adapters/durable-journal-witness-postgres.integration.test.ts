...
const REQUIRED_ENV = [
  ...
  "SANDBOXES_WITNESS_PRIVILEGED_MEMBER_URL",
  "SANDBOXES_WITNESS_DATABASE",
  "SANDBOXES_WITNESS_MIGRATION_ROLE",
  "SANDBOXES_WITNESS_READER_ROLE",
  "SANDBOXES_WITNESS_ACK_ROLE",
  "SANDBOXES_WITNESS_READER_DELEGATE_ROLE",
  "SANDBOXES_WITNESS_ACK_DELEGATE_ROLE",
  "SANDBOXES_WITNESS_UNRELATED_DATABASE_ROLE",
...
test.skipIf(!ENABLED)("durable witness is independent, signed, CAS-linearized, restart-safe, and least-privilege", async () => {
  const config = {
    ...
    privilegedMemberUrl: required("SANDBOXES_WITNESS_PRIVILEGED_MEMBER_URL"),
    ...
    readerDelegateRole: required("SANDBOXES_WITNESS_READER_DELEGATE_ROLE"),
    acknowledgementDelegateRole: required("SANDBOXES_WITNESS_ACK_DELEGATE_ROLE"),
    unrelatedDatabaseRole: required("SANDBOXES_WITNESS_UNRELATED_DATABASE_ROLE"),
  ...
  try {
    ...
    const witnessMigrationState = async () => ({
      schema_objects: await migration.query<{ object_identity: string }>(`
        SELECT 'relation:' || relation.relkind::text || ':' || relation.relname::text
          AS object_identity
        FROM pg_catalog.pg_class relation
        JOIN pg_catalog.pg_namespace namespace ON namespace.oid = relation.relnamespace
        WHERE namespace.nspname = 'sandboxes_durable_journal_witness'
        UNION ALL
        SELECT 'routine:' || procedure.prokind::text || ':' || procedure.proname::text || '(' ||
          pg_catalog.oidvectortypes(procedure.proargtypes) || ')' AS object_identity
      ...
      ledger: await migration.query<{
        ...
        FROM sandboxes_durable_journal_witness.schema_migrations
        ORDER BY migration_name`),
    })
    const seedBareWitnessLedger = async (
      rows: readonly Readonly<{ migration_name: string; checksum_sha256: string }>[],
    ): Promise<void> => {
      await migration.query("CREATE SCHEMA sandboxes_durable_journal_witness")
      await migration.query(`CREATE TABLE sandboxes_durable_journal_witness.schema_migrations (
        migration_name text PRIMARY KEY, checksum_sha256 text NOT NULL,
        applied_at timestamptz NOT NULL DEFAULT clock_timestamp())`)
      for (const row of rows) {
        await migration.query(
          `INSERT INTO sandboxes_durable_journal_witness.schema_migrations
            (migration_name, checksum_sha256) VALUES ($1, $2)`,
          [row.migration_name, row.checksum_sha256],
        )
      }
    }
    const privilegedMigration = new RoleSwitchingClient(
      new Client(config.privilegedMemberUrl, config.ca),
    ...
    expect(await privilegedMigration.query(`
      ...
      session_user: "witness_privileged_member",
      current_user: config.migrationRole,
    }])
    const privilegedMigrationAttempt = await Promise.allSettled([
      applyPostgresDurableJournalWitnessMigrationV1(privilegedMigration, migrationOptions),
    ])
    ...
    const privilegedMigrationState = await migration.query<{
      ...
      pg_catalog.to_regnamespace('sandboxes_durable_journal_witness') IS NOT NULL AS schema_exists,
      pg_catalog.to_regclass(
        'sandboxes_durable_journal_witness.schema_migrations'
    ...
    for (const invalidLedger of [
      [{
        migration_name: "0002_unknown_witness_migration.sql",
        checksum_sha256: d("unknown-witness-migration"),
      }],
      [{
        migration_name: POSTGRES_DURABLE_JOURNAL_WITNESS_MIGRATION_V1.name,
        checksum_sha256: d("drifted-witness-migration"),
      }],
    ] as const) {
      await seedBareWitnessLedger(invalidLedger)
      const before = await witnessMigrationState()
      await expect(applyPostgresDurableJournalWitnessMigrationV1(
        migration,
        migrationOptions,
      )).rejects.toMatchObject({ code: "integrity_failed" })
      expect(await witnessMigrationState()).toEqual(before)
      await migration.query("DROP SCHEMA sandboxes_durable_journal_witness CASCADE")
    }
    ...
    await migration.query(
      `INSERT INTO sandboxes_durable_journal_witness.schema_migrations
        (migration_name, checksum_sha256) VALUES ($1, $2)`,
      ["9999_unknown_witness_migration.sql", d("extra-witness-migration")],
    )
    const extraLedgerBefore = await witnessMigrationState()
    await expect(applyPostgresDurableJournalWitnessMigrationV1(
      migration,
      migrationOptions,
    )).rejects.toMatchObject({ code: "integrity_failed" })
    expect(await witnessMigrationState()).toEqual(extraLedgerBefore)
    await migration.query(`DELETE FROM sandboxes_durable_journal_witness.schema_migrations
      WHERE migration_name = '9999_unknown_witness_migration.sql'`)

    const databaseIdentifier = quoteTestIdentifier(config.database)
    const readerIdentifier = quoteTestIdentifier(config.readerRole)
    const acknowledgementIdentifier = quoteTestIdentifier(config.acknowledgementRole)
    const readerDelegateIdentifier = quoteTestIdentifier(config.readerDelegateRole)
    const acknowledgementDelegateIdentifier = quoteTestIdentifier(config.acknowledgementDelegateRole)
    ...
    const migrationInvariantState = async () => ({
      ...
      config: await migration.query<{ row: string }>(
        `SELECT pg_catalog.row_to_json(config)::text AS row
          FROM sandboxes_durable_journal_witness.config config`,
      ),
      ledger: await migration.query<{
        migration_name: string
        checksum_sha256: string
        ...
        FROM sandboxes_durable_journal_witness.schema_migrations ORDER BY migration_name`),
    })
    ...
    await migration.query(`GRANT CREATE, TEMPORARY ON DATABASE ${databaseIdentifier}
      TO ${readerIdentifier}, ${acknowledgementIdentifier}`)
    await migration.query(`GRANT CONNECT ON DATABASE ${databaseIdentifier}
      TO ${readerIdentifier}, ${acknowledgementIdentifier} WITH GRANT OPTION`)
    const readerGrantor = new Client(config.readerUrl, config.ca)
    const acknowledgementGrantor = new Client(config.acknowledgementUrl, config.ca)
    clients.push(readerGrantor, acknowledgementGrantor)
    await readerGrantor.query(`GRANT CONNECT ON DATABASE ${databaseIdentifier}
      TO ${readerDelegateIdentifier}`)
    await acknowledgementGrantor.query(`GRANT CONNECT ON DATABASE ${databaseIdentifier}
      TO ${acknowledgementDelegateIdentifier}`)
    ...
    expect(driftedAclKeys).toContain(`${config.readerRole}:CREATE:false`)
    expect(driftedAclKeys).toContain(`${config.readerRole}:TEMPORARY:false`)
    expect(driftedAclKeys).toContain(`${config.readerRole}:CONNECT:true`)
    expect(driftedAclKeys).toContain(`${config.acknowledgementRole}:CREATE:false`)
    expect(driftedAclKeys).toContain(`${config.acknowledgementRole}:TEMPORARY:false`)
    expect(driftedAclKeys).toContain(`${config.acknowledgementRole}:CONNECT:true`)
    expect(driftedAclKeys).toContain(`${config.readerDelegateRole}:CONNECT:false`)
    expect(driftedAclKeys).toContain(`${config.acknowledgementDelegateRole}:CONNECT:false`)
    await applyPostgresDurableJournalWitnessMigrationV1(migration, migrationOptions)
    ...
    expect(databaseAcls.map((row) =>
      ...
      `${config.readerRole}:CONNECT:false`,
      `${config.acknowledgementRole}:CONNECT:false`,
    ...
    const witnessInitializationState = async () => await migration.query<{
      config_count: bigint | string
      receipt_count: bigint | string
      head_count: bigint | string
    }>(`SELECT
      (SELECT count(*) FROM sandboxes_durable_journal_witness.config) AS config_count,
      (SELECT count(*) FROM sandboxes_durable_journal_witness.receipts) AS receipt_count,
      (SELECT count(*) FROM sandboxes_durable_journal_witness.heads) AS head_count`)
    const expectSetRoleInitializationRejected = async (
      switchedRole: "reader" | "acknowledgement",
    ): Promise<void> => {
      const readerBase = new Client(
        switchedRole === "reader" ? config.privilegedMemberUrl : config.readerUrl,
        config.ca,
      )
      const acknowledgementBase = new Client(
        switchedRole === "acknowledgement"
          ? config.privilegedMemberUrl
          : config.acknowledgementUrl,
        config.ca,
      )
      const readerClient = switchedRole === "reader"
        ? new RoleSwitchingClient(readerBase, config.readerRole)
        : readerBase
      const acknowledgementClient = switchedRole === "acknowledgement"
        ? new RoleSwitchingClient(acknowledgementBase, config.acknowledgementRole)
        : acknowledgementBase
      const switchedClient = switchedRole === "reader" ? readerClient : acknowledgementClient
      expect(await switchedClient.query(`
        SELECT session_user::text AS session_user, current_user::text AS current_user`)).toEqual([{
        session_user: "witness_privileged_member",
        current_user: switchedRole === "reader" ? config.readerRole : config.acknowledgementRole,
      }])
      const before = await witnessInitializationState()
      const attempt = await Promise.allSettled([
        PostgresDurableJournalWitnessV1.fromClients(
          readerClient,
          options(acknowledgementClient),
        ),
      ])
      if (attempt[0]?.status === "fulfilled") {
        await attempt[0].value.close().catch(() => undefined)
      } else {
        await Promise.allSettled([readerBase.close(), acknowledgementBase.close()])
      }
      const after = await witnessInitializationState()
    ...
    await expectSetRoleInitializationRejected("reader")
    ...
    try {
      const publicConnectPair = newClients()
      await expect(PostgresDurableJournalWitnessV1.fromClients(
        publicConnectPair.reader,
        options(publicConnectPair.acknowledgement),
      )).rejects.toMatchObject({ code: "integrity_failed" })
    } finally {
    ...
    await applyPostgresDisposableTaskJournalMigrationV1(mainMigration, {
      ...
      expected_journal_cluster_system_identifier: config.journalCluster,
