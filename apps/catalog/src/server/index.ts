...
export function createCatalogHandler(options: CatalogApiOptions = {}): (request: Request) => Response {
  ...
  return (request: Request): Response => {
    try {
      return routeRequest(request, store);
    } catch {
      // A store or parsing failure must become a JSON 500, never an unhandled
      // exception escaping into Bun.serve (which would tear the connection
...
function routeRequest(request: Request, store: CatalogStoreLike): Response {
