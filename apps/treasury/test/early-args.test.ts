 ...
 *
 * treasury-mcp --help previously fell through main()'s --version guard into
 ...
 * plain treasury-mcp must STILL reach the transport path — with stdin closed
 * it ends, printing neither version nor usage (negative).
 */

const TREASURY_ROOT = join(import.meta.dir, "..");
...
describe("treasury-mcp answers --help/--version before any transport (row 7e5f8f3d)", () => {
  test("--help prints usage and exits", async () => {
    const result = await runMcp(["--help"]);
    expect(result.timedOut).toBe(false);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain("Usage:");
    expect(result.stdout).toContain("treasury-mcp");
  ...
  test("plain treasury-mcp still reaches the transport path (negative probe)", async () => {
