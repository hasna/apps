...
const SCOPED_CRED = "fleet-parity-" + "scoped-cred-000000000000";
const UNSCOPED_CRED = "fleet-parity-" + "unscoped-cred-0000000000";
const PARITY_CREDENTIALS = JSON.stringify([
  { id: "parity-scoped", token: SCOPED_CRED, type: "session", roles: ["editor"], scopes: ["fleet:read", "fleet:write"], entity_ids: [HASNA_INC] },
  { id: "parity-unscoped", token: UNSCOPED_CRED, type: "session", roles: ["editor"], scopes: ["fleet:read", "fleet:write"] },
]);
...
beforeEach(() => {
  ...
  scopedPrincipal = authenticateToken(SCOPED_CRED)!;
...
describe("interface parity — unscoped principal is DENIED config CRUD (deny-by-default §1c)", () => {
  ...
  it("refuses create/get/update/delete on MCP and /v1 with ENTITY_ACCESS_DENIED", async () => {
    const unscoped = authenticateToken(UNSCOPED_CRED)!;
    ...
    const httpCreate = await callHttp<{ code: string }>(createOp, createInput, UNSCOPED_CRED);
    ...
    const httpGet = await callHttp<{ code: string }>(getOp, { id: slo.id }, UNSCOPED_CRED);
    ...
    const httpUpd = await callHttp<{ code: string }>(updateOp, { id: slo.id, name: "hijacked" }, UNSCOPED_CRED);
    ...
    const httpDel = await callHttp<{ code: string }>(deleteOp, { id: slo.id }, UNSCOPED_CRED);
...
describe("interface parity — unscoped principal gets an EMPTY list (deny-by-default §1c)", () => {
  ...
  it("returns an empty list on MCP and /v1 for every config LIST op", async () => {
    ...
    for (const op of listOps) {
      ...
      const http = await callHttp<unknown[]>(op, {}, UNSCOPED_CRED);
