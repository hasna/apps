...
`hasna.npm-release-agent-review.v2`.

Fix one independent coding-agent session before reviewing the candidate. Set
`RELEASE_REVIEWER_AGENT` to its canonical registered agent name. Record the
registry row ID, actual runtime, durable session/run ID, exact release commit,
and review evidence on the release task. A seat or runtime brand alone does not
identify the reviewer.

The reviewer and publisher must be different coding-agent sessions with
different registered names. The release verifier checks identifier syntax,
equality with the configured reviewer, reviewer/publisher inequality, and the
Ed25519 signature. It does not query the agent registry or authenticate runtime
lineage.

Store the canonical base64 PKCS8 DER Ed25519 private key in the secrets vault.
Inject it only into the fixed reviewer's receipt-issuance process after its
verdict is final. The publishing workflow receives the public key and signed
receipt; it never receives the private key.

This is an independent-agent review gate. No human release-review approval is
required. Issue `GO` only after reviewing the exact candidate and resolving all
concrete reachable in-scope P0/P1 blockers.

The `npm-release` environment has no required reviewers or wait timer. Its
non-secret variables provide the fixed reviewer identity, deterministic key id,
and Ed25519 public key consumed by the package-owned verifier:

- `RELEASE_REVIEWER_AGENT`, set to the fixed reviewer's canonical registered
  agent name, such as `review-todos-release-0160`;
...
```json
{
  ...
  "reviewer": {
    "type": "coding-agent",
    "agent": "review-todos-release-0160"
  },
  "publisher": {
    "type": "coding-agent",
...
The publisher identity must match the annotated tag's single final `Agent:`
trailer and must differ from the reviewer. Missing, unsigned, malformed,
...
commit or version is still a replay and fails the live commit, package, tag,
and procedure-revision comparisons.
...
Fix the independent reviewer and publisher identities before review begins. The
reviewer checks out the exact clean release commit already on protected main,
records its canonical registered agent name and durable review-run id on the
release task, and independently evaluates that candidate. The same reviewer may
perform at most two focused remediation checks for concrete reachable in-scope
P0/P1 defects. It issues `GO` only when both blocking counts are zero.

The fixed reviewer uses the package release-review vault item containing a
canonical base64 PKCS8 DER Ed25519 private key. That key is delivered only to
the same independent reviewer process after its review verdict is final. Replace
the five bracketed values below with the actual reviewer agent name, vault item,
exact release SHA, intended publisher agent, and task evidence directory. No
private-key value is printed or stored in the repository.

```bash
evidence_dir=<task-evidence-directory>
reviewer_agent=review-todos-release-0160
reviewer_agent_file="$(mktemp)"
reviewer_key_id_file="$(mktemp)"
reviewer_public_key_file="$(mktemp)"
receipt_file="$(mktemp)"

printf '%s\n' "${reviewer_agent}" | gh variable set RELEASE_REVIEWER_AGENT \
  --repo hasna/apps --env npm-release
gh variable get RELEASE_REVIEWER_AGENT \
  --repo hasna/apps --env npm-release > "${reviewer_agent_file}"
test "$(sed -n '1p' "${reviewer_agent_file}")" = "${reviewer_agent}"
...
release SHA, registered agent name, review-run id, verdict, and blocker counts
