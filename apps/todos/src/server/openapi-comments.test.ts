...
describe("task list and completion OpenAPI contract", () => {
  test("documents exhaustive task pagination, total, filters, and completion evidence", () => {
    ...
    expect(document.components.schemas.UpdateTaskInput.properties.parent_id).toMatchObject({
      type: "string",
      nullable: true,
    });
...
describe("project mutation OpenAPI contract", () => {
  test("preserves create routing compatibility, closes generic update, and exposes atomic rename", () => {
    ...
    expect(Object.keys(createProperties)).toEqual(["status", "short_id", "metadata", "name", "path", "description", "task_list_id", "task_prefix", "parent_id"]);
    expect(Object.keys(updateProperties)).toEqual(["status", "short_id", "metadata", "name", "path", "description", "parent_id"]);
