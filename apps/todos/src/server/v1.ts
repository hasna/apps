    ...
    if (resource === "tasks") {
      ...
      if (!id) {
        if (method === "GET") {
          ...
          if(includeArchived!==null&&includeArchived!=="true"&&includeArchived!=="false")return error(400,"include_archived must be true or false");
          const planRead=url.searchParams.get("plan_read_contract");
          if(planRead!==null){const keys=[...url.searchParams.keys()];const allowed=new Set(["plan_read_contract","plan_id","include_subtasks","include_archived","limit","offset"]);if(new Set(keys).size!==keys.length||keys.some(key=>!allowed.has(key)))return error(400,"Plan read contract requires an exact unambiguous plan selection");}
          if(planRead!==null&&(planRead!=="1"||!url.searchParams.get("plan_id")||includeSubtasks!=="true"||includeArchived===null||url.searchParams.has("parent_id")))return error(400,"plan_read_contract=1 requires plan_id and explicit complete selection flags");
          ...
          return json({ tasks, count: tasks.length, total, ...(planRead==="1"?{selection:{schema_version:1,plan_id:filter.plan_id,include_subtasks:true,include_archived:filter.include_archived}}:{}) });
        }
        if (method === "POST") {
    ...
    if (resource === "plans") {
      ...
      if (id && action === "comments") {
        ...
        if (method === "GET") {
          ...
          if(url.searchParams.has("plan_read_contract")){
            if(url.searchParams.get("plan_read_contract")!=="1")return error(400,"Unsupported plan read contract");
            if(!store.plans.getComments)return error(503,"Complete plan history is unavailable");
          }
    ...
    if (resource === "task-lists") {
      ...
      if (id && (method === "PATCH" || method === "PUT")) {
        ...
        if (body.project_id !== undefined) {
          if (body.project_id !== null && (typeof body.project_id !== "string" || !body.project_id.trim())) {
            return error(400, "project_id must be a non-empty string or null");
          }
          if (typeof body.project_id === "string" && body.project_id.trim() !== "") {
            const target = await store.projects.get(body.project_id.trim());
            if (!target) {
              return error(400, `task-list project_id must reference a registered project: ${body.project_id}`);
            }
