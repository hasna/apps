...
/**
 * O15-04049 — bun 1.3.14 ESM default-export interop regression.
 *
 * The bundled CLI and its external `ink` dependency load `signal-exit` at
 * runtime. signal-exit 4.x ships an ESM build selected through the package
 * `exports` "import" condition; bun 1.3.14 crashes evaluating that mjs
 * build's default-export interop (`SyntaxError: Missing 'default' export in
 * module signal-exit/dist/mjs/index.js`, reproduced locally as "Multiple
 * exports with the same name onExit"). Observed on station01 as `todos add`
 * and `todos comment` crashing under bun 1.3.14 while `list`/`--help`
 * worked; a workaround rerouted the installed package's exports to the cjs
 * build. The CJS v3 build (no `exports` map) is immune.
 *
 ...
 * packages (execa, foreground-child, spawndamnit via @changesets/git)
 * hoist signal-exit@4.1.0 to the shared root — the conflicting direct range
 * forces a CJS 3.x copy inside the @hasna/todos tree, so both the bundle's
 * ink import chain and ink's own `require("signal-exit")` resolve the CJS
 * build.
 */
describe("signal-exit resolves to the CJS v3 build (O15-04049)", () => {
