  ...
  test("built project/list/plan/task lifecycle stays on HTTP with a read-only TODOS_DB_PATH", async () => {
    ...
    try {
      ...
      const runRemoteOk = async (invocation: string[]): Promise<string> => {
        ...
        // This converted family refuses database selectors instead of merely
        // ignoring them. The no-file checks still cover both HOME and cwd.
        if(invocation.includes("lists"))delete invocationEnv.TODOS_DB_PATH;
