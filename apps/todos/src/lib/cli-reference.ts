...
export const ENV_VARS: EnvVarRef[] = [
  ...
  { name: "HASNA_TODOS_API_URL", description: "Todos service authority; the Keychain api-url item and ~/.hasna/todos/config/credentials also supply it", default: "https://api.hasna.com/todos" },
  { name: "HASNA_TODOS_API_KEY", description: "Todos credential (lowest tier: the Keychain item hasna.credentials.todos.api-key and ~/.hasna/todos/config/credentials outrank it)", default: "(none — hosted mode fails closed)" },
  { name: "HASNA_TODOS_LOCAL", description: "Deliberate unhosted opt-in: use the on-box SQLite store and never read a credential", default: "(unset)" },
