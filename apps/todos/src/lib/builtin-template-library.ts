...
import type { TemplateExport } from "../db/templates.js";
...
export const BUILTIN_TEMPLATES: BuiltinTemplate[] = [
  ...
  },
  {
    ...
    description:
      "Threat model, test, remediate, and report on security posture.",
    category: "security",
    version: BUILTIN_TEMPLATE_LIBRARY_VERSION,
    variables: [
      {
    ...
    tasks: [
      ...
      },
      {
        position: 2,
        title_pattern:
          "Review authz, data access, and dependency risks in {target}",
        priority: "critical",
  ...
  },
  {
    name: "release",
    description:
      "Prepare, verify, publish, install, and smoke test a package release.",
    category: "release",
    version: BUILTIN_TEMPLATE_LIBRARY_VERSION,
    variables: [
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        position: 4,
        title_pattern:
          "Run migration {migration} verification and drift checks",
        priority: "high",
  ...
  },
  {
    name: "incident",
    description:
      "Triage, mitigate, repair, verify, and retrospect an incident.",
    category: "incident",
    version: BUILTIN_TEMPLATE_LIBRARY_VERSION,
    variables: [
  ...
  },
  {
    name: "qa",
    description:
      "Build a focused QA plan, execute checks, file defects, and sign off.",
    category: "qa",
    version: BUILTIN_TEMPLATE_LIBRARY_VERSION,
    variables: [
      {
  ...
  },
  {
    ...
    tasks: [
      ...
      },
      {
        position: 10,
        title_pattern:
          "Add agent tools (register_agent, heartbeat, set_focus, list_agents)",
        priority: "medium",
        depends_on_positions: [3],
      },
      {
...
export function getBuiltinTemplate(name: string): BuiltinTemplate | null {
  return (
    listBuiltinTemplates().find((template) => template.name === name) ?? null
  );
}
...
export function exportBuiltinTemplateFiles(): Array<{
  filename: string;
...
export function writeBuiltinTemplateFiles(directory: string): {
  directory: string;
  written: number;
  files: string[];
} {
...
/** The server-owned bundled initialization payload; no storage or API access. */
export function builtinTemplateInputs(): CreateTemplateInput[] {
