...
describe("secret redaction", () => {
  ...
  it("detects OpenAI and GitHub tokens", () => {
    const scan = scanTextForSecrets(`key=sk-1234567890abcdef and gh${"p_"}1234567890123456789012345678901234`);
  ...
  it("allows safe placeholders through allowlist", () => {
    const scan = scanTextForSecrets(`Use sk-test or gh${"p_"}xxx in docs`);
  ...
  it("assertNoSecrets throws with context", () => {
    expect(() => assertNoSecrets(`token gh${"p_"}1234567890123456789012345678901234`, "comment")).toThrow(/comment/i);
  });
