...
function handleCliFlags(argv: string[]): boolean {
  if (argv.includes("--help") || argv.includes("-h")) {
    console.log("Usage: search-mcp [options]");
    console.log("");
    console.log("MCP server for @hasna/search (stdio default, optional Streamable HTTP)");
    console.log("");
    console.log("Options:");
    console.log("  --http         Start Streamable HTTP transport on 127.0.0.1");
    console.log("  --port <n>     HTTP port (default 8832, or MCP_HTTP_PORT env)");
