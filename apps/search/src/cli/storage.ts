...
export function registerStorageCommands(program: Command): void {
  const storage = program
    .command("storage")
    .description("Manage search local/remote storage sync");
  ...
  storage
    .command("push")
    .description("Push local search data to PostgreSQL")
  ...
  storage
    .command("pull")
    .description("Pull PostgreSQL search data into the local database")
    .option("--tables <tables>", "Comma-separated table names")
    .option("--json", "Output as JSON")
    .action(async (opts) => {
      try {
  ...
  storage
    ...
    .action(async (opts) => {
      try {
        ...
        const result = await applyPgMigrations(opts.connectionString || getStorageConnectionString("search"));
        if (opts.json) {
          printJson(result);
          return;
        }
