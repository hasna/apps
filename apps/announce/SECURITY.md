...
Hasna Announce stores local JSONL data in `~/.hasna/announce` by default. Set `ANNOUNCE_DATA_DIR` to use a project-specific or ephemeral data directory.
...
When publishing overwrites an existing file, Hasna Announce stores a local backup under `~/.hasna/announce/backups` by default. Treat backups as local artifacts and do not commit them.
