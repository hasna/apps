...
import { assertSupportedNpmVersion, packCommand, scanPackedArtifact, scannerCommand } from "../scripts/scan-artifact";
...
describe("npm artifact gate", () => {
  ...
  test("scans exactly the local npm tarball, suppresses lifecycle recursion, and cleans its workspace", () => {
    ...
    spyOn(Bun, "spawnSync").mockImplementation(((command: string[]) => {
      ...
      if (calls.length === 2) {
        ...
        writeFileSync(join(workspace, "hasna-guardrails-0.2.0.tgz"), "fixture");
        return output(JSON.stringify([{ filename: "hasna-guardrails-0.2.0.tgz" }]));
      }
      expect(command).toEqual(scannerCommand(join(workspace, "hasna-guardrails-0.2.0.tgz")));
  ...
  test("real npm creates a tarball under inherited dry-run and does not run lifecycle hooks", () => {
    const fixture = mkdtempSync(join(tmpdir(), "guardrails-npm-pack-test-"));
    const destination = join(fixture, "packed");
    mkdirSync(destination);
    writeFileSync(join(fixture, "package.json"), JSON.stringify({
      name: "guardrails-pack-fixture", version: "1.0.0", files: ["index.js"],
    ...
    try {
      ...
      // Prove both hooks really fail when suppression is removed; the passing
      // arm above cannot be explained by inert or skipped fixture scripts.
      for (const [hook, exitCode] of [["prepack", 91], ["prepare", 92]] as const) {
        const rejected = join(fixture, hook);
        mkdirSync(rejected);
        writeFileSync(join(fixture, "package.json"), JSON.stringify({
          name: "guardrails-pack-fixture", version: "1.0.0", files: ["index.js"],
          scripts: { [hook]: `node -e 'process.exit(${exitCode})'` },
        }));
        const unsuppressed = originalSpawnSync(packCommand(rejected).filter((arg) => arg !== "--ignore-scripts"), spawnOptions);
        expect(unsuppressed.exitCode).toBe(exitCode);
        expect(existsSync(join(rejected, packed[0].filename))).toBe(false);
      }
  ...
  test("symlinks and non-regular archives fail closed", () => {
    const external = mkdtempSync(join(tmpdir(), "guardrails-test-target-"));
    try {
      ...
      for (const symlink of [true, false]) {
        ...
        expect(calls).toHaveLength(2);
        expect(existsSync(workspace)).toBe(false);
        expect(existsSync(join(external, "target.tgz"))).toBe(true);
      }
