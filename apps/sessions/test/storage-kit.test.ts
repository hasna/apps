...
describe("generated storage kit backend and TLS helpers", () => {
  it("exports its version and selects the server data backend by DATABASE_URL", () => {
    expect(KIT_VERSION).toBe("0.11.1");
    expect(envToken("open-sessions")).toBe("OPEN_SESSIONS");
    expect(serverDataBackendEnvKeys("open-sessions")).toEqual({
    ...
    expect(resolveServerDataBackend("sessions", canonical)).toEqual({
      backend: "postgresql",
      source: "HASNA_SESSIONS_DATABASE_URL",
      databaseUrlPresent: true,
      databaseUrlSource: "HASNA_SESSIONS_DATABASE_URL",
    });
  ...
  it("rejects the removed storage-mode variables with migration guidance", () => {
    expect(() =>
      assertNoLegacyStorageMode("sessions", { HASNA_SESSIONS_STORAGE_MODE: "cloud" }),
    ).toThrow(/HASNA_SESSIONS_STORAGE_MODE was removed/);
    expect(() =>
      assertNoLegacyStorageMode("sessions", { SESSIONS_MODE: "self_hosted" }),
    ).toThrow(/SESSIONS_MODE was removed/);
    expect(() => assertNoLegacyStorageMode("sessions", {})).not.toThrow();
  });
  ...
  it("resolves disabled, relaxed, and verified TLS using every CA source", () => {
    ...
    expect(resolveTlsConfig("postgres://db/x?sslmode=disable", { env: {} })).toBe(false);
...
describe("generated pool factory", () => {
  ...
  it("builds the server pool from env and returns a closable query client", async () => {
    expect(() => createServerPoolFromEnv("open-sessions", { env: {} })).toThrow(
      "postgresql storage for open-sessions needs a database URL",
    );
    expect(() =>
      createServerPoolFromEnv("open-sessions", {
        env: { HASNA_OPEN_SESSIONS_STORAGE_MODE: "cloud" },
      }),
    ).toThrow(/was removed/);

    const result = createServerPoolFromEnv("open-sessions", {
      env: {
