// Client-side session store resolver (local SQLite vs hosted /v1 HTTP).
//
// This is the ONE seam the CLI uses for session-record reads/writes. When the
// client-flip resolves to the hosted transport — HASNA_SESSIONS_API_URL +
// HASNA_SESSIONS_API_KEY are set — every read and write is routed to the
// app's cloud `/v1` HTTP API (the configured HASNA_SESSIONS_API_URL, e.g.
// https://sessions.your-deployment.example/v1) with the bearer key, using the
...
// API URL + API key pair alone (HASNA_SESSIONS_API_URL + HASNA_SESSIONS_API_KEY,
// or their unprefixed aliases). The deployment-mode vocabulary was removed by
// the owner directive of 2026-07-29 and the contracts client no longer reads
// any *_MODE variable; the resolver fails closed when exactly one of the pair
// is set rather than silently reading the wrong dataset.
...
/**
 * Resolve the active session store. Hosted /v1 when the API URL + API key pair
 * is set (the contracts resolver throws if only one of the pair is set — no
 * silent local drift); local SQLite otherwise.
 */
