...
describe("appId alignment with hasna.app.v1", () => {
  test("normalizes npm names to the bare member convention", () => {
    expect(normalizeAppId("@hasna/todos")).toBe("todos");
  ...
  test("parseAppId validates the slug", () => {
    expect(parseAppId("@hasna/changelog")).toBe("changelog");
...
describe("publishRelease", () => {
  ...
  test("uses the site base URL for the ref and tolerates zero promoted entries", async () => {
    ...
    expect(result.changelogRef.uri).toBe("https://changelog.hasna.com/apps/todos/");
  });

  test("buildChangelogRef and changelogSiteAppUrl are stable", () => {
    expect(changelogSiteAppUrl("https://x.example//", "todos")).toBe("https://x.example/apps/todos/");
    expect(buildChangelogRef({ appId: "todos", version: "1.0.0" }).name).toBe("todos 1.0.0 changelog");
  });
});
