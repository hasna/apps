...
Hasna Changelog stores local JSONL data in `~/.hasna/changelog` by default. Set `CHANGELOG_DATA_DIR` to use a project-specific or ephemeral data directory.
...
When publishing overwrites an existing file, Hasna Changelog stores a local backup under `~/.hasna/changelog/backups` by default. Treat backups as local artifacts and do not commit them.
