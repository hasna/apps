...
artifact that P4 (`stations sync` convergence, script absorption) builds on.
...
stations setup --template station,dgx-spark

# apply on a physical box (mutation-gated like every setup apply)
stations setup --template station,dgx-spark --apply --yes
...
stations setup --template station,dgx-spark --check

# cloud render: user-data for an EC2 station (same source, second serialization)
stations setup --template station,ec2 --render cloud-init --station station17 > user-data.yaml
```

## Roster controller (station01)

Copy `templates/station/roster.example.json` to
`~/.hasna/stations/roster.json`, replace every placeholder, and keep
`applyMode` set to `manual` through the first kill drill. A manual pass is:

```bash
HASNA_STATIONS_ALLOW_MUTATIONS=1 stations-daemon roster reconcile \
  --config "$HOME/.hasna/stations/roster.json" --apply --drill-level tmux-kill
```
...
systemctl --user enable --now stations-roster.service
systemctl --user status stations-roster.service
```
...
| `stations-roster.service` (dgx-spark) | station01's old `/opt/fixture` unit was not the packaged controller and its heartbeat had gone stale |
...
carries the operator's home and office stations makes every cloud instance a
...
`--no-fail-on-findings` restores the pre-1.8.0 behaviour of always exiting 0.
It silences the exit code, not the report — the JSON still says `drift`.

**This is a breaking change for callers.** Before template 1.8.0 `--check` exited 0
...
contract until template 1.8.0, which made 12 of the 42 items version-blind: a station
...
`@hasna/stations` package, and is stamped into
`~/.hasna/stations/template-state.json` on cloud-init apply. Rolling a change:
bump version → publish → `stations sync --apply` per station (P4) / AMI or
