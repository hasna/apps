...
one atomic `--all-stations` wave), per-machine verification, and one-command
revert.

Built into `@hasna/stations` because fleet responsibility (manifest, remote
...
`stations flip apps` lists the per-app profile. All **25** `@hasna` OSS apps are
registered:

```
accounts attachments calendar contacts conversations domains economy emails
files identities instructions knowledge logs loops stations mementos projects
...
(drain shadow to divergence==0) before the atomic `--all-stations` cutover, so
stations never split-brain. See below.

## Commands

```
stations flip apps [--json]                         # list registered apps
stations flip plan <app> [opts]                     # waves + generated script, no execution
stations flip script <app> --mode self_hosted       # print the remote script only
stations flip apply <app> [opts] --execute          # roll out (dry-run without --execute)
stations flip revert <app> [opts] --execute         # revert to local
```

Selection / rollout options (apply, plan, revert):

- `--stations <ids>` restrict to explicit machine ids (comma/space separated).
- `--tags <tags>` restrict to stations carrying ALL tags.
- `--exclude <ids>` exclude stations.
- `--all-stations` flip the ENTIRE fleet in one **atomic** wave (ignores
  `--stations`; used for the coordination-store cutover so the fleet flips
...
Machine list comes from the fleet manifest (`stations manifest list`), which is
...
stations flip plan knowledge --json

# 1. CANARY — one machine. Dry-run, then execute.
stations flip apply knowledge --stations <canary-id>
stations flip apply knowledge --stations <canary-id> --execute
...
stations flip apply knowledge --exclude <canary-id> --batch 4 --execute

# 3. ALL — remainder (canary + batches already cover the fleet in one apply too):
stations flip apply knowledge --execute
```

Revert any time (one command):

```bash
stations flip revert knowledge --execute            # whole fleet back to local
stations flip revert knowledge --stations <id> --execute
```
...
to cut over. Use `--all-stations` so the whole fleet flips in one atomic wave (no
split-brain):

```bash
# Dry-run shows freeze-required and the single atomic wave.
stations flip plan todos --all-stations
...
- To roll back what already flipped: `stations flip revert <app> --execute`
  (or `--all-stations` for the coordination stores).
