...
export const HTTP_NAME = "stations";
...
export function resolveHttpSecurityConfig(
  ...
  host = "127.0.0.1",
): StationsHttpSecurityConfig {
  const allowUnauthenticated = env.STATIONS_ALLOW_UNAUTHENTICATED === "1" && isLoopbackHost(host);
  const allowedOrigins = (env.STATIONS_HTTP_ALLOWED_ORIGINS ?? "")
    .split(",")
    .map((origin) => origin.trim())
    .filter(Boolean);
  const maxBodyBytes = parsePositiveInteger(env.STATIONS_HTTP_MAX_BODY_BYTES, DEFAULT_MAX_BODY_BYTES);
  return {
    apiKey: env.STATIONS_API_KEY,
...
export function authorizeHttpOrigin(
  ...
  return {
    ok: false,
    status: 403,
    reason: "Untrusted Origin header for stations MCP HTTP request.",
  };
}
...
function requestBearerToken(req: IncomingMessage): string | undefined {
  ...
  const apiKey = req.headers["x-stations-api-key"];
...
export function authorizeHttpRequest(
  ...
  if (!expected) {
    return { ok: false, status: 401, reason: "stations MCP HTTP requires STATIONS_API_KEY or loopback-only STATIONS_ALLOW_UNAUTHENTICATED=1." };
  }

  const received = requestBearerToken(req);
  if (received && safeTokenEquals(received, expected)) return { ok: true };
  return { ok: false, status: 401, reason: "Invalid or missing stations MCP HTTP API key." };
}

function corsHeaders(req: IncomingMessage, host: string, security: StationsHttpSecurityConfig): Record<string, string> {
  const origin = originValue(req);
  if (!origin || !isTrustedHttpOrigin(origin, host, security.allowedOrigins)) return {};
  return {
    "access-control-allow-origin": origin,
    "access-control-allow-methods": "GET, POST, DELETE, OPTIONS",
    "access-control-allow-headers": "authorization, content-type, x-stations-api-key, mcp-session-id",
    "access-control-expose-headers": "mcp-session-id",
    "vary": "Origin",
  };
}

function applyCorsHeaders(req: IncomingMessage, res: ServerResponse, host: string, security: StationsHttpSecurityConfig): void {
...
export function startHttpServer(options: StartHttpServerOptions = {}): Server {
  ...
  httpServer.listen(port, host, () => {
    ...
    console.error(`stations-mcp HTTP listening on http://${host}:${boundPort}`);
  });
