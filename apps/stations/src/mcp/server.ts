...
import { diffStations } from "../commands/diff.js";
...
export const MACHINE_MCP_TOOL_NAMES = [
  ...
  "stations_self_test",
  "stations_apps_list",
  "stations_apps_status",
  "stations_apps_diff",
  "stations_apps_plan",
  "stations_apps_apply",
  "stations_manifest",
  "stations_manifest_validate",
  "stations_manifest_bootstrap",
  "stations_manifest_get",
  "stations_friendly_name_get",
  "stations_friendly_name_set",
  "stations_friendly_name_clear",
  "stations_details",
  "stations_browserplan_fleet",
  "stations_notes_context",
  "stations_notes_trash_policies",
  "stations_manifest_remove",
  "stations_agent_status",
  "stations_daemon_status",
  "stations_daemon_service_plan",
  "stations_setup_preview",
  "stations_setup_apply",
  "stations_sync_preview",
  "stations_sync_apply",
  "stations_topology",
  "stations_machine_health",
  "stations_routing",
  "stations_command_matrix",
  "stations_loop_preflight",
  "stations_dispatch_fleet_smoke",
  "stations_compatibility",
  "stations_diff",
  "stations_install_tailscale_preview",
  "stations_install_tailscale_apply",
  "stations_install_claude_status",
  "stations_install_claude_diff",
  "stations_install_claude_preview",
  "stations_install_claude_apply",
  "stations_route_resolve",
  "stations_workspace_resolve",
  "stations_ssh_resolve",
  "stations_ports",
  "stations_backup_preview",
  "stations_backup_apply",
  "stations_cert_preview",
  "stations_cert_apply",
  "stations_dns_add",
  "stations_dns_list",
  "stations_dns_render",
  "stations_notifications_add",
  "stations_notifications_list",
  "stations_notifications_test",
  "stations_notifications_dispatch",
  "stations_notifications_remove",
  "stations_webhooks_add",
  "stations_webhooks_list",
  "stations_webhooks_test",
  "stations_webhooks_remove",
  "stations_events_emit",
  "stations_events_list",
  "stations_events_replay",
  "stations_serve_info",
  "stations_serve_dashboard",
  "storage_status",
  "storage_push",
  "storage_pull",
  "storage_sync",
...
function mutationCallerId(): string {
  return process.env["HASNA_STATIONS_MUTATION_CALLER_ID"]?.trim() || "mcp";
}

function mutationRunId(): string {
  return process.env["HASNA_STATIONS_MUTATION_RUN_ID"]?.trim() || "mcp";
}
...
export function createMcpServer(version: string, options: McpServerOptions = {}): McpServer {
  const server = new McpServer({ name: "stations", version });
  ...
  server.tool("stations_self_test", "Run local package smoke checks.", {}, async () => ({
  ...
  server.tool(
    ...
    async ({ machine_id, yes, expected_plan_digest, approval_token }) => {
      ...
      requireMcpMutation("stations_apps_apply", approval_token, {
        machineId: resolvedMachineId,
        resourceId: mcpPlanResourceId("stations_apps_apply", resolvedMachineId, plan),
  ...
  server.tool("stations_manifest", "Read the current fleet manifest.", {}, async () => ({
    content: [{ type: "text", text: JSON.stringify(manifestList(), null, 2) }],
  }));
  server.tool("stations_manifest_validate", "Validate the current fleet manifest.", {}, async () => ({
  ...
  server.tool(
    ...
    async ({ approval_token }) => {
      requireMcpMutation("stations_manifest_bootstrap", approval_token, { resourceId: "manifest:bootstrap", args: {} });
  ...
  server.tool(
    ...
    async ({ machine_id, friendly_name, approval_token }) => {
      ...
      requireMcpMutation("stations_friendly_name_set", approval_token, {
  ...
  server.tool(
    ...
    async ({ machine_id, approval_token }) => {
      ...
      requireMcpMutation("stations_friendly_name_clear", approval_token, {
        machineId: input.machineId,
        resourceId: machineFriendlyNameResourceId(input.machineId),
  ...
  server.tool(
    ...
    "Resolve note origin/source/target machine display names, sync targets, and actor provenance for Hasna Notes consumers.",
    {
      ...
      source: z.enum(["notes", "open-notes", "agent", "sync", "import", "stations", "open-stations", "unknown"]).optional().describe("Provenance source (legacy open-* values open-notes/open-stations accepted on read)"),
  ...
  server.tool(
    ...
    "List per-machine note trash retention metadata with latest-10/View-more pagination.",
    {
      ...
      limit: z.number().int().min(1).nullable().optional().describe("Maximum stations to return; default is 10, null returns all"),
  ...
  server.tool(
    ...
    async ({ machine_id, approval_token }) => {
      requireMcpMutation("stations_manifest_remove", approval_token, { machineId: machine_id, args: { machine_id } });
  ...
  server.tool(
    "stations_daemon_service_plan",
    "Plan launchd/systemd lifecycle commands for the stations-daemon daemon.",
    {
      ...
      executable: z.string().optional().describe("stations-daemon executable path"),
  ...
  server.tool(
    ...
    async ({ machine_id, yes, approval_token }) => {
      ...
      requireMcpMutation("stations_setup_apply", approval_token, {
        machineId: resolvedMachineId,
        resourceId: mcpPlanResourceId("stations_setup_apply", resolvedMachineId, plan),
        args: mcpPlanApprovalArgs({ machine_id: resolvedMachineId, yes }, plan),
      });
  ...
  server.tool(
    ...
    async ({ machine_id, yes, approval_token }) => {
      ...
      requireMcpMutation("stations_sync_apply", approval_token, {
        machineId: resolvedMachineId,
        resourceId: mcpPlanResourceId("stations_sync_apply", resolvedMachineId, plan),
        args: mcpPlanApprovalArgs({ machine_id: resolvedMachineId, yes }, plan),
      });
  ...
  server.tool(
    "stations_machine_health",
    "Return compact local/remote loop-readiness health for stations.",
    {
      ...agentSelectorSchema,
  ...
  server.tool(
    "stations_routing",
    "Return compact route readiness for local and remote stations.",
    {
      ...agentSelectorSchema,
  ...
  server.tool(
    "stations_diff",
    "Show manifest differences between two stations.",
    {
    ...
    async ({ left_machine_id, right_machine_id }) => ({
      content: [{ type: "text", text: JSON.stringify(diffStations(left_machine_id, right_machine_id), null, 2) }],
    })
  );
  ...
  server.tool(
    ...
    async ({ machine_id, tools, yes, approval_token }) => {
      ...
      requireMcpMutation("stations_install_claude_apply", approval_token, {
        machineId: resolvedMachineId,
        resourceId: mcpPlanResourceId("stations_install_claude_apply", resolvedMachineId, plan),
  ...
  server.tool(
    ...
    async ({ machine_id, yes, approval_token }) => {
      ...
      requireMcpMutation("stations_install_tailscale_apply", approval_token, {
        machineId: resolvedMachineId,
        resourceId: mcpPlanResourceId("stations_install_tailscale_apply", resolvedMachineId, plan),
        args: mcpPlanApprovalArgs({ machine_id: resolvedMachineId, yes }, plan),
      });
  ...
  server.tool("stations_ports", "List listening ports on a machine.", { machine_id: z.string().optional().describe("Machine identifier") }, async ({ machine_id }) => ({
  ...
  server.tool(
    ...
    { bucket: z.string().optional().describe("S3 bucket name; defaults to HASNA_STATIONS_S3_BUCKET or STATIONS_S3_BUCKET"), prefix: z.string().optional().describe("S3 key prefix; defaults to HASNA_STATIONS_S3_PREFIX, STATIONS_S3_PREFIX, or stations") },
  ...
  server.tool(
    ...
    { bucket: z.string().optional().describe("S3 bucket name; defaults to HASNA_STATIONS_S3_BUCKET or STATIONS_S3_BUCKET"), prefix: z.string().optional().describe("S3 key prefix; defaults to HASNA_STATIONS_S3_PREFIX, STATIONS_S3_PREFIX, or stations"), yes: z.boolean().describe("Confirmation flag for execution"), approval_token: approvalTokenSchema },
    async ({ bucket, prefix, yes, approval_token }) => {
      requireMcpMutation("stations_backup_apply", approval_token, { resourceId: mutationResourceId("backup", bucket, prefix), args: { bucket, prefix, yes } });
  ...
  server.tool(
    ...
    async ({ domains, yes, approval_token }) => {
      requireMcpMutation("stations_cert_apply", approval_token, { resourceId: mutationResourceId("cert", domains.join(",")), args: { domains, yes } });
  ...
  server.tool(
    ...
    async ({ domain, port, target_host, approval_token }) => {
      ...
      requireMcpMutation("stations_dns_add", approval_token, { resourceId: mutationResourceId("dns", domain), args: { domain, port, target_host: resolvedTargetHost } });
      return { content: [{ type: "text", text: JSON.stringify(addDomainMapping(domain, port, resolvedTargetHost), null, 2) }] };
    }
  );
  server.tool("stations_dns_list", "List local domain mappings.", {}, async () => ({ content: [{ type: "text", text: JSON.stringify(listDomainMappings(), null, 2) }] }));
  server.tool(
    "stations_dns_render",
  ...
  server.tool(
    ...
    async ({ channel_id, type, target, command_args, events, enabled, approval_token }) => {
      ...
      requireMcpMutation("stations_notifications_add", approval_token, { resourceId: mutationResourceId("notification", channel_id), args: { channel_id, type, target, command_args: commandArgs, events: resolvedEvents, enabled: resolvedEnabled } });
      return {
  ...
  server.tool("stations_notifications_list", "List notification channels.", {}, async () => ({
  ...
  server.tool(
    ...
    async ({ channel_id, event, message, yes, approval_token }) => {
      if (yes === true) requireMcpMutation("stations_notifications_test", approval_token, { resourceId: mutationResourceId("notification-test", channel_id, event), args: { channel_id, event, message, yes: true } });
      return {
  ...
  server.tool(
    ...
    async ({ event, message, channel_id, approval_token }) => {
      requireMcpMutation("stations_notifications_dispatch", approval_token, { resourceId: mutationResourceId("notification-dispatch", channel_id, event), args: { event, message, channel_id } });
  ...
  server.tool(
    ...
    async ({ channel_id, approval_token }) => {
      requireMcpMutation("stations_notifications_remove", approval_token, { resourceId: mutationResourceId("notification", channel_id), args: { channel_id } });
  ...
  server.tool(
    ...
    "Add or replace a shared event webhook channel.",
    {
      ...
      event_type: z.string().optional().describe("Optional event type filter, e.g. stations.*"),
    ...
    async ({ channel_id, url, event_type, source, secret, enabled, approval_token }) => {
      const resolvedEnabled = enabled ?? true;
      requireMcpMutation("stations_webhooks_add", approval_token, { resourceId: mutationResourceId("webhook", channel_id), args: { channel_id, url, event_type, source, secret, enabled: resolvedEnabled } });
  ...
  server.tool("stations_webhooks_list", "List shared event webhook channels.", {}, async () => ({
  ...
  server.tool(
    ...
    async ({ channel_id, event_type, message, approval_token }) => {
      requireMcpMutation("stations_webhooks_test", approval_token, { resourceId: mutationResourceId("webhook-test", channel_id, event_type), args: { channel_id, event_type, message } });
      return {
        content: [{ type: "text", text: JSON.stringify(await events.testChannel(channel_id, { source: "stations", type: event_type ?? "events.test", message }), null, 2) }],
      };
    }
  );
  ...
  server.tool(
    ...
    { channel_id: z.string().describe("Channel identifier"), approval_token: approvalTokenSchema },
    async ({ channel_id, approval_token }) => {
      requireMcpMutation("stations_webhooks_remove", approval_token, { resourceId: mutationResourceId("webhook", channel_id), args: { channel_id } });
  ...
  server.tool(
    "stations_events_emit",
    "Emit a shared event from stations.",
    {
    ...
    async ({ event_type, subject, severity, message, data, metadata, dedupe_key, deliver, approval_token }) => {
      ...
      requireMcpMutation("stations_events_emit", approval_token, { resourceId: mutationResourceId("event", event_type, subject, dedupe_key), args: { event_type, subject, severity, message, data: resolvedData, metadata: resolvedMetadata, dedupe_key, deliver: resolvedDeliver } });
      return {
  ...
  server.tool("stations_events_list", "List shared events.", {}, async () => ({
  ...
  server.tool(
    ...
    async ({ event_id, source, event_type, dry_run, approval_token }) => {
      if (dry_run !== true) requireMcpMutation("stations_events_replay", approval_token, { resourceId: mutationResourceId("event-replay", event_id, source, event_type), args: { event_id, source, event_type, dry_run: false } });
      return {
  ...
  server.tool("stations_serve_dashboard", "Render the current dashboard HTML.", {}, async () => ({
    content: [{ type: "text", text: renderDashboardHtml() }],
  }));

  server.tool("storage_status", "Show stations storage sync configuration and local sync history.", {}, async () => ({
  ...
  server.tool(
    "storage_sync",
    "Bidirectional stations storage sync: pull then push.",
