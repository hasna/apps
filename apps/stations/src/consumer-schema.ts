...
export type StationsConsumerSchemaEnvelope =
  | "contract"
  | "topology"
  | "route"
  | "workspace"
...
export const STATIONS_CONSUMER_SCHEMA_BUNDLE: StationsConsumerSchemaBundle = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: STATIONS_CONSUMER_SCHEMA_URI,
  title: "@hasna/stations consumer contract schema bundle",
  type: "object",
  $defs: {
    cacheability: {
      type: "object",
      ...
      properties: {
        ...
        source_authority: { enum: ["stations", "open-stations", "manifest", "manifest_metadata", "live_topology", "argument", "inferred", "fallback", "unresolved", "mixed", "unknown"] }, // stations and open-stations accepted on read (open-stations is the legacy stored value)
    ...
    contract: {
      ...
      properties: {
        schema_version: { const: STATIONS_CONSUMER_CONTRACT_VERSION },
        package_name: { const: "@hasna/stations" },
        entrypoint: { const: "@hasna/stations/consumer" },
        schema_uri: { const: STATIONS_CONSUMER_SCHEMA_URI },
        schema_artifact: { const: "schemas/stations-consumer.schema.json" },
    ...
    command_matrix_plan: {
      ...
      properties: {
        ...
        mcp: {
          ...
          required: ["tool", "args"],
          properties: {
            tool: { const: "stations_ssh_resolve" },
            args: {
              type: "object",
    ...
    machine_health: {
      type: "object",
      required: ["schema_version", "package", "capabilities", "generated_at", "kind", "pagination", "summary", "stations", "artifacts", "warnings"],
      properties: {
        schema_version: { const: STATIONS_CONSUMER_CONTRACT_VERSION },
        package: { type: "object" },
        capabilities: { type: "object" },
        ...
        stations: { type: "array", items: { "$ref": "#/$defs/machine_health_row" } },
    ...
    loop_preflight: {
      type: "object",
      required: ["schema_version", "package", "capabilities", "generated_at", "kind", "mode", "selection_mode", "ok", "pagination", "summary", "stations", "artifacts", "warnings"],
      properties: {
        schema_version: { const: STATIONS_CONSUMER_CONTRACT_VERSION },
        package: { type: "object" },
        capabilities: { type: "object" },
        ...
        stations: { type: "array", items: { "$ref": "#/$defs/loop_preflight_machine" } },
        artifacts: { type: "array", items: { "$ref": "#/$defs/agent_artifact_ref" } },
        warnings: { type: "array", items: { type: "string" } },
      },
    },
    browserplan_fleet: {
      type: "object",
      required: ["schema_version", "package", "capabilities", "generated_at", "kind", "target", "coverage", "operation_contract", "stations", "warnings"],
      properties: {
        schema_version: { const: STATIONS_CONSUMER_CONTRACT_VERSION },
        package: { type: "object" },
        capabilities: { type: "object" },
        ...
        stations: {
          ...
          items: {
            ...
            properties: {
              ...
              install_state: {
                type: "object",
                required: ["checked", "source", "browserplan_cli", "stations_cli", "bun", "git", "node", "chrome", "warnings"],
                properties: {
                  checked: { type: "boolean" },
    ...
    note_actor_context: {
      ...
      properties: {
        ...
        source: { enum: ["notes", "open-notes", "agent", "sync", "import", "stations", "open-stations", "unknown"] }, // legacy open-* values (open-notes, open-stations) accepted on read
        display_name: { type: "string" },
      },
    },
    note_machine_context: {
    ...
    topology: {
      type: "object",
      required: ["schema_version", "package", "capabilities", "generated_at", "local_machine_id", "pagination", "stations", "warnings"],
      properties: {
        schema_version: { const: STATIONS_CONSUMER_CONTRACT_VERSION },
        package: { type: "object" },
        capabilities: { type: "object" },
    ...
    project_assignments: {
      type: "object",
      required: ["schema_version", "package", "generated_at", "assignments", "projects", "stations", "warnings"],
      properties: {
        schema_version: { const: STATIONS_CONSUMER_CONTRACT_VERSION },
        package: { type: "object" },
        generated_at: { type: "string", format: "date-time" },
...
function validateBrowserPlanSafeRunner(value: unknown, path: string, machineId: string, errors: string[]): void {
  ...
  if (isRecord(value.mcp)) {
    if (value.mcp.tool !== "stations_ssh_resolve") errors.push(`${path}.mcp.tool`);
...
function validateCommandMatrixPlan(value: unknown, path: string, errors: string[]): void {
  ...
  if (!isRecord(value.mcp)) {
    ...
    if (mcp.tool !== "stations_ssh_resolve") errors.push(`${path}.mcp.tool`);
...
export function getStationsConsumerSchemaBundle(): StationsConsumerSchemaBundle {
  return JSON.parse(JSON.stringify(STATIONS_CONSUMER_SCHEMA_BUNDLE)) as StationsConsumerSchemaBundle;
}

export function validateStationsConsumerEnvelope(
  envelope: StationsConsumerSchemaEnvelope,
  value: unknown,
): StationsConsumerValidationResult {
  const errors: string[] = [];
  if (!isRecord(value)) {
    return { ok: false, envelope, schema_id: STATIONS_CONSUMER_SCHEMA_URI, errors: ["not_object"] };
  }
  if (value.schema_version !== STATIONS_CONSUMER_CONTRACT_VERSION) errors.push(`schema_version:${String(value.schema_version)}`);
  ...
  if (envelope === "contract") {
    ...
    if (value.package_name !== "@hasna/stations") errors.push("package_name");
    if (value.entrypoint !== "@hasna/stations/consumer") errors.push("entrypoint");
    ...
    requireFields(value, ["package", "capabilities", "generated_at", "local_machine_id", "pagination", "stations", "warnings"], errors);
    validatePagination(value.pagination, "pagination", errors);
    if (!hasArray(value, "stations")) errors.push("stations");
    if (Array.isArray(value.stations)) {
      for (const [index, machine] of value.stations.entries()) {
        if (!isRecord(machine)) {
          errors.push(`stations.${index}`);
          continue;
        }
        requireFields(machine, ["machine_id", "friendly_name", "display_name", "updated_at"], errors);
        if (!hasString(machine, "machine_id")) errors.push(`stations.${index}.machine_id`);
        if (machine.aliases !== undefined && (!Array.isArray(machine.aliases) || machine.aliases.some((alias) => typeof alias !== "string"))) {
          errors.push(`stations.${index}.aliases`);
        }
        if (!hasString(machine, "display_name")) errors.push(`stations.${index}.display_name`);
        if (machine.friendly_name !== null && typeof machine.friendly_name !== "string") errors.push(`stations.${index}.friendly_name`);
        if (machine.updated_at !== null && typeof machine.updated_at !== "string") errors.push(`stations.${index}.updated_at`);
      }
    }
    ...
    requireFields(value, ["package", "generated_at", "assignments", "projects", "stations", "warnings"], errors);
    ...
    if (!hasObject(value, "actor")) {
      ...
      if (!["notes", "open-notes", "agent", "sync", "import", "stations", "open-stations", "unknown"].includes(String(actor.source))) errors.push("actor.source");
    ...
    requireFields(value, ["package", "capabilities", "generated_at", "kind", "pagination", "summary", "stations", "artifacts", "warnings"], errors);
    if (value.kind !== "machine_health") errors.push("kind");
    validatePagination(value.pagination, "pagination", errors);
    validateAgentSummary(value.summary, "summary", errors);
    validateAgentMachineRows(value.stations, "stations", errors);
    ...
    requireFields(value, ["package", "capabilities", "generated_at", "kind", "mode", "selection_mode", "ok", "pagination", "summary", "stations", "artifacts", "warnings"], errors);
    ...
    validateLoopPreflightRows(value.stations, "stations", errors);
    validateAgentArtifacts(value.artifacts, "artifacts", errors);
    if (!hasArray(value, "warnings")) errors.push("warnings");
  } else if (envelope === "browserplan_fleet") {
    requireFields(value, ["package", "capabilities", "generated_at", "kind", "target", "coverage", "operation_contract", "stations", "warnings"], errors);
    ...
    if (!hasArray(value, "stations")) {
      errors.push("stations");
    } else {
      const stations = value.stations as unknown[];
      const coverage = isRecord(value.coverage) ? value.coverage : null;
      if (coverage) {
        if (typeof coverage.expected === "number" && coverage.expected !== stations.length) errors.push("coverage.expected");
        if (typeof coverage.returned === "number" && coverage.returned !== stations.length) errors.push("coverage.returned");
        if (typeof coverage.known === "number" && Array.isArray(coverage.missing) && coverage.known !== stations.length - coverage.missing.length) errors.push("coverage.known");
      }
      for (const [index, machine] of stations.entries()) {
        if (!isRecord(machine)) {
          errors.push(`stations.${index}`);
          continue;
        }
        ...
        if (!hasString(machine, "machine_id")) errors.push(`stations.${index}.machine_id`);
        if (!BROWSERPLAN_MACHINE_ID_SET.has(String(machine.machine_id))) errors.push(`stations.${index}.machine_id`);
        if (machine.slug !== machine.machine_id) errors.push(`stations.${index}.slug`);
        if (BROWSERPLAN_EXCLUDED_MACHINE_ID_SET.has(String(machine.machine_id))) errors.push(`stations.${index}.excluded_machine_id`);
        if (!hasNullableString(machine, "friendly_name")) errors.push(`stations.${index}.friendly_name`);
        if (!hasNullableString(machine, "friendlyName")) errors.push(`stations.${index}.friendlyName`);
        if (!hasString(machine, "display_name")) errors.push(`stations.${index}.display_name`);
        if (!hasString(machine, "displayName")) errors.push(`stations.${index}.displayName`);
        if (machine.displayName !== machine.display_name) errors.push(`stations.${index}.displayName`);
        if (typeof machine.known !== "boolean") errors.push(`stations.${index}.known`);
        if (typeof machine.eligible !== "boolean") errors.push(`stations.${index}.eligible`);
        if (!hasObject(machine, "status")) {
          errors.push(`stations.${index}.status`);
        } else {
          const status = machine.status as Record<string, unknown>;
          requireFields(status, ["state", "label", "online"], errors);
          if (!["online", "offline", "unknown"].includes(String(status.state))) errors.push(`stations.${index}.status.state`);
          if (!["Online", "Offline", "Unknown"].includes(String(status.label))) errors.push(`stations.${index}.status.label`);
          if (status.online !== null && typeof status.online !== "boolean") errors.push(`stations.${index}.status.online`);
        }
        if (!hasObject(machine, "reachability")) {
          errors.push(`stations.${index}.reachability`);
        } else {
          ...
          if (typeof reachability.ok !== "boolean") errors.push(`stations.${index}.reachability.ok`);
          if (!BROWSERPLAN_ROUTE_KINDS.has(String(reachability.route))) errors.push(`stations.${index}.reachability.route`);
          if (!BROWSERPLAN_ROUTE_KINDS.has(String(reachability.source))) errors.push(`stations.${index}.reachability.source`);
          if (!BROWSERPLAN_ROUTE_CONFIDENCES.has(String(reachability.confidence))) errors.push(`stations.${index}.reachability.confidence`);
          if (typeof reachability.local !== "boolean") errors.push(`stations.${index}.reachability.local`);
          if (reachability.tailscale_online !== null && typeof reachability.tailscale_online !== "boolean") errors.push(`stations.${index}.reachability.tailscale_online`);
          if (typeof reachability.cacheable !== "boolean") errors.push(`stations.${index}.reachability.cacheable`);
          if (!hasArray(reachability, "warnings")) errors.push(`stations.${index}.reachability.warnings`);
        }
        if (!hasObject(machine, "install_state")) {
          errors.push(`stations.${index}.install_state`);
        } else {
          const install = machine.install_state as Record<string, unknown>;
          requireFields(install, ["checked", "source", "browserplan_cli", "stations_cli", "bun", "git", "node", "chrome", "warnings"], errors);
          if (typeof install.checked !== "boolean") errors.push(`stations.${index}.install_state.checked`);
          if (!["compatibility", "not_checked", "failed"].includes(String(install.source))) errors.push(`stations.${index}.install_state.source`);
          for (const key of ["browserplan_cli", "stations_cli", "bun", "git", "node", "chrome"]) {
            validateBrowserPlanCapability(install[key], `stations.${index}.install_state.${key}`, errors);
          }
          if (!hasArray(install, "warnings")) errors.push(`stations.${index}.install_state.warnings`);
        }
        if (!hasArray(machine, "operation_hooks")) {
          errors.push(`stations.${index}.operation_hooks`);
        } else {
          for (const [hookIndex, hook] of (machine.operation_hooks as unknown[]).entries()) {
            const path = `stations.${index}.operation_hooks.${hookIndex}`;
            if (!isRecord(hook)) {
              errors.push(path);
              continue;
            }
        ...
        if (!hasArray(machine, "warnings")) errors.push(`stations.${index}.warnings`);
      }
    }
    if (!hasArray(value, "warnings")) errors.push("warnings");
    ...
    if (!hasObject(value, "source")) {
      ...
      if (source.authority !== "stations" && source.authority !== "open-stations") errors.push("source.authority");
  ...
  return { ok: errors.length === 0, envelope, schema_id: STATIONS_CONSUMER_SCHEMA_URI, errors };
}
