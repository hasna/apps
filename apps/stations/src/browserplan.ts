...
export const BROWSERPLAN_ROUTE_OWNER = "stations";
...
const CAPABILITY_COMMANDS = ["browserplan", "stations", "bun", "git", "node", "google-chrome", "chromium", "chromium-browser"] as const;

function packageInfo(): StationsContractPackage {
  return {
...
function installState(
  ...
  try {
    const report = checkMachineCompatibility({
      ...
      packages: [
        { name: STATIONS_PACKAGE_NAME, command: "stations", required: false },
    ...
    return {
      ...
      stations_cli: capability(report, "stations"),
...
function safeRunner(machineId: string): BrowserPlanSafeRunnerContract {
  return {
    ...
    cli: {
      command: ["stations", "ssh", "--machine", machineId, "--cmd", "<browserplan-owned command>", "--json"],
...
function operationHook(input: {
  ...
  if (!input.known) blockedBy.push("machine_missing_from_open_stations_topology");
...
function eligibilityReasons(machine: MachineTopologyEntry | null, reach: BrowserPlanMachineReachability): string[] {
  ...
  if (!machine) reasons.push("machine_missing_from_open_stations_topology");
...
export function getBrowserPlanFleet(options: BrowserPlanFleetOptions = {}): BrowserPlanFleet {
  ...
  const stations = machineIds.map((machineId) => browserPlanMachine(machineId, topology, options));
  const missing = stations.filter((machine) => !machine.known).map((machine) => machine.machine_id);
  const unreachable = stations.filter((machine) => machine.known && !machine.reachability.ok).map((machine) => machine.machine_id);
  ...
  return {
    ...
    coverage: {
      ...
      known: stations.length - missing.length,
      missing,
      unreachable,
    ...
    stations,
    warnings: [...new Set([...topology.warnings, ...warnings, ...stations.flatMap((machine) => machine.warnings)])],
  };
}
