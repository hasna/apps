...
import {
  ...
  type StationsConsumerCapabilities,
  type StationsContractPackage,
} from "./topology.js";
...
interface Selection {
  topology: MachineTopology;
  stations: SelectedMachine[];
  pagination: MachineListPagination;
  selectionMode: "explicit" | "discovered";
  warnings: string[];
}

function packageMetadata(): StationsContractPackage {
  return { name: STATIONS_PACKAGE_NAME, version: getPackageVersion() };
}
...
function selectStations(options: AgentMachineSelectorOptions): Selection {
  ...
  const topologyById = new Map(topology.stations.map((machine) => [machine.machine_id, machine]));
  if (!hasExplicitIds) {
    return {
      topology,
      stations: topology.stations.map((entry) => ({ machineId: entry.machine_id, entry })),
  ...
  const selected = hasExplicitIds
    ...
    : topology.stations.map((entry) => ({ machineId: entry.machine_id, entry }));
  ...
  const stations = limit === null ? selected.slice(offset) : selected.slice(offset, offset + limit);
...
function detailRefs(machineId: string): AgentApiDetailRefs {
  return {
    cli: `stations details --machine ${shellQuote(machineId)} --json`,
    mcp: "stations_details",
...
function routingDetailRefs(machineId: string): AgentApiDetailRefs {
  return {
    cli: `stations route --machine ${shellQuote(machineId)} --json`,
    mcp: "stations_route_resolve",
...
function artifactRefs(limit: number | null, offset: number, privateMetadata = false): AgentApiArtifactRef[] {
  ...
  return [
    { kind: "topology", ref: `stations topology ${paginationArgs} --json`, format: "json", private: false },
    { kind: "doctor", ref: "stations doctor --machine <machine-id> --json", format: "json", private: false },
    { kind: "route", ref: `stations route --machine <machine-id>${privateMetadata ? " --private-metadata" : ""} --json`, format: "json", private: privateMetadata },
  ];
}
...
export function getFleetMachineHealth(options: MachineHealthOptions = {}): MachineHealthReport {
  const selection = selectStations(options);
  const stations = selection.stations.map((selected) => buildHealthRow({ selected, topology: selection.topology, options }));
  return {
    ...
    artifacts: [
      ...
      { kind: "compatibility", ref: "stations compatibility --machine <machine-id> --json", format: "json", private: false },
    ],
...
export function getFleetRouting(options: FleetRoutingOptions = {}): FleetRoutingReport {
  const selection = selectStations(options);
  const routes = selection.stations.map((selected) => buildRoutingRow({ selected, topology: selection.topology, options }));
  return {
    schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
    package: packageMetadata(),
    capabilities: getStationsConsumerCapabilities(),
...
function buildCommandPlan(input: {
  ...
  const cliCommand = input.route.local
    ...
    : `stations ssh --machine ${shellQuote(input.machineId)} --cmd ${shellQuote(publicCommand)}${input.options.privateMetadata ? " --private-metadata" : ""}`;
...
export function getCommandMatrix(options: CommandMatrixOptions = {}): CommandMatrixReport {
  const selection = selectStations(options);
  const commands = selection.stations.map((selected) => buildCommandMatrixRow({ selected, topology: selection.topology, options }));
  return {
    schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
    package: packageMetadata(),
    capabilities: getStationsConsumerCapabilities(),
    ...
    artifacts: [
      ...artifactRefs(selection.pagination.limit, selection.pagination.offset, options.privateMetadata),
      { kind: "command_matrix", ref: "stations command-matrix --machine <machine-id> --cmd '<command>'", format: "json", private: false },
    ],
    warnings: bounded([...new Set(selection.warnings)]),
  };
}
...
export function getFleetLoopPreflight(options: FleetLoopPreflightOptions = {}): FleetLoopPreflightReport {
  const selection = selectStations(options);
  const healthRows = selection.stations.map((selected) => buildHealthRow({ selected, topology: selection.topology, options }));
  const commandRows = selection.stations.map((selected) => buildCommandMatrixRow({ selected, topology: selection.topology, options }));
  const commandByMachine = new Map(commandRows.map((row) => [row.machine_id, row]));
  const stations = healthRows.map((health) => {
  ...
  const baseSummary = summary(stations);
  const anyReady = stations.some((machine) => machine.ready);
  const allReady = stations.length > 0 && stations.every((machine) => machine.ready);
  return {
    schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
    package: packageMetadata(),
    capabilities: getStationsConsumerCapabilities(),
    ...
    summary: {
      ...baseSummary,
      runnable: stations.filter((machine) => machine.can_run).length,
      any_ready: anyReady,
      all_ready: allReady,
    },
    stations,
    artifacts: [
      ...
      { kind: "command_matrix", ref: "stations command-matrix --json", format: "json", private: false },
    ],
    warnings: bounded([...new Set(selection.warnings)]),
  };
}
