...
function renderMachineHealthResult(result: MachineHealthReport): string {
  const lines = result.stations.map((machine) =>
  ...
  return [
    renderKeyValueTable([
      ["stations", `${result.pagination.count}/${result.pagination.total}`],
...
function renderFleetOpsCheck(result: FleetOpsCheck): string {
  ...
  return [
    renderKeyValueTable([
      ...
      ["ok", String(result.ok)],
      ["stations", String(result.summary.stations)],
      ["ready", String(result.summary.ready)],
      ["blocked", String(result.summary.blocked)],
...
function renderDispatchFleetSmoke(result: DispatchFleetSmokeReport): string {
  ...
  return [
    renderKeyValueTable([
      ...
      ["stations", String(result.summary.total)],
...
function renderBrowserPlanFleet(result: BrowserPlanFleet): string {
  const lines = [
    renderKeyValueTable([
      ...
      ["stations", `${result.coverage.known}/${result.coverage.expected} known`],
  ...
  for (const machine of result.stations) {
...
const clipboardCommand = program.command("clipboard").description("Real-time clipboard sync across fleet stations");
const installClaudeCommand = program.command("install-claude").description("Install or inspect Claude, Codex, and Gemini CLIs");
const daemonCommand = program.command("daemon").description("Install and inspect the stations-daemon fleet daemon service");
const heartbeatCommand = program.command("heartbeat").description("Collect and inspect stations-daemon heartbeat rows");
...
function cliMutationCallerId(): string {
  return process.env["HASNA_STATIONS_MUTATION_CALLER_ID"]?.trim() || "cli";
}

function cliMutationRunId(): string {
  return process.env["HASNA_STATIONS_MUTATION_RUN_ID"]?.trim() || "cli";
}
...
function runtimeTmuxCommand(): string {
  return process.env["HASNA_STATIONS_TMUX_BIN"]?.trim() || "tmux";
}

function runtimeTmuxEventTypes(once: boolean): string[] {
  return once ? ["stations.tmux.pane_missing"] : ["stations.tmux.pane_died"];
}
...
eventWebhooksCommand
  ...
  .action(async (target: string, options: WebhookAddCliOptions, command: Command) => {
    ...
    requireCliMutation("stations_webhooks_add", options.approvalToken, {
...
eventWebhooksCommand
  ...
  .action(async (id: string, options: { approvalToken?: string; json?: boolean }, command: Command) => {
    requireCliMutation("stations_webhooks_remove", options.approvalToken, {
...
eventWebhooksCommand
  ...
  .action(async (id: string, options: WebhookTestCliOptions, command: Command) => {
    ...
    requireCliMutation("stations_webhooks_test", options.approvalToken, {
...
eventsCommand
  ...
  .action(async (options: EventsReplayCliOptions, command: Command) => {
    if (options.dryRun !== true) {
      requireCliMutation("stations_events_replay", options.approvalToken, {
...
function addDaemonLifecycleCommand(action: DaemonServiceAction, description: string): void {
  daemonCommand
    ...
    .option("--service-name <name>", "Service name/label", "stations-daemon")
    .option("--executable <path>", "Absolute stations-daemon executable path")
...
addDaemonLifecycleCommand("install", "Plan or install the stations-daemon daemon service");
addDaemonLifecycleCommand("uninstall", "Plan or uninstall the stations-daemon daemon service");
addDaemonLifecycleCommand("restart", "Plan or restart the stations-daemon daemon service");
...
heartbeatCommand
  ...
  .option("--stations-command <command>", "stations CLI command or absolute path", "stations")
  .option("-j, --json", "Print JSON output", false)
  .action((options: { machine?: string[]; timeoutMs?: string; stationsCommand?: string; json?: boolean }, command: Command) => {
    const plan = buildHeartbeatCollectorCommand({
      stations: options.machine,
      timeoutMs: options.timeoutMs ? parseIntegerOption(options.timeoutMs, "timeout-ms", { min: 1 }) : undefined,
      stationsCommand: options.stationsCommand,
    });
...
heartbeatCommand
  .command("collect")
  .description("Run one-shot stations-daemon heartbeats over machine routes and import public rows locally")
  .option("--machine <id...>", "Machine identifier to collect; repeat for multiple stations", collectOptionValues, [])
  .option("--timeout-ms <ms>", "Per-machine command timeout in milliseconds")
...
manifestFriendlyNameCommand
  ...
  .action((id: string, name: string, options: { approvalToken?: string; json?: boolean }) => {
    ...
    requireCliMutation("stations_friendly_name_set", options.approvalToken, {
...
manifestFriendlyNameCommand
  ...
  .action((id: string, options: { approvalToken?: string; json?: boolean }) => {
    ...
    requireCliMutation("stations_friendly_name_clear", options.approvalToken, {
      machineId: input.machineId,
      resourceId: machineFriendlyNameResourceId(input.machineId),
...
program
  ...
  .action(async (options: {
    ...
    const machineId = cliMachineId(options.machine ?? process.env["HASNA_STATIONS_MACHINE_ID"]);
...
program
  ...
  .option("--limit <n>", `Maximum stations to return (default ${DEFAULT_MACHINE_LIST_LIMIT})`)
  ...
  .action((options: { tailscale?: boolean; limit?: string; offset?: string; all?: boolean; privateMetadata?: boolean; json?: boolean }) => {
    ...
    console.log(renderKeyValueTable([
      ...
      ["stations", `${topology.pagination.count}/${topology.pagination.total}`],
    ...
    for (const machine of topology.stations) {
...
browserPlanCommand
  ...
  .description("List BrowserPlan target stations and safe remote operation hooks")
...
notesCommand
  ...
  .option("--source <source>", "notes | agent | sync | import | stations | unknown")
...
program
  ...
  .description("Return compact local/remote loop-readiness health for stations")
...
program
  .command("routing")
  .description("Return compact route readiness for local and remote stations")
  .option("--machine <id...>", "Limit to machine ids; comma-separated values are accepted")
  .option("--no-tailscale", "Skip tailscale status probing")
  .option("--limit <n>", `Maximum stations to return (default ${DEFAULT_MACHINE_LIST_LIMIT})`)
  .option("--offset <n>", "Machine list offset for pagination")
...
opsCommand
  ...
  .action((options: AgentApiCliOptions & {
    ...
    const result = getFleetOpsCheck({
      ...
      command: options.cmd,
      commandLabel: options.commandLabel,
      expectedStations: parseMachineIdList(options.expectMachine),
...
projectsAssignmentsCommand
  .command("assign")
  .description("Write a machine/project location assignment into the stations manifest")
  .requiredOption("--machine <id>", "Machine identifier")
  ...
  .action((options: {
    ...
    requireCliMutation("stations_projects_assign", options.approvalToken, {
      machineId: input.machineId,
...
projectsAssignmentsCommand
  .command("remove")
  .description("Remove a machine/project assignment from the stations manifest")
  .requiredOption("--machine <id>", "Machine identifier")
  .requiredOption("--project <id>", "Project or workspace identifier")
  .option("--approval-token <token>", "Scoped mutation approval token")
  .option("-j, --json", "Print JSON output", false)
  .action((options: { machine: string; project: string; approvalToken?: string; json?: boolean }) => {
    ...
    requireCliMutation("stations_projects_unassign", options.approvalToken, {
      machineId: input.machineId,
      resourceId: projectAssignmentResourceId(input.machineId, input.projectId),
...
workspaceCommand
  ...
  .option("--allow-untrusted", "Allow writing mappings for stations not marked trusted", false)
  .option("--no-tailscale", "Skip tailscale status probing")
  .option("--approval-token <token>", "Scoped mutation approval token")
  .option("-j, --json", "Print JSON output", false)
  .action((options: {
...
program
  .command("diff")
  .description("Show manifest differences between two stations")
  ...
  .action((options: { left: string; right?: string; json?: boolean }) => {
    const result = diffStations(options.left, options.right);
    console.log(JSON.stringify(result, null, 2));
  });
...
program
  ...
  .option("--bucket <name>", "S3 bucket name; defaults to HASNA_STATIONS_S3_BUCKET or STATIONS_S3_BUCKET")
  .option("--prefix <prefix>", "S3 key prefix; defaults to HASNA_STATIONS_S3_PREFIX, STATIONS_S3_PREFIX, or stations")
...
function printHostsResult(plan: ReturnType<typeof planFleetHosts>, applied: boolean, viaSudo = false): void {
  ...
  console.log(applied ? `applied ${plan.entries.length} entries${viaSudo ? " (via sudo)" : ""}` : "dry run — re-run `stations hosts apply` to write");
}
...
notificationsCommand
  ...
  .option("--message <message>", "Test message", "stations notification test")
...
runtimeCommand
  ...
  .description("Print a tmux pane-died hook command that emits stations events")
  .option("--stations-command <command>", "stations CLI executable", "stations")
  ...
  .option("-j, --json", "Print JSON output", false)
  .action((options: { stationsCommand?: string; tmuxCommand?: string; deliver?: boolean; approvalToken?: string; trustedLocalMutation?: boolean; json?: boolean }) => {
...
runtimeCommand
  .command("tmux-watch")
  .description("Watch a tmux pane and emit stations.tmux.pane_died if it disappears")
  ...
  .option("--once", "Probe once and emit stations.tmux.pane_missing when absent", false)
  ...
  .action(async (target: string, options: RuntimeTmuxWatchCliOptions) => {
    ...
    if (deliver) {
      requireCliMutation("stations_runtime_tmux_watch_deliver", options.approvalToken, {
...
program
  ...
  .action((machineArg: string | undefined, options: { machine?: string; all?: boolean; print?: boolean; json?: boolean }) => {
    if (options.all) {
      ...
      const results: ScreenAllResult[] = topology.stations.map((m) => {
        try {
    ...
    if (!machineId) {
      console.error("Provide a machine: stations screen <id>  (or --all)");
      process.exitCode = 1;
      return;
    }
...
program
  ...
  .action((options: { machine?: string; all?: boolean; checkSecret?: boolean; secretsCommand: string; tailscale?: boolean; strict?: boolean; json?: boolean }) => {
    ...
    const machineIds = options.all
      ? topology.stations.map((machine) => machine.machine_id)
...
program
  ...
  .action((options: { host: string; port: string; json?: boolean }) => {
    ...
    console.log(chalk.green(`stations dashboard listening on http://${server.hostname}:${server.port}`));
  });

const dbCommand = program.command("db").description("Cloud database schema operations (Amendment A1: shared RDS stations database)");

dbCommand
  .command("migrate")
  .description("Apply pending cloud migrations (api-keys + stations registry). Connects as the owner role.")
...
function requireRoot(): boolean {
  ...
  if (uid !== 0) {
    console.error(chalk.red("error: this command must run as root (try: sudo stations heal install)"));
    return false;
  }
  return true;
}
...
healCommand
  ...
  .action((options: { determinism?: boolean; watchdog?: boolean; service?: boolean }) => {
    ...
    if (!config.preferredSsid) {
      console.error(chalk.red("error: set preferredSsid first: stations heal config --set '{\"preferredSsid\":\"X81ND\"}'"));
      process.exitCode = 1;
      return;
    }
    const out: string[] = [];
...
flipCommand
  ...
  .option("--stations <ids>", "Restrict to these machine ids (comma/space separated)")
  .option("--tags <tags>", "Restrict to stations carrying ALL of these tags")
  .option("--exclude <ids>", "Exclude these machine ids")
  .option("--all-stations", "Flip the ENTIRE fleet in one atomic wave (coordination cutover)", false)
  .option("--canary <n>", "Canary wave size", "1")
  .option("--batch <n>", "Batch size after canary", "4")
  .option("-j, --json", "Print JSON output", false)
  .action((app: string, options: Record<string, string | undefined> & { json?: boolean; allStations?: boolean }) => {
    ...
    for (const wave of plan.waves) console.log(`  ${wave.name}: ${wave.stations.join(", ") || "(none)"}`);
...
flipCommand
  ...
  .option("--mode <mode>", "api (route the client to the hosted API) or local (revert)", "api")
  .option("--stations <ids>", "Restrict to these machine ids")
  .option("--tags <tags>", "Restrict to stations carrying ALL of these tags")
  .option("--exclude <ids>", "Exclude these machine ids")
  .option("--all-stations", "Flip the ENTIRE fleet in one atomic wave (coordination cutover)", false)
  .option("--canary <n>", "Canary wave size", "1")
  ...
  .option("-j, --json", "Print JSON output", false)
  .action((app: string, options: Record<string, string | undefined> & { execute?: boolean; json?: boolean; freezeCheck?: string; allStations?: boolean }) => {
    const spec = getFlipApp(app);
    const mode = normalizeFlipMode(options["mode"]);
    const { waves } = resolveFlipWaves(spec, options as never);
...
flipCommand
  ...
  .option("--tags <tags>", "Restrict to stations carrying ALL of these tags")
  .option("--exclude <ids>", "Exclude these machine ids")
  .option("--all-stations", "Revert the ENTIRE fleet in one atomic wave", false)
  .option("--canary <n>", "Canary wave size", "1")
  .option("--batch <n>", "Batch size after canary", "4")
  .option("--execute", "Actually run (default is dry-run)", false)
  .option("-j, --json", "Print JSON output", false)
  .action((app: string, options: Record<string, string | undefined> & { execute?: boolean; json?: boolean; allStations?: boolean }) => {
    const spec = getFlipApp(app);
    const { waves } = resolveFlipWaves(spec, options as never);
...
const registryCommand = program
  .command("registry")
  .description("Machine registry CRUD (routes to <stations.host>/v1/stations when flipped to the hosted API, else local store)");
...
registryCommand
  ...
  .action(async (
    ...
    requireCliMutation("stations_registry_register", options.approvalToken, {
...
registryCommand
  ...
  .option("-j, --json", "Print JSON output", false)
  .action(async (id: string, options: { approvalToken?: string; json?: boolean }, command: Command) => {
    requireCliMutation("stations_registry_remove", options.approvalToken, {
