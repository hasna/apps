// Machine registry data-access layer for the stations control-plane service.
//
// The registry is the canonical fleet inventory: the set of stations the
...
export class MachineRegistry {
  ...
  async list(options: ListStationsOptions = {}): Promise<MachineRecord[]> {
    ...
    const rows = await this.client.many<Row>(
      `SELECT * FROM stations ${where} ORDER BY updated_at DESC ${limitClause} ${offsetClause}`,
      params,
    );
  ...
  async get(id: string): Promise<MachineRecord | null> {
    ...
    const row = await this.client.get<Row>(`SELECT * FROM stations WHERE id = $1`, [cleanId]);
  ...
  async upsert(input: UpsertMachineInput): Promise<MachineRecord> {
    ...
    const row = await this.client.one<Row>(
      `INSERT INTO stations (id, friendly_name, platform, arch, status, labels, metadata, updated_at)
  ...
  async remove(id: string): Promise<boolean> {
    const cleanId = requireId(id);
    const result = await this.client.query(`DELETE FROM stations WHERE id = $1`, [cleanId]);
    return result.rowCount > 0;
  }

  async count(status?: string): Promise<number> {
    const row = status
      ? await this.client.get<Row>(`SELECT count(*)::int AS c FROM stations WHERE status = $1`, [status])
      : await this.client.get<Row>(`SELECT count(*)::int AS c FROM stations`);
