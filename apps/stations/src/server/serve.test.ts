...
function keyFor(scopes: string[]): string {
  return mintApiKey({ app: "stations", scopes, signingSecret: SIGNING_SECRET }).token;
}
...
describe("stations-serve handler", () => {
  ...
  test("GET /openapi.json exposes the versioned paths", async () => {
    ...
    expect(doc.paths["/v1/stations"]).toBeDefined();
    expect(doc.paths["/v1/stations/{id}"]).toBeDefined();
  });

  test("unauthenticated /v1 request is rejected with 401", async () => {
    const res = await handler()(new Request("http://x/v1/stations"));
    expect(res.status).toBe(401);
  });

  test("a bad token is rejected with 401", async () => {
    const res = await handler()(new Request("http://x/v1/stations", { headers: { "x-api-key": "hasna_stations_garbage" } }));
    expect(res.status).toBe(401);
  });

  test("read-scoped key can list but not write (403 on write)", async () => {
    const h = handler();
    const readKey = keyFor(["stations:read"]);
    const list = await h(new Request("http://x/v1/stations", { headers: { "x-api-key": readKey } }));
    expect(list.status).toBe(200);

    const write = await h(
      new Request("http://x/v1/stations", {
        method: "POST",
  ...
  test("write-scoped key completes a full CRUD roundtrip", async () => {
    const h = handler();
    const key = keyFor(["stations:read", "stations:write"]);
    const auth = { "x-api-key": key, "content-type": "application/json" };

    const created = await h(new Request("http://x/v1/stations", { method: "POST", headers: auth, body: JSON.stringify({ id: "spark01" }) }));
    expect(created.status).toBe(200);

    const got = await h(new Request("http://x/v1/stations/spark01", { headers: auth }));
    expect(got.status).toBe(200);

    const patched = await h(new Request("http://x/v1/stations/spark01", { method: "PATCH", headers: auth, body: JSON.stringify({ status: "online" }) }));
    expect(patched.status).toBe(200);

    const deleted = await h(new Request("http://x/v1/stations/spark01", { method: "DELETE", headers: auth }));
    expect(deleted.status).toBe(200);
    expect((await deleted.json()).deleted).toBe(true);

    const missing = await h(new Request("http://x/v1/stations/spark01", { headers: auth }));
    expect(missing.status).toBe(404);
  });

  test("Bearer scheme is accepted", async () => {
    const key = keyFor(["stations:read"]);
