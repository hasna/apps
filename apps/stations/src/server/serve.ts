// stations-serve — the HTTP control-plane API for the machine registry.
//
// Surfaces:
...
//   /v1/stations[...]     registry CRUD (API-key auth, scopes stations:read/write)
//   /v1/heartbeats        fleet heartbeats (API-key auth, stations:read)
//
...
// stations Postgres directly through the vendored storage kit — no cache, no
// local mirror. The server data backend is `sqlite | postgresql` selected by
// HASNA_STATIONS_DATABASE_URL presence; any set storage-mode variable throws.
...
function resolveSigningSecret(): string {
  const secret = process.env["HASNA_STATIONS_API_SIGNING_KEY"]
    || process.env["API_KEY_SIGNING_SECRET"]
    || process.env["HASNA_API_SIGNING_KEY"];
  if (!secret || secret.trim().length === 0) {
    throw new Error(
      "stations-serve requires an API-key signing secret. Set HASNA_STATIONS_API_SIGNING_KEY "
...
export function createHandler(deps: {
  ...
  return async function handle(req: Request): Promise<Response> {
    ...
    if (path === "/v1/stations" && method === "GET") {
      const denied = await authorize(req, ["stations:read"]);
      if (denied) return denied;
      const status = url.searchParams.get("status") ?? undefined;
      const limit = url.searchParams.get("limit");
      const offset = url.searchParams.get("offset");
      const stations = await deps.registry().list({
        ...(status ? { status } : {}),
    ...
    if (path === "/v1/stations" && method === "POST") {
      const denied = await authorize(req, ["stations:write"]);
      if (denied) return denied;
    ...
    const machineMatch = path.match(/^\/v1\/stations\/([^/]+)$/);
    if (machineMatch) {
...
export function startServer(options: StartServerOptions = {}): StationsServer {
  ...
  const handler = createHandler({
    ...
    // ignore permission errors — the table is guaranteed by `stations db migrate`.
