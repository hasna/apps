// Migration runner for the stations cloud database. Connects as the OWNER role
// (DDL) and applies, in order: the api-keys auth schema (@hasna/contracts) then
// the stations registry schema. Idempotent and checksum-guarded by the vendored
...
import { STATIONS_MIGRATIONS } from "./migrations.js";

/** Full ordered migration set for the stations database. */
export function allMigrations(): Migration[] {
  const auth = apiKeyMigrations().map((m) => defineMigration(m.id, m.sql));
  return [...auth, ...STATIONS_MIGRATIONS];
}
