...
import { STATIONS_CONSUMER_CONTRACT_VERSION, STATIONS_PACKAGE_NAME, getStationsConsumerCapabilities, type StationsContractPackage, type StationsConsumerCapabilities } from "./topology.js";
...
const DEFAULT_COMMANDS: CompatibilityCommandSpec[] = [
  ...
  { command: "stations", required: true },
];

function defaultPackages(): CompatibilityPackageSpec[] {
  return [{ name: "@hasna/stations", command: "stations", expectedVersion: getPackageVersion(), required: true }];
}
...
function packageCommand(name: string): string {
  ...
  if (name === "@hasna/stations") return "stations";
