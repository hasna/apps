...
function runMachineCommandWithProcessGroupTimeout(
  ...
  const maxOutputChars = Number.isFinite(options.maxOutputChars) && (options.maxOutputChars ?? 0) > 0
    ...
    : null;
  const helperDir = mkdtempSync(join(tmpdir(), "stations-timeout-helper-"));
  ...
  const helper = spawnSync(process.execPath, ["--eval", PROCESS_GROUP_TIMEOUT_HELPER], {
    ...
    env: {
      ...process.env,
      HASNA_STATIONS_COMMAND_TIMEOUT_MS: String(timeoutMs),
      HASNA_STATIONS_COMMAND_KILL_GRACE_MS: String(killGraceMs),
      HASNA_STATIONS_COMMAND_PGID_FILE: pgidFile,
      HASNA_STATIONS_COMMAND_MAX_OUTPUT_CHARS: maxOutputChars === null ? "" : String(maxOutputChars),
      HASNA_STATIONS_COMMAND_REDACT_OUTPUT: options.redactOutput === true ? "1" : "",
    },
...
const PROCESS_GROUP_TIMEOUT_HELPER = `
	...
	const timeoutMs = Math.max(1, Number.parseInt(process.env.HASNA_STATIONS_COMMAND_TIMEOUT_MS || "1", 10));
	const killGraceMs = Math.max(1, Number.parseInt(process.env.HASNA_STATIONS_COMMAND_KILL_GRACE_MS || "1000", 10));
	const parsedMaxOutputChars = Number.parseInt(process.env.HASNA_STATIONS_COMMAND_MAX_OUTPUT_CHARS || "", 10);
	const maxOutputChars = Number.isFinite(parsedMaxOutputChars) && parsedMaxOutputChars > 0 ? parsedMaxOutputChars : null;
	const redactOutput = process.env.HASNA_STATIONS_COMMAND_REDACT_OUTPUT === "1";
	const pgidFile = process.env.HASNA_STATIONS_COMMAND_PGID_FILE || "";
