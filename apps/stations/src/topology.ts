...
export const STATIONS_CONSUMER_CONTRACT_VERSION = 1;
export const STATIONS_PACKAGE_NAME = "@hasna/stations";
export const STATIONS_CONSUMER_ENTRYPOINT = "@hasna/stations/consumer";
export const STATIONS_CONSUMER_SCHEMA_URI = "https://schemas.example.com/stations/consumer/v1/stations-consumer.schema.json";
export const STATIONS_CONSUMER_SCHEMA_ARTIFACT = "schemas/stations-consumer.schema.json";
...
export interface StationsContractPackage {
  name: typeof STATIONS_PACKAGE_NAME;
  version: string;
}

export interface StationsConsumerCapabilities {
  topology: true;
  compatibility: true;
...
export type StationsConsumerEnvelope =
  | "topology"
  | "route"
  | "workspace"
  | "compatibility"
...
export interface StationsConsumerFieldCapabilities {
  topology: {
...
export interface StationsConsumerContract {
  schema_version: typeof STATIONS_CONSUMER_CONTRACT_VERSION;
  package_name: typeof STATIONS_PACKAGE_NAME;
  entrypoint: typeof STATIONS_CONSUMER_ENTRYPOINT;
  schema_uri: typeof STATIONS_CONSUMER_SCHEMA_URI;
  schema_artifact: typeof STATIONS_CONSUMER_SCHEMA_ARTIFACT;
  capabilities: StationsConsumerCapabilities;
  field_capabilities: StationsConsumerFieldCapabilities;
  cacheability: {
    default_ttl_ms: typeof DEFAULT_MACHINE_RESOLVER_TTL_MS;
    stale_requires_refresh: true;
  };
  envelopes: StationsConsumerEnvelope[];
  stable_exports: string[];
}

export const STATIONS_CONSUMER_CAPABILITIES: StationsConsumerCapabilities = {
  topology: true,
  compatibility: true,
  route_resolution: true,
  cli_json_fallback: true,
...
export const STATIONS_CONSUMER_FIELD_CAPABILITIES: StationsConsumerFieldCapabilities = {
  topology: {
    machine_identity: true,
    friendly_names: true,
    display_name_fallback: true,
...
export const STATIONS_CONSUMER_CONTRACT: StationsConsumerContract = {
  schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
  package_name: STATIONS_PACKAGE_NAME,
  entrypoint: STATIONS_CONSUMER_ENTRYPOINT,
  schema_uri: STATIONS_CONSUMER_SCHEMA_URI,
  schema_artifact: STATIONS_CONSUMER_SCHEMA_ARTIFACT,
  capabilities: STATIONS_CONSUMER_CAPABILITIES,
  field_capabilities: STATIONS_CONSUMER_FIELD_CAPABILITIES,
  cacheability: {
  ...
  stable_exports: [
    ...
    "STATIONS_CONSUMER_FIELD_CAPABILITIES",
...
export function getStationsConsumerCapabilities(): StationsConsumerCapabilities {
  return { ...STATIONS_CONSUMER_CAPABILITIES };
}
...
function envReachableHosts(): Set<string> {
  const raw = process.env["HASNA_STATIONS_REACHABLE_HOSTS"];
...
function paginateMachineEntries(entries: MachineTopologyEntry[], options: MachineTopologyOptions): {
  ...
  const stations = limit === null
    ? ordered.slice(offset)
    : ordered.slice(offset, offset + limit);
  const nextOffset = offset + stations.length < total ? offset + stations.length : null;
...
export function discoverMachineTopology(options: MachineTopologyOptions = {}): MachineTopology {
  ...
  const manifestById = new Map(manifest.stations.map((machine) => [machine.id, machine]));
  ...
  const { stations, pagination } = paginateMachineEntries(allStations, options);
  return {
    schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
    package: {
...
export function redactTopologyForOutput(topology: MachineTopology, options: PublicOutputOptions = {}): MachineTopology {
  ...
  return {
    ...
    stations: topology.stations.map((machine) => ({
      ...machine,
...
export function findMachineTopologyEntry(topology: MachineTopology, machineId: string): MachineTopologyEntry | null {
  ...
  return topology.stations.find((machine) => normalizeMachineAlias(machine.machine_id) === requested)
    ?? topology.stations.find((machine) => (machine.aliases ?? []).some((alias) => normalizeMachineAlias(alias) === requested))
    ?? null;
}
...
function findRouteMachine(topology: MachineTopology, requestedMachineId: string): {
  ...
  if (requested === "local" || requested === "localhost" || requested === normalizeMachineAlias(hostname()) || requested === normalizeMachineAlias(topology.local_machine_id)) {
    return {
      machine: topology.stations.find((machine) => machine.machine_id === topology.local_machine_id) ?? null,
      matchedBy: "local_alias",
    };
  }

  const machineIdMatch = topology.stations.find((machine) => normalizeMachineAlias(machine.machine_id) === requested);
  if (machineIdMatch) return { machine: machineIdMatch, matchedBy: "machine_id" };
  const aliasMatch = topology.stations.find((machine) => (machine.aliases ?? []).some((alias) => normalizeMachineAlias(alias) === requested));
  if (aliasMatch) return { machine: aliasMatch, matchedBy: "alias" };

  const hostnameMatch = topology.stations.find((machine) => machine.hostname && normalizeMachineAlias(machine.hostname) === requested);
  if (hostnameMatch) return { machine: hostnameMatch, matchedBy: "hostname" };

  const tailscaleMatch = topology.stations.find((machine) => {
  ...
  const routeMatch = topology.stations.find((machine) => routeTargetMatches(machine, requestedMachineId));
...
export function resolveMachineRoute(machineId: string, options: MachineRouteOptions = {}): MachineRouteResolution {
  ...
  if (!machine) {
    ...
    return {
      schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
      package: { name: STATIONS_PACKAGE_NAME, version: getPackageVersion() },
      ok: false,
      machine_id: null,
...
export function getLocalMachineTopology(options: MachineTopologyOptions = {}): MachineTopologyEntry {
  ...
  return topology.stations.find((machine) => machine.machine_id === topology.local_machine_id) ?? {
