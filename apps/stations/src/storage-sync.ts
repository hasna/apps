...
export const STATIONS_STORAGE_TABLES = STORAGE_TABLES;
...
export const STATIONS_STORAGE_ENV = "HASNA_STATIONS_DATABASE_URL";
export const STATIONS_STORAGE_FALLBACK_ENV = "STATIONS_DATABASE_URL";
export const STATIONS_STORAGE_MODE_ENV = "HASNA_STATIONS_STORAGE_MODE";
export const STATIONS_STORAGE_MODE_FALLBACK_ENV = "STATIONS_STORAGE_MODE";
export const STORAGE_DATABASE_ENV = [STATIONS_STORAGE_ENV, STATIONS_STORAGE_FALLBACK_ENV] as const;
export const STORAGE_MODE_ENV = [STATIONS_STORAGE_MODE_ENV, STATIONS_STORAGE_MODE_FALLBACK_ENV] as const;
...
export async function getStoragePg(): Promise<PgAdapterAsync> {
  ...
  if (!url) {
    throw new Error("Missing HASNA_STATIONS_DATABASE_URL or STATIONS_DATABASE_URL");
  }
...
export function getSyncMetaAll(): SyncMeta[] {
  ...
  return db.query("SELECT table_name, last_synced_at, direction FROM _stations_sync_meta ORDER BY table_name, direction").all() as SyncMeta[];
}
...
export function resolveTables(tables?: string[]): StorageTable[] {
  ...
  if (invalid.length > 0) throw new Error(`Unknown stations storage table(s): ${invalid.join(", ")}`);
...
function recordSyncMeta(db: Database, direction: "push" | "pull", results: SyncResult[]): void {
  ...
  const statement = db.query(`
    INSERT INTO _stations_sync_meta (table_name, last_synced_at, direction)
    VALUES (?, ?, ?)
...
function ensureSyncMetaTable(db: Database): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS _stations_sync_meta (
