...
export const PRIVATE_MANIFEST_REF_ENV = "HASNA_STATIONS_PRIVATE_MANIFEST_REF";
export const PRIVATE_MANIFEST_BACKEND_ENV = "HASNA_STATIONS_PRIVATE_MANIFEST_BACKEND";
...
const exactBunRegistrySchema = z.object({
  schema: z.literal("stations.exact_bun_registry.v1"),
...
export const fleetSchema = z.object({
  ...
  stations: z.array(machineSchema),
  ...
  fleet.stations.forEach((machine, index) => {
    if (seenStations.has(machine.id)) {
      context.addIssue({ code: z.ZodIssueCode.custom, path: ["stations", index, "id"], message: `duplicate machine id ${machine.id}` });
    }
    seenStations.add(machine.id);
    ...
    for (const identity of identities) {
      ...
      if (previous) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["stations", index, identity.kind === "id" ? "id" : "aliases", ...(identity.aliasIndex === undefined ? [] : [identity.aliasIndex])],
...
function normalizeStations(stations: MachineManifest[]): MachineManifest[] {
  return [...stations].sort((left, right) => left.id.localeCompare(right.id));
}
...
function migrateLegacyManifest(raw: unknown): unknown {
  if (!isRecord(raw) || !Array.isArray(raw.stations)) return raw;

  let migrated = false;
  const stations = raw.stations.map((machine): unknown => {
  ...
  return migrated ? { ...raw, stations } : raw;
}
...
function privateRefFromOptions(options: ReadManifestWithSourceOptions): string | null {
  ...
  return options.privateRef?.trim()
    ...
    || env["STATIONS_PRIVATE_MANIFEST_REF"]?.trim()
    || null;
}
...
export function findManifestMachine(manifest: FleetManifest, machineId: string): MachineManifest | null {
  const direct = manifest.stations.find((machine) => machine.id === machineId);
  if (direct) return direct;
  const requested = machineId.trim().toLowerCase();
  return manifest.stations.find((machine) => (machine.aliases ?? []).some((alias) => alias.trim().toLowerCase() === requested)) ?? null;
}

export function detectCurrentMachineManifest(): MachineManifest {
  const machineId = process.env["HASNA_STATIONS_MACHINE_ID"] || hostname();
