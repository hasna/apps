...
program
  ...
  .option("--mode <mode>", "Agent mode recorded with heartbeat metadata", process.env["HASNA_STATIONS_AGENT_MODE"] || "daemon")
  .option("--storage-push", "Push heartbeat rows to configured remote storage after writing", false)
  .option("--storage-push-retries <count>", "Storage push retry count", process.env["HASNA_STATIONS_AGENT_STORAGE_PUSH_RETRIES"] || "2")
  .option("--storage-push-backoff-ms <ms>", "Storage push retry backoff in milliseconds", process.env["HASNA_STATIONS_AGENT_STORAGE_PUSH_BACKOFF_MS"] || "250")
...
program.action(async (options: HeartbeatOptions) => {
  ...
  const intervalMs = Number.isFinite(parsedIntervalSeconds)
    ...
    : Number.isFinite(parsedIntervalMs)
      ...
      : 30000;
  const storagePushEnabled = options.storagePush || envFlag("HASNA_STATIONS_AGENT_STORAGE_PUSH");
  const privateMetadataEnabled = options.privateMetadata || envFlag("HASNA_STATIONS_PRIVATE_METADATA") || envFlag("STATIONS_PRIVATE_METADATA");
  const doctorSummaryEnabled = options.doctorSummary || envFlag("HASNA_STATIONS_AGENT_DOCTOR_SUMMARY");
