...
function fileCrashloopTask(config: RosterConfig, runner: RosterCommandRunner, entry: RosterEntry, error: string): boolean {
  const tag = `stations-roster-${entry.id.replace(/[^A-Za-z0-9_-]/g, "-")}`;
  ...
  const args = [
    ...base,
    "add",
    `[stations roster] ${entry.id} is crashlooping`,
    "-d",
    [
    ...
    "urgent",
    "--tags",
    `stations,roster,crashlooping,${tag}`,
  ];
...
export async function runRosterReconcile(config: RosterConfig, options: RosterRunOptions = {}): Promise<RosterRunResult> {
  ...
  try {
    ...
    const summary = [
      `[stations roster] ${validated.machineId ?? hostname()} reconcile ${status}`,
    ...
    if (!summaryAttempted) {
      ...
      try {
        conversationPosted = postConversation(
          validated,
          runner,
          `[stations roster] ${validated.machineId ?? hostname()} reconcile failed controller_error=${message}`,
          true,
        );
      } catch {
        conversationPosted = false;
