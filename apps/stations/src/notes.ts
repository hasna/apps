...
export const LEGACY_NOTE_CONTEXT_SOURCES = ["notes", "stations"] as const;
...
const NOTE_CONTEXT_SOURCES = new Set<NoteMachineContextSource | LegacyNoteContextSource>([...LEGACY_NOTE_CONTEXT_SOURCES, "notes", "agent", "sync", "import", "stations", "unknown"]);
...
function mergeTopologyForMachineLookup(primary: MachineTopology, fallback: MachineTopology, warning: string): MachineTopology {
  const stationsById = new Map(fallback.stations.map((machine) => [machine.machine_id, machine]));
  for (const machine of primary.stations) stationsById.set(machine.machine_id, machine);
  return {
    ...fallback,
...
function normalizeContextSource(source: NoteMachineContextSourceInput | undefined, actorType: NoteActorType): NoteMachineContextSource {
  const canonical = source === "open-notes" || source === "open-stations" ? LEGACY_NOTE_CONTEXT_SOURCE_ALIASES[source] : source;
...
export function listMachineTrashPolicies(options: MachineTrashPoliciesOptions = {}): MachineTrashPolicies {
  ...
  const pagination: MachineListPagination = machineId
    ...
    : topology.pagination;
  const warnings = [...topology.warnings];
  if (machineId && stations.length === 0) warnings.push(`unknown_machine:trash_owner:${machineId}`);
  return {
    schema_version: STATIONS_CONSUMER_CONTRACT_VERSION,
    package: packageInfo(),
    capabilities: getStationsConsumerCapabilities(),
    ...
    pagination,
    policies: stations.map(trashPolicyForMachine),
    warnings,
  };
}
