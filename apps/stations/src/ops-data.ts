...
import { STATIONS_CONSUMER_CONTRACT_VERSION, STATIONS_PACKAGE_NAME } from "./topology.js";
...
export interface CriticalDbIntegrityReport {
  ...
  package: { name: typeof STATIONS_PACKAGE_NAME; version: string };
  generated_at: string;
...
function dbIntegrityTaskSuggestion(finding: CriticalDbIntegrityFinding): MachineDataTaskSuggestion {
  if (finding.check_tool === "none" && finding.message?.includes("unavailable")) {
    return {
      ...
      dedupe_key: `stations:data:db-integrity:${finding.fingerprint}`,
      title: `[stations:data] Restore sqlite3 for DB integrity checks`,
      description: [
        "source: @hasna/stations ops db-integrity",
        `fingerprint: ${finding.fingerprint}`,
        `tool: ${finding.path}`,
      ...
      priority: "high",
      tags: ["stations", "ops-data", "db-integrity", "sqlite3"],
    };
  }
  return {
    fingerprint: finding.fingerprint,
    dedupe_key: `stations:data:db-integrity:${finding.fingerprint}`,
    title: `[stations:data] Fix DB integrity failure in ${basename(finding.path)}`,
    description: [
      "source: @hasna/stations ops db-integrity",
      `fingerprint: ${finding.fingerprint}`,
      `path: ${finding.path}`,
    ...
    priority: "critical",
    tags: ["stations", "ops-data", "db-integrity"],
  };
}
...
function snapshotFailureTaskSuggestion(item: OpsStateSnapshotItem): MachineDataTaskSuggestion {
  if (item.method === "sqlite_backup" && item.message?.includes("unavailable")) {
    return {
      ...
      dedupe_key: `stations:data:state-snapshot:${item.fingerprint}`,
      title: `[stations:data] Restore sqlite3 for state snapshots`,
      description: [
        "source: @hasna/stations ops state-snapshot",
        `fingerprint: ${item.fingerprint}`,
        `tool: ${item.path}`,
      ...
      priority: "high",
      tags: ["stations", "ops-data", "state-snapshot", "sqlite3"],
    };
  }
  return {
    fingerprint: item.fingerprint,
    dedupe_key: `stations:data:state-snapshot:${item.fingerprint}`,
    title: `[stations:data] Fix state snapshot failure for ${basename(item.path)}`,
    description: [
      "source: @hasna/stations ops state-snapshot",
      `fingerprint: ${item.fingerprint}`,
      `path: ${item.path}`,
    ...
    priority: "high",
    tags: ["stations", "ops-data", "state-snapshot"],
  };
}
...
function taskUpsertDescription(
  ...
  return [
    `dedupe_key: ${suggestion.dedupe_key}`,
    "source: @hasna/stations ops data",
    `kind: ${result.kind}`,
...
function packageInfo(): { name: typeof STATIONS_PACKAGE_NAME; version: string } {
