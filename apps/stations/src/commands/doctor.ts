...
function buildDoctorCommand(): string {
  return [
    'data_dir="${HASNA_STATIONS_DIR:-$HOME/.hasna/stations}"',
    'manifest_path="${HASNA_STATIONS_MANIFEST_PATH:-$data_dir/stations.json}"',
    'db_path="${HASNA_STATIONS_DB_PATH:-$data_dir/stations.db}"',
    'notifications_path="${HASNA_STATIONS_NOTIFICATIONS_PATH:-$data_dir/notifications.json}"',
    "printf 'data_dir=%s\\n' \"$data_dir\"",
    ...
    "printf 'stations=%s\\n' \"$(command -v stations 2>/dev/null || printf missing)\"",
    "printf 'stations_daemon=%s\\n' \"$(command -v stations-daemon 2>/dev/null || printf missing)\"",
    "printf 'stations_mcp=%s\\n' \"$(command -v stations-mcp 2>/dev/null || printf missing)\"",
...
export function runDoctor(machineId?: string, options: DoctorOptions = {}): DoctorReport {
  ...
  const collectedChecks: DoctorCheck[] = [
    ...
    makeCheck(
      "stations-cli",
      details["stations"] && details["stations"] !== "missing" ? "ok" : "warn",
      "stations CLI availability",
      details["stations"] || "missing"
    ),
    makeCheck(
      "stations-daemon-cli",
      details["stations_daemon"] && details["stations_daemon"] !== "missing" ? "ok" : "warn",
      "stations-daemon availability",
      details["stations_daemon"] || "missing"
    ),
    makeCheck(
      "stations-mcp-cli",
      details["stations_mcp"] && details["stations_mcp"] !== "missing" ? "ok" : "warn",
      "stations-mcp availability",
      details["stations_mcp"] || "missing"
    ),
    makeCheck(
      "ssh",
