...
export const HEARTBEAT_COLLECT_MUTATION_OPERATION = "stations_heartbeat_collect";
...
export function heartbeatCollectMutationArgs(options: Pick<HeartbeatCollectOptions, "stations" | "timeoutMs" | "doctorSummary"> = {}): Record<string, unknown> {
  return {
    stations: options.stations?.map((machine) => machine.trim()).filter(Boolean) ?? [],
    timeout_ms: options.timeoutMs ?? null,
    doctor_summary: options.doctorSummary !== false,
  };
}

export function heartbeatCollectResourceId(options: Pick<HeartbeatCollectOptions, "stations"> = {}): string {
  const stations = options.stations?.map((machine) => machine.trim()).filter(Boolean) ?? [];
  return `heartbeat:collect:${stations.length > 0 ? stations.join(",") : "all"}`;
}

function defaultMachineIds(): string[] {
  const ids = new Set<string>([getLocalMachineId()]);
  for (const machine of readManifest().stations) ids.add(machine.id);
  return [...ids].sort();
}
...
export function buildHeartbeatCollectorCommand(options: HeartbeatCollectorCommandOptions = {}): HeartbeatCollectorCommandPlan {
  const explicitMachineIds = options.stations ? uniqueSortedMachineIds(options.stations) : [];
  const stations = explicitMachineIds.length > 0 ? explicitMachineIds : defaultHeartbeatCollectorMachineIds();
  ...
  const stationsCommand = options.stationsCommand?.trim() || "stations";
  ...
  const collectArgs = [
    "heartbeat",
    "collect",
    ...stations.flatMap((machine) => ["--machine", machine]),
    "--timeout-ms",
    String(Math.floor(timeoutMs)),
    "--json",
  ];
  const collectCommand = [stationsCommand, ...collectArgs].map(shellArg).join(" ");
  const warnings: string[] = [];
  if (explicitMachineIds.length === 0) warnings.push("heartbeat_collector_defaulted_to_local_machine_only");
  if (stations.length > 2) warnings.push("heartbeat_collector_many_stations_may_exceed_freshness_ttl");
...
function heartbeatCommand(machineId: string, doctorSummary: boolean): string {
  const doctorArgs = doctorSummary
    ? `if stations-daemon --help 2>&1 | grep -q -- '--doctor-summary'; then doctor='--doctor-summary'; else doctor=''; fi`
    : "doctor=''";
  return [
    `PATH="$HOME/.bun/bin:$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin"`,
    "unset HASNA_STATIONS_MACHINE_ID STATIONS_MACHINE_ID HASNA_STATIONS_PRIVATE_METADATA STATIONS_PRIVATE_METADATA",
    "export PATH",
    "if ! command -v stations-daemon >/dev/null 2>&1; then printf 'stations-daemon not found\\n' >&2; exit 127; fi",
    doctorArgs,
    'stations-daemon --once $doctor --json',
  ].join("; ");
}
...
function parseHeartbeatSnapshot(
  ...
  if (!line) throw new Error("stations-daemon did not emit a JSON heartbeat");
...
export function collectHeartbeats(
  ...
  const requestedMachineIds = options.stations && options.stations.length > 0 ? options.stations : defaultMachineIds();
