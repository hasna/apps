...
export function startClipboardDaemon(port?: number): void {
  ...
  const machineId = process.env["HASNA_STATIONS_MACHINE_ID"] || "unknown";
  ...
  setInterval(async () => {
    // Skip polling on headless stations — only receive from peers
    ...
    // Push to all other known stations
...
async function discoverPeers(): Promise<{ host: string; port: number }[]> {
  ...
  const knownPeers = (process.env["HASNA_STATIONS_CLIPBOARD_PEERS"] || "")
    .split(",")
    .map((peer) => peer.trim())
    .filter(Boolean);
