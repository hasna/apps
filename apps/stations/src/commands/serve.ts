...
<html lang="en">
  ...
  <body>
    <main>
      <h1>Stations Dashboard <span class="refresh" id="last-updated"></span></h1>
      <div class="grid">
        <section class="card"><div>Manifest stations</div><div class="stat">${status.manifestMachineCount}</div></section>
        ...
        <section class="card"><div>Tailscale routes</div><div class="stat">${topology.stations.filter((machine) => machine.ssh.route === "tailscale").length}</div></section>
      </div>
    ...
    <script>
      ...
      async function refreshData() {
        try {
          ...
          if (tbody && status.stations) {
            tbody.innerHTML = status.stations
              .map((m) =>
                "<tr>" +
    ...
    async fetch(request) {
      ...
      if (url.pathname === "/api/routes") {
        ...
        return Response.json({
          ...
          routes: topology.stations.map((machine) => redactRouteForOutput(resolveMachineRoute(machine.machine_id, { topology }), { privateMetadata })),
          ...(privateWarnings.length > 0 ? { warnings: privateWarnings } : {}),
        });
      }
      if (url.pathname === "/api/stations/friendly-name") {
        ...
        if (request.method === "POST" || request.method === "PUT") {
          ...
          const denied = requireDashboardMutation("stations_friendly_name_set", request, body, {
        ...
        if (request.method === "DELETE") {
          ...
          if (!input.machineId) return jsonError("machine_id is required.");
          const denied = requireDashboardMutation("stations_friendly_name_clear", request, body, {
            machineId: input.machineId,
            resourceId: machineFriendlyNameResourceId(input.machineId),
      ...
      if (url.pathname === "/api/stations/details") {
        if (request.method !== "GET") {
      ...
      if (url.pathname === "/api/browserplan/fleet") {
        ...
        return Response.json(getBrowserPlanFleet({
          machineIds: queryMachineIdList(url, ["machine", "machine_id", "stations", "machine_ids"]),
          includeTailscale: url.searchParams.get("tailscale") === "true",
      ...
      if (url.pathname === "/api/projects/assignments") {
        ...
        if (request.method === "POST") {
          ...
          const denied = requireDashboardMutation("stations_projects_assign", request, body, {
            machineId: input.machineId,
        ...
        if (request.method === "DELETE") {
          ...
          const denied = requireDashboardMutation("stations_projects_unassign", request, body, {
            machineId: input.machineId,
            resourceId: projectAssignmentResourceId(input.machineId, input.projectId),
      ...
      if (url.pathname === "/api/events") {
        ...
        const source = typeof body["source"] === "string" ? body["source"] : "stations";
        ...
        const denied = requireDashboardMutation("stations_events_emit", request, body, {
      ...
      if (url.pathname === "/api/notifications/test") {
        ...
        const resolvedMessage = message ?? "stations notification test";
        const denied = requireDashboardMutation("stations_notifications_test", request, body, {
      ...
      if (url.pathname === "/api/webhooks/test") {
        ...
        const data = objectBodyValue(body["data"]);
        const denied = requireDashboardMutation("stations_webhooks_test", request, body, {
