 ...
 * `HASNA_<APP>_API_KEY`. The raw RDS DSN is NEVER distributed to stations and
 ...
 * `--all-stations` (used by the coordination-store cutover so stations flip
 ...
 * route, so keeping it registered would let `stations flip mailery` attempt to
 ...
 * divergence==0) so stations never split-brain.
 */
...
function defineFlipApp(app: string): FlipAppSpec {
  ...
  if (FREEZE_REQUIRED_APPS.has(app)) {
    ...
    spec.note = "Coordination store: drain dual-write shadow to divergence==0, then atomic --all-stations cutover.";
  }
...
export interface SelectTargetsOptions {
  /** Explicit station ids (comma/space accepted upstream, array here). */
  stations?: string[];
  /** Restrict to stations carrying ALL of these tags. */
  tags?: string[];
...
export function selectTargets(manifest: FleetManifest, options: SelectTargetsOptions = {}): FlipTarget[] {
  ...
  const explicit = options.stations && options.stations.length > 0 ? new Set(options.stations) : null;
  const requiredTags = options.tags ?? [];
  const seen = new Set<string>();
  const out: FlipTarget[] = [];
  for (const machine of manifest.stations) {
...
export interface PlanWavesOptions {
  /** Stations in the canary wave (default 1). */
  canarySize?: number;
  /** Stations per batch wave after the canary (default 4). */
  batchSize?: number;
  /**
   * Atomic mode: put EVERY target into a single wave (no canary, no batching).
   * Used by `--all-stations` for the coordination-store cutover so the fleet
...
export function planWaves(targets: FlipTarget[], options: PlanWavesOptions = {}): FlipWave[] {
  ...
  if (options.atomic) {
    return [{ name: "all-stations", index: 0, targets: targets.slice() }];
  }
...
export interface FlipPlan {
  ...
  mode: FlipMode;
  freezeRequired: boolean;
  waves: { name: string; stations: string[] }[];
...
export function buildFlipPlan(spec: FlipAppSpec, mode: FlipMode, waves: FlipWave[], scriptOptions?: BuildScriptOptions): FlipPlan {
  return {
    ...
    waves: waves.map((w) => ({ name: w.name, stations: w.targets.map((t) => t.id) })),
