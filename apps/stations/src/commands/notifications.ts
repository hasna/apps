...
async function dispatchCommand(
  ...
  const result = Bun.spawnSync([executable, ...(channel.commandArgs ?? [])], {
    ...
    env: {
      ...process.env,
      HASNA_STATIONS_NOTIFICATION_CHANNEL: channel.id,
      HASNA_STATIONS_NOTIFICATION_EVENT: event,
      HASNA_STATIONS_NOTIFICATION_MESSAGE: message,
    },
  });
