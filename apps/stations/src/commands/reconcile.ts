...
// Desired-state package reconcile loop for stations-daemon.
//
// Desired state comes from the stations.json manifest (fleet-wide `packages`
...
export function buildReconcilePlan(options: BuildReconcilePlanOptions = {}): ReconcilePlan {
  ...
  const machineId = options.machineId?.trim() || process.env["HASNA_STATIONS_MACHINE_ID"] || "local";
  ...
  // freeze.json gate (operator `stations freeze add`) is still merged in so
