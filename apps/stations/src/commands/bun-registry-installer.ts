...
export const EXACT_BUN_STATIONS_PACKAGE_NAME = "@hasna/stations" as const;
export const EXACT_BUN_STATIONS_PACKAGE_ORDER = 10 as const;
...
export interface ExactBunTargetTransactionPayload {
  schema: "stations.exact_bun_transaction.v1";
  machineId: string;
...
export function defaultExactBunSourceLoader(source: ExactBunRegistrySourceRef): Buffer {
  ...
  const directory = mkdtempSync(join(tmpdir(), "stations-exact-source-"));
...
export function parseExactBunPackageObservation(
  ...
  if (parsed["schema"] !== "stations.bun_package_probe.v1") throw new Error("probe_schema_mismatch");
...
export function validateExactBunMachine(machine: MachineManifest): string[] {
  ...
  const stationsOnly = packages.length === 1 && packages[0]!.name === EXACT_BUN_STATIONS_PACKAGE_NAME;
  if (!stationsOnly && packages.length !== EXACT_BUN_PACKAGE_NAMES.length) {
    errors.push(`exact_package_count:${packages.length}`);
    return errors;
  }
  const expectedNames: readonly string[] = stationsOnly
    ? [EXACT_BUN_STATIONS_PACKAGE_NAME]
    : EXACT_BUN_PACKAGE_NAMES;
  const expectedOrders: readonly number[] = stationsOnly
    ? [EXACT_BUN_STATIONS_PACKAGE_ORDER]
...
function exactBunPackageOrder(name: string): number | undefined {
  if (name === EXACT_BUN_STATIONS_PACKAGE_NAME) return EXACT_BUN_STATIONS_PACKAGE_ORDER;
...
function validateTargetSteps(steps: ExactBunRegistryPlanStep[]): void {
  ...
  if (steps.some((step) => step.package.name === EXACT_BUN_STATIONS_PACKAGE_NAME) && steps.length !== 1) {
  ...
  steps.forEach((step) => {
    ...
    if (step.kind !== "bun-registry-exact"
      ...
      || step.probe.schema !== "stations.bun_package_probe.v1"
...
function parseInstalledState(
  ...
  if (installedState.schema !== "stations.apps.status.v2"
...
function safeSourceEnvironment(base: NodeJS.ProcessEnv, step: ExactBunRegistryPlanStep, sourcePath: string, globalRoot: string): NodeJS.ProcessEnv {
  ...
  return {
    ...env,
    HASNA_STATIONS_EXACT_BUN_SCHEMA: "stations.exact_bun_registry.v1",
    HASNA_STATIONS_EXACT_BUN_MODE: "live-global",
    HASNA_STATIONS_EXACT_BUN_PACKAGE: step.package.name,
    HASNA_STATIONS_EXACT_BUN_VERSION: step.package.version,
    HASNA_STATIONS_EXACT_BUN_SELECTOR: step.package.selector,
    HASNA_STATIONS_EXACT_BUN_ARCHIVE_SHA256: step.package.archiveSha256,
    HASNA_STATIONS_EXACT_BUN_REGISTRY_INTEGRITY: step.package.registryIntegrity,
    HASNA_STATIONS_EXACT_BUN_SOURCE_PATH: sourcePath,
    HASNA_STATIONS_EXACT_BUN_GLOBAL_DIR: globalRoot,
  };
}
...
export function executeExactBunTargetStatus(payload: ExactBunTargetTransactionPayload): ExactBunTargetTransactionResult {
  if (payload.schema !== "stations.exact_bun_transaction.v1") throw new Error("transaction_schema_mismatch");
...
export function executeExactBunTargetTransaction(
  ...
  const transactionRoot = mkdtempSync(join(dependencies.temporaryRoot ?? tmpdir(), "stations-exact-bun-"));
...
export function decodeExactBunTargetPayload(value: string): ExactBunTargetTransactionPayload {
  ...
  if (!isRecord(decoded) || decoded["schema"] !== "stations.exact_bun_transaction.v1") {
    throw new Error("transaction_payload_invalid");
  }
...
export function parseExactBunTargetResult(
  ...
  if (parsed["schema"] !== "stations.exact_bun_transaction_result.v1"
...
function exactBunBootstrapInput(source: Buffer, envelope: ExactBunBootstrapEnvelope): Buffer {
  const prefix = Buffer.from(
    `globalThis.__STATIONS_EXACT_BUN_BOOTSTRAP_INPUT__=${JSON.stringify(envelope)};\n`,
    "utf8",
  );
