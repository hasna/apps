...
const DEFAULT_SERVICE_NAME = "stations-daemon";
const DEFAULT_EXECUTABLE = "/usr/local/bin/stations-daemon";
...
function buildEnvironment(
  ...
  const env: Record<string, string> = {
    HASNA_STATIONS_AGENT_MODE: "daemon",
    HASNA_STATIONS_AGENT_SERVICE: serviceName,
  };
  ...
  if (options.storagePush) {
    env["HASNA_STATIONS_AGENT_STORAGE_PUSH"] = "1";
    env["HASNA_STATIONS_AGENT_STORAGE_PUSH_BACKOFF_MS"] = "250";
    env["HASNA_STATIONS_AGENT_STORAGE_PUSH_RETRIES"] = "2";
    // No mode var: storage push needs only the DSN placeholder, and deployment
    // modes were removed (owner directive 2026-07-29).
    env["HASNA_STATIONS_DATABASE_URL"] = placeholderForEnv("HASNA_STATIONS_DATABASE_URL");
  ...
  if (options.doctorSummary) {
    env["HASNA_STATIONS_AGENT_DOCTOR_SUMMARY"] = "1";
  }

  if (options.privateMetadata === true) {
    env["HASNA_STATIONS_PRIVATE_METADATA"] = "1";
...
function buildServiceFile(options: ResolvedDaemonServiceOptions): DaemonServiceFile {
  if (options.platform === "macos") {
    return {
      id: "launchd-plist",
      description: "launchd property list for stations-daemon",
  ...
  return {
    id: "systemd-unit",
    description: "systemd unit for stations-daemon",
...
function buildLaunchdCommands(options: ResolvedDaemonServiceOptions): DaemonServiceCommand[] {
  ...
  return [
    command(
      "launchd-logs",
      "Stream logs for stations-daemon.",
      "log",
...
function systemdUnit(options: ResolvedDaemonServiceOptions): string {
  ...
  return `[Unit]
Description=Hasna stations agent
