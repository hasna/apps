...
export function startClipboardServer(options: ClipboardServerOptions = {}): ClipboardServerHandle {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname === "/health" && request.method === "GET") {
      return jsonResponse(response, 200, { ok: true, machineId: process.env["HASNA_STATIONS_MACHINE_ID"] || "unknown" });
    }
  ...
  try {
    const res = await fetch(`http://${host}:${port}/clipboard`, {
      ...
      body: JSON.stringify({
        content,
        contentType: "text",
        sourceMachine: process.env["HASNA_STATIONS_MACHINE_ID"] || "unknown",
        timestamp: new Date().toISOString(),
      }),
    });
