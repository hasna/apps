...
function isExactAppsPlan(plan: AppsPlanResult): plan is ExactBunAppsPlan {
  return "schema" in plan && plan.schema === "stations.apps.plan.v2";
}
...
function packageSpecsForApps(machine: MachineManifest, options: AppsManifestOptions): DesiredPackage[] {
  ...
  if (!manifest.stations.some((entry) => entry.id === machine.id)) return [];
...
function buildAppProbeCommand(machine: MachineManifest, app: ManifestAppSpec): string {
  ...
  return `if dpkg-query -W -f='${'${Version}'}' ${packageName} >/tmp/stations-app-version 2>/dev/null; then printf 'installed=1\\nversion='; cat /tmp/stations-app-version; printf '\\n'; rm -f /tmp/stations-app-version; else printf 'installed=0\\n'; fi`;
}
...
function resolveMachine(machineId?: string, options: AppsManifestOptions = {}, requireExactTarget = false): MachineManifest {
  ...
  if (manifestPath) {
    ...
    if (machine && exactBunPackages(machine).length > 0 && manifest.stations.length !== 1) {
...
export function validateAppsCandidate(machineId: string, options: AppsManifestOptions = {}): AppsValidationResult {
  ...
  try {
    ...
    const manifest = readManifest(manifestPath);
    if (manifest.stations.length !== 1) errors.push("candidate_manifest_not_target_only");
