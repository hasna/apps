...
export const HOSTS_BLOCK_BEGIN = "# >>> hasna stations fleet >>>";
export const HOSTS_BLOCK_END = "# <<< hasna stations fleet <<<";
...
export function getHostsPath(): string {
  const override = process.env["HASNA_STATIONS_HOSTS_PATH"];
...
export function renderHostsBlock(entries: FleetHostEntry[]): string {
  const lines = [
    HOSTS_BLOCK_BEGIN,
    "# Managed by `stations hosts apply`. Do not edit between these markers.",
