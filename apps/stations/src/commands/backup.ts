...
export const STATIONS_BACKUP_BUCKET_ENV = "HASNA_STATIONS_S3_BUCKET";
export const STATIONS_BACKUP_BUCKET_FALLBACK_ENV = "STATIONS_S3_BUCKET";
export const STATIONS_BACKUP_PREFIX_ENV = "HASNA_STATIONS_S3_PREFIX";
export const STATIONS_BACKUP_PREFIX_FALLBACK_ENV = "STATIONS_S3_PREFIX";
export const DEFAULT_BACKUP_PREFIX = "stations";
...
function readBackupBucketEnv(): Pick<BackupTarget, "bucket" | "bucketSource"> | null {
  const primary = readEnv(STATIONS_BACKUP_BUCKET_ENV);
  if (primary) return { bucket: primary, bucketSource: STATIONS_BACKUP_BUCKET_ENV };
  const fallback = readEnv(STATIONS_BACKUP_BUCKET_FALLBACK_ENV);
  if (fallback) return { bucket: fallback, bucketSource: STATIONS_BACKUP_BUCKET_FALLBACK_ENV };
  return null;
}

function readBackupPrefixEnv(): Pick<BackupTarget, "prefix" | "prefixSource"> | null {
  const primary = readEnv(STATIONS_BACKUP_PREFIX_ENV);
  if (primary) return { prefix: primary, prefixSource: STATIONS_BACKUP_PREFIX_ENV };
  const fallback = readEnv(STATIONS_BACKUP_PREFIX_FALLBACK_ENV);
  if (fallback) return { prefix: fallback, prefixSource: STATIONS_BACKUP_PREFIX_FALLBACK_ENV };
  return null;
}
...
export function resolveBackupTarget(options: { bucket?: string; prefix?: string } = {}): BackupTarget {
  ...
  if (!bucket) {
    throw new Error(
      `Missing S3 backup bucket. Pass --bucket or set ${STATIONS_BACKUP_BUCKET_ENV} or ${STATIONS_BACKUP_BUCKET_FALLBACK_ENV}.`,
    );
  }
...
export function buildBackupPlan(bucket?: string, prefix?: string): SetupResult {
  ...
  const archivePath = join(homedir(), ".hasna", "stations", "backup.tgz");
  ...
  return {
    machineId: process.env["HASNA_STATIONS_MACHINE_ID"] || "local",
    mode: "plan",
    steps,
    executed: 0,
  };
