...
function getToolBinary(tool: AiCliTool): string {
  if (tool === "claude") return process.env["HASNA_STATIONS_CLAUDE_BINARY"] || "claude";
  if (tool === "codex") return process.env["HASNA_STATIONS_CODEX_BINARY"] || "codex";
  return process.env["HASNA_STATIONS_GEMINI_BINARY"] || "gemini";
}
