...
const SERVICE_PATH = "/etc/systemd/system/stations-heal.service";
...
function log(msg: string): void {
  console.log(`${new Date().toISOString()} [stations-heal] ${msg}`);
}
...
export function startHealDaemon(): void {
  ...
  if (!config.preferredSsid) {
    log("refusing to start: preferredSsid is not configured (run `stations heal config --set ...`)");
    process.exit(1);
  }
...
function binPath(): string {
  // Resolve an absolute path to the `stations` binary for the systemd ExecStart.
  // Root's PATH won't include the user's bun bin, so prefer concrete candidates.
  const candidates: string[] = [];
  const which = sh("command -v stations").out.split("\n")[0]?.trim();
  if (which) candidates.push(which);
  if (process.argv[1]) candidates.push(process.argv[1]);
  const home = process.env["HOME"];
  if (home) candidates.push(`${home}/.bun/bin/stations`);
  candidates.push("/root/.bun/bin/stations", "/usr/local/bin/stations");
...
const ROOT_DATA_DIR = "/etc/stations-heal";
...
export function installHealService(): string[] {
  ...
  const unit = `[Unit]
Description=Hasna stations self-healing network watchdog
...
Environment=HASNA_STATIONS_DIR=${ROOT_DATA_DIR}
...
`;
  ...
  sh("systemctl enable --now stations-heal.service");
...
export function uninstallHealService(): string[] {
  const log: string[] = [];
  sh("systemctl disable --now stations-heal.service 2>/dev/null || true");
...
export function healServiceStatus(): { installed: boolean; active: boolean; enabled: boolean } {
  return {
    ...
    active: sh("systemctl is-active stations-heal.service").out === "active",
    enabled: sh("systemctl is-enabled stations-heal.service 2>/dev/null").out === "enabled",
  };
}
