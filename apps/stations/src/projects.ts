...
export function listMachineProjectAssignments(options: MachineProjectAssignmentsOptions = {}): MachineProjectAssignments {
  ...
  const localMachineId = process.env["HASNA_STATIONS_MACHINE_ID"]?.trim() || null;
  ...
  const allAssignments = mergeAssignments(manifest.stations.flatMap((machine) => [
  ...
  return {
    ...
    stations: machineSummaries(assignments),
    warnings: [],
  };
}
...
function updateManifestMachine(manifest: FleetManifest, machineId: string, updater: (machine: MachineManifest) => MachineManifest): FleetManifest {
  ...
  let found = false;
  const stations = manifest.stations.map((machine) => {
