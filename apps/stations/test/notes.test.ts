...
function setupTemp(name: string): string {
  ...
  process.env.HASNA_STATIONS_MACHINE_ID = "notes-node-00";
  manifestInit();
  return dir;
}
...
describe("Hasna Notes machine contract", () => {
  test("resolves note origin, source, target, sync targets, and agent provenance display names", () => {
    const dir = setupTemp("stations-notes-context-");
    try {
      manifestAdd({
        id: "notes-node-00",
      ...
      expect(validateStationsConsumerEnvelope("note_machine_context", context)).toMatchObject({ ok: true, errors: [] });
      ...
      expect(validateStationsConsumerEnvelope("note_machine_context", invalidActor)).toMatchObject({ ok: true, errors: [] });
    } finally {
  ...
  test("normalizes legacy open-* actor source aliases to canonical values on read", () => {
    const dir = setupTemp("stations-notes-legacy-source-");
    try {
      const openStations = resolveNoteMachineContext({
        originMachineId: "notes-node-00",
        actor: {
          actor_type: "human",
      ...
      expect(openStations.actor.source).toBe("stations");
      expect(openStations.actor.display_name).toBe("stations");
      expect(validateStationsConsumerEnvelope("note_machine_context", openStations)).toMatchObject({ ok: true, errors: [] });
      ...
      expect(validateStationsConsumerEnvelope("note_machine_context", openNotes)).toMatchObject({ ok: true, errors: [] });
      ...
      expect(canonical.actor.source).toBe("stations");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("expands paginated topology for explicit note machine ids", () => {
    const dir = setupTemp("stations-notes-context-pagination-");
    try {
      ...
      expect(firstPageTopology.stations.some((machine) => machine.machine_id === "notes-node-00")).toBe(false);
  ...
  test("lists per-machine trash policies with metadata and latest-10 pagination", () => {
    const dir = setupTemp("stations-notes-trash-");
    try {
      for (let index = 0; index < 12; index += 1) {
        const id = `notes-node-${String(index).padStart(2, "0")}`;
        manifestAdd({
      ...
      expect(validateStationsConsumerEnvelope("machine_trash_policies", policies)).toMatchObject({ ok: true, errors: [] });
