...
function setupTemp(name: string): string {
  ...
  process.env.HASNA_STATIONS_MACHINE_ID = "details-node";
  manifestInit();
  return dir;
}
...
describe("machine details consumer contract", () => {
  test("builds consumer-safe details with friendly names, status, metadata, and sync timestamps", () => {
    const dir = setupTemp("stations-details-");
    try {
      manifestAdd({
        id: "details-node",
      ...
      upsertHeartbeat("details-node", 100, "online", {
        ...
        storageSyncLastError: "postgres://user:pass@10.0.0.1/stations",
      });
      ...
      expect(validateStationsConsumerEnvelope("machine_details", {
        ...details,
  ...
  test("normalizes status from saved topology and uses Tailscale online state when heartbeat is unknown", () => {
    const dir = setupTemp("stations-details-status-");
    try {
      const topology = discoverMachineTopology({ includeTailscale: false });
      const machine = topology.stations.find((entry) => entry.machine_id === "details-node");
      ...
      expect(validateStationsConsumerEnvelope("machine_details", stale)).toMatchObject({ ok: true, errors: [] });
      ...
      expect(validateStationsConsumerEnvelope("machine_details", online)).toMatchObject({ ok: true, errors: [] });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("keeps registry lifecycle status separate from heartbeat-backed liveness", async () => {
    const dir = setupTemp("stations-details-registry-liveness-");
    try {
  ...
  test("uses neutral fallback details for unknown stations and expands paginated topology", () => {
    const dir = setupTemp("stations-details-fallback-");
    try {
      ...
      expect(firstPage.stations.some((machine) => machine.machine_id === "details-node-00")).toBe(false);
      ...
      expect(validateStationsConsumerEnvelope("machine_details", missing)).toMatchObject({ ok: true, errors: [] });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
