...
function topologyWith(entry: Record<string, unknown>) {
  return {
    ...
    package: { name: "@hasna/stations" as const, version: "0.0.24" },
    capabilities: {
...
describe("stations screen", () => {
  ...
  test("resolves screen password secret from manifest metadata", () => {
    const topology = topologyWith({
      user: "operator",
      metadata: {
        screenPasswordSecret: "stations/screen-sharing/screen-demo-mac-005-vnc-password",
      },
    });
    const credentials = resolveScreenCredentials("demo-mac-005", { topology });
    expect(credentials.passwordSecretKey).toBe("stations/screen-sharing/screen-demo-mac-005-vnc-password");
  ...
  test("screen-enable plan pipes secrets CLI output into SSH without exposing the password value", () => {
    ...
    expect(plan.secretsCommandArgs).toEqual(["secrets", "get", "stations/screen-sharing/screen-demo-mac-005-vnc-password", "--show"]);
    ...
    expect(plan.command).toContain("secrets' 'get' 'stations/screen-sharing/screen-demo-mac-005-vnc-password' '--show'");
...
describe("screen-credentials exit code", () => {
  ...
  test("does not fail closed when a full listing includes unroutable stations", () => {
  ...
  test("still fails on a missing checked secret in non-strict mode", () => {
    ...
    // absent stays fatal even though unroutable stations do not.
