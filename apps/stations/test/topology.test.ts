...
describe("machine topology SDK", () => {
  test("merges manifest stations, heartbeats, and tailscale peers", () => {
    const dir = setupTemp("stations-topology-");
    try {
      manifestAdd({
        id: "demo-node-01",
      ...
      expect(topology.package.name).toBe("@hasna/stations");
      expect(topology.capabilities.route_resolution).toBe(true);
      expect(topology.local_machine_id).toBe("demo-node-02");
      expect(topology.stations.map((machine) => machine.machine_id)).toContain("demo-node-01");
      expect(topology.stations.map((machine) => machine.machine_id)).toContain("demo-node-02");

      const demoNode01 = topology.stations.find((machine) => machine.machine_id === "demo-node-01");
      ...
      const demoNode02 = topology.stations.find((machine) => machine.machine_id === "demo-node-02");
  ...
  test("resolves a legacy manifest alias to the canonical topology route", () => {
    const dir = setupTemp("stations-topology-machine-alias-");
    try {
      manifestAdd({
        id: "station03",
      ...
      const canonical = topology.stations.find((machine) => machine.machine_id === "station03");
  ...
  test("retains Tailscale state when a peer advertises only the legacy alias", () => {
    const dir = setupTemp("stations-topology-legacy-peer-alias-");
    try {
      manifestAdd({
        id: "station03",
        friendlyName: "station03",
      ...
      const topology = discoverMachineTopology({
        ...
        runner: fakeRunner({
          ...
          Peer: {
            "nodekey:apple03": {
              HostName: "apple03",
              DNSName: "apple03.tailnet.ts.net.",
              OS: "linux",
              TailscaleIPs: ["203.0.113.54"],
              Online: true,
              Active: true,
            },
          },
      ...
      const canonical = topology.stations.find((machine) => machine.machine_id === "station03");
      expect(canonical?.tailscale.ips).toEqual(["203.0.113.54"]);
      expect(canonical?.tailscale.online).toBe(true);
      expect(canonical?.tailscale.active).toBe(true);

      const route = resolveMachineRoute("station03", { topology, now: new Date("2026-06-09T00:00:00.000Z") });
      expect(route.route).toBe("tailscale");
      expect(route.target).toBe("station03.tailnet.ts.net");
      expect(route.evidence.tailscale_online).toBe(true);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("resolves project paths from manifest metadata without exposing secrets", () => {
    const dir = setupTemp("stations-workspace-paths-");
    try {
      manifestAdd({
        id: "demo-node-01",
        hostname: "demo-node-01",
        ...
        metadata: {
          ...
          screenPasswordSecret: "stations/screen-sharing/demo-node-01-vnc-password",
        },
      });
  ...
  test("can skip tailscale probing and still report local topology", () => {
    const dir = setupTemp("stations-topology-local-");
    try {
  ...
  test("marks stale online heartbeats offline in topology", () => {
    const dir = setupTemp("stations-topology-stale-heartbeat-");
    try {
      upsertHeartbeat("demo-node-02", 123, "online");
      ...
      const local = topology.stations.find((machine) => machine.machine_id === "demo-node-02");
  ...
  test("returns latest 10 stations by default with display-name fallback and view-more metadata", () => {
    const dir = setupTemp("stations-topology-pagination-");
    try {
      ...
      expect(firstPage.stations.map((machine) => machine.machine_id)).toEqual([
        "demo-node-11",
        "demo-node-10",
        "demo-node-09",
        "demo-node-08",
      ...
      expect(firstPage.stations[0]).toMatchObject({
        machine_id: "demo-node-11",
        friendly_name: "Studio Linux",
        display_name: "Studio Linux",
        updated_at: "2026-06-12T00:00:00.000Z",
      });
      expect(firstPage.stations[1]).toMatchObject({
      ...
      expect(secondPage.stations.map((machine) => machine.machine_id)).toEqual(["demo-node-01", "demo-node-00"]);

      const full = discoverMachineTopology({ includeTailscale: false, limit: null });
      expect(full.pagination.limit).toBeNull();
      expect(full.stations).toHaveLength(12);
  ...
  test("resolves a tailscale peer without requiring a manifest entry", () => {
    const dir = setupTemp("stations-topology-peer-route-");
    try {
      const topology = discoverMachineTopology({
        now: new Date("2026-06-09T00:00:00.000Z"),
        runner: fakeRunner({
