...
afterEach(() => {
  delete process.env["HASNA_STATIONS_ALLOW_MUTATIONS"];
  ...
  delete process.env["HASNA_STATIONS_REACHABLE_HOSTS"];
  delete process.env["HASNA_STATIONS_DATABASE_URL"];
  delete process.env["STATIONS_DATABASE_URL"];
  delete process.env["HASNA_STATIONS_MUTATION_REPLAY_PATH"];
});

test("exports expected MCP tool surface", () => {
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_status");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_doctor");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_self_test");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_apps_status");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_install_claude_diff");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_notifications_dispatch");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_webhooks_add");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_events_emit");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_serve_info");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_sync_apply");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_compatibility");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_machine_health");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_routing");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_command_matrix");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_loop_preflight");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_dispatch_fleet_smoke");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_route_resolve");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_workspace_resolve");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_friendly_name_get");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_friendly_name_set");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_friendly_name_clear");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_details");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_browserplan_fleet");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_notes_context");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_notes_trash_policies");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_daemon_status");
  expect(MACHINE_MCP_TOOL_NAMES).toContain("stations_daemon_service_plan");
...
test("MCP dispatch fleet smoke is read-only and redacted", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-dispatch-smoke-"));
  ...
  process.env["HASNA_STATIONS_MACHINE_ID"] = "spark02";
  try {
...
`);
    ...
    try {
      const result = await client.callTool({
        name: "stations_dispatch_fleet_smoke",
        arguments: {
          machine_ids: ["local"],
...
test("MCP BrowserPlan fleet tool exposes target stations and operation hooks", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-browserplan-"));
  process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
  process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
  process.env["HASNA_STATIONS_MACHINE_ID"] = "control";
  manifestInit();
  manifestAdd({
    id: "machine001",
  ...
  try {
    const result = await client.callTool({
      name: "stations_browserplan_fleet",
    ...
    expect(payload.stations[0].operation_hooks.map((hook: { id: string }) => hook.id)).toContain("headless_launch");
...
test("MCP loop preflight returns compact redacted JSON", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-agent-apis-"));
  process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
  process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
  process.env["HASNA_STATIONS_MACHINE_ID"] = "control";
  process.env["HASNA_STATIONS_REACHABLE_HOSTS"] = "operator@worker";
  manifestInit();
  manifestAdd({
    id: "control",
  ...
  try {
    ...
    expect(payload.warnings).toContain("private_output_denied:set HASNA_STATIONS_ALLOW_PRIVATE_OUTPUT=1 to allow private metadata output");
  } finally {
    await client.close();
    await server.close();
    rmSync(dir, { recursive: true, force: true });
...
test("MCP mutation tools accept scoped tokens only for the exact operation and machine", async () => {
  process.env[MUTATION_APPROVAL_TOKEN_ENV] = "secret";
  process.env["HASNA_STATIONS_MANIFEST_PATH"] = `${process.env.TMPDIR ?? "/tmp"}/stations-mcp-scoped-${Date.now()}.json`;
  ...
  try {
    ...
    const text = (result.content as Array<{ type: string; text: string }>)[0]?.text;
    expect(JSON.parse(text)).toMatchObject({ version: 1, stations: [] });
  } finally {
    await client.close();
    await server.close();
    delete process.env["HASNA_STATIONS_MANIFEST_PATH"];
...
test("MCP friendly-name tools use scoped approvals and topology pagination", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-friendly-name-"));
  process.env[MUTATION_APPROVAL_TOKEN_ENV] = "secret";
  process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
  process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
  ...
  try {
    ...
    try {
      const result = await client.callTool({
        name: "stations_friendly_name_set",
    ...
    expect(topologyPayload.stations[0]).toMatchObject({
      machine_id: "demo-node-11",
      friendly_name: "Studio Linux",
      display_name: "Studio Linux",
    });
    ...
    const cleared = await client.callTool({
      name: "stations_friendly_name_clear",
...
test("MCP note contract tools expose provenance and trash metadata", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-notes-"));
  process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
  process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
  process.env["HASNA_STATIONS_MACHINE_ID"] = "origin-node";
  manifestInit();
  manifestAdd({
    id: "origin-node",
  ...
  try {
    ...
    const trashResult = await client.callTool({
      name: "stations_notes_trash_policies",
...
test("MCP mutation tokens reject same-resource notification argument tampering", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-args-hash-"));
  process.env[MUTATION_APPROVAL_TOKEN_ENV] = "secret";
  process.env["HASNA_STATIONS_NOTIFICATIONS_PATH"] = join(dir, "notifications.json");
  const token = createMutationApprovalToken({
    surface: "mcp",
    operation: "stations_notifications_add",
  ...
  try {
    ...
    try {
      const result = await client.callTool({
        name: "stations_notifications_add",
        arguments: {
          channel_id: "ops",
          type: "command",
          target: "/bin/sh",
...
test("MCP apps apply tokens are bound to the approved plan digest", async () => {
  const dir = mkdtempSync(join(tmpdir(), "stations-mcp-plan-digest-"));
  process.env[MUTATION_APPROVAL_TOKEN_ENV] = "secret";
  process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
  process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
  process.env["HASNA_STATIONS_MUTATION_REPLAY_PATH"] = "";
  manifestInit();
  const machine = {
    id: "demo-node-apply",
    platform: "linux" as const,
    workspacePath: "/tmp/stations-mcp-plan-digest",
    apps: [],
  };
  manifestAdd(machine);
  ...
  try {
    ...
    const token = createMutationApprovalToken({
      surface: "mcp",
      operation: "stations_apps_apply",
      machineId: "demo-node-apply",
      resourceId: `plan:stations_apps_apply:demo-node-apply:${planDigest}`,
      callerId: "mcp",
      runId: "mcp",
      transport: "mcp:stdio",
...
test("MCP storage tools hand off approved mutations to the storage layer", async () => {
  ...
  try {
    ...
    expect(failureText).toContain("Missing HASNA_STATIONS_DATABASE_URL");
    expect(failureText).not.toContain("sdk.stations_storage_push requires");
  } finally {
    await client.close();
    await server.close();
  }
