...
function createExactBunPackageFixture(expectedVersion: string) {
  const dir = mkdtempSync(join(tmpdir(), "stations-apps-package-exact-"));
...
function runProbeCommandUnderShell(shell: string, probeCommand: string, binaryOutput: string) {
  const dir = mkdtempSync(join(tmpdir(), "stations-apps-probe-bin-"));
  probeFixtureDirs.push(dir);
  const binDir = join(dir, "bin");
  mkdirSync(binDir);
  const binaryPath = join(binDir, "stations");
...
describe("apps", () => {
  ...
  test("lists apps from manifest", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-"));
  ...
  test("uses separate exact custom install and probe commands without collateral steps", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-custom-contract-"));
  ...
  test("rejects a custom install command without an exact probe command", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-custom-missing-probe-"));
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
  ...
  test("fails closed when a custom probe emits malformed installed output", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-custom-malformed-probe-"));
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
  ...
  test("fails closed when a custom probe reports a mismatched expected version", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-custom-version-mismatch-"));
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
    const probeCommand = "printf 'installed=1\\nversion=0.1.60\\n'";
    manifestAdd({
  ...
  test("fails closed when the exact Bun version banner does not match", () => {
    ...
    const result = runProbeCommand(probeCommand, "stations v0.2.20 (build 2026.08.13)");
  ...
  test("generated Bun package probe parses and verifies under the macOS zsh login shell, not only sh/bash", () => {
    ...
    for (const shell of ["sh", "bash", "zsh"]) {
      ...
      const result = runProbeCommandUnderShell(shell, probeCommand, "stations v0.2.21 (build 2026.08.13)");
  ...
  test("keeps legacy custom packageName install and command-v probe semantics", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-custom-legacy-"));
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
    manifestAdd({
      id: "remote-linux",
  ...
  test("installs exact Bun package specs through apps and verifies the declared executable version", () => {
    ...
    expect(plan.steps[0]).toMatchObject({
      id: "package-hasna-stations",
      title: "Install @hasna/stations@0.2.21 on remote-linux",
      command: "bun install -g '@hasna/stations@0.2.21'",
      manager: "bun",
      privileged: false,
    ...
    expect(plan.steps[0]?.probeCommand).not.toContain("@hasna/stations@0.2.21");
    ...
    const runner: MachineCommandRunner = (machineId, command, options) => {
      ...
      if (command === "bun install -g '@hasna/stations@0.2.21'") {
        return { machineId, source: "ssh", stdout: "", stderr: "", exitCode: 0 };
      }
      throw new Error(`unexpected command: ${command}`);
    };
    ...
    expect(calls).toEqual([
      ...
      { command: "bun install -g '@hasna/stations@0.2.21'", redactOutput: true },
      { command: plan.steps[0]?.probeCommand, redactOutput: true },
    ]);
  });

  test("reports a Bun owner/PATH collision instead of trusting a shadowed executable", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-bun-owner-collision-"));
    probeFixtureDirs.push(dir);
    const manifestPath = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = manifestPath;
    writeFileSync(manifestPath, `${JSON.stringify({
  ...
  test("fails verification when the Bun-managed executable is absent from active PATH", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-bun-owner-missing-path-"));
    probeFixtureDirs.push(dir);
    const manifestPath = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = manifestPath;
    writeFileSync(manifestPath, `${JSON.stringify({
      ...
      stations: [{ id: "remote-linux", platform: "linux", workspacePath: "/home/operator/workspace" }],
    }, null, 2)}\n`);
  ...
  test("accepts an active symlink that resolves to the Bun-managed executable", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-bun-owner-symlink-"));
    probeFixtureDirs.push(dir);
    const manifestPath = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = manifestPath;
    writeFileSync(manifestPath, `${JSON.stringify({
  ...
  test("extracts the version token from a banner with trailing build metadata", () => {
    const { probeCommand } = createExactBunPackageFixture("0.2.21");
    const result = runProbeCommand(probeCommand, "stations v0.2.21 (build 2026.08.13)");
    expect(result.status).toBe(0);
    expect(result.stdout).toBe("installed=1\nversion=0.2.21\n");
    expect(result.stderr).toBe("");
  });
  ...
  test("keeps apt, brew, and winget install command compatibility", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-manager-compat-"));
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
    manifestAdd({
      id: "linux-node",
    ...
    expect(probes).toEqual(new Map([
      [
        "linux-node",
        "if dpkg-query -W -f='${Version}' 'curl' >/tmp/stations-app-version 2>/dev/null; then printf 'installed=1\\nversion='; cat /tmp/stations-app-version; printf '\\n'; rm -f /tmp/stations-app-version; else printf 'installed=0\\n'; fi",
      ],
      [
        "mac-node",
  ...
  test("computes app status and diff", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-status-"));
  ...
  test("surfaces remote readiness failures for app status", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-remote-fail-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "remote-mac";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
    manifestAdd({
      id: "remote-mac",
  ...
  test("checks readiness for an explicit unmanaged machine id instead of falling back to local", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-unmanaged-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "local-fixture";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
  ...
  test("runs app install steps on the selected machine", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-runner-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "local-fixture";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
    manifestAdd({
  ...
  test("rejects stale expected plan digests before running app install commands", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-plan-digest-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "local-fixture";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
    manifestAdd({
  ...
  test("validates, plans, and carries an exact candidate source only through runner stdin", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-exact-candidate-"));
    ...
    expect("schema" in plan && plan.schema).toBe("stations.apps.plan.v2");
    ...
    const runner: MachineCommandRunner = (machineId, value, options) => {
      ...
      return {
        ...
        stdout: JSON.stringify({
          schema: "stations.exact_bun_transaction_result.v1",
          machineId,
          platform: "linux",
          state: "COMMITTED",
          executed: 2,
  ...
  test("resolves a legacy alias to the canonical machine in an exact candidate", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-exact-alias-candidate-"));
    const manifestPath = join(dir, "candidate.json");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = manifestPath;
    const candidate = exactBunCandidate("station03", { platform: "macos", bunPath: "/Users/hasna/.bun/bin/bun" });
    (candidate.stations as Array<Record<string, unknown>>)[0]!.aliases = ["apple03"];
  ...
  test("live-revalidates installed state before a zero-step no-op", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-exact-installed-state-"));
    const manifestPath = join(dir, "candidate.json");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = manifestPath;
    writeExactBunCandidate(manifestPath);
    ...
    const installedState = {
      schema: "stations.apps.status.v2" as const,
      machineId: initial.machineId,
      platform: initial.platform,
      source: "ssh" as const,
      packages: initial.steps.map((step) => ({
        schema: "stations.bun_package_probe.v1" as const,
        package: step.package.name,
        expectedVersion: step.package.version,
        observedVersion: step.package.version,
  ...
  test("derives and applies one exact Factory step on Linux 0.2.18 and macOS 0.2.17 fixtures", () => {
    ...
    for (const fixture of exactBunTargetFixtures) {
      const dir = mkdtempSync(join(tmpdir(), `stations-apps-exact-${fixture.id}-`));
      const manifestPath = join(dir, "candidate.json");
  ...
  test("rejects a non-target-only exact candidate before planning or applying", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-exact-multi-target-"));
    const manifestPath = join(dir, "candidate.json");
    const candidate = exactBunCandidate();
    (candidate.stations as Array<Record<string, unknown>>).push({
      id: "unrelated-target",
      platform: "linux",
  ...
  test("fails closed when explicit and environment manifest authorities differ", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-apps-manifest-authority-"));
    ...
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = configured;
