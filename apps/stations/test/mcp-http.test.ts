...
import {
  ...
  startHttpServer,
  type StationsHttpSecurityConfig,
...
const API_KEY = "test-stations-api-key";

function httpSecurity(overrides: Partial<StationsHttpSecurityConfig> = {}): StationsHttpSecurityConfig {
  return {
    apiKey: API_KEY,
...
async function withHttpServer<T>(
  ...
  security: StationsHttpSecurityConfig = httpSecurity(),
): Promise<T> {
...
describe("MCP HTTP transport", () => {
  ...
  test("resolves HTTP mode and default port", () => {
    ...
    expect(HTTP_NAME).toBe("stations");
  });
  ...
  test("resolves MCP HTTP security from API key or loopback-only unauthenticated opt-in", () => {
    ...
    expect(resolveHttpSecurityConfig({ STATIONS_ALLOW_UNAUTHENTICATED: "1" }, "127.0.0.1").allowUnauthenticated).toBe(true);
    expect(resolveHttpSecurityConfig({ STATIONS_ALLOW_UNAUTHENTICATED: "1" }, "0.0.0.0").allowUnauthenticated).toBe(false);
    expect(resolveHttpSecurityConfig({ STATIONS_HTTP_ALLOWED_ORIGINS: "https://ops.example, http://localhost:3000" }).allowedOrigins).toEqual([
      "https://ops.example",
      "http://localhost:3000",
    ]);
    expect(resolveHttpSecurityConfig({ STATIONS_HTTP_MAX_BODY_BYTES: "4096" }).maxBodyBytes).toBe(4096);
  });
  ...
  test("handles MCP initialize and tool call over Streamable HTTP", async () => {
    await withHttpServer(async (port) => {
      ...
      try {
        const tools = await client.listTools();
        expect(tools.tools.some((tool) => tool.name === "stations_status")).toBe(true);

        const result = await client.callTool({ name: "stations_status", arguments: {} });
  ...
  test("HTTP MCP mutations require scoped tokens beyond HTTP API key auth", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-http-scoped-"));
    ...
    try {
      await withHttpServer(async (port) => {
        ...
        try {
          ...
          const text = (result.content as Array<{ type: string; text: string }>)[0]?.text;
          expect(JSON.parse(text).stations).toEqual([]);
        } finally {
          await client.close();
        }
      });
    ...
    await withHttpServer(async (port) => {
      ...
      try {
        const results = await Promise.all(
          clients.map((client) => client.callTool({ name: "stations_status", arguments: {} }))
        );
