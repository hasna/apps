...
describe("workspace resolver CLI", () => {
  test("repairs inferred mappings through the SDK helper", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-workspace-repair-sdk-"));
    try {
      ...
      expect(preview.resolution.repair_hints[0].shell_command).toContain("stations");
      ...
      expect(readManifest().stations[0].metadata).not.toHaveProperty("workspace_paths");
      ...
      const machine = readManifest().stations[0];
  ...
  test("diagnoses untrusted inferred workspace mappings", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-workspace-doctor-untrusted-"));
    try {
      process.env.HASNA_STATIONS_MANIFEST_PATH = join(dir, "stations.json");
  ...
  test("diagnoses missing machine manifests as failed workspace readiness", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-workspace-doctor-missing-"));
    try {
      process.env.HASNA_STATIONS_MANIFEST_PATH = join(dir, "stations.json");
      process.env[MUTATION_APPROVAL_FLAG_ENV] = "1";
      manifestInit();
  ...
  test("prints machine workspace mapping as JSON", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-workspace-cli-"));
    try {
      process.env.HASNA_STATIONS_MANIFEST_PATH = join(dir, "stations.json");
      process.env.HASNA_STATIONS_MACHINE_ID = "demo-node-02";
      manifestInit();
  ...
  test("prints workspace doctor diagnostics as JSON", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-workspace-doctor-cli-"));
    try {
      process.env.HASNA_STATIONS_MANIFEST_PATH = join(dir, "stations.json");
      manifestInit();
      manifestAdd({
      ...
      expect(output.repair_hints[0].shell_command).toContain("stations");
  ...
  test("repairs inferred mappings through the CLI", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-workspace-repair-cli-"));
    try {
      process.env.HASNA_STATIONS_MANIFEST_PATH = join(dir, "stations.json");
      process.env[MUTATION_APPROVAL_FLAG_ENV] = "1";
      manifestInit();
