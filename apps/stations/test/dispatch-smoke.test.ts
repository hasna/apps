...
function setupDispatchFleet(): string {
  const dir = mkdtempSync(join(tmpdir(), "stations-dispatch-smoke-"));
  ...
  process.env.HASNA_STATIONS_REACHABLE_HOSTS = "operator@spark01,hasna@apple03";
  manifestInit();
  manifestAdd({
    id: "spark02",
...
describe("dispatch fleet smoke API", () => {
  test("reports default dispatch fleet package, route, and daemon readiness without mutation", () => {
    ...
    try {
      ...
      expect(report.stations.map((machine) => machine.target.target_id)).toEqual(["local", "spark01", "spark02:ssh", "apple03"]);
      expect(report.stations.find((machine) => machine.target.target_id === "spark02:ssh")?.route_health).toMatchObject({
        route: "ssh",
        forced_ssh: true,
        target: "[redacted]",
      });
      expect(report.stations.every((machine) => machine.daemon.restart_readiness.executed === false)).toBe(true);
      expect(report.stations.every((machine) => machine.daemon.restart_readiness.planned_mutates === true)).toBe(true);
  ...
  test("dispatch-smoke CLI emits JSON and does not call daemon restart", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-dispatch-smoke-cli-"));
...
`);
      ...
      const env = {
        ...
        HASNA_STATIONS_MACHINE_ID: "spark02",
      };
      ...
      expect(payload.stations[0].daemon.restart_readiness).toMatchObject({
