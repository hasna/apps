...
function eventEmitApproval(dir: string, type: string, args: {
  ...
  const source = args.source ?? "stations";
  const data = args.data ?? {};
  const metadata = args.metadata ?? {};
  return dashboardToken("stations_events_emit", eventStoreResourceId("event", dir, type, args.subject, args.dedupeKey), withEventStoreScope(dir, {
    event_type: type,
    source,
...
function notificationTestApproval(channelId: string, args: {
  ...
  const message = args.message ?? "stations notification test";
  const apply = args.apply === true;
  const yes = args.yes === true;
  return dashboardToken("stations_notifications_test", dashboardResourceId("notification-test", channelId, event), {
...
function webhookTestApproval(dir: string, channelId: string, args: {
  ...
  const data = args.data ?? {};
  return dashboardToken("stations_webhooks_test", eventStoreResourceId("webhook-test", dir, channelId, type), withEventStoreScope(dir, {
    channel_id: channelId,
    event_type: type,
    subject,
    message,
...
describe("serve", () => {
  afterEach(() => {
    ...
    delete process.env["HASNA_STATIONS_ALLOW_PRIVATE_OUTPUT"];
  ...
  test("returns default serve info", () => {
    ...
    expect(info.routes).toContain("/api/stations/friendly-name");
    expect(info.routes).toContain("/api/stations/details");
  ...
  test("renders dashboard html", () => {
    ...
    expect(html).toContain("<title>Stations Dashboard</title>");
    expect(html).toContain("Stations Dashboard");
  ...
  test("serves new JSON endpoints", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-serve-"));
    ...
    const machineDetails = await fetch(`${base}/api/stations/details?machine=demo-node-01&tailscale=false`).then((response) => response.json());
    ...
    const emitted = await fetch(`${base}/api/events`, {
      ...
      body: JSON.stringify({
        type: "stations.test",
        data: { ok: true },
        approval_token: eventEmitApproval(dir, "stations.test", { data: { ok: true } }),
      }),
    ...
    expect(Array.isArray(topology.stations)).toBe(true);
    ...
    expect(browserPlanFleet.stations[0]).toMatchObject({
    ...
    expect(emitted.event.type).toBe("stations.test");
  ...
  test("friendly-name API sets, reads, clears, and paginates topology", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-serve-friendly-name-"));
    ...
    const base = `http://127.0.0.1:${server.port}`;
    try {
      const denied = await fetch(`${base}/api/stations/friendly-name`, {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      const set = await fetch(`${base}/api/stations/friendly-name`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
      ...
      const read = await fetch(`${base}/api/stations/friendly-name?machine=demo-node-11`).then((response) => response.json());
      ...
      expect(topology.stations[0]).toMatchObject({
        machine_id: "demo-node-11",
        friendly_name: "Studio Linux",
        display_name: "Studio Linux",
      });
      ...
      const cleared = await fetch(`${base}/api/stations/friendly-name?machine=demo-node-11`, {
        method: "DELETE",
  ...
  test("project assignments API lists, assigns, and removes with scoped approval", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-serve-projects-"));
    ...
    try {
      ...
      const filtered = await fetch(`${base}/api/projects/assignments?project=stations`).then((response) => response.json());
      expect(filtered.filters).toEqual({ machine_id: null, project_id: "stations" });
      expect(filtered.assignments).toHaveLength(1);

      const removeInput = { machineId: input.machineId, projectId: input.projectId };
      const removed = await fetch(`${base}/api/projects/assignments?machine=demo-node-01&project=stations`, {
        method: "DELETE",
  ...
  test("dashboard POST mutation routes require scoped approval tokens", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-serve-security-"));
    ...
    await new EventsClient().addChannel({
      ...
      filters: [{ source: "stations", type: "stations.secure" }],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    });
    ...
    await new EventsClient().addChannel({
      ...
      filters: [{ source: "stations", type: "events.test" }],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    });
    ...
    try {
      const eventWithoutToken = await fetch(`${base}/api/events`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type: "stations.secure", message: "blocked" }),
      });
      ...
      const eventToken = eventEmitApproval(dir, "stations.secure", { message: "approved" });
      const tamperedEvent = await fetch(`${base}/api/events`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type: "stations.secure", message: "tampered", approval_token: eventToken }),
      });
      ...
      const cliSurfaceToken = createMutationApprovalToken({
        surface: "cli",
        operation: "stations_events_emit",
        transport: "cli",
        callerId: "dashboard",
        runId: "dashboard",
        resourceId: eventStoreResourceId("event", dir, "stations.secure", undefined, undefined),
        args: withEventStoreScope(dir, {
          event_type: "stations.secure",
          source: "stations",
          subject: undefined,
          severity: undefined,
          message: "approved",
      ...
      const wrongSurface = await fetch(`${base}/api/events`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type: "stations.secure", message: "approved", approval_token: cliSurfaceToken }),
      });
      expect(wrongSurface.status).toBe(403);
      expect(existsSync(eventProof)).toBe(false);

      const wrongStoreToken = eventEmitApproval(join(dir, "other-store-root"), "stations.secure", { message: "approved" });
      const wrongStore = await fetch(`${base}/api/events`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type: "stations.secure", message: "approved", approval_token: wrongStoreToken }),
      });
      ...
      const approvedEvent = await fetch(`${base}/api/events`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type: "stations.secure", message: "approved", approval_token: eventToken }),
      });
      expect(approvedEvent.status).toBe(200);
      expect((await approvedEvent.json()).event.type).toBe("stations.secure");
  ...
  test("explicit all-interface bind does not relax dashboard mutation approval", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-serve-bind-"));
    process.env["HASNA_EVENTS_DIR"] = join(dir, "events");
    process.env[MUTATION_APPROVAL_TOKEN_ENV] = dashboardMutationSecret;
    ...
    try {
      const response = await fetch(`http://127.0.0.1:${server.port}/api/events`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ type: "stations.exposed", message: "blocked" }),
      });
