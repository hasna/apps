...
describe("clipboard", () => {
  ...
  test("sanitizeClipboardForRead blocks secret patterns", () => {
    const result = sanitizeClipboardForRead("my secret token is AKI" + "A123", 1024 * 1024, ["secret", "AKIA"]);
    expect(result.ok).toBe(false);
  ...
  test("clipboard key creation and retrieval", () => {
    ...
    process.env["HASNA_STATIONS_CLIPBOARD_KEY_PATH"] = join(dir, "clipboard.key");
    ...
    delete process.env["HASNA_STATIONS_CLIPBOARD_KEY_PATH"];
  });
  ...
  test("getClipboardStatus returns status", () => {
    ...
    process.env["HASNA_STATIONS_CLIPBOARD_KEY_PATH"] = paths.keyPath;
    process.env["HASNA_STATIONS_CLIPBOARD_HISTORY_PATH"] = paths.historyPath;
    ...
    delete process.env["HASNA_STATIONS_CLIPBOARD_KEY_PATH"];
    delete process.env["HASNA_STATIONS_CLIPBOARD_HISTORY_PATH"];
  });
});
