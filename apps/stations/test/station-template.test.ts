...
describe("overlay merge", () => {
  ...
  test("dgx-spark overlay keeps the measured 121G-class slice values", () => {
    ...
    const rosterUnit = effective.files.find((file) => file.target.endsWith("stations-roster.service"));
    expect(rosterUnit?.content).toContain("/home/hasna/.bun/bin/stations-daemon roster daemon");
    expect(rosterUnit?.content).not.toContain("/opt/fixture");
    expect(effective.services).toContainEqual({
      name: "stations-roster.service",
      scope: "user",
      expectEnabled: true,
      expectActive: true,
    });
...
describe("physical render", () => {
  test("renders idempotent steps covering every template concern", () => {
    ...
    expect(commands).toContain("systemctl --user enable --now 'stations-roster.service'");
...
describe("drift check", () => {
  ...
  test("POSITIVE CONTROL: a missing bun global is detected (12 CLIs were checked by nothing)", () => {
    const { root, home, effective } = buildCleanFixture();
    rmSync(join(home, ".bun/install/global/node_modules/@hasna/stations"), { recursive: true, force: true });
    const result = checkStationTemplate(effective, { rootDir: root, homeDir: home, commandProbe: null });
    expect(result.verdict).toBe("drift");
    const item = result.items.find((candidate) => candidate.id === "package:bun:@hasna/stations");
  ...
  test("the shipped stations floor tracks this package version", () => {
    for (const overlays of [[], ["ec2"], ["dgx-spark"]]) {
      const pkg = effectiveFor(overlays).packages.bun.find((candidate) => candidate.name === "@hasna/stations");
  ...
  test("result names the machine it describes so a fleet sweep cannot mis-attribute it", () => {
    ...
    expect(defaulted.machineId).toBe(process.env["HASNA_STATIONS_MACHINE_ID"] || hostname());
...
describe("boot criticality (owner ruling 2026-07-29: tailscale must never be boot-critical)", () => {
  ...
  test("setup writes the final template-state stamp with the same effective-template payload as cloud-init", () => {
    ...
    expect(setupStamp?.command).toContain('"$HOME/.hasna/stations/template-state.json"');
  ...
  function exitCodeFixture(template: Record<string, unknown>) {
    ...
    return {
      ...
      env: {
        ...
        HASNA_STATIONS_TEMPLATES_DIR: join(dir, "templates"),
        HASNA_STATIONS_MANIFEST_PATH: join(dir, "stations.json"),
        HASNA_STATIONS_DB_PATH: join(dir, "stations.db"),
        HASNA_STATIONS_MACHINE_ID: "control",
...
describe("apt download-only policy and archive cache bound (template 1.7.0)", () => {
  test("base layer ships a download-only drop-in that overrides 20auto-upgrades", () => {
    const effective = effectiveFor(["ec2"]);
    ...
    expect(file?.content.split("\n")).toContain('APT::Periodic::Unattended-Upgrade "0";');
    ...
    // or the declared install policy and cache bound can be silently overridden.
