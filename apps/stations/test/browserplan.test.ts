...
import { STATIONS_PACKAGE_NAME, discoverMachineTopology } from "../src/topology.js";
...
const ENV_KEYS = [
  ...
  "HASNA_STATIONS_REACHABLE_HOSTS",
] as const;
...
function setupTemp(name: string): string {
  ...
  process.env.HASNA_STATIONS_MACHINE_ID = "machine001";
  manifestInit();
  return dir;
}

function addBrowserPlanFixtureStations(): void {
  process.env.HASNA_STATIONS_REACHABLE_HOSTS = "operator@machine001,operator@machine002";
  manifestAdd({
    id: "machine001",
...
describe("BrowserPlan fleet contract", () => {
  test("derives owner ids from the npm package names rather than repeating literals", () => {
    ...
    expect(BROWSERPLAN_ROUTE_OWNER).toBe(defaultAppIdForPackage(STATIONS_PACKAGE_NAME));
  });
  ...
  test("app_install_update validation is an allowlist of install shapes, not a git denylist", () => {
    const dir = setupTemp("stations-browserplan-allowlist-");
    try {
      addBrowserPlanFixtureStations();
      ...
      const withTemplate = (template: string) => {
        ...
        const hook = clone.stations[0]?.operation_hooks.find((entry) => entry.id === "app_install_update");
        if (hook) hook.command_template = template;
        return validateStationsConsumerEnvelope("browserplan_fleet", clone).ok;
      };
      ...
      expect(withTemplate("bun install -g @hasna/stations@latest")).toBe(false);
      ...
      // return alternating results for successive stations in one payload.
  ...
  test("lists machine001-machine011 coverage without mixing spark stations", () => {
    const dir = setupTemp("stations-browserplan-");
    try {
      addBrowserPlanFixtureStations();
      ...
      expect(fleet.stations.map((machine) => machine.machine_id)).toEqual(["machine001", "machine002"]);
      expect(fleet.stations.map((machine) => machine.machine_id)).not.toContain("spark01");
      ...
      expect(fleet.stations[0]?.display_name).toBe("Browser One");
      expect(fleet.stations[0]?.workspace.project_root).toBe("/Users/hasna/Workspace/open-chrome");
      expect(fleet.stations[0]?.install_state.checked).toBe(false);
      expect(fleet.stations[0]?.operation_hooks.map((hook) => hook.id)).toEqual([
        "profile_setup",
        "headed_launch",
        "headless_launch",
        "daemon_status",
      ...
      expect(fleet.stations[0]?.operation_hooks.every((hook) => hook.safe_runner.mcp.args.private_metadata === false)).toBe(true);
      expect(fleet.stations[0]?.operation_hooks.find((hook) => hook.id === "supervisor_status")?.command_template).toBe("browserplan remote status --machine <machine-id> --json");
      const installHook = fleet.stations[0]?.operation_hooks.find((hook) => hook.id === "app_install_update");
      ...
      expect(validateStationsConsumerEnvelope("browserplan_fleet", fleet)).toMatchObject({ ok: true, errors: [] });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("detects remote capabilities and blocks hooks when BrowserPlan is missing", () => {
    const dir = setupTemp("stations-browserplan-installs-");
    try {
      addBrowserPlanFixtureStations();
      const topology = discoverMachineTopology({ includeTailscale: false, limit: null });
      const fleet = getBrowserPlanFleet({
      ...
      const machine001 = fleet.stations.find((machine) => machine.machine_id === "machine001");
      const machine002 = fleet.stations.find((machine) => machine.machine_id === "machine002");
  ...
  test("does not report transport-unavailable BrowserPlan capabilities as missing", () => {
    const dir = setupTemp("stations-browserplan-unavailable-");
    try {
      addBrowserPlanFixtureStations();
      const topology = discoverMachineTopology({ includeTailscale: false, limit: null });
      const fleet = getBrowserPlanFleet({
      ...
      const machine = fleet.stations[0];
  ...
  test("reports missing BrowserPlan target stations as coverage gaps", () => {
    const dir = setupTemp("stations-browserplan-missing-");
    try {
      const fleet = getBrowserPlanFleet({
      ...
      expect(fleet.stations[0]).toMatchObject({
        machine_id: "machine011",
        known: false,
        eligible: false,
        eligibility_reasons: ["machine_missing_from_open_stations_topology", "route_unavailable", "route_confidence_none"],
      });
      expect(fleet.stations[0]?.operation_hooks.every((hook) => hook.available === false)).toBe(true);
      expect(validateStationsConsumerEnvelope("browserplan_fleet", fleet)).toMatchObject({ ok: true, errors: [] });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  ...
  test("rejects malformed BrowserPlan fleet contract payloads", () => {
    const dir = setupTemp("stations-browserplan-schema-");
    try {
      addBrowserPlanFixtureStations();
      const fleet = getBrowserPlanFleet({
        machineIds: ["machine001"],
      ...
      malformed.stations[0].operation_hooks[0].safe_runner.mcp.args.private_metadata = true;
      malformed.stations[0].operation_hooks.find((hook: { id: string }) => hook.id === "supervisor_status").command_template = "browserplan remote start --machine <machine-id> --json";
      ...
      malformed.stations[0].operation_hooks.find((hook: { id: string }) => hook.id === "app_install_update").command_template = "cd <open-chrome-project-root> && git fetch origin && git reset --hard origin/main";

      const result = validateStationsConsumerEnvelope("browserplan_fleet", malformed);
      ...
      expect(result.errors).toEqual(expect.arrayContaining([
        ...
        "stations.0.operation_hooks.0.safe_runner.mcp.args.private_metadata",
        "stations.0.operation_hooks.4.command_template",
        "stations.0.operation_hooks.7.command_template",
      ]));
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
