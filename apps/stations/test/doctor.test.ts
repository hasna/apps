...
describe("doctor", () => {
  afterEach(() => {
    ...
    delete process.env["HASNA_STATIONS_PRIVATE_MANIFEST_REF"];
    delete process.env["STATIONS_PRIVATE_MANIFEST_REF"];
  ...
  test("reports local machine checks", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-"));
    ...
    writeFileSync(process.env["HASNA_STATIONS_DB_PATH"]!, "", "utf8");
    writeFileSync(process.env["HASNA_STATIONS_NOTIFICATIONS_PATH"]!, "{}", "utf8");
  ...
  test("reports GitHub App references without leaking values", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-github-app-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-01";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
  ...
  test("does not expose inferred local machine id in default JSON report", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-implicit-local-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "private-host-real";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    manifestInit();
  ...
  test("marks remote checks unverified when an unmanaged machine probe fails", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-unmanaged-remote-"));
  ...
  test("redacts manifest and adapter details in JSON output", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-redact-"));
    const secretToken = `gh${"p"}_${"PRIVATE".repeat(5)}`;
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-01";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
    process.env["HASNA_STATIONS_NOTIFICATIONS_PATH"] = join(dir, "notifications.json");
    process.env["HASNA_STATIONS_PRIVATE_MANIFEST_REF"] = "vault://private-fleet/prod/operator-hosts";
    manifestInit();
    manifestAdd({
      id: "demo-node-01",
      ...
      checks: {
        secrets: () => ({
          ...
          data: {
            ...
            secretRef: "stations/screen-sharing/demo",
          },
        }),
      },
    };
    ...
    expect(payload).toContain("stations/screen-sharing/demo");
  });

  test("reports optional adapter fallback checks when no adapters are configured", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-adapter-fallback-"));
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-01";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
    process.env["HASNA_STATIONS_NOTIFICATIONS_PATH"] = join(dir, "notifications.json");
  ...
  test("hides raw private refs from manifest and optional adapter failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-doctor-private-adapter-"));
    const rawPrivateRef = "s3://private-fleet.example/demo/stations.json?token=github_pat_123456789012345678901234";
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-04";
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_PRIVATE_MANIFEST_REF"] = rawPrivateRef;
    manifestInit();
