...
// real freeze.json.
const emptyFreezePath = join(mkdtempSync(join(tmpdir(), "stations-reconcile-freeze-")), "freeze.json");
...
describe("desired state and installed parsing", () => {
  ...
  test("merges fleet packages with per-machine overrides and filters non-bun managers", () => {
    ...
    expect(defaultBinForPackage("@hasna/stations")).toBe("stations");
  });
});
...
describe("reconcile plan", () => {
  test("keeps exact Bun packages in contract order and builds one atomic plan", () => {
    ...
    manifest.stations[0]!.packages!.reverse();
  ...
  test("on-disk freeze.json still blocks when an in-memory manifest is provided", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-reconcile-disk-freeze-"));
    const freezePath = join(dir, "freeze.json");
    writeFileSync(freezePath, `${JSON.stringify({
      version: 1,
      packages: [{ name: "left-pad", reason: "operator freeze via stations freeze add" }],
    })}\n`, "utf8");
    const plan = buildReconcilePlan({
...
describe("reconcile execution", () => {
  ...
  test("applies install/update, verifies, emits rollout records, and persists them", async () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-reconcile-apply-"));
...
describe("release.published trigger", () => {
  test("extracts triggers only from valid release.published envelopes", () => {
    expect(releaseEventTrigger({ type: "stations.tmux.pane_died" })).toBeNull();
...
describe("stations reconcile CLI", () => {
  ...
  function cliEnv(dir: string): NodeJS.ProcessEnv {
    return {
      ...process.env,
      HASNA_STATIONS_DIR: dir,
      HASNA_STATIONS_MANIFEST_PATH: join(dir, "stations.json"),
      HASNA_STATIONS_DB_PATH: join(dir, "stations.db"),
      HASNA_STATIONS_FREEZE_PATH: join(dir, "freeze.json"),
      HASNA_STATIONS_MACHINE_ID: "demo-node-01",
      HASNA_STATIONS_ALLOW_MUTATIONS: "1",
    };
  }

  function writeFixtures(dir: string): { installedPath: string } {
    writeFileSync(join(dir, "stations.json"), `${JSON.stringify(manifestFixture(), null, 2)}\n`, "utf8");
  ...
  test("stations reconcile --dry-run plans without mutating anything", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-cli-reconcile-"));
  ...
  test("stations reconcile --dry-run --event-json scopes the plan to the released package", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-cli-reconcile-event-"));
    const { installedPath } = writeFixtures(dir);
  ...
  test("stations reconcile rejects non release.published event envelopes", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-cli-reconcile-bad-event-"));
    const { installedPath } = writeFixtures(dir);
    const eventPath = join(dir, "event.json");
  ...
  test("stations reconcile --apply exits non-zero when an action is freeze-blocked", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-cli-reconcile-blocked-"));
    const { installedPath } = writeFixtures(dir);
  ...
  test("stations freeze add/check/list/remove drive the reconcile freeze gate", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-cli-freeze-"));
    const { installedPath } = writeFixtures(dir);
