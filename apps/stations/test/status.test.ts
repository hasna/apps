...
describe("fleet status", () => {
  test("combines manifest stations and heartbeats", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-status-"));
    ...
    expect(status.stations.some((machine) => machine.machineId === "demo-node-01")).toBe(true);
  });

  test("merges alias-keyed heartbeats into the canonical manifest machine", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-status-alias-"));
    process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MACHINE_ID"] = "apple03";
    manifestInit();
    manifestAdd({
      id: "station03",
      aliases: ["apple03"],
  ...
  test("uses the latest heartbeat row for status summaries", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-status-latest-"));
    process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-01";
    manifestInit();
    ...
    const machine = getStatus({ privateMetadata: true, heartbeatTtlMs: null }).stations.find((entry) => entry.machineId === "demo-node-01");
  ...
  test("marks stale online heartbeats offline in status summaries", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-status-stale-"));
    process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-01";
    manifestInit();
    ...
    }).stations.find((entry) => entry.machineId === "demo-node-01");

    expect(machine?.heartbeatStatus).toBe("offline");
  });

  test("redacts local paths and machine identifiers by default", () => {
    const dir = mkdtempSync(join(tmpdir(), "stations-status-redacted-"));
    process.env["HASNA_STATIONS_DB_PATH"] = join(dir, "stations.db");
    process.env["HASNA_STATIONS_MANIFEST_PATH"] = join(dir, "stations.json");
    process.env["HASNA_STATIONS_MACHINE_ID"] = "demo-node-01";
    manifestInit();
    ...
    expect(status.stations[0]?.machineId).toBe(REDACTED_VALUE);
    expect(status.stations[0]?.doctorSummary).toBeNull();
