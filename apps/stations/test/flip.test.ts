...
describe("flip registry", () => {
  test("registers all 25 @hasna OSS apps (emails in, retired mailery out)", () => {
    ...
    for (const app of [
      ...
      "loops", "stations", "mementos", "projects", "recordings", "sandboxes", "secrets",
...
describe("target selection", () => {
  test("returns all stations in manifest order by default", () => {
    expect(selectTargets(manifest).map((t) => t.id)).toEqual(["apple01", "apple03", "spark01", "station02"]);
  });
  test("filters by explicit ids", () => {
    expect(selectTargets(manifest, { stations: ["spark01"] }).map((t) => t.id)).toEqual(["spark01"]);
  });
...
describe("wave planning", () => {
  ...
  test("atomic (--all-stations) puts every target in a single wave", () => {
    const waves = planWaves(targets, { atomic: true });
    expect(waves).toHaveLength(1);
    expect(waves[0]?.name).toBe("all-stations");
...
describe("script generation", () => {
  ...
  test("captured default get is empty, while the generated script consumes the key through exec", () => {
    const root = mkdtempSync(join(tmpdir(), "stations-flip-secrets-"));
    try {
...
describe("orchestration", () => {
  ...
  test("atomic --all-stations wave flips every machine together", () => {
...
describe("plan", () => {
  test("plan lists waves and referenced api-key secret paths without values", () => {
    ...
    expect(plan.waves[0]?.stations).toEqual(["apple01"]);
