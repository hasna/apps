...
describe("runtime tmux monitor", () => {
  test("builds an explicit trusted-local tmux pane-died hook plan", () => {
    const plan = buildTmuxPaneDiedHookPlan({ tmuxCommand: "tmux", stationsCommand: "stations", trustedLocalMutation: true });
    expect(plan.eventType).toBe("stations.tmux.pane_died");
    ...
    expect(plan.shellCommand).toContain("stations");
    ...
    expect(plan.shellCommand).toContain("HASNA_STATIONS_ALLOW_MUTATIONS=1");
    expect(plan.shellCommand).toContain("--no-deliver");
  });

  test("rejects dependency-owned event binaries for tmux hook plans", () => {
    for (const stationsCommand of ["events", "hasna-events", "/tmp/node_modules/.bin/events", "/tmp/node_modules/.bin/hasna-events"]) {
      expect(() => buildTmuxPaneDiedHookPlan({ stationsCommand, trustedLocalMutation: true })).toThrow("must invoke the stations CLI");
    }
  });

  test("does not include trusted mutation env by default", () => {
    const plan = buildTmuxPaneDiedHookPlan({ tmuxCommand: "tmux", stationsCommand: "stations" });
    expect(plan.trustedLocalMutation).toBe(false);
    expect(plan.shellCommand).not.toContain("HASNA_STATIONS_ALLOW_MUTATIONS=1");
  });

  test("builds a tmux hook plan with a scoped approval token when provided", () => {
    const plan = buildTmuxPaneDiedHookPlan({
      tmuxCommand: "tmux",
      stationsCommand: "stations",
  ...
  test("emits pane_died after a previously present pane disappears", async () => {
    ...
    expect(emitted[0]?.input).toMatchObject({
      source: "stations",
      type: "stations.tmux.pane_died",
      subject: "tmux:%11",
      data: { target: "%11", paneId: "%11" },
      metadata: { monitor: "tmux-pane", runtime: "stations" },
    });
  });
  ...
  test("can emit an initial missing event for one-shot checks", async () => {
    ...
    expect(emitted[0]?.type).toBe("stations.tmux.pane_missing");
  });
});
