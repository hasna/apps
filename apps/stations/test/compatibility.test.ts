...
describe("machine compatibility checks", () => {
  test("does not classify remote authentication failure as missing subjects", () => {
    const report = checkMachineCompatibility({
      ...
      commands: [{ command: "bun", required: true }, { command: "stations", required: true }],
  ...
  test("preserves zsh PATH while probing command paths", () => {
    ...
    const report = checkMachineCompatibility({
      ...
      packages: [{ name: "@hasna/stations", command: "stations", expectedVersion: "0.2.42", required: true }],
    });
    ...
    expect(report.checks.find((check) => check.id === "package:@hasna/stations:version")?.actual).toBe("0.2.42");
  });
  ...
  test("checks command, package, and workspace compatibility", () => {
    ...
    expect(report.package.name).toBe("@hasna/stations");
