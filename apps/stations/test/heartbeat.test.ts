...
function setupEmptyTemp(name: string): string {
  ...
  process.env.HASNA_STATIONS_DB_PATH = join(dir, "stations.db");
  process.env.HASNA_STATIONS_MANIFEST_PATH = join(dir, "stations.json");
  process.env.HASNA_STATIONS_MACHINE_ID = "spark01";
  manifestInit();
  return dir;
}
...
describe("heartbeat collection", () => {
  test("builds the canonical trusted OpenLoops collector command from explicit stations", () => {
    const dir = setupEmptyTemp("stations-heartbeat-collector-command-");
    try {
      manifestAdd({
        id: "spark02",
        platform: "linux",
      ...
      const plan = buildHeartbeatCollectorCommand({ stations: ["spark02", "spark01"] });
      ...
      expect(plan.command).toBe("HASNA_STATIONS_ALLOW_MUTATIONS=1 stations heartbeat collect --machine spark01 --machine spark02 --timeout-ms 90000 --json");
  ...
  test("defaults collector commands to the local machine to keep one-minute loops bounded", () => {
    const dir = setupEmptyTemp("stations-heartbeat-collector-local-default-");
    try {
      manifestAdd({
        id: "spark02",
        platform: "linux",
      ...
      expect(plan.stations).toEqual(["spark01"]);
      expect(plan.command).toBe("HASNA_STATIONS_ALLOW_MUTATIONS=1 stations heartbeat collect --machine spark01 --timeout-ms 90000 --json");
  ...
  test("does not bake the deprecated --fail-on-error flag into the canonical collector command", () => {
    const dir = setupEmptyTemp("stations-heartbeat-collector-no-deprecated-flag-");
    try {
      const explicit = buildHeartbeatCollectorCommand({ stations: ["spark01"] });
  ...
  test("builds deterministic collector commands from explicit machine options", () => {
    const dir = setupEmptyTemp("stations-heartbeat-collector-explicit-");
    try {
      const plan = buildHeartbeatCollectorCommand({
        stations: ["spark02", "spark01", "spark02", "  "],
        timeoutMs: 12_000,
        stationsCommand: "/opt/stations/bin/stations",
      });

      expect(plan.stations).toEqual(["spark01", "spark02"]);
      expect(plan.command).toBe("HASNA_STATIONS_ALLOW_MUTATIONS=1 /opt/stations/bin/stations heartbeat collect --machine spark01 --machine spark02 --timeout-ms 12000 --json");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("requires scoped SDK mutation approval before route execution", () => {
    const dir = setupTemp("stations-heartbeat-approval-");
    const calls: string[] = [];
    try {
      ...
      expect(() => collectHeartbeats({ stations: ["demo-node-01"] }, runner)).toThrow("sdk.stations_heartbeat_collect requires");
  ...
  test("imports a public one-shot heartbeat from a routed machine command", () => {
    const dir = setupTemp("stations-heartbeat-collect-");
    try {
      const runner: MachineCommandRunner = (machineId, command) => {
        expect(machineId).toBe("demo-node-01");
        expect(command).not.toContain("HASNA_STATIONS_MACHINE_ID='demo-node-01'");
        expect(command).toContain("unset HASNA_STATIONS_MACHINE_ID");
        expect(command).toContain("stations-daemon --once");
        return {
          machineId,
          source: "ssh",
      ...
      const results = collectHeartbeats(approved({ stations: ["demo-node-01"] }), runner);
  ...
  test("reports a failed machine without importing a heartbeat", () => {
    const dir = setupTemp("stations-heartbeat-failed-");
    try {
      const runner: MachineCommandRunner = (machineId) => ({
        machineId,
        source: "ssh",
        stdout: "",
        stderr: "stations-daemon not found",
        exitCode: 127,
      });
      ...
      expect(results[0]).toMatchObject({
        ...
        status: "failed",
        source: "ssh",
        error: "stations-daemon not found",
      });
      expect(listHeartbeats("demo-node-01")).toEqual([]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  test("rejects a heartbeat from the wrong remote identity", () => {
    const dir = setupTemp("stations-heartbeat-mismatch-");
    try {
      const runner: MachineCommandRunner = (machineId) => ({
        machineId,
        source: "ssh",
  ...
  test("does not trust route-facing manifest hostnames as heartbeat identity", () => {
    const dir = setupTemp("stations-heartbeat-route-identity-");
    try {
      manifestAdd({
        id: "demo-node-03",
      ...
      const results = collectHeartbeats(approved({ stations: ["demo-node-03"] }), runner);
  ...
  test("accepts a declared heartbeat alias but stores the canonical machine id", () => {
    const dir = setupTemp("stations-heartbeat-metadata-alias-");
    try {
      manifestAdd({
        id: "demo-node-04",
        platform: "macos",
      ...
      const results = collectHeartbeats(approved({ stations: ["demo-node-04"] }), runner);
  ...
  test("rejects explicit collection by heartbeat alias instead of canonical id", () => {
    const dir = setupTemp("stations-heartbeat-explicit-alias-");
    try {
      manifestAdd({
        id: "demo-node-04",
        platform: "macos",
      ...
      const results = collectHeartbeats(approved({ stations: ["MacBook-Pro"] }), runner);
  ...
  test("canonicalizes explicit local collection aliases to the local machine id", () => {
    const dir = setupTemp("stations-heartbeat-local-alias-");
    try {
      const runner: MachineCommandRunner = (machineId) => ({
        machineId,
        source: "local",
      ...
      const results = collectHeartbeats(approved({ stations: ["localhost"] }), runner);
  ...
  test("rejects private heartbeat metadata", () => {
    const dir = setupTemp("stations-heartbeat-private-");
    try {
      const runner: MachineCommandRunner = (machineId) => ({
        machineId,
        source: "ssh",
  ...
  test("keeps collecting after a runner throws for one machine", () => {
    const dir = setupTemp("stations-heartbeat-throw-");
    try {
      const runner: MachineCommandRunner = (machineId) => {
      ...
      const results = collectHeartbeats(approved({ stations: ["demo-node-01", "demo-node-02"] }), runner);
