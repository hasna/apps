...
function runFixture(source: string, timeoutMs?: number, env: Record<string, string | undefined> = process.env) {
  const dir = mkdtempSync(join(tmpdir(), "stations-test-budget-"));
...
describe("shared test harness budget", () => {
  ...
  test("isolates the suite from an incompatible operator manifest", () => {
    const operatorHome = mkdtempSync(join(tmpdir(), "stations-operator-home-"));
    const operatorData = join(operatorHome, ".hasna", "stations");
    mkdirSync(operatorData, { recursive: true });
    writeFileSync(join(operatorData, "stations.json"), JSON.stringify({
      version: 1,
      stations: [{
        id: "operator-machine",
        platform: "linux",
    ...
    delete env.HASNA_STATIONS_DIR;
    delete env.HASNA_STATIONS_DB_PATH;
    delete env.HASNA_STATIONS_MANIFEST_PATH;
    delete env.HASNA_STATIONS_NOTIFICATIONS_PATH;
...
test("renders from isolated state", () => {
  expect(renderDashboardHtml()).toContain("<title>Stations Dashboard</title>");
});`,
        undefined,
        env,
      );
