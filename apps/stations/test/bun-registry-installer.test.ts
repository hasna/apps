...
function delivery(order: 10 | 20, packageName: string, bin: string, archiveSha256: string, integrity: string): ExactBunRegistryDeliveryV1 {
  return {
    schema: "stations.exact_bun_registry.v1",
    order,
    mode: "live-global",
    source: { ...source },
    archiveSha256,
...
function machineFixture(): MachineManifest {
  const root = mkdtempSync(join(tmpdir(), "stations-exact-bun-test-"));
  roots.push(root);
...
function stationsSelfUpgradeFixture(): MachineManifest {
...
describe("exact Bun registry plan", () => {
  test("builds exactly two ordered safe steps and resolves one verified source", () => {
    ...
    expect(plan.schema).toBe("stations.apps.plan.v2");
  ...
  test("builds and executes one exact Stations self-upgrade step", () => {
    const machine = stationsSelfUpgradeFixture();
    const plan = buildExactBunAppsPlan(machine);
    expect(plan.steps.map((step) => [step.order, step.package.selector])).toEqual([
      [10, "@hasna/stations@0.2.20"],
    ]);
    ...
    writeFileSync(bunfig, [
      ...
      'registry = "https://registry.npmjs.org"',
      "minimumReleaseAge = 604800",
      'minimumReleaseAgeExcludes = ["@hasnaxyz/infinity", "@hasnaxyz/factory", "@hasna/secrets", "@hasna/events", "@hasna/stations"]',
      "",
    ].join("\n"));
    ...
    const result = executeExactBunTargetTransaction(payload, sourceBytes, {
      ...
      runSource(_command, env) {
        selectors.push(env["HASNA_STATIONS_EXACT_BUN_SELECTOR"]!);
    ...
    expect(selectors).toEqual(["@hasna/stations@0.2.20"]);
  ...
  test("uses a bounded controller bootstrap without candidate target commands and fails before source when unavailable", () => {
    ...
    const runner: MachineCommandRunner = (machineId, command, options) => {
      ...
      expect(String(options?.stdin)).toContain("stations.exact_bun_bootstrap.v1");
...
describe("exact Bun target transaction", () => {
  ...
  test("accepts a quarantine exclusion superset and rejects any missing required entry", () => {
    ...
    writeFileSync(bunfig, [
      ...
      'registry = "https://registry.npmjs.org"',
      "minimumReleaseAge = 604800",
      'minimumReleaseAgeExcludes = ["@hasnaxyz/infinity", "@hasnaxyz/factory", "@hasna/secrets", "@hasna/events", "@hasna/stations", "@hasna/contracts"]',
      "",
    ].join("\n"));
    ...
    writeFileSync(bunfig, [
      ...
      'registry = "https://registry.npmjs.org"',
      "minimumReleaseAge = 604800",
      'minimumReleaseAgeExcludes = ["@hasnaxyz/infinity", "@hasnaxyz/factory", "@hasna/secrets", "@hasna/stations", "@hasna/contracts"]',
      "",
    ].join("\n"));
