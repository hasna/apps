...
const PACKAGE_NAME = "@hasna/stations";
const MAX_PACKAGE_BYTES = 4 * 1024 * 1024;
const STATIONS_BIN_NAMES = ["stations", "stations-mcp", "stations-daemon", "stations-serve"] as const;
...
const REQUIRED_FILES = [
  ...
  "package/schemas/stations-consumer.schema.json",
...
async function main(): Promise<void> {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "stations-release-"));
  try {
  ...
  console.log("stations release verification passed");
}
...
async function assertPackedPackageBinBoundary(packed: string): Promise<void> {
  ...
  assertStationsOwnedBinBoundary(packedPackage.bin ?? {});
}

function assertReleaseScripts(): void {
  assertStationsOwnedBinBoundary(packageJson.bin ?? {});
...
async function smokeInstalledPackage(appDir: string): Promise<void> {
  await run(["bun", "-e", "import('@hasna/stations').then((m)=>{ if (!m.getPackageVersion || !m.checkMachineCompatibility || !m.resolveMachineWorkspace) throw new Error('missing root exports') })"], { cwd: appDir, quiet: true });
  await run(["bun", "-e", "import('@hasna/stations').then((m)=>{ for (const name of ['writeManifest','getDb','upsertHeartbeat','writeHeartbeat','watchTmuxPane','getStoragePg','PgAdapterAsync','createTrustedSdkMutationApproval']) if (name in m) throw new Error('raw root export: '+name); })"], { cwd: appDir, quiet: true });
  await run(["bun", "-e", "import('@hasna/stations/consumer').then((m)=>{ if (!m.resolveMachineWorkspace || !m.STATIONS_CONSUMER_CONTRACT_VERSION) throw new Error('missing consumer exports') })"], { cwd: appDir, quiet: true });
  await run(["bun", "-e", "import('@hasna/stations/storage').then((m)=>{ if (!m.getStorageStatus || !m.storagePush) throw new Error('missing storage exports') })"], { cwd: appDir, quiet: true });
  await run(["bun", "-e", "import('@hasna/stations/storage').then((m)=>{ for (const name of ['getStoragePg','PgAdapterAsync']) if (name in m) throw new Error('raw storage export: '+name); })"], { cwd: appDir, quiet: true });
  await run(["sh", "-lc", "command -v ./node_modules/.bin/stations ./node_modules/.bin/stations-mcp ./node_modules/.bin/stations-daemon"], { cwd: appDir, quiet: true });
  assertInstalledDependencyBinBoundary(appDir);
  await run(["./node_modules/.bin/stations", "--version"], { cwd: appDir, quiet: true, expect: packageJson.version });
  await run(["./node_modules/.bin/stations", "--help"], { cwd: appDir, quiet: true, expect: "Usage:" });
  await run(["./node_modules/.bin/stations", "self-test", "--json"], { cwd: appDir, quiet: true, expect: "\"checks\"" });
  await run(["./node_modules/.bin/stations-mcp", "--version"], { cwd: appDir, quiet: true, expect: packageJson.version });
  await run(["./node_modules/.bin/stations-mcp", "--help"], { cwd: appDir, quiet: true, expect: "Usage:" });
  await run(["./node_modules/.bin/stations-daemon", "--version"], { cwd: appDir, quiet: true, expect: packageJson.version });
  await run(["./node_modules/.bin/stations-daemon", "--help"], { cwd: appDir, quiet: true, expect: "Usage:" });
  await run([
      "bun",
      join(appDir, "node_modules", "@hasna", "stations", "scripts", "consumer-conformance.mjs"),
    "--package-dir",
    join(appDir, "node_modules", "@hasna", "stations"),
    "--cli-command",
    join(appDir, "node_modules", ".bin", "stations"),
  ], { cwd: appDir, quiet: true, expect: "stations consumer conformance: ok" });
}

function assertStationsOwnedBinBoundary(bin: Record<string, string>): void {
  const actual = Object.keys(bin).sort();
  assert(JSON.stringify(actual) === JSON.stringify([...STATIONS_BIN_NAMES].sort()), `package bin names must be stations-owned only, got ${actual.join(", ")}`);
...
function assertInstalledDependencyBinBoundary(appDir: string): void {
  for (const name of DEPENDENCY_EVENT_BIN_NAMES) {
    ...
    assert(!target.includes(join("node_modules", "@hasna", "stations")), `dependency-owned ${name} bin must not resolve inside @hasna/stations`);
  }
}
...
async function run(
  ...
  const dataRoot = join(tmpdir(), "stations-release-data");
  const proc = Bun.spawn(cmd, {
    ...
    env: {
      ...process.env,
      HASNA_STATIONS_DIR: dataRoot,
      HASNA_STATIONS_DB_PATH: join(dataRoot, "stations.db"),
      HASNA_STATIONS_MANIFEST_PATH: join(dataRoot, "stations.json"),
      HASNA_STATIONS_NOTIFICATIONS_PATH: join(dataRoot, "notifications.json"),
      HASNA_STATIONS_MACHINE_ID: "release-smoke-local",
    },
  });
