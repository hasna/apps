...
export interface CollectionOptions { limit?: number; cursor?: number; verbose?: boolean }
...
export function windowPage<K extends string, T, U>(key: K, rows: T[], options: CollectionOptions, summarize: (item: T) => U) {
  ...
  return {
    [key]: options.verbose ? selected : selected.map(summarize), count: selected.length, total: null,
    limit, cursor, next_cursor: hasMore ? cursor + selected.length : null, has_more: hasMore, compact: !options.verbose,
    hint: hasMore ? `Continue with cursor=${cursor + selected.length}; set verbose=true for full fields in a page, or full=true for the legacy response.`
      : "Set verbose=true for full fields in this page, or full=true for the legacy response.",
  } as Record<K, Array<T | U>> & { count:number; total:null; limit:number; cursor:number; next_cursor:number|null; has_more:boolean; compact:boolean; hint:string };
}
