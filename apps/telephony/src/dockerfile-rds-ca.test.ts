...
/**
 * Regression test for O15-00669 — the one-shot migrate task
 * (`bun scripts/apply-cloud-migrations.mjs`, run as `telephony-prod-migrate`)
 * exited 1 against the shared RDS with `self signed certificate in certificate
 * chain`, blocking every telephony deploy (oss-fleet-prod ECS).
 *
 * The telephony image is the single artifact used by BOTH the serve task and
 * the migrate task (the migrate task overrides the CMD to
 * `bun scripts/apply-cloud-migrations.mjs`). The vendored storage kit
 ...
 * logs/emails/todos/files/loops/economy/conversations/knowledge images ship),
 * so a stale or wrong bundle fails CI.
 */
...
describe("telephony image RDS CA contract (O15-00669)", () => {
  ...
  test("wires the connection's CA path to the bundled CA", () => {
    ...
    // runner (scripts/apply-cloud-migrations.mjs) builds its client through
