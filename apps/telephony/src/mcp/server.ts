...
export function buildServer(): McpServer {
  ...
  server.tool("telephony_list_messages", "List messages", {
    ...
    const rows = await getStore().listMessages({ agent_id: args.agent_id, project_id: args.project_id, limit: limit + 1, offset: cursor });
    return text(windowPage("messages", rows, { limit, cursor, verbose: args.verbose }, compactMessage));
  });
  ...
  server.tool("telephony_list_calls", "List call log", {
    ...
    const limit = args.limit ?? 20; const cursor = args.cursor ?? 0;
    const rows = await getStore().listCalls({ agent_id: args.agent_id, project_id: args.project_id, limit: limit + 1, offset: cursor });
    return text(windowPage("calls", rows, { limit, cursor, verbose: args.verbose }, compactCall));
  });
