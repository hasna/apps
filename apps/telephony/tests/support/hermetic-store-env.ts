 ...
 * `hasna.credentials.telephony.api-key` the shared @hasna/contracts resolver
 * (which consults the Keychain for the live `process.env`, and only for it)
 * resolves the fleet key, the opt-in yields, and `bun test` transacts against
 * the live fleet. It did: two `SMsignedreplay` fixture rows reached the hosted
 * service from the serve suite before this helper existed.
 *
