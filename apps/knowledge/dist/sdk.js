...
function adoptResolverDataHome(resolved, env = process.env) {
  ...
  if (typeof dataOverride === "string" && dataOverride.trim().length > 0)
    return true;
  return existsSync2(join2(resolved, "knowledge.db")) || existsSync2(join2(resolved, "config.json"));
}
...
function knowledgeCredentialsPath(env = process.env) {
  return credentialDiskSources2(KNOWLEDGE_APP_SLUG, env)[0] ?? join5(getDataHome(env), "config", "credentials");
}
