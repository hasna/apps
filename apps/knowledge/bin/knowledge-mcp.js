...
var MCP_HTTP_SERVICE_NAME = "knowledge", DEFAULT_MCP_HTTP_PORT = 8819;
var init_mcp_http = () => {};
...
import { existsSync as existsSync15, readFileSync as readFileSync14, writeFileSync as writeFileSync6 } from "fs";
// package.json
var package_default = {
...
class HalfConfiguredKnowledgeClientError extends Error {
  ...
  constructor(urlKeysPresent) {
    ...
    super(`knowledge: ${urlKeysPresent.join(", ")} names an API store, but no mode variable says to use it, ` + "so this command would silently read and write the on-box store instead. " + `Set ${canonical}=postgres to use the API, or ${canonical}=sqlite to confirm you want the on-box store. ` + `Run 'knowledge mode' to see the full resolution.`);
...
import { chmodSync as chmodSync3, existsSync as existsSync4, mkdirSync as mkdirSync2, readFileSync as readFileSync4, statSync as statSync2, writeFileSync as writeFileSync3 } from "fs";
...
class LocalArtifactStore {
  ...
  async put(entry) {
    ...
    const path = join4(this.root, key);
    assertInside(this.root, path);
    mkdirSync2(dirname3(path), { recursive: true, mode: 448 });
    writeFileSync3(path, entry.body, { mode: 384 });
    chmodSync3(path, 384);
    return { key, uri: pathToFileURL(path).href, modified_at: statSync2(path).mtime.toISOString() };
  }
  async getText(key) {
    const normalizedKey = normalizeArtifactKey(key);
    const path = join4(this.root, normalizedKey);
    assertInside(this.root, path);
...
// src/auth.ts
import { existsSync as existsSync5, mkdirSync as mkdirSync3, readFileSync as readFileSync5, unlinkSync as unlinkSync2, writeFileSync as writeFileSync4 } from "fs";
...
function knowledgeAuthPath(env = process.env) {
  ...
  const root = env.HASNA_KNOWLEDGE_AUTH_DIR ?? join5(homedir2(), ".hasna", "knowledge");
...
function validateStorageConfig(config, workspace) {
  ...
  for (const file of forbiddenFiles) {
    errors2.push(`Forbidden Knowledge workspace file present: ${file}. Move secrets to open-secrets/runtime env and remove or replace legacy backups/exports with redacted owner-only artifacts.`);
  }
  ...
  if (config.storage.type === "s3") {
    if (!config.storage.s3?.bucket)
      errors2.push("storage.s3.bucket is required when storage.type is s3.");
  ...
  if (!config.sources.allowed_schemes.includes("open-files")) {
    errors2.push("sources.allowed_schemes must include open-files.");
  }
  if (config.mode === "hosted" && config.hosted?.api_url) {
    try {
      normalizeKnowledgeApiOrigin(config.hosted.api_url);
    } catch {
      errors2.push("hosted.api_url must be an http(s) URL when mode is hosted.");
    }
  }
  return {
    ok: errors2.length === 0,
...
function auditId(input) {
  return `audit_${createHash2("sha256").update(`${input.event_type}\x00${input.action}\x00${input.target_uri ?? ""}\x00${input.created_at ?? ""}\x00${JSON.stringify(input.metadata ?? {})}\x00${randomUUID3()}`).digest("hex").slice(0, 24)}`;
}
...
import { existsSync as existsSync12, lstatSync as lstatSync2, readdirSync as readdirSync2, readFileSync as readFileSync11, statSync as statSync3 } from "fs";
...
function isMigrationTombstone(workspace, summary, currentHome) {
  ...
  if (summary.files.some((file) => !isRetainedTombstoneFile(file)))
    return false;
  try {
    const metadata = JSON.parse(readFileSync12(join8(workspace.home, "migration.json"), "utf8"));
...
function truncateText2(value, max = 160) {
...
function compactItem(item, options = {}) {
  ...
  const metadataKeys2 = Object.keys(metadata);
  const row = {
    id: item.id,
    short_id: item.short_id,
    title: truncateText2(item.title, 120),
    url: item.url ? truncateText2(item.url, 160) : null,
    tags: tags.slice(0, 12).map((tag) => truncateText2(tag, 48)),
    tag_count: tags.length,
    archived: item.archived ?? false,
    created_at: item.created_at,
    updated_at: item.updated_at,
    content_preview: truncateText2(item.content, 160),
    content_length: item.content?.length ?? 0,
    metadata_keys: metadataKeys2.slice(0, 20).map((key) => truncateText2(key, 64)),
    metadata_key_count: metadataKeys2.length
  };
...
function parseJsonObject7(value) {
  if (!value)
    return {};
  try {
    const parsed = JSON.parse(value);
...
function rowWithJson(row, fields = ["metadata_json", "acl_json"]) {
  ...
  for (const field of fields) {
    if (field in next) {
      ...
      next[name] = parseJsonObject7(next[field]);
      delete next[field];
    }
  }
  return next;
...
function openFilesSnapshot(service = projectService()) {
  ...
  try {
    ...
    return {
      ...
      refs: rows.map((row) => {
        const metadata = parseJsonObject7(row.metadata_json);
        return {
          id: row.id,
          uri: row.uri,
...
function buildServer() {
  ...
  registerTool(server, "knowledge_app_wiki_init", "Initialize app wiki scope", "Create the scoped Hasna app/project wiki workspace and SQLite catalog through the Knowledge service", {
    scope: scopeField,
    allow_global: exports_external.boolean().optional().describe("Required for global-scope app wiki writes")
  ...
  registerTool(server, "knowledge_app_wiki_note_add", "Add app wiki note", "Write a scoped app/project wiki note through Knowledge artifacts and catalog rows", {
    scope: scopeField,
    title: exports_external.string().describe("Note title"),
    content: exports_external.string().describe("Note content"),
    tags: exports_external.array(exports_external.string()).optional().describe("Optional note tags"),
    source_refs: exports_external.array(exports_external.string()).optional().describe("Optional source refs to cite"),
    allow_global: exports_external.boolean().optional().describe("Required for global-scope app wiki writes")
  ...
  registerTool(server, "knowledge_app_wiki_source_add", "Add app wiki source ref", "Ingest a read-only source ref into the scoped app/project wiki catalog", {
    scope: scopeField,
    source_ref: exports_external.string().describe("Source reference URI, e.g. file://... or open-files://..."),
    purpose: exports_external.string().optional().describe("Purpose label, default knowledge_index"),
    allow_global: exports_external.boolean().optional().describe("Required for global-scope app wiki writes")
  ...
  registerTool(server, "knowledge_sync_doctor", "Knowledge sync doctor", "Read-only readiness report for machine sync, storage, open-files source refs, route/workspace resolution, and next commands", {
    scope: scopeField,
    machine: exports_external.string().optional().describe("Optional remote machine id or SSH alias"),
    peer_workspace: exports_external.string().optional().describe("Optional peer repo root or .hasna/knowledge path"),
    tables: exports_external.array(exports_external.string()).optional().describe("Optional knowledge.db tables to include in recommended dry-run commands"),
    ...
    try {
      return jsonText({
        package: { name: package_default.name, version: package_default.version },
  ...
  registerTool(server, "knowledge_sync_peer", "Knowledge peer sync", "Dry-run, pull, push, or bidirectionally sync with a local or remote peer knowledge workspace", {
    scope: scopeField,
    peer_workspace: exports_external.string().optional().describe("Peer repo root or .hasna/knowledge path; optional for remote machine sync when machines path mapping is configured"),
  ...
  registerTool(server, "knowledge_context_pack", "Bounded knowledge context pack", "Return compact cited JSON for agents under token and item budgets", {
    ...
    dedupe: exports_external.boolean().optional().describe("Include duplicate candidates for run/loop evidence"),
    model: exports_external.string().optional().describe("Embedding model ref, default openai:text-embedding-3-small"),
    dimensions: exports_external.number().optional().describe("Embedding dimensions for deterministic fake mode"),
    fake: exports_external.boolean().optional().describe("Use deterministic fake embeddings for local tests")
  ...
  registerTool(server, "ok_import", "Import knowledge items", "Import items from an exported JSON file, skipping duplicate IDs", {
    ...
    const imported = JSON.parse(readFileSync14(file, "utf8"));
...
async function main() {
  ...
  if (isHttpMode2()) {
    const handle = await startMcpHttpServer2(buildServer, {
      port: resolveMcpHttpPort2()
    });
