...
function forbiddenWorkspaceFilesPresent(workspace) {
  const present = [];
  if (existsSync6(join5(workspace.home, "cloud.env")))
    present.push("cloud.env");
  if (existsSync6(join5(workspace.home, "migration-exports")))
...
// src/auth.ts
var DEFAULT_KNOWLEDGE_API_URL = KNOWLEDGE_DEFAULT_API_URL;
function credentialSourceKind(tier) {
  if (tier === "keychain")
    return "keychain";
  if (tier === "disk" || tier === "profile")
    return "file";
  return "env";
}
...
function resolveKnowledgeApiAuthority(env = process.env) {
...
async function probeKnowledgeAuth(env = process.env) {
  ...
  if (!key.apiKey && key.tier !== "pointer")
    return notProbed;
...
var AI_PROVIDER_IDS = ["openai", "anthropic", "deepseek"];
...
${answer}`;
  ...
  const writePolicy = {
    approved: options.approveWrite === true,
    durable_writes_performed: false,
    reason: options.approveWrite ? "Approval flag recorded; durable wiki writes are deferred to an approval-gated wiki compile step (knowledge wiki compile --approve-write)." : "Dry-run mode: proposed wiki updates require approval before durable writes."
  };
  return {
    run_id: runId,
    prompt,
...
import { basename as basename5, extname as extname2, join as join7, relative as relative4, resolve as resolve5, sep as sep4 } from "path";
...
function isMigrationTombstone(workspace, summary, currentHome) {
  ...
  if (summary.files.some((file) => !isRetainedTombstoneFile(file)))
    return false;
  try {
    const metadata = JSON.parse(readFileSync11(join8(workspace.home, "migration.json"), "utf8"));
...
function resolvePeerWorkspace(input) {
  ...
  if (existsSync15(join9(target, "knowledge.db")) || existsSync15(join9(target, "config.json"))) {
