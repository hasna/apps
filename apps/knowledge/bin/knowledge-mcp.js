...
async function getKnowledgeRecord(kind, id, options = {}) {
  ...
  if (usesKnowledgeHttpTransport()) {
    if (normalized !== "auto" && normalized !== "item") {
      throw new Error(`knowledge: reading a '${normalized}' record targets the on-box sqlite RAG catalog, which is not available to the HTTP client ` + `(a credential resolved through the @hasna/contracts chain). The shared corpus is the server ` + `knowledge-items; use kind 'item' (or 'auto'), or pass an explicit store_path to read the full ` + `local catalog.`);
    }
