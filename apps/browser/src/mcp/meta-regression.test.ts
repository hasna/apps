...
// ─── batch action parsing ────────────────────────────────────────────────────

describe("browser_batch action dispatch", () => {
  it("strips browser_ prefix from tool names", () => {
    const toolNames = ["browser_navigate", "navigate", "browser_click", "click"];
    const normalized = toolNames.map(n => n.replace(/^browser_/, ""));
    expect(normalized).toEqual(["navigate", "navigate", "click", "click"]);
  });

  it("tracks success/failure counts correctly", () => {
    const results = [
      { tool: "navigate", success: true, result: { url: "https://example.com" } },
      { tool: "click", success: true },
      { tool: "unknown", success: false, error: "Unknown tool" },
    ];
...
// ─── cron manager basic API ─────────────────────────────────────────────────

describe("cron manager basic API", () => {
  it("createCronJob and listCronJobs are functions", async () => {
    const { createCronJob, listCronJobs } = await import("../lib/cron-manager.js");
    expect(typeof createCronJob).toBe("function");
    expect(typeof listCronJobs).toBe("function");
  });

  it("listCronJobs returns array", async () => {
    const { listCronJobs } = await import("../lib/cron-manager.js");
    const jobs = listCronJobs();
    expect(Array.isArray(jobs)).toBe(true);
  });

  it("deleteCronJob returns false for nonexistent id", async () => {
    const { deleteCronJob } = await import("../lib/cron-manager.js");
    expect(deleteCronJob("nonexistent")).toBe(false);
  });
});

// ─── url watcher basic API ─────────────────────────────────────────────────

describe("url watcher basic API", () => {
  it("createWatchJob and listWatchJobs are functions", async () => {
    const { createWatchJob, listWatchJobs } = await import("../lib/url-watcher.js");
    expect(typeof createWatchJob).toBe("function");
    expect(typeof listWatchJobs).toBe("function");
  });

  it("listWatchJobs returns array", async () => {
    const { listWatchJobs } = await import("../lib/url-watcher.js");
    const jobs = listWatchJobs();
    expect(Array.isArray(jobs)).toBe(true);
  });

  it("deleteWatchJob returns false for nonexistent id", async () => {
    const { deleteWatchJob } = await import("../lib/url-watcher.js");
    expect(deleteWatchJob("nonexistent")).toBe(false);
  });
});

// ─── task queue basic API ───────────────────────────────────────────────────

describe("task queue basic API", () => {
  it("queueBrowserTask and getBrowserTasks are functions", async () => {
    const { queueBrowserTask, getBrowserTasks } = await import("../lib/task-queue.js");
    expect(typeof queueBrowserTask).toBe("function");
    expect(typeof getBrowserTasks).toBe("function");
  });

  it("getBrowserTasks returns array", async () => {
    const { getBrowserTasks } = await import("../lib/task-queue.js");
    const tasks = await getBrowserTasks();
