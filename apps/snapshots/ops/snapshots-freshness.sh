...
if ! grep -q '"ok"' "$OUT_FILE" 2>/dev/null; then
  echo "$(date -u +%FT%TZ) check-error rc=$RC unexpected output $(head -c 300 "$OUT_FILE" 2>/dev/null)" >> "$LOG"
  rm -f "$OUT_FILE" "$ERR_FILE"
  exit 0
fi

OK="$(sed -n 's/.*"ok": \(true\|false\).*/\1/p' "$OUT_FILE" | head -1)"
REASON="$(sed -n 's/.*"reason": "\([a-z-]*\)".*/\1/p' "$OUT_FILE" | head -1)"
LAST_RUN="$(sed -n 's/.*"last_capture_run_at": "\([^"]*\)".*/\1/p' "$OUT_FILE" | head -1)"
AGE="$(sed -n 's/.*"last_capture_run_age_seconds": \([0-9]*\).*/\1/p' "$OUT_FILE" | head -1)"
rm -f "$OUT_FILE" "$ERR_FILE"
...
case "$REASON" in
  ...
  capture-run-stale)
    ...
    ;;
  *)
    echo "$(date -u +%FT%TZ) ALERT unexpected freshness result reason=${REASON:-unknown}" >> "$LOG"
    exit 0
    ;;
esac
exit 1
