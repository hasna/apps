/**
 * snapshots data-root resolution — thin app wrapper over the single paths
 * resolver in `@hasna/contracts` (ruling hasna/apps#1668). The resolver owns
 * platform placement (`~/.hasna/snapshots` on macOS, XDG data root on Linux)
 * and the `HASNA_{CONFIG,DATA,STATE,CACHE}_HOME` kind overrides; this module
 * layers the snapshots-specific exact-app override on top.
 */
...
/**
 * The resolver snapshots data root: kind overrides honored,
 * `~/.hasna/snapshots` on macOS, `~/.local/share/hasna/snapshots` on Linux.
 */
export function getResolverDataRoot(env: NodeJS.ProcessEnv = process.env): string {
  return resolverDataDir({ app: "snapshots", home: getHomeDir(env), env, });
}

/**
 * The pre-ruling legacy root (`~/.hasna/snapshots`). On macOS this equals the
...
/**
 * The effective snapshots data root: an exact-app override wins
