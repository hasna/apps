...
function sandbox(check: (root: string, env: Record<string, string>) => void): void {
  const root = mkdtempSync(join(tmpdir(), "draw-install-isolation-"));
...
describe("draw library filesystem boundary", () => {
  ...
  test("CLI retains explicit board-file writes and stdout-only exports", () => sandbox((root, env) => {
    const files = join(root, "files");
    mkdirSync(files);
    const board = join(files, "board.json");
    const cli = (...args: string[]) => run([process.execPath, join(packageRoot, "src/cli/index.ts"), ...args], files, env);
    expect(JSON.parse(cli("create")).board.cards).toEqual([]);
    expect(readdirSync(files)).toEqual([]);
    cli("create", board, "--title", "Install boundary");
    cli("add", board, "--note", "Hello world.", "--pin");
    cli("add", board, "--drawing");
    expect(cli("list", board)).toContain("Hello world.");
    expect(cli("stats", board)).toMatch(/Total\s+2/);
    const saved = readFileSync(board, "utf8");
    expect(JSON.parse(cli("export", board)).board.cards).toHaveLength(2);
    expect(JSON.parse(cli("export", board, "--to", "excalidraw")).type).toBe("excalidraw");
    expect(readFileSync(board, "utf8")).toBe(saved);
    expect(readdirSync(files)).toEqual(["board.json"]);
    expectNoImplicitState(env);
  }));
  ...
  test("SDK and React entry points retain their public capabilities without implicit state", () => sandbox((_root, env) => {
    const output = run([process.execPath, "-e", `
      import { createBoard, loadBoard } from "./src/index.ts";
      import { Board, Card, DrawSurface, NoteEditor } from "./src/react/index.ts";
      if (![Board, Card, DrawSurface, NoteEditor].every((value) => typeof value === "function")) throw new Error("React exports missing");
      const board = createBoard({ title: "Boundary" }).addNote({ text: "Hello world." }).addDrawing();
      const loaded = loadBoard(board.serialize());
      console.log(JSON.stringify({ title: loaded.title, stats: loaded.stats(), text: loaded.cards[0].text }));
    `], packageRoot, env);
    expect(JSON.parse(output)).toMatchObject({
      title: "Boundary", stats: { total: 2, notes: 1, drawings: 1 }, text: "Hello world.",
    });
    expectNoImplicitState(env);
  }));
});
