...
export async function recurringProtocol<T>(action: (f: ReturnType<typeof createRecurringProtocolFixture>) => Promise<T>): Promise<T> {
  const f = createRecurringProtocolFixture(), original = globalThis.fetch;
...
export function createRecurringProtocolFixture(origin = "https://recurring.example.com/prefix") {
  const root = mkdtempSync(join(realpathSync(tmpdir()), "recurring-surface-unit-"));
