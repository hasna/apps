...
export interface DiscoverySource { path: string; sha256: string | null; format?: "json" | "toml"; fields?: string[] }
...
function projected(source: DiscoverySource, changes?: Map<string, string>): string | null {
  ...
  const object = parseConfig(text, source.path, source.format === "toml");
...
export function resolveAgentDiscovery(options: { home: string; agent: IntegrationAgent; reviewed?: ReviewedDiscoveryInputs; canonical?: (path: string) => string }): AgentDiscoveryBinding {
  ...
  const witness = (path: string, format?: "json" | "toml", fields?: string[]) => {
  ...
  const configPath = join(home, agent === "opencode" ? ".config/opencode/opencode.json" : `.${agent}/${agent === "codex" ? "config.toml" : agent === "cursor" ? "hooks.json" : "settings.json"}`);
  const configText = witness(configPath, agent === "codex" ? "toml" : "json", agent === "claude" ? ["enabledPlugins", "extraKnownMarketplaces"] : agent === "codex" ? ["plugins", "marketplaces", "skills"] : agent === "gemini" ? ["skills", "extensions", "security"] : agent === "opencode" ? ["plugin", "skills"] : ["version"]);
  const config: any = configText === null ? {} : parseConfig(configText, configPath, agent === "codex");
  ...
  if (review) {
    ...
    return { ...supplied, sources: [...sources, ...review.sources], ...(agent === "gemini" ? { builtinNames } : {}) };
  }
