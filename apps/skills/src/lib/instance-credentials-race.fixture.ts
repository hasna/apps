...
writeFileSync(credentialFile, text(oldKey, "http://127.0.0.1:4001"), { mode: 0o600 });
...
mock.module("node:fs", () => ({ ...realFs, readFileSync: (...args: any[]) => {
  ...
  if (!rotated && args[0] === capturedFd) {
    writeFileSync(`${credentialFile}.next`, text(newKey, "http://127.0.0.1:4002"), { mode: 0o600 });
...
try {
  ...
  const result = await resolveSkillsConnection({ HASNA_HOME: scratch, HASNA_SKILLS_API_URL: "http://127.0.0.1:4002" }, { credentials: { keychain: { enabled: false } } });
  console.log(JSON.stringify({ rotated, wrongInstanceCredentialPair: result?.apiKey === oldKey && result.apiOrigin === "http://127.0.0.1:4002" }));
