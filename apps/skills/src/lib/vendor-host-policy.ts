...
export const APPROVED_CODE_HOSTS: readonly { domain: string; reason: string }[] = [
  ...
  { domain: "signin.aws", reason: "AWS SSO OIDC signin endpoint ({Region}.oauth.signin.aws) from the bundled AWS SDK signin endpoint tables; BYO AWS credentials." },
...
export const DYNAMIC_HOST_SITES: readonly { file: string; path: string; expr: string; reason: string }[] = [
  ...
  },
  {
    ...
    path: "",
    expr: "parsed.host",
    reason: "fast-uri normalize(): ASCII-converts a caller's parsed URI host (new URL('http://' + parsed.host).hostname); the value is caller-supplied input, never a fixed endpoint.",
  },
  {
    file: "bin/mcp.js",
    path: "",
    expr: "parsed.host",
    reason: "Bundled copy of fast-uri normalize() (see bin/index.js entry).",
  },
  {
    file: "bin/worker.js",
    path: "",
