...
import { existsSync, lstatSync, mkdirSync, readFileSync, readdirSync, renameSync, writeFileSync, unlinkSync, chmodSync } from "node:fs";
...
export interface NativeSkillEntry { agent: string; path: string; hash: string; managed: boolean; vendor: boolean }
export interface AgentConfigChange { path: string; before: string | null; after: string }
export interface AgentIntegrationPlan { dataDir: string; profileId: string; changes: AgentConfigChange[]; nativeSkills: NativeSkillEntry[] }
...
export function inventoryNativeSkills(home = homedir(), options: { includeVendor?: boolean; projectDir?: string } = {}): NativeSkillEntry[] {
  const roots: Array<readonly [string, string]> = ROOTS.map(([agent, path]) => [agent, join(home, path)]);
  ...
  function visit(agent: string, path: string, vendor = false, depth = 0): void {
    ...
    if (existsSync(join(path, "SKILL.md"))) {
      ...
      entries.push({ agent, path, hash: treeHash(path), managed, vendor }); return;
    }
  ...
  if (options.includeVendor) {
    for (const agent of ["codex", "claude"]) visit(agent, join(home, `.${agent}`, "plugins", "cache"), true);
  }
  return entries;
}
...
function disableCodexSkills(text: string, skills: NativeSkillEntry[]): string {
  Bun.TOML.parse(text);
  let result = text;
  for (const skill of skills.filter(entry => entry.agent === "codex")) {
    ...
    result = result.replace(/^\[\[skills\.config\]\][^\n]*(?:\n(?!\s*\[)[^\n]*)*/gm, section => {
      ...
      if (!declared || ![resolve(path), resolve(skill.path)].includes(resolve(declared))) return section;
      found = true;
...
export function planAgentIntegration(options: { home?: string; dataDir?: string; agents: IntegrationAgent[]; command?: string; profileId?: string; includeVendor?: boolean; projectDir?: string }): AgentIntegrationPlan {
  ...
  const nativeSkills = inventoryNativeSkills(home, { includeVendor: options.includeVendor, projectDir: options.projectDir });
  ...
  for (const agent of [...new Set(options.agents)]) {
    ...
    const path = join(home, `.${agent}`, agent === "claude" ? "settings.json" : "hooks.json");
    ...
    if (agent === "codex") {
      const configPath = join(home, ".codex", "config.toml"), previous = readOptional(configPath);
      const next = disableCodexSkills(previous ?? "", nativeSkills);
      if (next !== (previous ?? "")) changes.push({ path: configPath, before: previous, after: next });
    }
  }
  return { dataDir, profileId, changes, nativeSkills };
}
...
export function applyAgentIntegration(plan: AgentIntegrationPlan): { changed: string[]; backups: string[] } {
  ...
  try {
    for (const change of plan.changes) { atomicWrite(change.path, change.after); written.push(change); }
  ...
  atomicWrite(join(backupRoot, "receipt.json"), JSON.stringify({ version: 1, changes: plan.changes.map(change => ({ path: change.path, beforeHash: change.before === null ? null : sha(change.before), afterHash: sha(change.after) })), backups }) + "\n");
  return { changed: plan.changes.map(change => change.path), backups };
}

export function archiveNativeSkills(inventory: NativeSkillEntry[], options: { dataDir?: string; includeUnmanaged?: boolean }): { entries: Array<{ source: string; archive: string; hash: string }> } {
  ...
  try {
    ...
    atomicWrite(join(archiveRoot, "receipt.json"), JSON.stringify({ version: 1, entries }) + "\n");
  } catch (error) {
