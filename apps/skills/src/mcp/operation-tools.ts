...
export function registerOperationTools(server: McpServer): void {
  server.registerTool("scaffold_skill", {
    ...
    description: "Create a portable skill folder under <skills data root>/installed/<name> with SKILL.md, skill.json, AGENTS.md, package.json, and src/index.ts.",
    inputSchema: {
      name: z.string(),
  ...
  server.registerTool("port_skill", {
    title: "Port Skill",
    description: "Import an existing skill folder into the portable <skills data root>/installed/<name> standard and add missing standard files.",
    inputSchema: {
      path: z.string(),
