...
import { SqliteGovernanceStore, MemoryGovernanceStore } from "./governance-store.js";
...
for (const [name, amount] of malformed) test("malformed " + name + " never writes admission, reservation, settlement or events", async () => owned(async (store, governance) => {
...
for (const [estimate, actual] of [[0, 0], [25, 9], [Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER]] as const) test("valid cents preserve reserve and first-reconciliation replay: " + estimate, async () => owned(async (store, governance) => {
...
test("memory store refuses invalid direct writes and preserves terminal reconciliation", async () => {
  ...
  for (const [, amount] of malformed) await expect(store.createReservation({ orgId: "owned", runId: "owned", estimatedCents: amount })).rejects.toBeInstanceOf(RangeError);
  ...
  for (const [, amount] of malformed) await expect(store.reconcileReservation(reservation.id, amount, "charged")).rejects.toBeInstanceOf(RangeError);
