  ...
  describe("tags", () => {
    ...
    test("outputs remote tags with --remote --json", async () => {
      ...
      try {
        ...
        expect(exitCode).toBe(0);
        // The configured authority owns discovery; bundled authoring tags cannot leak in.
        const data = JSON.parse(stdout);
        ...
        expect(counts.get("remote")).toBe(2);
        expect(counts.get("audio")).toBe(1);
        expect(data.length).toBe(2);
        const localOnlyTag = [...baseline.keys()].find(tag => tag !== "audio" && tag !== "remote");
        expect(localOnlyTag).toBeDefined();
        expect(counts.has(localOnlyTag!)).toBe(false);
      } finally {
        server.stop(true);
      }
    });
