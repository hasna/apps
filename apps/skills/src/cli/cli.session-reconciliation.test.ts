...
test("the CLI plans and applies one exact session migration using only API reads", async () => {
  ...
  expect(metadata.profileId).toBe("legacy"); expect(requests).toEqual([]);
  ...
  const applied = await runCliInCwd([...args, "--apply", "--plan-digest", plan.planDigest], root, env);
  ...
  expect(JSON.parse(after.stdout).profileRevision).toBe("new-revision");
  const repeat = await runCliInCwd([...args, "--apply", "--plan-digest", plan.planDigest], root, env);
