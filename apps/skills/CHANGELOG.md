...
- a96a031: Include the current project and ancestor native skill directories in migration, matching the prompt hook's discovery scope. Report bounded offending paths when native skill copies block a prompt so the archive plan can be reviewed and applied from the affected project.
