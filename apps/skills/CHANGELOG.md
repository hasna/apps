...
- Preserve Codex native hook trust reconciliation when its writer moves a simple table across an unchanged array-of-tables run, while continuing to reject repeated or nested layouts and unrelated configuration drift.
...
- 9512b08: Keep existing native sessions on their validated pinned profile when the managed hook default changes, while new sessions use the configured profile. Report safe context error codes and the selected profile for genuine conflicts. Preserve fail-closed validation and discard foreign child messages and stderr.
- 1c77d2b: Preserve caller-owned credit checkout idempotency keys across SDK, CLI and MCP, including bounded unresolved-response errors. Explicit retries retain the same key; no automatic checkout retry is performed.
...
- b95cedd: Align the public administration schemas with the Zod v3 compatibility API used by the canonical build, so their inferred declarations also work with the declared Zod 4 dependency. Check all four public exports in a fresh strict TypeScript consumer, including administrative role and suspension literal inference.
...
- 2dc5814: Use hyphenated names when scaffolding or importing local skills, preserving CamelCase and acronym boundaries and converting dots and underscores to hyphens. Preserve existing installed identities and declared command names; align name declarations in copied instruction files without changing their original source. Make single-folder `port` / `add` return a failing exit status when JSON reports invalid validation, consistent with human output; imported folders remain available for correction.
- 4c66df8: Compute governed artifact hashes and byte counts from the text after redaction so stored metadata and sequential cumulative output checks match the persisted UTF-8 bytes. Concurrent admission remains a separate follow-up. Refuse missing or non-text bodies before storage instead of fabricating integrity metadata.
...
> Release note (2026-09-04): published from 5310b18f3 after an independent adversarial review (three merge rounds and a release review, all recorded on hasna/apps#1639). The `ci` run on that commit was red in exactly one job, `build + test (affected)`, and only on `/ui`, a package this release does not touch and that was removed from the repository the same afternoon (4144075d2); `/skills` itself was 1540 pass / 0 fail on Linux in that run. `prepublishOnly` (`typecheck && test`) cannot pass on macOS because of three environment-only tests (the macOS data-home layout, and GNU `find -printf`), so the publish ran with `--ignore-scripts` under the owner directive of 2026-09-04 after typecheck, build, `verify:release` and the full suite were run by hand at the release commit; `npm pack` with and without `--ignore-scripts` was verified byte-identical by the reviewer. Residual review findings: hasna/apps#1671. The stale changeset `.changeset/skills-paths-resolver.md` (the in-package path resolver, already shipped and described under 0.1.72) is consumed here so the next `changeset version` does not re-describe it as 0.2.1.
...
- 9d2514e: fix(skills): add the oss-app-two-backend-storage recipe to the canonical corpus with schema-valid frontmatter. The skill's description previously carried an unquoted colon+space that strict YAML loaders reject — Codewith 0.1.95 failed to load it on every startup. Ships a strict-frontmatter regression over the whole corpus plus the Development Tools catalog entry.
...
- 8b70821: skills-server answers --version/-V/--help before any bind (todos row 7e5f8f3d). Previously `skills-server --version`/`--help` fell through to resolveServerConfig()/startSkillsServer() and bound :8787 with no output.
...
- d44a19ceb: Hosted pins (T7): migration 0004, store methods, `/api/v1/pins` routes, and client pin/unpin/listPins.
- eb13fab5a: Hosted tag surface (T7): tags list, `skills?tag=` filtering, `tags/:tag/skills`, and `pins?tag=` querying.
- 5aab3c09f: RemoteSkillsClient pin/tag/incremental-sync methods with a fail-closed version-skew guard (T10).
- ee57c1414: When a remote registry is configured, its skills merge into the default list/search results (T5).
- 3d5431ee4: Cloud sync reconcile verb (T9 successor): write-free dry-run resolution and pack-then-sample pull re-check.
- e210a4c0c: Mirror the unified fleet session monitor into the canonical corpus.
...
- d9032bc7b: `skills auth.json` follows HASNA_SKILLS_DIR instead of always resolving under the default data root.
- cee14f862: The server reads HASNA_SKILLS_API_SIGNING_KEY as the canonical bundle signing key.
- 21902dd69: Display name "Hasna Skills" (open- prefix retired).
- cacc1c13a: The deploy lane is ported to the discoverable root workflow path and gate.
...
- **`skills sync [names...] [--for <agent>] [--dry-run] [--all] [--force]`** — the last
  ...
  `~/.config/opencode/…`, `~/.cursor/…`), per-tool adapted (Claude keeps
  `user_invocable`; Codewith/Codex/OpenCode/Cursor have it stripped). Instruction skills
  are written as prose the agent auto-loads; executable skills as a pointer to
  `skills run <name>`. Named bundled workflow skills copy their complete resource
  directory. **Non-clobbering**: every synced directory carries a
  `.hasna-skills.json` ownership marker. An unmanaged directory with SKILL.md is skipped
  unless `--force` explicitly adopts it; any other pre-existing unmarked directory is
  always left untouched. Managed updates and force adoption replace the owned directory
  from a staged exact mirror, removing stale files while restoring the original if the
  swap fails. This replaces the disabled legacy `sync` command and reverses the
  `installSkillForAgent()` stub — that function now writes an agent skill folder
  (single-skill entry point to the same non-clobbering writer) instead of returning
  `success: false`. New library exports: `syncSkillsToAgents`, `writeManagedAgentSkill`,
  `writeManagedSkillDir`, `adaptSkillMdForAgent`, `agentGlobalSkillsDir`,
  `pointerSkillMd`, `resolveSyncAgents`, `removeManagedAgentSkill`, `SYNC_AGENTS`.
