...
|---|---|---|
| Catalog skills | 87 | `SKILLS.length` (`src/lib/registry-data/index.ts`) |
| Instruction-kind skills | 21 | `SKILLS` entries with `kind: "instruction"` |
...
`skills/` holds those 87 catalog directories plus `_common`. The catalog is
**mixed**: 21 `kind: "instruction"` skills (SKILL.md prose, no `src/`) plus 66
