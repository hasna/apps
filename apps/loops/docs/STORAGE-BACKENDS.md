...
loudly instead of downgrading). No connection-mode selector participates in this decision: the resolver selects hosted authority, and `HASNA_LOOPS_LOCAL=1` is the only local selector. A database URL never
...
available only through the explicit `HASNA_LOOPS_LOCAL=1` opt-in. There is no connection-mode value to flip back.
...
1.1.0, and no published version of the package carries the field any more.
