...
- fix(release): harden the packed-artifact gate — the boundary scanner now blocks AWS ARNs and 12-digit AWS account ids in packed content (previously filename-only in the monorepo guard), and the packed-boundary check runs a content-level `secrets scan` over the extracted tarball. `secrets scan workspace` reports findings in its JSON payload while still exiting 0 (measured), so the gate parses the payload and fails closed on any finding, any scan error, or any unparseable output. Two-sided known-positive/known-negative fixtures guard the new patterns, including a planted-credential regression through the exact gate.
- fix(release): regenerate both lockfiles from the 0.5.8 manifest — the monorepo root `bun.lock` (loops workspace record 0.5.7→0.5.8, machines 0.2.32→0.2.29) and the standalone `apps/loops/bun.lock` used by the app's Docker build (contracts 0.10.6→0.13.1, machines 0.0.49→0.2.29) — and verify the frozen-install paths (`bun install --frozen-lockfile` at the workspace root and standalone) resolve the exact candidate versions. The runtime-design docs example now uses `<vault:...>` placeholder syntax so the packed-content scan stays silent on documented vault-path references.
