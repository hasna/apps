...
- e0f6dde: Loop bundles: a loop's definition and its scripts on disk, published as immutable `loop@version` artifacts, with an append-only revision ledger (hasna/apps#1724).

  A loop has always been a database row, and the row stays the runtime authority. But a loop may carry _scripts_, and a row cannot — which is why `loops hygiene scripts` reports every command target referencing `~/.hasna/loops/scripts` as a defect: such a loop is by definition un-portable. Bundles are the fix.
...
- e2d0c7d: Search the Bun global install's dependency bin directory when resolving executables.
