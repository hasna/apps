...
- `--knowledge-feedback` wires failed loop outcomes to the Knowledge CLI and appends bounded Knowledge context before agent starts. It is opt-in per target or via `LOOPS_KNOWLEDGE_FEEDBACK=1`, uses `knowledge upsert` and `knowledge context pack`, and never uses ad hoc home-level Markdown as knowledge storage.
