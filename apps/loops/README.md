...
- `--knowledge-feedback` opt-in emits failed loop outcomes after finalization through `knowledge upsert` and appends schema-validated, bounded, scrubbed `knowledge context pack` evidence before agent runs. Emission is always best-effort; only a context lookup configured with `required: true` can fail agent execution.
