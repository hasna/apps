...
describe("loops-daemon bin fails closed at startup (spawned)", () => {
  ...
  test("an option value equal to 'help' cannot bypass the startup gate", () => {
    const result = runDaemon(["run", "--interval-ms", "help"]);
    expect(result.status).not.toBe(0);
  ...
  test("`install --local` bootstraps without a pre-exported opt-in and writes an explicit local unit", () => {
    const result = runDaemon(["install", "--local"]);
    expect(result.status, result.stderr).toBe(0);
    const receipt = JSON.parse(result.stdout) as { local: boolean; path: string };
    expect(receipt.local).toBe(true);
    expect(result.installedUnit).toContain("HASNA_LOOPS_LOCAL");
    expect(result.sqliteFiles).toEqual([]);
  }, 30_000);

  test("`install --local` is explicit enough to bootstrap even when the shell has hosted credentials", () => {
    const result = runDaemon(["install", "--local"], { ...HOSTED_ENV });
    expect(result.status, result.stderr).toBe(0);
    expect(result.installedUnit).toContain("HASNA_LOOPS_LOCAL");
