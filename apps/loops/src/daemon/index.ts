...
export function resolveLoopsDaemonStartupGate(env: Record<string, string | undefined> = process.env):
  ...
  let issue: string;
  try {
    if (selectsLoopsLocalStore(env)) return { ok: true };
...
program
  ...
  .action((opts: { local: true }) => {
    console.log(
      JSON.stringify(
        installStartup(process.argv[1] ?? "loops-daemon", process.execPath, ["run"], process.platform, { local: opts.local }),
        null,
        2,
      ),
    );
...
if (import.meta.main) {
  ...
  if (!informational) {
    // `install --local` is the one bootstrap exception: it opens no store and
    // writes the explicit local opt-in into the unit it creates. Every other
    // daemon command, including bare `install`, must pass the startup gate.
    const installingExplicitLocalUnit = argv[0] === "install" && argv.includes("--local");
    const gate = installingExplicitLocalUnit ? { ok: true as const } : resolveLoopsDaemonStartupGate();
    if (!gate.ok) {
      console.error(gate.message);
      process.exit(1);
    }
    if (!installingExplicitLocalUnit) noticeLocalLoopsMode();
