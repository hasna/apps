 ...
 * to refuse outright whenever the server selected the hosted API
...
function serveHosted(
  loops: Loop[],
  runsByLoop: Record<string, LoopRun[]>,
  behavior: { loopResponse?: Loop; runsResponse?: LoopRun[] } = {},
) {
  const paths: string[] = [];
    ...
    fetch(request) {
      ...
      if (url.pathname === "/v1/loops/count") {
        ...
        return Response.json({
          ok: true,
          count: loops.filter((loop) => (status ? loop.status === status : true)).length,
        });
      }
      ...
      if (url.pathname.startsWith("/v1/loops/")) {
        ...
        const found = behavior.loopResponse ?? loops.find((loop) => loop.id === id || loop.name === id);
        return found
      ...
      if (url.pathname === "/v1/runs") {
        ...
        return Response.json({ ok: true, runs: (behavior.runsResponse ?? filtered).slice(0, limit) });
      }
...
describe("hosted MCP diagnostics (W13 PORT-TO-API)", () => {
  test("loops_health, loops_health_scan, loops_doctor and loops_diagnose all answer from /v1", async () => {
    ...
    try {
      ...
      expect(doctor.checks.every((check) => check.scope === "control-plane")).toBe(true);
      expect(doctor.unchecked.map((entry) => entry.id)).toContain("machine-runtime");
  ...
  test("refuses identity-mismatched hosted diagnostic rows", async () => {
    const root = mkdtempSync(join(tmpdir(), "loops-mcp-hosted-diag-wrong-id-"));
    roots.push(root);
    const requested = hostedLoop({ id: "requested-loop", name: "requested-loop" });
    const wrong = hostedLoop({ id: "different-loop", name: "different-loop" });
    const { server } = serveHosted([requested], { [requested.id]: [hostedRun(requested)] }, { loopResponse: wrong });
    const { client, transport } = await connectHostedMcp(root, server.port as number);
    try {
      const result = await client.callTool({ name: "loops_diagnose", arguments: { idOrName: requested.id } });
      expect(result.isError).toBe(true);
      const error = payload(result) as { error: { code: string; message: string } };
      expect(error.error.code).toBe("HOSTED_RESPONSE_MALFORMED");
      expect(error.error.message).toContain("diagnosed loop identity to match requested id or exact name");
      expect(dbFilesUnder(root)).toEqual([]);
    } finally {
      await client.close();
      await transport.close();
  ...
  test("refuses a run that belongs to a different loop", async () => {
    const root = mkdtempSync(join(tmpdir(), "loops-mcp-hosted-diag-wrong-run-loop-"));
    roots.push(root);
    const requested = hostedLoop({ id: "requested-loop", name: "requested-loop" });
    const wrongRun = hostedRun(requested, { loopId: "different-loop" });
    const { server } = serveHosted([requested], { [requested.id]: [] }, { runsResponse: [wrongRun] });
    const { client, transport } = await connectHostedMcp(root, server.port as number);
    try {
      const result = await client.callTool({ name: "loops_diagnose", arguments: { idOrName: requested.id } });
      expect(result.isError).toBe(true);
      const error = payload(result) as { error: { code: string; message: string } };
      expect(error.error.code).toBe("HOSTED_RESPONSE_MALFORMED");
      expect(error.error.message).toContain("run.loopId to equal requested loop id");
      expect(dbFilesUnder(root)).toEqual([]);
    } finally {
      await client.close();
      await transport.close();
  ...
  test("diagnose resolves bounded child-loop evidence instead of reporting a successful route drain as healthy", async () => {
    const root = mkdtempSync(join(tmpdir(), "loops-mcp-hosted-diag-child-"));
    roots.push(root);
    const parent = hostedLoop({
      id: "parent-loop",
      name: "parent-loop",
      target: { type: "command", command: "loops", args: ["routes", "drain", "todos-task"] },
    });
    const child = hostedLoop({ id: "child-loop", name: "child-loop" });
    const parentRun = hostedRun(parent, {
      id: "parent-run",
      stdout: JSON.stringify({
        results: [{ kind: "created", task: { id: "task-1", status: "pending", tags: [] }, loop: { id: child.id } }],
      }),
    });
    const childRun = hostedRun(child, { id: "child-run", status: "failed", exitCode: 1, stderr: "child failed" });
    const { server, paths } = serveHosted([parent, child], {
      [parent.id]: [parentRun],
      [child.id]: [childRun],
    });
    const { client, transport } = await connectHostedMcp(root, server.port as number);
    try {
      const diagnose = payload(await client.callTool({
        name: "loops_diagnose",
        arguments: { idOrName: parent.id },
      })) as { expectation: { ok: boolean; failure?: { classification?: string } }; unchecked: Array<{ id: string }> };
      expect(diagnose.expectation.ok).toBe(false);
      expect(diagnose.expectation.failure?.classification).toBe("route_functional");
      expect(diagnose.unchecked.map((entry) => entry.id)).not.toContain(`runs:${child.id}`);
