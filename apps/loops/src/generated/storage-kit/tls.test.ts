...
describe("generated storage kit TLS", () => {
  test("leaves local non-TLS URLs without pg ssl config", () => {
    ...
    expect(resolveTlsConfig("postgres://localhost/openloops?sslmode=disable")).toBeUndefined();
  });

  test("requires a CA bundle for sslmode=require", () => {
    expect(() => resolveTlsConfig("postgres://rds.example.test/openloops?sslmode=require", { env: {} })).toThrow(
  ...
  test("normalizes legacy ssl=true to the verified require path", () => {
    ...
    expect(() => resolveTlsConfig("postgres://rds.example.test/openloops?ssl=true", { env: {} })).toThrow(
      "requires verified TLS",
    );
  });
});
