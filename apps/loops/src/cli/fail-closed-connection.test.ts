...
import { loopsAuthorityEnvKeys } from "../lib/local-opt-in.js";
...
const SCRUBBED = new Set([...loopsAuthorityEnvKeys(), LOCAL_KEY, LOCAL_ALIAS]);
...
function runCli(args: string[], extraEnv: Record<string, string> = {}) {
  ...
  try {
    const result = spawnSync(process.execPath, ["--no-env-file", cliPath, ...args], {
      env: connectionEnv({
        ...extraEnv,
        HOME: scratch,
        HASNA_HOME: scratch,
    ...
    let installedUnit: string | null = null;
    if (result.status === 0 && args.includes("install")) {
      try {
        const receipt = JSON.parse(result.stdout) as { path?: unknown };
        if (typeof receipt.path === "string") installedUnit = readFileSync(receipt.path, "utf8");
      } catch {
        // The calling assertion will report malformed output.
      }
    }
    return { ...result, installedUnit };
  } finally {
...
describe("loops CLI fail-closed connection (owner rulings 2026-09-04 / 2026-09-07)", () => {
  ...
  test("import refuses authority before reading a missing or malformed local file", () => {
    withDataDir("import", (dataDir) => {
      const missing = runCli(["import", join(dataDir, "missing.json")], { LOOPS_DATA_DIR: dataDir });
      expect(missing.status).not.toBe(0);
      expect(output(missing)).toContain("no loops client connection is configured");
      expect(output(missing)).not.toContain("ENOENT");
      expect(readdirSync(dataDir)).toEqual([]);
  ...
  test("a guarded local-only command on the HOSTED route is REMOTE_COMMAND_UNSUPPORTED naming the command, with no store", () => {
    withDataDir("daemon-status-hosted", (dataDir) => {
      ...
      expect(text).toContain("loops-runner");
      expect(text).not.toContain("test-key-not-a-real-secret");
      expect(readdirSync(dataDir)).toEqual([]);
    });
  });

  test("`loops daemon install --local` bootstraps without a pre-exported opt-in and writes an explicit local unit", () => {
    withDataDir("umbrella-install", (dataDir) => {
      const result = runCli(["--json", "daemon", "install", "--local"], { LOOPS_DATA_DIR: dataDir });
      expect(result.status, output(result)).toBe(0);
      const receipt = JSON.parse(result.stdout) as { local: boolean };
      expect(receipt.local).toBe(true);
      expect(result.installedUnit).toContain("HASNA_LOOPS_LOCAL");
      expect(readdirSync(dataDir)).toEqual([]);
    });
  });

  test("umbrella `install --local` remains an explicit bootstrap under hosted shell credentials", () => {
    withDataDir("umbrella-install-hosted", (dataDir) => {
      const result = runCli(["--json", "daemon", "install", "--local"], { LOOPS_DATA_DIR: dataDir, ...HOSTED_ENV });
      expect(result.status, output(result)).toBe(0);
      expect(result.installedUnit).toContain("HASNA_LOOPS_LOCAL");
      expect(output(result)).not.toContain("test-key-not-a-real-secret");
      expect(readdirSync(dataDir)).toEqual([]);
    });
  });

  test("bare `loops daemon install` preserves fail-closed startup refusal and writes no unit", () => {
    withDataDir("umbrella-install-refusal", (dataDir) => {
      const result = runCli(["--json", "daemon", "install"], { LOOPS_DATA_DIR: dataDir });
      expect(result.status).not.toBe(0);
      expect(output(result)).toContain("required option '--local'");
      expect(result.installedUnit).toBeNull();
      expect(readdirSync(dataDir)).toEqual([]);
    });
  });
  ...
  test("a direct local Store command announces LOCAL mode once before opening SQLite", () => {
    withDataDir("direct-store-notice", (dataDir) => {
      const result = runCli(["export", "--dry-run", "--json"], { LOOPS_DATA_DIR: dataDir, [LOCAL_KEY]: "1" });
      expect(result.status, output(result)).toBe(0);
      expect(result.stderr.match(/loops: LOCAL mode/g)?.length ?? 0).toBe(1);
      expect(existsSync(join(dataDir, "loops.db"))).toBe(true);
    });
