...
describe("public package cloud boundary", () => {
  ...
  test("skips generated temporary and Bun cache directories", () => {
    const hostedSuffix = ["hasna", "xyz"].join(".");
    withBoundaryFixture(
      {
        ".tmp/generated.ts": `export const endpoint = "https://api.${hostedSuffix}/v1";`,
        ".bun-cache/generated.js": `export const endpoint = "https://api.${hostedSuffix}/v1";`,
        "src/client.ts": 'export const endpoint = "https://service.example/v1";',
      },
      (result) => {
        expect(result.status).toBe(0);
        expect(result.stdout).toContain("boundary scan passed");
