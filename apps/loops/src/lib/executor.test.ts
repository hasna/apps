...
describe("executeLoop", () => {
  ...
  describe("worktree enforcement", () => {
    ...
    test("passes --json when stopping timed out codewith durable agents", async () => {
      const root = mkdtempSync(join(tmpdir(), "loops-codewith-stop-json-"));
      const bin = join(root, "bin");
      mkdirSync(bin, { recursive: true });
      const argsFile = join(root, "codewith-args");
      const codewith = join(bin, "codewith");
      writeFileSync(
        ...
        [
          ...
          `printf -- '--\\n' >> ${JSON.stringify(argsFile)}`,
          'case " $* " in',
          "  *' agent start '*) echo '{\"agent\":{\"agentId\":\"a1\",\"status\":\"running\"},\"created\":true}' ;;",
          "  *' agent read '*) echo '{\"agent\":{\"agentId\":\"a1\",\"status\":\"running\"}}' ;;",
          "  *' agent stop '*) echo '{\"agent\":{\"agentId\":\"a1\",\"status\":\"cancelled\"}}' ;;",
          "  *) echo '{}' ;;",
          "esac",
        ].join("\n"),
      );
      ...
      try {
        const loop = store.createLoop({
          name: "codewith-stop-json-contract",
          schedule: { type: "once", at: new Date().toISOString() },
          target: {
            type: "agent",
            provider: "codewith",
        ...
        const recorded = readFileSync(argsFile, "utf8");
        const invocations = recorded.split("--\n").filter((chunk) => chunk.trim().length > 0);
        const stopArgs = invocations.find((chunk) => chunk.includes("stop\n"));
        expect(stopArgs).toBeDefined();
        expect(stopArgs).toContain("--json");
      } finally {
        store.close();
        rmSync(root, { recursive: true, force: true });
      }
