...
function workerVerifierAgentVariables(opts: { addDirs: boolean; branchNoun: string }): LoopTemplateVariable[] {
  return [
    ...
    { name: "allowTools", description: "Comma-separated provider session tools the agent may use." },
    { name: "allowCommands", description: "Comma-separated shell commands the agent may run." },
    { name: "safetyReason", description: "Reason for relaxed sandbox or per-session allowlist policy." },
