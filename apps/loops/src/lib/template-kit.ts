...
function workerVerifierAgentVariables(opts: { addDirs: boolean; branchNoun: string }): LoopTemplateVariable[] {
  return [
    ...
    { name: "worktreeBranchPrefix", default: "loops", description: `Branch prefix for generated ${opts.branchNoun} worktree branches.` },
...
const ROUTING_REMEDIATION_PREFLIGHT_SCRIPT = [
  ...
  "};",
  "const todosProject = required('LOOPS_ROUTING_REMEDIATION_TODOS_PROJECT');",
  "const doctorJsonPath = optional('LOOPS_ROUTING_REMEDIATION_DOCTOR_JSON');",
  "const doctorOutputPath = required('LOOPS_ROUTING_REMEDIATION_DOCTOR_OUTPUT');",
  "const preflightOutputPath = required('LOOPS_ROUTING_REMEDIATION_PREFLIGHT_OUTPUT');",
  "const idempotencyKey = required('LOOPS_ROUTING_REMEDIATION_IDEMPOTENCY_KEY');",
  "const applyCommand = required('LOOPS_ROUTING_REMEDIATION_APPLY_COMMAND');",
  ...
  "const maxRepairs = Number(env.LOOPS_ROUTING_REMEDIATION_MAX_REPAIRS || '25');",
  "if (!Number.isInteger(maxRepairs) || maxRepairs < 0) throw new Error('maxRepairs must be a non-negative integer');",
  "const dryRun = !['0', 'false', 'no', 'off'].includes(String(env.LOOPS_ROUTING_REMEDIATION_DRY_RUN || 'true').toLowerCase());",
  "let doctor;",
  "let sourceDoctorRun;",
  "if (doctorJsonPath) {",
  ...
  "}, {});",
  "const preflight = {",
  "  schema_version: 'loops.routing_remediation_preflight.v1',",
...
export function routingRemediationPreflightCommand(opts: RoutingRemediationPreflightCommandOptions): string {
  return [
    "set -euo pipefail",
    `export LOOPS_ROUTING_REMEDIATION_TODOS_PROJECT=${shellQuote(opts.todosProjectPath)}`,
    `export LOOPS_ROUTING_REMEDIATION_DOCTOR_JSON=${shellQuote(opts.doctorJsonPath ?? "")}`,
    `export LOOPS_ROUTING_REMEDIATION_DOCTOR_OUTPUT=${shellQuote(opts.doctorOutputPath)}`,
    `export LOOPS_ROUTING_REMEDIATION_PREFLIGHT_OUTPUT=${shellQuote(opts.preflightOutputPath)}`,
    `export LOOPS_ROUTING_REMEDIATION_IDEMPOTENCY_KEY=${shellQuote(opts.idempotencyKey)}`,
    `export LOOPS_ROUTING_REMEDIATION_MAX_REPAIRS=${shellQuote(String(opts.maxRepairs))}`,
    `export LOOPS_ROUTING_REMEDIATION_DRY_RUN=${shellQuote(opts.dryRun ? "true" : "false")}`,
    `export LOOPS_ROUTING_REMEDIATION_SCOPE_ARGS=${shellQuote(JSON.stringify(routingRemediationDoctorScopeArgs(opts)))}`,
    `export LOOPS_ROUTING_REMEDIATION_APPLY_COMMAND=${shellQuote(opts.applyCommand)}`,
    "bun - <<'BUN'",
...
/** PR handoff script asset (bun heredoc body); fully env-driven via LOOPS_PR_HANDOFF_*. */
 ...
 * same `loops:pr-handoff=done` evidence the artifact path records, so the
 ...
 * is tolerated, never fails the step. Fully env-driven via LOOPS_PR_HANDOFF_*.
 */
...
export function prHandoffCommand(opts: PrHandoffCommandOptions): string {
  return [
    "set -euo pipefail",
    `export LOOPS_PR_HANDOFF_ARTIFACT=${shellQuote(opts.artifactPath)}`,
    `export LOOPS_PR_HANDOFF_TASK_ID=${shellQuote(opts.taskId)}`,
    `export LOOPS_PR_HANDOFF_TODOS_PROJECT=${shellQuote(opts.todosProjectPath)}`,
    `export LOOPS_PR_HANDOFF_WORKTREE=${shellQuote(opts.worktreeCwd)}`,
    `export LOOPS_PR_HANDOFF_WORKTREE_ROOT=${shellQuote(opts.worktreeRoot)}`,
    `export LOOPS_PR_HANDOFF_EXPECTED_BRANCH=${shellQuote(opts.expectedBranch)}`,
