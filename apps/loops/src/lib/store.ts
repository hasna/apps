...
export class Store {
   ...
   * `recoverExpiredRunLeasesDetailed` with `preserveLiveProcesses` would mark
   * abandoned; `liveDeferred` is exactly the set it would leave alone because
   * it still has a live process. Sharing the SELECT and the classifier (rather
   * than reimplementing them) is deliberate: a preview built from separate
   * logic could drift from what apply actually does, which is exactly the
