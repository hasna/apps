...
import {
  ...
  type BundleExecutionRefusal,
} from "./bundle/executor-bundle.js";
 ...
 */
const BUFFERED_OUTPUT_PROVIDERS: ReadonlySet<AgentProvider> = new Set([
  "claude",
  "codewith",
  "opencode",
  "aicopilot",
...
type ResultFields = Partial<
  Omit<ExecutorResult, "status" | "startedAt" | "durationMs">
>;
 ...
 */
function agentPromptDeliveryFailure(
  ...
  if (isRedactionPlaceholder(prompt)) {
    return (
      `agent prompt was redacted before delivery (received ${JSON.stringify(prompt)}): ` +
 ...
 */
function agentProducedNoOutput(
  spec: CommandSpec,
  fields: ResultFields,
): boolean {
  if (!spec.agentProvider) return false;
  return (
    (fields.stdout ?? "").trim().length === 0 &&
    (fields.stderr ?? "").trim().length === 0
  );
}
...
const BWRAP_LOOPBACK_SETUP_ERROR =
  /\bbwrap:\s*loopback:\s*Failed RTM_NEWADDR:\s*Operation not permitted\b/i;
...
function jsonValueContainsExecutedCommand(value: unknown): boolean {
  ...
  if (
    completedCommand &&
    !BWRAP_LOOPBACK_SETUP_ERROR.test(JSON.stringify(record))
  ) {
    return true;
  }
...
function agentCommandExecutionFailure(
  spec: CommandSpec,
  fields: ResultFields,
): string | undefined {
  if (spec.agentProvider !== "codewith" && spec.agentProvider !== "codex")
    return undefined;
  const output = `${fields.stdout ?? ""}\n${fields.stderr ?? ""}`;
  if (
    !BWRAP_LOOPBACK_SETUP_ERROR.test(output) ||
    jsonlContainsExecutedCommand(fields.stdout ?? "")
  )
    return undefined;
  return (
    "agent could not execute any command because sandbox setup failed: " +
...
function codewithJsonlHasTerminalSuccess(stdout: string): boolean {
  for (const line of stdout.split(/\r?\n/)) {
    ...
    if (
      ...
      (payload as Record<string, unknown>).type === "task_complete"
    ) {
      return true;
    }
  }
...
function codewithJsonlReconciledSuccess(
  spec: CommandSpec,
  fields: ResultFields,
): boolean {
  return (
    spec.agentProvider === "codewith" &&
    codewithJsonlHasTerminalSuccess(fields.stdout ?? "")
  );
}
...
function codewithExecFailureLooksTransient(detail: string): boolean {
  ...
  return /(?:SQLITE_BUSY|database is locked|Resource temporarily unavailable)/i.test(
    normalized,
  );
}
...
function shouldRetryCodewithExecFailure(
  ...
  const durationMs =
    new Date(attemptFinishedAt).getTime() -
    new Date(attemptStartedAt).getTime();
  if (
    !Number.isFinite(durationMs) ||
    durationMs > CODEWITH_EXEC_FAST_FAILURE_MAX_MS
  )
    return false;
  return codewithExecFailureLooksTransient(
    `${fields.stderr ?? ""}\n${fields.stdout ?? ""}`,
  );
}
...
function codewithExecRetrySummary(
  attemptFields: ResultFields,
  attemptIndex: number,
): string {
...
function stderrWithRetrySummaries(
  ...
  const tailBudget = Math.max(
    0,
    maxOutputBytes - Buffer.byteLength(prefix, "utf8"),
  );
...
function notifySpawn(pid: number | undefined, opts: ExecuteOptions): void {
  ...
  opts.onSpawnProcess?.({
    ...
    pgid: pid,
    processStartedAt:
      startedMs !== undefined ? new Date(startedMs).toISOString() : nowIso(),
  });
}
...
function codewithProfileInventoryFromJson(
  value: unknown,
): CodewithProfileInventory | undefined {
  const root =
...
function codewithProfileInventoryFromOutput(
  output: string,
): CodewithProfileInventory | undefined {
  ...
  try {
    return codewithProfileInventoryFromJson(
...
function assertCodewithAuthProfileSupported(profile: string): void {
  if (profile.includes("\0")) {
    throw new Error(
      `codewith auth profile contains unsupported NUL byte: ${codewithProfileForError(profile)}`,
    );
  }
}
...
function allowlistEnv(
  allowlist: CommandSpec["allowlist"],
...
export function defaultAgentIdleTimeoutMs(
  target: AgentTarget,
  opts: ExecuteOptions,
): number | undefined {
  if (target.timeoutMs !== undefined || target.idleTimeoutMs !== undefined)
    return undefined;
  const raw =
    opts.env?.LOOPS_AGENT_IDLE_TIMEOUT_MS ??
    process.env.LOOPS_AGENT_IDLE_TIMEOUT_MS;
...
function commandSpec(
  ...
  if (target.type === "command") {
    ...
    return {
      ...
      timeoutMs:
        commandTarget.timeoutMs === undefined
          ? DEFAULT_TIMEOUT_MS
  ...
  return {
    ...
    idleTimeoutMs:
      agentTarget.idleTimeoutMs ?? defaultAgentIdleTimeoutMs(agentTarget, opts),
    account: agentTarget.account,
    accountTool:
      agentTarget.account?.tool ?? accountToolForProvider(agentTarget.provider),
...
async function executionEnv(
  ...
  const accountEnv = spec.account
    ? await resolveAccountEnv(spec.account, spec.accountTool, {
        ...(opts.env ?? process.env),
      })
    : undefined;
  const activeConfigEnv = accountEnv
    ? undefined
    : await activeProfileConfigEnv(spec, opts);
...
function executionEnvSync(
  ...
  const accountEnv = spec.account
    ? resolveAccountEnvSync(spec.account, spec.accountTool, {
        ...(opts.env ?? process.env),
      })
    : undefined;
...
function hereDoc(
  value: string,
  destinationVariable = "__OPENLOOPS_STDIN",
): string[] {
...
function remoteBootstrapLines(
  ...
  const worktreeManaged =
    worktree?.enabled &&
    (worktree.mode === "auto" || worktree.mode === "required");
  ...
  if (spec.account) {
    if (!spec.accountTool)
      throw new Error(
        "account.tool is required when no provider tool can be inferred",
      );
    lines.push(
  ...
  for (const [key, value] of Object.entries(
    allowlistEnv(spec.allowlist, spec.sessionContract),
  )) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  return lines;
 ...
 */
function remoteWorktreeEnterLines(
  worktree: AgentWorktreeSpec,
...
function remoteScript(
  ...
  const hasAutoFallback = Boolean(
    spec.worktree?.enabled && spec.worktree.mode === "auto" && fallbackSpec,
  );
  ...
  if (hasAutoFallback) {
    if (spec.stdin !== undefined || fallbackSpec?.stdin !== undefined) {
      lines.push(
        '__OPENLOOPS_STDIN=""',
        "trap 'rm -f \"$__OPENLOOPS_STDIN\"' EXIT",
      );
    }
  } else if (spec.stdin !== undefined) {
    lines.push(
      '__OPENLOOPS_STDIN="$(mktemp -t openloops-stdin.XXXXXX)"',
      "trap 'rm -f \"$__OPENLOOPS_STDIN\"' EXIT",
    );
  ...
  const fallbackBranchLines = (invocationSpec: CommandSpec): string[] => {
    ...
    branchLines.push(
      sessionContractLine(invocationSpec),
      invocationFor(invocationSpec, stdinRedirect),
    );
    return branchLines;
  };
...
function remotePreflightScript(
  ...
  if (spec.nativeAuthProfile?.provider === "codewith") {
    const profileForError = codewithProfileForError(
      spec.nativeAuthProfile.profile,
    );
    lines.push(
...
function remotePreflightFailureMessage(
  machineId: string,
  detail: string,
): string {
  const normalized = detail.trim();
  if (
    /Host key verification failed|REMOTE HOST IDENTIFICATION HAS CHANGED|Offending .*key in .*known_hosts/i.test(
      normalized,
    )
  ) {
    return [
...
function assertCodewithProfileListed(
  ...
  if (!profiles.has(profile)) {
    throw new Error(
      `codewith auth profile not found: ${codewithProfileForError(profile)}`,
    );
  }
}
...
function codewithProfileSetFromResult(result: {
  ...
  if ((result.status ?? 1) !== 0) {
    ...
    throw new Error(
      `codewith auth profile preflight failed${detail ? `: ${detail}` : ""}`,
    );
  }
...
function assertCodewithJsonProfileListed(
  ...
  if (inventory.unusable.has(profile)) {
    throw new Error(
      `codewith auth profile preflight failed: profile is unusable: ${codewithProfileForError(profile)}`,
    );
  }
...
function preflightNativeAuthProfileSync(
  spec: CommandSpec,
  env: NodeJS.ProcessEnv,
): void {
  ...
  if (
    assertCodewithJsonProfileListed(spec.nativeAuthProfile.profile, jsonResult)
  )
    return;
  assertCodewithProfileListed(
    spec.nativeAuthProfile.profile,
    runProfileList(["profile", "list"]),
  );
}

async function preflightNativeAuthProfile(
  spec: CommandSpec,
  env: NodeJS.ProcessEnv,
): Promise<void> {
  if (spec.nativeAuthProfile?.provider !== "codewith") return;
  const jsonResult = await spawnCapture(
    spec.command,
    ["profile", "list", "--json"],
  ...
  if (
    ...
    return;
  const tableResult = await spawnCapture(spec.command, ["profile", "list"], {
    env,
    timeoutMs: 15_000,
  });
 ...
 */
export function isStaleWorktreeRegistration(
  detail: string | undefined,
): boolean {
  return (
    typeof detail === "string" &&
    /missing but already registered worktree/i.test(detail)
  );
}
...
async function ensureLocalWorktree(
  ...
  if (!repoRoot || !path || !branch) {
    return {
      error:
        "worktree preparation requires repoRoot, path, and branch metadata",
    };
  }
  const git = (
    args: string[],
  ): Promise<Awaited<ReturnType<typeof spawnCapture>>> =>
  ...
  if (stats) {
    ...
    if (!resolvedDirEquals(top.stdout.trim(), path)) {
      return {
        error: `existing worktree top-level mismatch for ${path}: ${top.stdout.trim()}`,
      };
    }
    ...
    if (!expectedCommon || expectedCommon !== actualCommon) {
      return {
        error: `existing worktree ${path} belongs to a different git common dir`,
      };
    }
    ...
    if (current.error || (current.status ?? 1) !== 0) {
      return {
        error: `existing worktree ${path} branch could not be inspected${spawnDetail(current) ? `: ${spawnDetail(current)}` : ""}`,
      };
    }
    if (actualBranch !== branch) {
      ...
      if (
        recovered.error ||
        (recovered.status ?? 1) !== 0 ||
        recovered.stdout.trim() !== branch
      ) {
  ...
  try {
    ...
    return {
      error: `could not create worktree parent directory: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
  const hasBranch = await git([
    "-C",
  ...
  if (
    (add.error || (add.status ?? 1) !== 0) &&
    isStaleWorktreeRegistration(spawnDetail(add))
  ) {
  ...
  if (add.error || (add.status ?? 1) !== 0) {
    ...
    return {
      error: `git worktree add failed for ${path}${detail ? `: ${detail}` : ""}`,
    };
  }
  return { cwd: worktree.cwd };
}
...
async function enterWorktree(
    ...
    if (worktree.mode === "required") {
      return {
        failure: failureResult(
          startedAt,
          `worktree preparation failed (mode=required): ${prepared.error}`,
        ),
      };
    }
    opts.log?.(
      `worktree preparation failed (mode=${worktree.mode}); falling back to ${worktree.originalCwd}: ${prepared.error}`,
    );
  ...
  opts.log?.(
    `entered worktree ${worktree.path ?? spec.cwd}${worktree.branch ? ` branch ${worktree.branch}` : ""}`,
  );
  return undefined;
}
...
function worktreeFallbackSpec(
  ...
  return {
    ...
    preflightAnyOf: invocation.preflightAnyOf,
    stdin: invocation.stdin,
    sessionContract: spec.sessionContract
      ? { ...spec.sessionContract, cwd: fallbackCwd }
      : undefined,
    worktree: spec.worktree
      ? {
          ...
          enabled: false,
          cwd: fallbackCwd,
          reason: "auto worktree preparation failed",
        }
      : undefined,
  };
}
...
function preflightRemoteSpec(
  ...
  const plan = (opts.machineCommandResolver ?? resolveMachineCommand)(
    machine.id,
    "bash -s",
  );
  ...
  if (result.error)
    throw new Error(
      `remote preflight failed on ${machine.id}: ${result.error.message}`,
    );
  if ((result.status ?? 1) !== 0) {
    const detail = (
      result.stderr ||
...
async function executeRemoteSpec(
  ...
  try {
    plan = (opts.machineCommandResolver ?? resolveMachineCommand)(
      machine.id,
      "bash -s",
    );
  ...
  for (let attemptIndex = 0; ; attemptIndex += 1) {
    ...
    const attemptFields: ResultFields = {
      ...fields,
      stderr: attemptStderr.value(),
    };
    if (timedOut || idleTimedOut) {
      return timeoutResult(
        startedAt,
        idleTimedOut
          ? `idle timed out after ${spec.idleTimeoutMs}ms without stdout/stderr`
          : `timed out after ${spec.timeoutMs}ms`,
        fields,
      );
    }
    const commandExecutionFailure = agentCommandExecutionFailure(spec, fields);
    if (commandExecutionFailure)
      return failureResult(startedAt, commandExecutionFailure, fields);
    if (
      !error &&
      exitCode !== 0 &&
      codewithJsonlReconciledSuccess(spec, fields)
    ) {
      return successResult(startedAt, fields);
    }
    if (error || exitCode !== 0) {
      if (
        shouldRetryCodewithExecFailure(
          spec,
          attemptFields,
          error,
          attemptStartedAt,
        ...
        const retrySummary = codewithExecRetrySummary(
          attemptFields,
          attemptIndex,
        );
        opts.log?.(codewithExecRetryMessage(attemptIndex));
        retrySummaries.push(retrySummary);
        fields.stderr = stderrWithRetrySummaries(
          stderr,
          retrySummaries,
          maxOutputBytes,
        );
        if (
          await waitForRetryDelay(
            CODEWITH_EXEC_RETRY_DELAYS_MS[attemptIndex]!,
            opts.signal,
          )
        )
          continue;
      ...
      return failureResult(
        startedAt,
        error ??
          `remote process on ${machine.id} exited with code ${exitCode ?? "unknown"}`,
        fields,
      );
    }
    if (agentProducedNoOutput(spec, attemptFields))
      return failureResult(startedAt, AGENT_NO_OUTPUT_ERROR, fields);
    return successResult(startedAt, fields);
  }
}
...
export function preflightTarget(
  ...
  if (
    spec.preflightAnyOf?.length &&
    !spec.preflightAnyOf.some((command) => executableExists(command, env))
  ) {
    throw new Error(
      `none of required executables found: ${spec.preflightAnyOf.join(", ")}`,
    );
  }
...
export async function executeTarget(
  ...
  for (let attemptIndex = 0; ; attemptIndex += 1) {
    ...
    if (error || exitCode !== 0) {
      ...
      return failureResult(
        startedAt,
        error ?? `process exited with code ${exitCode ?? "unknown"}`,
        fields,
      );
    }
    if (agentProducedNoOutput(spec, attemptFields))
 ...
 */
export type BundleExecutionDecision =
  | {
      target: ExecutableTarget;
      opts: ExecuteOptions;
...
/**
 * The run result a bundle refusal produces.
 *
 * One place, because two callers need it: `executeLoop` for a plain command
 * loop and `executeLoopTarget` for the goal and workflow shapes, which never
 * reach `executeLoop` at all. `loop_runs.error` is what an operator greps, so
 * the coded prefix has to be identical on both paths.
 */
export function bundleRefusalResult(
  refusal: BundleExecutionRefusal,
): ExecutorResult {
  return failureResult(nowIso(), `${refusal.error}: ${refusal.message}`);
}

export function applyBundleExecution(
  loop: Loop,
  opts: ExecuteOptions,
): BundleExecutionDecision {
...
export async function executeLoop(
  ...
  opts: ExecuteOptions = {},
  /**
   * The bundle decision, when the caller has already taken it.
   *
   * `executeLoopTarget` gates BEFORE it branches to a goal or a workflow, so
   * for a plain command loop it hands the verdict down rather than making the
   * tree be read, hashed and secret-scanned a second time in the same run.
   * Absent, this verifies for itself — a direct `executeLoop` caller is gated
   * exactly as before.
   */
  decision?: BundleExecutionDecision,
): Promise<ExecutorResult> {
  if (loop.target.type === "workflow") {
    throw new Error(
      "workflow loop targets must be executed with executeLoopTarget",
    );
  }
  const bundled = decision ?? applyBundleExecution(loop, opts);
  if ("refusal" in bundled) {
    ...
    return bundleRefusalResult(bundled.refusal);
  }
  ...
  if (loop.target.preflight?.beforeRun) {
    ...
    try {
      ...
      return stampBundle(
        failureResult(
          startedAt,
          `runtime preflight failed: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
    }
  }
