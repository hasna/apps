...
export function loopsResolverEnv<T extends Env>(env: T): T {
  ...
  const blanks = [...keys.apiUrlKeys, ...keys.apiKeyKeys].filter(
    (key) => key in env && (env[key] ?? "").trim() === "",
  );
  if (blanks.length === 0) return env;
  const next = { ...env } as T;
  for (const key of blanks) delete next[key];
  if (isAmbientLoopsEnv(env)) {
    Object.defineProperty(next, CONTRACTS_AMBIENT_ENVIRONMENT, {
      value: true,
      enumerable: false,
      configurable: false,
      writable: false,
...
export interface LoopsResolverInputs<T extends Env> {
  /** The environment with declared-but-blank URL/key aliases removed; deliberate selectors remain terminal. */
  env: T;
 ...
 *
 * The hosted decision comes from `@hasna/contracts` 1.1.0's shared resolver,
...
export function resolveCloudStorage(name: string, env: Env = process.env, options: CloudStorageOptions = {}): CloudStorageResolution {
  ...
  const hasExplicitCredentialIntent = credentials.apiKey !== undefined || credentials.profile !== undefined;
  if (!hasExplicitCredentialIntent && selectsLoopsLocalStore(env)) {
