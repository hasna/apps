...
describe("executor-native worktree specs", () => {
  ...
  test("worktree specs preserve ordinary repo subdirectories whose names begin with two dots", () => {
    const subdir = join(repoPath, "..sdk");
    mkdirSync(subdir, { recursive: true });
    const workflow = renderTodosTaskWorkerVerifierWorkflow({
      taskId: "task-dotdot-prefix",
      projectPath: subdir,
      worktreeRoot,
    ...
    expect(worker.worktree?.repoRoot).toBe(resolvedRepoRoot);
    expect(worker.worktree?.originalCwd).toBe(subdir);
    expect(worker.cwd).toBe(join(worker.worktree!.path!, "..sdk"));
  });
