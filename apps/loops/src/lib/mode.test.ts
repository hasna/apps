...
describe("deployment mode contract", () => {
  ...
  test("redacts credential-bearing control-plane URLs in status output", () => {
    const status = buildDeploymentStatus({
      env: {
        LOOPS_API_URL: "https://user:fake-password@loops.example.test/api?token=fake-token&ok=true#frag",
        LOOPS_API_TOKEN: "present-but-not-returned",
      },
    });
  ...
  test("keeps cloud contract-only until cloud URL and cloud token are configured", () => {
    ...
    expect(urlOnly.warnings.join(" ")).toContain("LOOPS_CLOUD_TOKEN");

    const genericApiUrl = buildDeploymentStatus({
      env: {
        LOOPS_MODE: "cloud",
        LOOPS_API_URL: "https://self-hosted.example.test",
        LOOPS_API_TOKEN: "self-hosted-token",
      },
    });
    expect(genericApiUrl.controlPlane.configured).toBe(false);
    expect(genericApiUrl.controlPlane.apiUrl).toBeUndefined();
    expect(genericApiUrl.warnings.join(" ")).toContain("cloud mode uses LOOPS_CLOUD_API_URL");
  });
