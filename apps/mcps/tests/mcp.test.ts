...
async function createClientServer() {
  const server = createMcpServer({ name: "mcps-test", version: "0.0.1", storageTools: false });
...
async function createClientServerWithStorage() {
...
describe("MCP server tools", () => {
  ...
  it("lists storage sync tools by default", async () => {
    const { client } = await createClientServerWithStorage();
    const result = await client.listTools();
    const toolNames = result.tools.map((t) => t.name);
    ...
    expect(toolNames).toContain("storage_sync");
    expect(toolNames).not.toContain(["cloud", "status"].join("_"));
    expect(toolNames).not.toContain(["cloud", "push"].join("_"));
    expect(toolNames).not.toContain(["cloud", "pull"].join("_"));
    expect(toolNames).not.toContain(["cloud", "sync"].join("_"));
