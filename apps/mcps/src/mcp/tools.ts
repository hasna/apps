  ...
  const definitions: InternalMcpToolDefinition[] = [
    ...
    },
    {
      ...
      paramsSchema: {
        ...
        verbose: z.boolean().optional().describe("Return full per-package health reports"),
      },
      run: async ({ machine_ids, packages, refresh_catalog, timeout_ms, verbose }) => {
        ...
        return jsonContent({
          items: reports.map(compactFleetHealthReport),
          total: reports.length,
          hint: "Use fleet_health({verbose:true}) for per-package health details.",
        });
      },
    },
    {
      ...
      paramsSchema: {
        ...
        timeout_ms: z.number().int().min(1000).optional().describe("Remote timeout in milliseconds"),
        verbose: z.boolean().optional().describe("Return full per-package install reports"),
      },
      run: async ({ machine_ids, packages, mode, installer, refresh_catalog, timeout_ms, verbose }) => {
        ...
        if (verbose === true) return jsonContent(reports);
        return jsonContent({
          items: reports.map(compactFleetInstallReport),
          total: reports.length,
          hint: "Use fleet_install({verbose:true}) for per-package install details and command output.",
        });
      },
    },
    {
