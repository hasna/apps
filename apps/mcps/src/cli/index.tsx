...
function renderFleetHealth(reports: FleetHealthReport[], opts: { verbose?: boolean } = {}): void {
  if (reports.length === 0) {
  ...
  if (!opts.verbose) console.log(chalk.dim("\nUse --verbose for per-package health details or -j for full JSON."));
}

function renderFleetInstall(reports: FleetInstallReport[], opts: { verbose?: boolean } = {}): void {
  if (reports.length === 0) {
    console.log(chalk.dim("No machines selected."));
    return;
  }
  ...
  if (!opts.verbose) console.log(chalk.dim("\nUse --verbose for per-package install results or -j for full JSON."));
}
  ...
  .action(async (query: string | undefined, opts) => {
    try {
      ...
      for (let i = 0; i < page.items.length; i++) {
        ...
        const idx = opts.install ? chalk.dim(`${page.offset + i + 1}. `) : "  ";
...
program
  ...
  .description("Sync local MCP registry tables with configured remote Postgres storage")
  .option("--tables <tables>", "comma-separated table list")
  .option("--json", "Output as JSON")
  .option("--verbose", "Show full sync metadata and table results")
  .action(async (action = "status", opts: { tables?: string; json?: boolean; verbose?: boolean }) => {
    const { getStorageStatus, storagePull, storagePush, storageSync } = await import("../lib/storage-sync.js");
    const tables = parseTables(opts.tables);
    const printSyncSummary = (label: string, results: Awaited<ReturnType<typeof storagePush>>) => {
      const rowsRead = results.reduce((sum, result) => sum + result.rowsRead, 0);
      const rowsWritten = results.reduce((sum, result) => sum + result.rowsWritten, 0);
      ...
      console.log(`  Errors:       ${errors === 0 ? chalk.green("0") : chalk.red(String(errors))}`);
      for (const result of results.filter((item) => item.errors.length > 0)) {
        const compact = compactSyncResult(result);
        console.log(`  ${chalk.red(compact.table)}: ${compact.firstError}`);
      }
    };
    try {
      switch (action) {
        case "status": {
          ...
          console.log(`  Sync meta:  ${status.sync.length} record(s)`);
          console.log(chalk.dim("Use --verbose or --json for table names and sync timestamps."));
          break;
        }
        case "push": {
          const results = await storagePush({ tables });
          if (opts.json || opts.verbose) printJson(results);
          else printSyncSummary("Storage Push", results);
          break;
        }
        case "pull": {
          const results = await storagePull({ tables });
          if (opts.json || opts.verbose) printJson(results);
          else printSyncSummary("Storage Pull", results);
          break;
        }
        case "sync": {
          ...
          if (opts.json || opts.verbose) {
            ...
            printSyncSummary("Storage Sync Push", results.push);
            printSyncSummary("Storage Sync Pull", results.pull);
          }
          break;
        }
        default:
          console.error(chalk.red(`Unknown storage action: ${action}. Valid actions: status, push, pull, sync`));
          closeDb();
          process.exit(1);
      }
    } catch (err) {
