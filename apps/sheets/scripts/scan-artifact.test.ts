...
function fixture(check: (root: string) => void): void {
  const root = mkdtempSync(join(tmpdir(), "sheets-artifact-test-"));
  try { check(root); }
...
describe("npm release artifact gate", () => {
  ...
  test("real npm materializes the archive under inherited dry-run without prepack or prepare recursion", () => fixture((root) => {
    ...
    writeFileSync(join(root, "package.json"), JSON.stringify({
      name: "sheets-pack-fixture", version: "1.0.0", files: ["index.js"],
    ...
    writeFileSync(join(root, "package.json"), JSON.stringify({
      name: "sheets-pack-fixture", version: "1.0.0", scripts: { prepare: "node -e 'process.exit(92)'" },
    }));
