...
// API-only: there is no local SQLite. These guards prove that send routes
...
describe("MCP api guards", () => {
  it("routes send_email through the api API without touching a local DB", async () => {
    const result = await callTool("send_email", {
      from: "ops@example.com",
      to: ["user@example.com"],
      subject: "Api MCP send",
      text: "hello",
      idempotency_key: "mcp-api-send",
    });
  ...
  it("fails api-client-only tools without touching a local DB", async () => {
    // These read/write server-owned state; the api client refuses them.
    //
    ...
    for (const [name, args] of cases) {
      ...
      expect(result.isError).toBe(true);
      expect(resultText(result)).toContain("not available in the api client");
    }
  });
  ...
  it("tells the truth about provisioning tools that no mode implements", async () => {
    ...
    // the API server exposes no /v1 provisioning route. Claiming these
    // "run on the API server" sent operators looking for a service that
    ...
    for (const [name, args, alternative] of cases) {
      ...
      expect(text).not.toContain("runs on the api server");
      expect(text).not.toContain("not available in the api client");
    }
  });
