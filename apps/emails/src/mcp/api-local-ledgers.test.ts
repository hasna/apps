...
// API-only: no local SQLite. list_groups, the warming tools and the
...
describe("MCP api repository-backed tools", () => {
  it("lists groups through the api API with exact member counts", async () => {
  ...
  it("runs the warming tools through the api API instead of refusing", async () => {
  ...
  it("runs the alias, group-member, and sequence tools through /v1 instead of refusing", async () => {
    ...
    // src/mcp/api-unguarded-tools.test.ts asserts the resulting server rows;
