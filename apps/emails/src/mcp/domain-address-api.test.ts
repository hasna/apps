...
const server = Bun.serve({
  ...
  fetch(req) {
    ...
    // The provider resource mirrors the real /v1 api service: metadata
...
async function startApi(): Promise<string> {
  ...
  if (!port) throw new Error(`api domain/address API fixture did not report a port: ${buf}`);
...
beforeEach(async () => {
  ...
  resetEnv();
  tempHome = mkdtempSync(join(tmpdir(), "emails-mcp-domain-address-api-"));
...
describe("MCP domain/address api API-only guards", () => {
  ...
  it("answers the credential-free no-provider DNS path in api mode", async () => {
  ...
  it("refuses the provider-scoped DNS path in api mode with the recorded credential reason", async () => {
  ...
  it("no longer refuses the repository-backed domain/address tools — they reach the wire", async () => {
    ...
    // ships ("is API-backed in api mode and requires ..."). So each tool must
    ...
    // there; src/mcp/api-unguarded-tools.test.ts drives them to completion
    // against a full stub.
    const REFUSAL_WORDINGS = ["is disabled for the API client", "is API-backed in api mode", "not available in the api client"];
    ...
    // with others that mutate the api env, and under that pollution the
    // fixture's own key stops matching and every call fails 401 — which is still
    // proof the tool got to the wire. `api-unguarded-tools.test.ts` drives all
    ...
    const WIRE = /\/v1\/|\/(domains|addresses|aliases)\b|Api (GET|POST|PATCH|PUT|DELETE)|HTTP \d{3}|\b(?:domains|addresses|aliases)\.(?:list|get|create|update|remove):/;
    ...
    for (const [name, args] of [
      ...
      // src/mcp/api-unguarded-tools.test.ts, which drives `remove_alias` to completion
      ...
      if (result.isError) {
        if (name === "remove_alias") {
          ...
          // suite shares a process with others that mutate the api environment: under
