...
// API-only: no local SQLite. `prepare_inbox` genuinely has no route in this
...
describe("MCP api local-state guards", () => {
  ...
  it("no longer refuses list_replies — it serves replies from /v1/messages", async () => {
    ...
    // tracking runs on the api server". It does not: src/db/inbound.ts routes
    ...
    expect(resultText(result)).not.toContain("not available in the api client");
