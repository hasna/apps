...
// needed when the API client is configured: a `/v1` route in src/server/api/resources.ts, a
...
// src/mcp/domain-address-api.test.ts pins the guards themselves.
...
function ok<T>(result: { content: Array<{ text: string }>; isError?: boolean }): T {
  expect(text(result)).not.toContain("API-only mode");
...
describe("MCP list_replies when the API client is configured (twin of `emails replies <id>`)", () => {
  ...
  // runs on the api server" and that no API-backed implementation existed.
  ...
  it("returns the reply the SERVER holds instead of refusing", async () => {
    ...
    expect(text(result)).not.toContain("not available in the api client");
