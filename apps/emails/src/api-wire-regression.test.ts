...
import { ApiError, EmailsApiClient } from "./api.js";
...
function schemaSample(schema: JsonSchema): unknown {
  ...
  if (typeof ref === "string") {
    const component = API_RESPONSE_COMPONENTS[
...
function responseSample(method: string, path: string, status: number): Record<string, unknown> {
  const contract = API_RESPONSE_CONTRACTS.find((candidate) => {
...
// on-disk config, which resolves to api and fails unrelated suites.
...
const API_ENV_KEYS = [
  ...
  "EMAILS_API_HTTP_MAX_RESPONSE_BYTES",
] as const;

let INHERITED_API_ENV: Record<string, string | undefined>;

function clearApiEnv(): void {
  for (const key of API_ENV_KEYS) {
    const inherited = INHERITED_API_ENV[key];
...
async function expectSafeRejection(operation: Promise<unknown>): Promise<void> {
  ...
  expect(String(thrown)).toMatch(/api.*invalid|invalid.*api/i);
...
beforeEach(() => {
  INHERITED_API_ENV = Object.fromEntries(
...
describe("shared-process environment hygiene", () => {
  ...
  test("scrubs or fixes every active runtime environment input before the shared process starts", () => {
    ...
    expect(covered).toContain("EMAILS_API_HTTP_CONNECT_TIMEOUT");
    expect(covered).toContain("EMAILS_API_HTTP_TIMEOUT");
    expect(covered).toContain("EMAILS_API_HTTP_MAX_RESPONSE_BYTES");
  });
});

describe("api successful-response wire contract", () => {
  test("generic stores reject malformed JSON instead of synthesizing an empty list", () => {
    applyApiEnv();
    expect(() => apiStoreFor("providers").list()).toThrow(/invalid.*JSON/i);
  });

  test("generic stores reject wrong list envelopes and rows missing their identity", () => {
    applyApiEnv();
    expect(() => apiStoreFor("domains").list()).toThrow(/invalid successful response/i);
    expect(() => apiStoreFor("contacts").list()).toThrow(/invalid successful response/i);
    expect(() => apiStoreFor("addresses").list()).toThrow(/invalid.*id/i);
  });

  test("inbox list and search reject malformed, wrong-shaped, and incomplete pages", async () => {
    const dataSource = new ApiMailDataSource({
      baseUrl: `${baseUrl}/v1`,
  ...
  test("the public api SDK rejects malformed successful JSON without leaking it", async () => {
  ...
  test("TUI and identity safe wrappers do not disguise invalid 2xx contracts as empty state", () => {
    applyApiEnv();
    expect(() => activeProviderId()).toThrow(ApiWireResponseError);
    expect(() => fetchIdentitySafe()).toThrow(ApiWireResponseError);
  });
  ...
  test("every generic resource operation validates list rows, records, and delete receipts", () => {
    const generic = API_RESPONSE_CONTRACTS.filter((contract) =>
      contract.status >= 200
      && contract.status < 300
    ...
    for (const contract of generic) {
      ...
      const invalid = contract.operationId.startsWith("listResource")
        ...
        : contract.operationId.startsWith("deleteResource")
          ...
          : {};
      expect(
        () => validateApiSdkSuccessResponse(contract.method, path, contract.status, invalid),
  ...
  test("mailbox mutation and send receipts fail closed while a real in-progress send remains valid", () => {
    ...
    expect(() => validateApiSdkSuccessResponse(
      "PATCH",
  ...
  test("send-intent list, reconcile, and cancel enforce their discriminated response contracts", () => {
    for (const [method, path, body] of [
      ...
      expect(() => validateApiSdkSuccessResponse(method, path, 200, body)).toThrow(
  ...
  test("auth, identity, bootstrap, key, login, and switch responses validate nested required fields", () => {
    ...
    for (const [method, path, status, body] of cases) {
      expect(
        () => validateApiSdkSuccessResponse(method, path, status, body),
  ...
  test("public auth 403s retain only schema-approved human text after validation", () => {
    expect(projectApiMailErrorBody(
      "POST",
      "/v1/auth/signup",
      403,
      {
  ...
  test("canonical and native-client identity paths use exactly the same response contracts", () => {
    for (const [method, canonical, alias] of [
      ...
      expect(() => validateApiSdkSuccessResponse(method, canonical, 200, valid)).not.toThrow();
      expect(() => validateApiSdkSuccessResponse(method, alias, 200, valid)).not.toThrow();
      expect(() => validateApiSdkSuccessResponse(method, alias, 200, {})).toThrow(
        /invalid successful response/i,
      );
    }
  });
  ...
  test("the runtime explicitly supports every generated response format and no others", () => {
    ...
    inspect(API_RESPONSE_CONTRACTS);
    inspect(API_RESPONSE_COMPONENTS);
    expect([...formats].sort()).toEqual([...SUPPORTED_API_RESPONSE_FORMATS].sort());
  });
  ...
  test("registry identity keys reject null and every served resource publishes PUT", () => {
    for (const [path, key] of [
      ...
      body[key] = null;
      expect(() => validateApiSdkSuccessResponse("POST", path, 201, body))
        .toThrow(new RegExp(`body\\.${key} must be a string`, "i"));
    }

    const genericPuts = API_RESPONSE_CONTRACTS.filter((contract) =>
      contract.method === "PUT"
      && contract.status === 200
      && contract.operationId.startsWith("replaceResource"));
    expect(genericPuts).toHaveLength(
      API_RESPONSE_CONTRACTS.filter((contract) =>
  ...
  test("both fetch clients enforce response-size limits before contract projection", async () => {
    ...
    await expect(sdk.getHealth()).rejects.toBeInstanceOf(ApiResponseSizeError);

    const mailbox = new ApiMailDataSource({
      baseUrl: `${baseUrl}/v1`,
      apiKey: "loopback-test-key",
      maxResponseBytes: 128,
    ...
    await expect(mailbox.listMailbox("inbox")).rejects.toBeInstanceOf(ApiResponseSizeError);
  });
  ...
  test("curl, fetch, and SDK errors never retain or print response bodies", async () => {
    applyApiEnv();
    expect(() => apiStoreFor("groups").list()).toThrow(ApiHttpError);
    try {
      apiStoreFor("groups").list();
    } catch (error) {
  ...
  test("the synchronous curl bridge enforces a response-size bound", () => {
    applyApiEnv();
    process.env["EMAILS_API_HTTP_MAX_RESPONSE_BYTES"] = "128";
    expect(() => apiStoreFor("templates").list()).toThrow(ApiResponseSizeError);
  });
});
