...
/** The canonical opt-in, the ONLY thing that selects the local store. */
...
describe("configured store resolution — the quadrants and the boot-error rows", () => {
  ...
  it("REFUSES a database path on its own — a path never selects the local store", () => {
  ...
  it("requires the exact local opt-in value 1 and refuses typos without quoting them", () => {
    for (const setting of LOCAL_OPT_IN_SETTINGS) {
      for (const value of ["0", "false", "yes", "on", "01"]) {
        let thrown: unknown;
        try {
          planEmailStore(bare({ [setting]: value }));
        } catch (error) {
          thrown = error;
        }
        expect(thrown).toBeInstanceOf(StoreConfigurationError);
        expect((thrown as StoreConfigurationError).settings).toEqual([setting]);
        expect(String(thrown)).toContain("must be exactly 1");
        expect(String(thrown)).not.toContain(`'${value}'`);
      }
    }
  });

  it("never lets blank deliberate hosted selectors or a principal pointer fall into SQLite", () => {
    for (const key of ["HASNA_EMAILS_API_KEY_OVERRIDE", "HASNA_EMAILS_API_KEY_REF", "HASNA_PROFILE"]) {
      expect(() => planEmailStore(local({ [key]: "" })), key).toThrow(StoreConfigurationError);
    }
    expect(() => planEmailStore(local({ EMAILS_CLIENT_ENV_SECRET: "vault/principals" })))
...
describe("configured store resolution — configurations it will not guess at", () => {
  ...
  it("treats blank local paths as absent but blank client declarations as terminal", () => {
    for (const blank of ["", "   ", "\t\n"]) {
      // A declared client URL never falls into the local arm, even when blank.
      expect(() => planEmailStore(local({
        [DATABASE_PATH_SETTINGS[1]]: "/tmp/x.db",
        [API_BASE_URL_SETTING]: blank,
      }))).toThrow(StoreConfigurationError);
      // A blank opt-in is not an opt-in: the same path alone is the refused row.
      expect(() => planEmailStore(bare({ [DATABASE_PATH_SETTINGS[1]]: "/tmp/x.db", [LOCAL]: blank })))
        .toThrow(StoreConfigurationError);
      // A blank database path names no local store and reaches the all-unset refusal.
      expect(() => planEmailStore(bare({ [DATABASE_PATH_SETTINGS[1]]: blank })))
        .toThrow(StoreConfigurationError);
      // A declared blank canonical credential is a terminal configuration error.
      expect(() => planEmailStore(bare({ [API_CREDENTIAL_SETTINGS[2]]: blank })))
        .toThrow(StoreConfigurationError);
    }
  });
...
describe("the store the resolution actually hands back", () => {
  ...
  it("re-resolves the configured store credential per request and refuses authority drift", async () => {
    let acceptedCredential = "first-key";
    ...
    const server = Bun.serve({
      ...
      fetch(request) {
        ...
        if (authorization !== `Bearer ${acceptedCredential}`) {
          return Response.json({ error: "unauthorized" }, { status: 401 });
        }
        return Response.json({ domains: [] });
      },
    });
    const drift = Bun.serve({
      port: 0,
      hostname: "127.0.0.1",
      fetch() {
        return Response.json({ domains: [] });
    ...
    try {
      only({
        [API_BASE_URL_SETTING]: `http://127.0.0.1:${server.port}`,
        [EMAILS_API_KEY_ENV]: acceptedCredential,
      });
      const store = createConfiguredEmailStore();
      await expect(store.domains.listDomains({ limit: 1 })).resolves.toEqual({ ok: true, value: [] });

      acceptedCredential = "second-key";
      process.env[EMAILS_API_KEY_ENV] = acceptedCredential;
      await expect(store.domains.listDomains({ limit: 1 })).rejects.toThrow("credential binding changed");
      expect(authorizations).toEqual(["Bearer first-key"]);
      expect(requests).toBe(1);

      process.env[API_BASE_URL_SETTING] = `http://127.0.0.1:${drift.port}`;
      await expect(store.domains.listDomains({ limit: 1 })).rejects.toThrow("authority changed");
      expect(requests).toBe(1);
    } finally {
      server.stop(true);
      drift.stop(true);
