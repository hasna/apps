// @generated from src/server/api/openapi.ts by scripts/generate-api-sdk.ts — DO NOT EDIT.
// Regenerate: bun run scripts/generate-api-sdk.ts
import {
  DEFAULT_API_HTTP_TIMEOUT_MS,
...
export interface EmailsApiClientOptions {
...
export class EmailsApiClient {
  ...
  constructor(options: EmailsApiClientOptions) {
    if (!options.baseUrl) throw new Error("EmailsApiClient requires a baseUrl.");
    ...
    if (parsedBaseUrl.protocol !== "https:" && !(parsedBaseUrl.protocol === "http:" && loopback)) {
      throw new Error("EmailsApiClient requires HTTPS except for loopback development URLs.");
    }
    ...
    this.timeoutMs = apiTransportLimit(
      options.timeoutMs,
      DEFAULT_API_HTTP_TIMEOUT_MS,
      "timeoutMs",
    );
    this.maxResponseBytes = apiTransportLimit(
  ...
  private async request<T>(method: string, path: string, opts: { body?: unknown; query?: Record<string, unknown>; init?: RequestInit }): Promise<T> {
    ...
    try {
      ...
      if (timeoutSignal.aborted || (error as Error)?.name === "TimeoutError") {
        throw new Error(`Api ${method} ${path} timed out after ${this.timeoutMs}ms`);
      }
      if (opts.init?.signal?.aborted || (error as Error)?.name === "AbortError") {
        throw new Error(`Api ${method} ${path} request was aborted`);
      }
      throw new Error(`Cannot reach the Emails api service for ${method} ${path}`);
    }
    const text = await readApiResponseText(response, { method, path }, this.maxResponseBytes);
    const data = response.ok
      ? parseApiSuccessJson(text, { status: response.status, method, path })
      : projectApiErrorBody(
        method,
        path,
        response.status,
        parseApiErrorJson(text, { status: response.status, method, path }),
      );
    if (response.ok) validateApiSdkSuccessResponse(method, path, response.status, data);
    if (!response.ok) {
    ...
    /** Report optional api sign-in providers */
