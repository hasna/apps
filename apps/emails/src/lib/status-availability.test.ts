...
describe("status availability primitives", () => {
  it("formats a reason as <code>:<detail> — <prose> and exposes the code", () => {
    const availability = statusUnavailable("server_route_absent", "/v1/senders", "api", "why");
...
function nullPayload(): EmailSystemStatus {
  const unreachable = statusUnavailable("source_unreachable", "GET /v1/providers -> HTTP 503", "api:/v1/providers");
  const notModelled = statusUnavailable("not_modelled_over_v1", "domain_dns_evidence", "api:/v1/domains");
  const absent = statusUnavailable("server_route_absent", "/v1/senders", "api:/v1/addresses");
  const capped = statusAvailable("api:/v1/addresses", "client_enumeration", false);
  return {
    ...
    database: { availability: statusUnavailable("not_applicable", "local_database_absent_in_api", "api"), data_dir: null },
    providers: { availability: unreachable, total: null, active: null, by_type: null },
    domains: {
      availability: statusAvailable("api:/v1/domains", "client_enumeration"),
    ...
    inbox: {
      ...
      inbound_buckets: { availability: statusUnavailable("not_applicable", "server_owned_ingestion", "api"), items: null, total: null },
      realtime: { availability: statusUnavailable("not_modelled_over_v1", "realtime_queue_state", "api"), queue_configured: null, queue_url: null, last_poll_at: null, last_error: null },
    },
    mailboxes: { counts: { inbox: 0, unread: 0, sent: 0, archived: 0, spam: 0, trash: 0, starred: 0 }, folders: [] },
    sources: {
      availability: statusAvailable("api_mail_view", "client_enumeration"),
    ...
    provisioning: { availability: statusAvailable("api", "client_enumeration"), domains_pending: 0, domains_failed: 0, addresses_pending: 0, addresses_failed: 0 },
