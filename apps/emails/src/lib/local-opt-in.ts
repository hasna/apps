...
// this process may keep mail there. A path alone used to be the opt-in,
...
// behind. The local decision stays package-specific because Emails also has its own
// session and identity principals, which count as hosted intent.
...
const CLIENT_ENV_PRINCIPAL_POINTER = "EMAILS_CLIENT_ENV_SECRET";
const TRANSPORT_ENV_KEYS = clientTransportEnvKeys(APP);
const PRESENCE_BASED_AUTHORITY_ENV_KEYS = Object.freeze([
  ...TRANSPORT_ENV_KEYS.apiUrlKeys,
  ...TRANSPORT_ENV_KEYS.apiKeyKeys,
...
function enabled(env: EmailsLocalOptInEnv, key: string): boolean {
  return (env[key] ?? "").trim() === "1";
}

/** Nonblank local selectors whose value is not the exact opt-in `1`. */
export function invalidEmailsLocalOptInSettings(env: EmailsLocalOptInEnv = process.env): string[] {
  return EMAILS_LOCAL_OPT_IN_ENV_KEYS.filter((key) => configured(env, key) && !enabled(env, key));
}
...
export function isEmailsLocalOptIn(env: EmailsLocalOptInEnv = process.env): boolean {
  return EMAILS_LOCAL_OPT_IN_ENV_KEYS.some((key) => enabled(env, key));
}

/** The opt-in key set to the exact value `1`, canonical first, or null. Never a value. */
export function emailsLocalOptInSetting(env: EmailsLocalOptInEnv = process.env): string | null {
  return EMAILS_LOCAL_OPT_IN_ENV_KEYS.find((key) => enabled(env, key)) ?? null;
}
...
export function emailsAuthorityEnvKeys(): string[] {
  return [
    ...PRESENCE_BASED_AUTHORITY_ENV_KEYS,
    CLIENT_ENV_PRINCIPAL_POINTER,
...
export function configuredEmailsAuthorityEnvKeys(env: EmailsLocalOptInEnv = process.env): string[] {
  return emailsAuthorityEnvKeys().filter((key) =>
    PRESENCE_BASED_AUTHORITY_ENV_KEYS.includes(key as typeof PRESENCE_BASED_AUTHORITY_ENV_KEYS[number])
      ? env[key] !== undefined
      : configured(env, key),
  );
}
 ...
 * dictionary and nothing else. Resolver URL/key declarations and deliberate
 * override, reference, and profile selectors count by presence even when blank,
 * because the shared resolver treats that as a terminal configuration error. App
 * session/identity principals and the principal-vault pointer count when nonblank.
 * The local selectors accept only the exact value `1`.
 */
