...
describe("the standard local opt-in for emails", () => {
  ...
  it("is set by either spelling and never by a blank", () => {
    ...
    expect(isEmailsLocalOptIn({ EMAILS_LOCAL: "1" })).toBe(true);
    expect(isEmailsLocalOptIn({ EMAILS_LOCAL: "yes" })).toBe(false);
    expect(isEmailsLocalOptIn({ HASNA_EMAILS_LOCAL: "0" })).toBe(false);
  ...
  it("reports every nonblank local selector that is not the exact value 1", () => {
    expect(invalidEmailsLocalOptInSettings({})).toEqual([]);
    expect(invalidEmailsLocalOptInSettings({ HASNA_EMAILS_LOCAL: "1" })).toEqual([]);
    expect(invalidEmailsLocalOptInSettings({ HASNA_EMAILS_LOCAL: "0", EMAILS_LOCAL: "yes" }))
      .toEqual(["HASNA_EMAILS_LOCAL", "EMAILS_LOCAL"]);
  });
  ...
  it("a configured environment outranks the flag; a clean environment honours it", () => {
    ...
    expect(selectsEmailsLocalMode({ HASNA_EMAILS_LOCAL: "1", HASNA_PROFILE: "" })).toBe(false);
    expect(selectsEmailsLocalMode({ HASNA_EMAILS_LOCAL: "1", HASNA_EMAILS_API_KEY_REF: "" })).toBe(false);
    expect(selectsEmailsLocalMode({ HASNA_EMAILS_LOCAL: "1", EMAILS_CLIENT_ENV_SECRET: "ref" })).toBe(false);
    // Declared-but-blank authority values are terminal hosted intent, never local.
    expect(selectsEmailsLocalMode({ HASNA_EMAILS_LOCAL: "1", HASNA_EMAILS_API_KEY: "  " })).toBe(false);
    expect(selectsEmailsLocalMode({ HASNA_EMAILS_LOCAL: "1", EMAILS_API_URL: "" })).toBe(false);
    expect(selectsEmailsLocalMode({ HASNA_EMAILS_LOCAL: "false" })).toBe(false);
  ...
  it("names exactly the configured authority keys, in resolver order, never values", () => {
    ...
    expect(configured).toEqual(["HASNA_EMAILS_API_URL", "HASNA_EMAILS_API_KEY", "EMAILS_IDP_TOKEN"]);
    ...
    expect(hasEmailsEnvAuthorityIntent({ HASNA_PROFILE: "" })).toBe(true);
    expect(hasEmailsEnvAuthorityIntent({ EMAILS_CLIENT_ENV_SECRET: "vault/ref" })).toBe(true);
  });
});
