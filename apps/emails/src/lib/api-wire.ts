...
} from "./api-response-contracts.generated.js";
import { canonicalizeApiPathname } from "./api-paths.js";
...
export const DEFAULT_API_HTTP_TIMEOUT_MS = 30_000;
export const DEFAULT_API_MAX_RESPONSE_BYTES = 8 * 1024 * 1024;
export const SUPPORTED_API_RESPONSE_FORMATS = [
  "date-time",
  "email",
  "uuid",
] as const;

export function apiTransportLimit(
...
function pathOnly(path: string): string {
  ...
  return canonicalizeApiPathname(
...
export class ApiWireResponseError extends Error {
  constructor(
    ...
    super(`Api ${method.toUpperCase()} ${safePath(path)} returned an invalid ${responseKind} response: ${detail}`);
    this.name = "ApiWireResponseError";
  }
}
...
export class ApiResponseSizeError extends Error {
  constructor(
    ...
    super(`Api ${method.toUpperCase()} ${safePath(path)} response exceeded the ${maxBytes}-byte limit`);
    this.name = "ApiResponseSizeError";
  }
}

/** Preserve safe-wrapper fallbacks while never disguising a bad 2xx contract. */
export function rethrowApiResponseFailure(error: unknown): void {
  if (error instanceof ApiWireResponseError || error instanceof ApiResponseSizeError) {
    throw error;
  }
}

function fail(context: WireContext, detail: string): never {
  throw new ApiWireResponseError(
    context.method,
    context.path,
    detail,
...
function responseContract(context: WireContext & { status: number }): ApiResponseContract {
...
function resolveSchema(schema: JsonSchema, context: WireContext): JsonSchema {
  ...
  const resolved = API_RESPONSE_COMPONENTS[name];
...
export function parseApiSuccessJson(
  text: string,
...
export function validateApiSdkSuccessResponse(
  method: string,
  path: string,
  status: number,
  value: unknown,
...
export const validateApiMailSuccessResponse = validateApiSdkSuccessResponse;

function validateDeclaredApiErrorResponse(
  method: string,
  path: string,
  status: number,
  value: unknown,
...
export async function readApiResponseText(
  ...
  context: WireContext,
  maxBytes = DEFAULT_API_MAX_RESPONSE_BYTES,
): Promise<string> {
  maxBytes = apiTransportLimit(
    maxBytes,
  ...
  if (Number.isFinite(contentLength) && contentLength > maxBytes) {
    throw new ApiResponseSizeError(context.method, context.path, maxBytes);
  }
 ...
 */
export function redactApiErrorBody(value: unknown): Record<string, unknown> | undefined {
 ...
 */
export function projectApiErrorBody(
  method: string,
  path: string,
  status: number,
  value: unknown,
): Record<string, unknown> | undefined {
  validateDeclaredApiErrorResponse(method, path, status, value);
  const pathname = pathOnly(path);
  if (
  ...
  return redactApiErrorBody(value);
}

export const projectApiMailErrorBody = projectApiErrorBody;
