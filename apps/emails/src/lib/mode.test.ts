...
// retired aliases are refused), or an explicit database path — and this module
...
describe("resolveClientModeSelection — the storage-plan mapping", () => {
  it("maps the local opt-in plus a database path to the local arm, naming the path setting", () => {
    ...
    // Local is EXPLICIT only (fail-closed ruling, 2026-09-04; standard opt-in):
  ...
  it("a database path WITHOUT the opt-in is refused, naming the opt-in", () => {
    process.env["EMAILS_DB_PATH"] = ":memory:";
