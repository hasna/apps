...
describe("createWebhookServer storage gate", () => {
  ...
  it("creates NO local data directory on the way to refusing, at the path the code actually uses", async () => {
    ...
    // POSITIVE CONTROL FIRST. Automatic local selection requires the explicit opt-in
    // (fail-closed ruling, 2026-09-04 — the all-unset default is gone); the path only
    // names its file. Construction creates nothing before the first real delivery, so the
