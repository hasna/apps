// ApiMailDataSource maps the operator-configured Emails service onto the
// common mailbox interface. The service speaks a versioned resource API — the
// same shape `src/db/api-store.ts` already uses for `domains`:
...
import {
  ...
  mailboxLabel,
  renderMarkdown,
  API_PROVIDER_CLEAR_UNSUPPORTED,
} from "./mail-types.js";
...
import {
  ...
  parseApiErrorJson,
  parseApiSuccessJson,
  projectApiMailErrorBody,
  readApiResponseText,
  apiTransportLimit,
  validateApiMailSuccessResponse,
  ApiWireResponseError,
} from "./api-wire.js";
// ── the /v1 message row (snake_case, as the api serve returns) ────────
...
export type ApiFetch = (url: string, init: RequestInit) => Promise<{
...
const MAIL_CHANGES_CURSOR_NAMESPACE_PREFIX = "emails-api:v";
...
function filterWalkExhausted(mailbox: Mailbox, scannedRows: number): Error {
  return new Error(
    `api emails: listing the ${mailbox} folder scanned ${scannedRows} rows over `
...
// api seam has no server-side aggregate, so it can only tally rows it
...
function scopedCountWalkExhausted(scannedRows: number, requests: number): Error {
  const error = new Error(
    `api emails: scoped folder counts scanned ${scannedRows} rows over `
...
// fields are already lower-cased by apiScopeOf, and the separator cannot
// occur in either, so distinct scopes cannot collide on one key.
function scopedCountsKey(scope: ApiScope): string {
...
export const API_SOURCE_ID = "server";
...
// The conversation key of a subject. The api store has NO thread_id
...
function validCursorTimestamp(value: unknown, field: string): string | null {
  ...
  if (typeof value !== "string" || value.length > 64) {
    throw new Error(`api emails: malformed changes cursor ${field}`);
  }
  const time = Date.parse(value);
...
function advanceCursorCycle(state: CursorCycleState, nextCursor: string): CursorCycleState {
  ...
  if (nextCursor === state.cycleAnchor) {
    throw new Error("api emails: pagination cursor repeated or did not advance");
  }
...
function encodeMailChangesCursor(state: DecodedMailChangesCursor): string {
  ...
  if (decoded.length > MAIL_CHANGES_CURSOR_MAX_DECODED_BYTES) {
    throw new Error("api emails: changes cursor exceeded its bounded size limit");
  }
...
function decodeMailChangesCursor(raw: string): DecodedMailChangesCursor | null {
  ...
  try {
    ...
    throw new Error("api emails: malformed changes cursor envelope");
  }
}

function validateRawChangesCursor(raw: string): string {
  if (!validServerCursor(raw)) {
    throw new Error("api emails: malformed raw changes cursor");
  }
  return raw;
}
...
function nextCursorPageCount(pageCount: number): number {
  ...
  if (next > MAIL_CHANGES_CURSOR_MAX_PAGES) {
    throw new Error(
      `api emails: pagination exceeded the ${MAIL_CHANGES_CURSOR_MAX_PAGES}-page safety limit`,
    );
  }
  return next;
}
...
function v1ToTuiMessage(m: V1Message, rules: PrioritySenderRule[] = []): TuiMessage {
  ...
  return {
    ...
    // Genuinely absent: the api store keeps no upstream/provider thread
...
/**
 * The subset of a MailboxSource the api /v1 store can actually express.
...
/**
 * Narrow a caller's MailboxSource to what the api serve can honour.
 *
 ...
 *
 * The whole-store ids this backend itself publishes (`api`, plus local
 * mode's `all`) resolve to no narrowing, so the id printed by
 * `emails inbox sources` round-trips into `--source`.
 */
function apiScopeOf(source?: MailboxSource): ApiScope | undefined {
  ...
  const wholeStore = !sourceId || sourceId === API_SOURCE_ID || sourceId === ALL_SOURCE_ID;
  ...
  if (unsupported.length > 0) {
    throw new Error(
      `Api mail is one shared store with no ingestion-source or provider provenance on its messages, `
      + `so ${unsupported.join(" and ")} cannot be applied. Scope by mailbox instead `
      + `(--address <email> or --domain <domain>), or use --source ${API_SOURCE_ID} for the whole store. `
...
function scopeMatch(m: V1Message, scope?: ApiScope): boolean {
...
function scopeServerFilterSets(scope: ApiScope | undefined): Array<{ to?: string; from?: string }> {
...
// Bounded per-request timeout so a slow/unreachable api serve FAILS
...
function apiTimeoutMs(): number {
  const raw = process.env["EMAILS_API_HTTP_TIMEOUT"];
...
export interface ApiMailDataSourceOptions {
  ...
  fetchImpl?: ApiFetch;
  now?: () => number;
  /** Per-request timeout in ms (default: EMAILS_API_HTTP_TIMEOUT or 30s). */
  timeoutMs?: number;
...
export class ApiMailDataSource implements MailDataSource {
  ...
  constructor(options: ApiMailDataSourceOptions) {
    ...
    if (url.protocol !== "https:" && !(url.protocol === "http:" && loopback)) {
      throw new Error("Api Emails requires HTTPS except for loopback development URLs.");
    }
    ...
    this.timeoutMs = apiTransportLimit(options.timeoutMs, apiTimeoutMs(), "timeoutMs");
    ...
    this.fetchImpl = options.fetchImpl
      ?? ((url, init) => fetch(url, init) as unknown as ReturnType<ApiFetch>);
  }
  ...
  private async requestOnce(
    ...
    init.signal = timer;
    let res: Awaited<ReturnType<ApiFetch>>;
    try {
      res = await this.fetchImpl(`${this.baseUrl}${path}`, init);
    } catch (error) {
      if (timer.aborted || (error as Error)?.name === "TimeoutError" || (error as Error)?.name === "AbortError") {
        throw new Error(`api emails: ${method} ${path} timed out after ${this.timeoutMs}ms`);
      }
      throw new Error(`api emails: cannot reach ${this.baseUrl} for ${method} ${path}`);
    }
    if (res.status >= 300 && res.status < 400) {
      throw new Error(`api emails: ${method} ${path} redirect refused`);
    }
    const text = await readApiResponseText(res, { method, path }, this.maxResponseBytes);
    let json: unknown = null;
    if (res.status >= 200 && res.status < 300) {
      json = parseApiSuccessJson(text, { status: res.status, method, path });
      validateApiMailSuccessResponse(method, path, res.status, json);
    } else {
      ...
      json = projectApiMailErrorBody(method, path, res.status, parsed);
    }
  ...
  private async listPage(
    ...
    if (status < 200 || status >= 300) {
      throw new Error(`api emails: GET /messages failed (HTTP ${status})`);
    }
    ...
    if (nextCursor !== null && !validServerCursor(nextCursor)) {
      throw new Error("api emails: GET /messages returned a malformed next_cursor");
    }
  ...
  private async *listPages(
    ...
    while (true) {
      ...
      if (page.nextCursor === undefined
        ...
        throw new Error(
          "api emails: legacy pagination reached offset cap 100000 on a full page; "
  ...
  private async serverStats(): Promise<{ counts: MailboxCounts; total: number; latestReceivedAt: string | null }> {
    ...
    if (status < 200 || status >= 300) {
      throw new Error(`api emails: GET /messages/counts failed (HTTP ${status})`);
    }
  ...
  private async scanAll(): Promise<V1Message[]> {
    ...
    for await (const page of this.listPages(PAGE_LIMIT)) {
      if (rows.length + page.length > MAX_SCAN_ROWS) {
        throw new Error(
          `api emails: full-store scan exceeded the ${MAX_SCAN_ROWS}-row safety limit`,
        );
      }
      rows.push(...page);
    }
  ...
  private async priorityRules(): Promise<PrioritySenderRule[]> {
    ...
    try {
      ...
      if (
        error instanceof ApiWireResponseError
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new Error(`api emails: GET /priority-sender-rules failed (HTTP ${response.status})`);
    }
    const rows = (response.json as { items?: unknown } | null)?.items;
    if (!Array.isArray(rows)) throw new Error("api emails: priority rule response was malformed");
  ...
  private async listFilteredMailboxPage(mailbox: Mailbox, scope: ApiScope | undefined, opts?: MailboxListOptions, rules: PrioritySenderRule[] = []): Promise<V1Message[]> {
    ...
    let walkRequests = 0;
    let scannedRows = 0;
    const budgeted = async function* (this: ApiMailDataSource, requestLimit: number, requestOpts: Parameters<ApiMailDataSource["listPage"]>[2]): AsyncGenerator<V1Message[]> {
  ...
  private async scanScopeRows(scope?: ApiScope): Promise<V1Message[]> {
    ...
    const collect = async (filters: { direction?: "inbound" | "outbound"; to?: string; from?: string }) => {
      for await (const page of this.listPages(PAGE_LIMIT, filters)) {
        ...
        if (seen.size > MAX_SCAN_ROWS) {
          throw new Error(
            `api emails: scoped scan exceeded the ${MAX_SCAN_ROWS}-row safety limit`,
          );
        }
      }
    };
  ...
  private async getRaw(id: string): Promise<V1Message | null> {
    ...
    if (status < 200 || status >= 300) {
      throw new Error(`api emails: GET /messages/<id> failed (HTTP ${status})`);
    }
  ...
  async listMailbox(mailbox: Mailbox, opts?: MailboxListOptions): Promise<TuiMessage[]> {
    const scope = apiScopeOf(opts?.source);
  ...
  async listMailboxFilters(options: { limit?: number; offset?: number } = {}): Promise<MailboxFilter[]> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: GET /mailbox-filters failed (HTTP ${status})`);
  ...
  async getMailboxFilter(identifier: string): Promise<MailboxFilter | null> {
    ...
    if (status === 404) return null;
    if (status < 200 || status >= 300) throw new Error(`api emails: GET /mailbox-filters/<id> failed (HTTP ${status})`);
  ...
  async createMailboxFilter(input: MailboxFilterInput): Promise<MailboxFilter> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: POST /mailbox-filters failed (HTTP ${status})`);
    return json as MailboxFilter;
  }
  ...
  async updateMailboxFilter(identifier: string, input: Partial<MailboxFilterInput>): Promise<MailboxFilter> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: PATCH /mailbox-filters/<id> failed (HTTP ${status})`);
    return json as MailboxFilter;
  }
  ...
  async deleteMailboxFilter(identifier: string): Promise<void> {
    ...
    if (status === 404) throw new Error(`mailbox filter not found: ${identifier}`);
    if (status < 200 || status >= 300) throw new Error(`api emails: DELETE /mailbox-filters/<id> failed (HTTP ${status})`);
  }
  ...
  async applyMailboxFilter(identifier: string, options: { limit?: number; offset?: number } = {}): Promise<import("./mail-data-source.js").MailboxFilterApplyResult> {
    ...
    if (status === 404) throw new Error(`mailbox filter not found: ${identifier}`);
    if (status < 200 || status >= 300) throw new Error(`api emails: POST /mailbox-filters/<id>/apply failed (HTTP ${status})`);
   ...
   */
  private async scopedCounts(scope: ApiScope): Promise<MailboxCounts> {
  ...
  async unreadByAddress(opts?: { limit?: number; offset?: number }): Promise<UnreadByAddressRow[]> {
    ...
    if (status < 200 || status >= 300) {
      throw new Error(`api emails: GET /messages/unread-by-address failed (HTTP ${status})`);
    }
    const body = (json as { rows?: unknown } | null)?.rows;
    if (!Array.isArray(body)) {
      throw new Error("api emails: GET /messages/unread-by-address returned no rows array");
    }
  ...
  async getAttachmentContent(id: string, index: number, opts?: { maxBytes?: number }): Promise<AttachmentContent> {
    ...
    if (status < 200 || status >= 300) {
      ...
      throw new Error(`api emails: attachment download failed (HTTP ${status}, ${String(body["code"] ?? "unknown_error")})`);
    }
  ...
  async listInsertionsSince(opts?: MailInsertionsQuery): Promise<MailInsertionsPage> {
    ...
    if (decodedCursor
      ...
      throw new Error(
        "api emails: receivedSince must match the value bound to the insertion cursor",
      );
    }
    ...
    while (true) {
      ...
      if (page.nextCursor === undefined && page.messages.length === pageLimit) {
        if (rawCursor) {
          throw new Error(
            "api emails: raw cursor pagination returned a full page without next_cursor; "
            + "upgrade the server or rerun without cursor because the traversal depth is unknown",
          );
        }
        if (consumedAfterPage === null) {
          throw new Error(
            "api emails: a legacy page omitted next_cursor after a raw server cursor; "
  ...
  // Mailbox mutations are persisted by the api serve.
  async setRead(id: string, read: boolean): Promise<void> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: mark read failed (HTTP ${status})`);
  }

  async setArchived(id: string, archived: boolean): Promise<void> {
    this.invalidate();
    const { status } = await this.request("PATCH", `/messages/${encodeURIComponent(id)}`, { archived });
    if (status < 200 || status >= 300) throw new Error(`api emails: archive update failed (HTTP ${status})`);
  }
  ...
  async setStarred(id: string, starred: boolean): Promise<void> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: star update failed (HTTP ${status})`);
  }
  ...
  async addLabel(id: string, label: string): Promise<string[]> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: add label failed (HTTP ${status})`);
  ...
  async removeLabel(id: string, label: string): Promise<string[]> {
    ...
    if (status < 200 || status >= 300) throw new Error(`api emails: remove label failed (HTTP ${status})`);
    return labelsOf((json as { message?: V1Message } | null)?.message ?? {} as V1Message);
  }
  ...
  async deleteMessage(id: string): Promise<void> {
    ...
    if (status !== 404 && (status < 200 || status >= 300)) {
      throw new Error(`api emails: DELETE /messages/<id> failed (HTTP ${status})`);
    }
  }
  ...
  async bulk(input: MailBulkInput): Promise<MailBulkResult> {
    ...
    for (const id of ids) {
      ...
      else throw new Error(`Bulk '${action}' is not supported on the api emails serve.`);
      affected += 1;
    }
  ...
  async send(input: MailSendInput): Promise<MailSendResult> {
    if (input.scheduledAt) {
      throw new Error("Scheduled send is not supported on the api emails serve.");
    }
    if (input.providerId) {
    ...
    if (status < 200 || status >= 300) {
      const reason = typeof (json as Record<string, unknown> | null)?.["reason"] === "string"
        ...
        : "";
      throw new Error(`api Emails: POST /messages/send failed (HTTP ${status})${reason}`);
    }
  ...
  async clear(filter?: MailClearFilter): Promise<MailClearResult> {
    ...
    if (filter?.providerId) throw new Error(API_PROVIDER_CLEAR_UNSUPPORTED);
    const scope = apiScopeOf(filter?.source);
...
/**
 * Build the api data source only from explicit operator configuration.
 * The client storage contract decides: an API origin plus a credential names
 * the API client, and nothing else does.
 */
export function resolveApiMailDataSource(fetchImpl?: ApiFetch): ApiMailDataSource | null {
  if (!isApiClientConfigured()) return null;
  const config = resolveApiConfig();
  if (!config) return null;
  // `apiKey` here is the Bearer credential slot — a user session token when
  // present, else the next configured credential (resolveApiConfig decides).
  return new ApiMailDataSource({
