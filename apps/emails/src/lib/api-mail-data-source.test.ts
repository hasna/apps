...
import {
  ApiMailDataSource,
  type ApiFetch,
  resolveApiMailDataSource,
} from "./api-mail-data-source.js";
...
// A api /v1 message row (snake_case, as the API returns).
...
function encodedChangesCursor(version: number, envelope: Record<string, unknown>): string {
  return `emails-api:v${version}:${Buffer.from(JSON.stringify(envelope), "utf8").toString("base64url")}`;
}
...
): { fetchImpl: ApiFetch; requests: string[]; deleted: string[] } {
...
// A fake api /v1 serve backed by an in-memory row list.
function fakeServe(
  initial: Array<Record<string, unknown>>,
  options: {
...
): { fetchImpl: ApiFetch; rows: Map<string, Record<string, unknown>>; posted: unknown[]; deleted: string[]; requests: string[] } {
...
): { ds: ApiMailDataSource; serve: ReturnType<typeof fakeServe> } {
  const serve = fakeServe(rows, options);
  const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "test-key", fetchImpl: serve.fetchImpl });
  return { ds, serve };
}
...
describe("ApiMailDataSource — /v1 resource mapping", () => {
  it("rejects remote plaintext HTTP before retaining an API key", () => {
    expect(() => new ApiMailDataSource({ baseUrl: "http://emails.example/v1", apiKey: "must-not-leak" }))
      .toThrow(/requires HTTPS/);
    expect(() => new ApiMailDataSource({ baseUrl: "http://localhost:8080/v1", apiKey: "local" }))
      .not.toThrow();
  });
  ...
  it("degrades to an empty rule set when an old serve 404s the rules endpoint with an undeclared body", async () => {
    ...
    const ds = new ApiMailDataSource({
  ...
  it("pushes timezone-aware since filters to the api server before pagination", async () => {
    const { ds, serve } = make([
  ...
  it("walks multiple full legacy offset pages for scoped counts and oldest reads", async () => {
    ...
    const countDs = new ApiMailDataSource({
      baseUrl: "https://emails.example/v1",
      apiKey: "test-key",
    ...
    const oldestDs = new ApiMailDataSource({
      baseUrl: "https://emails.example/v1",
      apiKey: "test-key",
  ...
  it("fails closed when a current cursor page omits its required continuation", async () => {
    const requests: string[] = [];
    const fetchImpl: ApiFetch = async (url) => {
      const u = new URL(url);
  ...
  it("rejects a malformed versioned insertion cursor before contacting the server", async () => {
    ...
    await expect(ds.listInsertionsSince({
      cursor: "emails-api:v1:not-base64-json",
      limit: 1,
  ...
  it("keeps starred api mailbox semantics aligned with local received folders", async () => {
    const { ds, serve } = make([
  ...
  it("keeps archived api mailbox lists aligned with server count semantics", async () => {
    const { ds } = make([
  ...
  it("reports api source totals as received mail, not global sent+received totals", async () => {
    const { ds } = make([
      v1("1"),
  ...
  it("surfaces a safe machine reason without retaining the server's sensitive send detail", async () => {
    ...
    const serveFail: ApiFetch = async () => ({
      status: 422,
      async text() {
        return JSON.stringify({
    ...
    const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "k", fetchImpl: serveFail });
    let failure: unknown;
    try {
  ...
  it("returns a sent-with-warning response as SUCCESS carrying the warning (a sent message must never look failed)", async () => {
    const serveWarn: ApiFetch = async () => ({
      status: 202,
      async text() {
        return JSON.stringify({
          message: v1("m1", {
    ...
    const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "k", fetchImpl: serveWarn });
  ...
  it("rejects a sent response that omits the schema-required provider message id", async () => {
    const serveRecOnly: ApiFetch = async () => ({
      status: 202,
      async text() {
        return JSON.stringify({
          message: v1("m2", {
    ...
    const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "k", fetchImpl: serveRecOnly });
  ...
  it("returns an in-progress send distinctly and does not require sent=true", async () => {
    const serveInProgress: ApiFetch = async () => ({
      status: 202,
      async text() {
        return JSON.stringify({
  ...
  it("rejects --provider instead of silently ignoring it (the server owns the sender)", async () => {
    ...
    let called = 0;
    const serve: ApiFetch = async () => {
      called += 1;
      return { status: 202, async text() { return JSON.stringify({ message: { id: "m" }, sent: true }); } };
    };
    const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "k", fetchImpl: serve });
    await expect(ds.send({
  ...
  it("never turns a provider REJECT into a resolved send", async () => {
    const serveReject: ApiFetch = async () => ({
      status: 422,
      async text() {
        return JSON.stringify({
    ...
    const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "k", fetchImpl: serveReject });
  ...
  it("never turns an INDETERMINATE outcome into a resolved send", async () => {
    const serveUnknown: ApiFetch = async () => ({
      status: 502,
      async text() {
        return JSON.stringify({
    ...
    const ds = new ApiMailDataSource({ baseUrl: "https://emails.example/v1", apiKey: "k", fetchImpl: serveUnknown });
    let failure: unknown;
    try {
      await ds.send({ to: "x@external.example", from: "me@example.com", subject: "s", body: "b" });
    } catch (error) {
  ...
  it("pushes verification sender and subject filters to the api server", async () => {
    const { ds, serve } = make([
  ...
  it("persists mailbox mutations through the api serve", async () => {
  ...
  it("keeps api attachment access metadata-only without local writes", async () => {
  ...
  it("refuses redirects before reading a response body or allowing fetch to forward the bearer header", async () => {
    ...
    let bodyReads = 0;
    let requests = 0;
    const redirectingFetch: ApiFetch = async (_url, init) => {
      requests++;
  ...
  it("fails fast and loud (never hangs) when the serve stalls past the timeout", async () => {
    ...
    const hangingFetch: ApiFetch = (_url, init) =>
      ...
      }) as unknown as ReturnType<ApiFetch>;
    const ds = new ApiMailDataSource({
      baseUrl: "https://emails.example/v1",
      apiKey: "test-key",
...
describe("resolveMailDataSource — api seam selection", () => {
  it("selects the API data source from URL and credential", () => {
    ...
    expect(ds.constructor.name).toBe("ApiMailDataSource");
    expect(ds.backend).toBe("api");
    expect(resolveApiMailDataSource()).toBeInstanceOf(ApiMailDataSource);
  });

  it("does not construct the API data source without the API configuration", () => {
    resetApiConfigCache();
    resetMailDataSource();
    expect(resolveApiMailDataSource()).toBeNull();
  });
});

// ── conversation / thread projection ────────────────────────────────────────
//
// The api store has no thread_id column, so getConversation() used to
...
describe("ApiMailDataSource — conversations", () => {
...
describe("ApiMailDataSource — source scoping", () => {
  ...
  // while the api pushdown's message_recipients spans only to/cc. The
  ...
  it("honors the --search and --limit it is handed on listMailboxSources", async () => {
    const { ds } = make([v1("2")]);
    expect((await ds.listMailboxSources({ search: "server" })).map((s) => s.id)).toEqual(["server"]);
...
describe("ApiMailDataSource — read filter", () => {
...
describe("ApiMailDataSource — a blocked message carries its reason", () => {
...
// call, with no cache).
// ---------------------------------------------------------------------------
describe("ApiMailDataSource — listLabelSummaries scan budget", () => {
  ...
  function deepStoreServe(pageCount: number): { fetchImpl: ApiFetch; requests: string[] } {
    const requests: string[] = [];
    const fetchImpl: ApiFetch = async (url, init) => {
      const u = new URL(url);
      const method = (init.method ?? "GET").toUpperCase();
  ...
  function dsFor(serve: { fetchImpl: ApiFetch }, now: () => number): ApiMailDataSource {
...
// ---------------------------------------------------------------------------
describe("ApiMailDataSource — scoped mailboxCounts scan budget", () => {
  ...
  function exhaustingServe(pageCount: number): { fetchImpl: ApiFetch; requests: string[] } {
    const requests: string[] = [];
  ...
  it("does NOT break scoped stores that complete today with many small pages", async () => {
    ...
    const dsDeep = new ApiMailDataSource({
      baseUrl: "https://emails.example/v1",
      apiKey: "test-key",
    ...
    const dsWide = new ApiMailDataSource({
      baseUrl: "https://emails.example/v1",
      apiKey: "test-key",
