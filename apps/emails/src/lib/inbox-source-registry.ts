...
function operationBinding(): HostedEmailsTransport {
  ...
  return resolveEmailsHostedTransport(process.env);
}
async function request<T>(transport: HostedEmailsTransport, operation: (client: EmailsSelfHostClient) => Promise<T>): Promise<T> {
...
async function rows(transport: HostedEmailsTransport): Promise<Row[]> {
  const result: Row[] = [], seen = new Set<string>();
  for (let offset = 0; offset < 100000; offset += 1000) {
    const page = (await request(transport, client => client.listResourceSources({ limit: 1000, offset }))).items;
...
export async function listRegisteredS3Sources(): Promise<RegisteredS3Source[]> {
  const transport = operationBinding();
  return (await rows(transport)).map(project);
}
export async function registerApiS3Source(input: { bucket: string; prefix?: string; region?: string; providerId?: string; name?: string; status: SourceStatus; liveSyncEnabled: boolean }): Promise<RegisteredS3Source> {
  ...
  const transport = operationBinding();
  const matches = (await rows(transport)).filter(row => settings(row).bucket === bucket && (settings(row).prefix ?? "") === prefix);
  ...
  return project(await request(transport, client => previous ? client.updateResourceSources(previous.id, record) : client.createResourceSources(record)));
}
export async function retireApiS3Source(ref: string): Promise<RegisteredS3Source> {
  ...
  const transport = operationBinding();
  const all = await rows(transport); const exact = all.filter(row => row.id === ref);
  ...
  const row = matches[0]!;
  return project(await request(transport, client => client.updateResourceSources(row.id, { status: "retired", settings_json: { ...settings(row), source_status: "retired", live_sync_enabled: false, retired_at: new Date().toISOString() } })));
}
