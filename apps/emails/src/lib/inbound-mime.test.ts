...
const rawMultipart = [
  ...
  `To: andrei@example.com, ops@example.com`,
  `Cc: audit@example.com`,
  `Subject: Your invoice is ready`,
  `Message-ID: <inv-9001@acme.com>`,
  `In-Reply-To: <req-42@example.com>`,
...
describe("parseInboundMime", () => {
  it("normalizes a multipart/alternative message to store fields", async () => {
    ...
    expect(r.to_addrs).toEqual(["andrei@example.com", "ops@example.com"]);
    expect(r.cc_addrs).toEqual(["audit@example.com"]);
    ...
    expect(r.in_reply_to).toBe("req-42@example.com");
  ...
  it("handles a plain text message with no date", async () => {
    ...
    expect(r.to_addrs).toEqual(["andrei@example.com"]);
