// LOCAL-backend honesty for the status payload.
//
...
describe("local status payload", () => {
  ...
  it("never proposes a command that refuses for the local SQLite client", async () => {
    ...
    // `isCommandAvailableForBackend(action.command, "sqlite")`, i.e. it validated the
    ...
    const refused = proposed
      .map((command) => ({ command, prefix: cliRefusalFor(command, "sqlite") }))
      .filter((entry) => entry.prefix !== null)
      .map((entry) => `${entry.command} (refused by ${entry.prefix})`);
    expect(refused, "next_actions proposes commands that throw for the local SQLite client").toEqual([]);
