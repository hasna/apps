// API clients execute forwarding on the server. Explicit local database callers retain
// the existing local pipeline; these stores never share a delivery ledger.
...
export interface ForwardingRunOptions {
   ...
   * caller has already answered the question the gate exists to ask. Omitting it uses the configured API service or explicit local database.
   * Missing or contradictory storage settings fail closed.
   */
  db?: Database;
  /**
...
/** Resolve explicitly configured local storage after API execution has been dispatched. */
function configuredForwardingLedger(): Database {
  ...
  switch (wiring.kind) {
    ...
    case "database_in_memory":
      ...
      return getDatabase();
    case "api":
      throw new Error("API forwarding must execute through the authenticated service.");
    case "unresolved":
...
export async function processForwardingRules(opts: ForwardingRunOptions = {}): Promise<ForwardingRunResult> {
  if (!opts.db && readStorageWiring(process.env).kind === "api") {
    const { runApiForwarding } = await import("./forwarding-api.js");
    return runApiForwarding(opts);
  }
  ...
  for (const pendingItem of pending) {
    ...
    const sendOpts = {
      ...
      // This legacy local key is consumed only by the sent-mail ledger, after
      // the provider call.
