// REGRESSION GUARD for the fabricated api status payload.
//
...
describe("api status reports REAL server inventory (G1)", () => {
...
describe("honest-unavailable contract (G3)", () => {
  ...
  it("reports null (never 0) and a source_unreachable reason when the API cannot be read", async () => {
    ...
    const { resetApiConfigCache } = await import("../db/api-store.js");
