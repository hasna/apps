...
beforeEach(() => {
  ...
  fixtureRoot = mkdtempSync(join(tmpdir(), "emails-groups-configured-"));
...
describe("canonical configured-store boundary", () => {
  it("persists and reads the default operation through authenticated HTTP", async () => {
    ...
    const created = await createGroup("canonical-default");
    expect(service().requestCount()).toBeGreaterThan(before);
    expect(db.query("SELECT * FROM groups WHERE id = ?").get(created.id)).toMatchObject({ id: created.id, name: "canonical-default" });
    const afterWrite = service().requestCount();
    expect(await getGroup(created.id)).toMatchObject({ id: created.id, name: "canonical-default" });
  ...
  it("rejects a missing credential before dispatch or mutation", async () => {
    ...
    const before = service().requestCount();
    await expect(createGroup("canonical-default")).rejects.toThrow(/API credential is required/);
  ...
  it("rejects a wrong credential at HTTP without exposing it or falling back", async () => {
    ...
    const before = service().requestCount();
    const error = await createGroup("canonical-default").catch(error => error);
