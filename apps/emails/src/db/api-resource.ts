...
/**
 * Return the api-backed store for `resource`. This client is
 * api-only: EVERY repository read/write routes to the app's `/v1` API.
 * Missing or invalid configuration throws (fail loud) via apiStoreFor().
 */
export function apiResource(resource: string): ApiResourceStore {
