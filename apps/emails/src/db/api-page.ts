...
// (src/server/api/store.ts clampLimit -> min(max(1, limit), 500),
 ...
 * src/server/api/store.ts. Asking for more silently returns 500.
 */
export const API_SERVER_PAGE_MAX = 500;
 ...
 */
export const API_ENUMERATION_PAGE_BUDGET = 40;

export interface ApiEnumeration<T = Record<string, unknown>> {
  rows: T[];
  /**
...
export interface EnumerateOptions<T = Record<string, unknown>> {
   ...
   * server-side filters (`src/server/api/resources.ts` `filters`).
...
function clampPageSize(value: number | undefined): number {
  if (!value || !Number.isFinite(value)) return API_SERVER_PAGE_MAX;
  return Math.min(Math.max(1, Math.floor(value)), API_SERVER_PAGE_MAX);
}
...
function clampBudget(value: number | undefined): number {
  if (!value || !Number.isFinite(value)) return API_ENUMERATION_PAGE_BUDGET;
 ...
 *
 * Throws (ApiHttpError) if the server rejects a page. The caller is
 ...
 */
export function enumerateApiRows<T = Record<string, unknown>>(
  resource: string | ApiResourceStore,
  ...
  const store = typeof resource === "string" ? apiResource(resource) : resource;
 ...
 * `apiListQuery` asks for `max(1000, limit + offset)` rows in ONE request
 * and sends no server-side offset, then windows the result locally. The server
 * clamps every list to API_SERVER_PAGE_MAX (see
 * src/server/api/store.ts clampLimit), so that helper can never see past
 ...
 */
export function assertHonestApiRead(
  enumeration: ApiEnumeration<unknown>,
  bound: number | null,
