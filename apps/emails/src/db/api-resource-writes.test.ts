// End-to-end proof that the resource repositories route WRITES to the api /v1
// API in api mode (not the local SQLite island) — the write half of the
// split-brain fix. Reads were already routed (see api-resource-routing.test.ts);
...
//
// A stateful stub /v1 server runs OUT OF PROCESS (the repo layer's api client
// is synchronous curl, which cannot reach an in-process Bun.serve). The local DB
// is left empty so a local write could not masquerade as the api state asserted.
...
describe("resource repos route writes to api in api mode", () => {
