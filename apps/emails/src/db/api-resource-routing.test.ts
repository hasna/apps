// End-to-end proof that the resource repositories route reads to the api /v1
// API in api mode (not the local SQLite island), and FAIL CLOSED when the
// endpoint is absent — the split-brain fix. A stub /v1 server runs OUT OF
// PROCESS (the repo layer's api client is synchronous curl, which cannot reach
...
import { ApiHttpError, resetApiConfigCache } from "./api-store.js";
...
const server = Bun.serve({ port: 0, fetch(req) {
  ...
  if (p === "/v1/contacts") return ok({ items: [{ id: "c1", tenant_id: tenantId, email: "api@example.com", name: "Api", send_count: 3, bounce_count: 0, complaint_count: 0, last_sent_at: null, suppressed: false, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-02T00:00:00Z" }] });
  if (p === "/v1/groups") return ok({ items: [{ id: "g1", tenant_id: tenantId, name: "api-group", description: null, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" }] });
  if (p === "/v1/owners") return ok({ items: [{ id: "o1", tenant_id: tenantId, type: "agent", name: "Api Agent", contact_email: null, external_id: null, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" }] });
  if (p === "/v1/providers") return ok({ items: [{ id: "p1", tenant_id: tenantId, name: "api-ses", type: "ses", region: "us-east-1", active: true, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" }] });
...
describe("resource repos route reads to api in api mode", () => {
  ...
  test("listContacts reads the configured API store", async () => {
    ...
    try {
      ...
      expect(rows[0]!.email).toBe("api@example.com");
  ...
  test("listGroups reads the configured API store", async () => {
    ...
    for (const [key] of restore) delete process.env[key];
    try {
      expect((await listGroups()).map((g) => g.name)).toEqual(["api-group"]);
    } finally {
      for (const [key, value] of restore) {
        if (value === undefined) delete process.env[key];
        else process.env[key] = value;
  ...
  test("listProviderSummaries returns api rows without secrets", () => {
    const rows = listProviderSummaries();
    expect(rows).toHaveLength(1);
    expect(rows[0]!.name).toBe("api-ses");
  ...
  test("missing endpoint FAILS CLOSED (no silent local read)", () => {
    expect(() => listDomains()).toThrow(ApiHttpError);
  });
});
