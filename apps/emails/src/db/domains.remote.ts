...
function apiToDomain(e: Record<string, unknown>): Domain {
  ...
  return {
    ...
    inbound_status: ["inbound_ready", "verified_inbound_ready"].includes(String(e["provisioning_status"])) ? "ready" : "pending",
    outbound_status: e["status"] === "outbound_disabled" ? "disabled" : verified && e["status"] === "active" && ["ready", "active", "verified", "verified_inbound_ready"].includes(String(e["provisioning_status"])) ? "ready" : "pending",
    monitoring_status: "none",
    dkim_status: dns,
    spf_status: dns,
    // Provider identity verification does not prove a published DMARC policy.
