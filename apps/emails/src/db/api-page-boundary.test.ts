...
//
// `apiListQuery` asked for `max(1000, limit + offset)` rows in ONE request and sent no
// server-side offset, then windowed the result locally. Every `/v1` list is clamped to 500
// server-side (src/server/api/store.ts clampLimit), so that helper could never see
...
import { API_SERVER_PAGE_MAX } from "./api-page.js";

const OVER_CLAMP = API_SERVER_PAGE_MAX + 100; // 600
...
describe("the stub enforces production's 500-row list clamp", () => {
  ...
  it("clamps /v1/messages, not just the generic resources", async () => {
    ...
    expect(messageRows).toHaveLength(API_SERVER_PAGE_MAX);
    expect(genericRows).toHaveLength(API_SERVER_PAGE_MAX);
  }, 30_000);
});
