...
// 500 rows (src/server/api/store.ts clampLimit). The client's convention
...
// crosses 500 rows.
//
// enumerateApiRows pages until a short page and reports `complete`, so a
...
describe("enumerateApiRows", () => {
  ...
  it("pages past the server cap instead of silently truncating at 500", async () => {
    ...
    const single = apiResource("addresses").list({ limit: 1000 });
    expect(single.length).toBeLessThanOrEqual(API_SERVER_PAGE_MAX);
  ...
  it("reports complete: false rather than a plausible total when the budget runs out", async () => {
    ...
    const result = enumerateApiRows("addresses", { pageSize: 100, pageBudget: 3 });
  ...
  it("throws instead of returning an empty page set when the endpoint is unreachable", async () => {
    ...
    const { resetApiConfigCache } = await import("./api-store.js");
    resetApiConfigCache();

    expect(() => enumerateApiRows("addresses")).toThrow();
  });
});
...
describe("enumerateApiRows with a bounded need", () => {
  it("stops at the window and reports filled, without claiming completeness", async () => {
    await stub.seed({ addresses: rows(1200) });

    const result = enumerateApiRows("addresses", { need: 100 });
  ...
  it("pages to fill a window wider than the server's page cap", async () => {
    ...
    const result = enumerateApiRows("addresses", { need: 800 });
  ...
  it("reports neither filled nor complete when the budget runs out first", async () => {
    ...
    const result = enumerateApiRows("addresses", { need: 900, pageSize: 100, pageBudget: 3 });
  ...
  it("fills the window with rows that pass select, not rows merely read", async () => {
    ...
    const result = enumerateApiRows<string>("addresses", {
      need: 100,
  ...
  it("proves a forward window shift that produces no duplicate at all", async () => {
    ...
    const result = enumerateApiRows("addresses", { need: 1000 });
