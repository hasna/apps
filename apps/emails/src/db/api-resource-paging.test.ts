...
// `.local` api resource helpers.
//
// `apiListQuery` used to send `offset` to the server AND `apiPage`
...
// The canonical `api-resource.ts` twin documents the same fix; the
...
import { apiListQuery, apiPage } from "./api-resource.sqlite.js";
...
// api branch — the one the ten `*.sqlite.ts` repositories still run behind
...
describe("apiListQuery windows the page exactly once", () => {
  test("never sends a server-side offset", () => {
    expect(apiListQuery({ limit: 2, offset: 2 }).query).not.toHaveProperty("offset");
    expect(apiListQuery({ offset: 40 }).query).not.toHaveProperty("offset");
  });
