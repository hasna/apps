...
beforeEach(() => {
  ...
  fixtureRoot = mkdtempSync(join(tmpdir(), "emails-owners-configured-"));
...
describe("canonical configured-store boundary", () => {
  it("persists and reads the default operation through authenticated HTTP", async () => {
    ...
    const created = await createOwner({ type: "agent", name: "Canonical Default" });
    expect(service().requestCount()).toBeGreaterThan(before);
    expect(db.query("SELECT * FROM owners WHERE id = ?").get(created.id)).toMatchObject({ id: created.id, name: "Canonical Default", type: "agent" });
    const afterWrite = service().requestCount();
    expect(await getOwner(created.id)).toMatchObject({ id: created.id, name: "Canonical Default", type: "agent" });
  ...
  it("rejects a missing credential before dispatch or mutation", async () => {
    ...
    const before = service().requestCount();
    await expect(createOwner({ type: "agent", name: "Canonical Default" })).rejects.toThrow(/API credential is required/);
  ...
  it("rejects a wrong credential at HTTP without exposing it or falling back", async () => {
    ...
    const before = service().requestCount();
    const error = await createOwner({ type: "agent", name: "Canonical Default" }).catch(error => error);
...
describe.each(STORE_VARIANTS)("owner registry (%s)", (_label, variant) => {
  ...
  it("lists owner names and rows for selected ids only, tolerating orphaned nulls", async () => {
    ...
    expect([...names.entries()].sort()).toEqual([
      [first.id, "First"] as [string, string],
      [second.id, "Second"] as [string, string],
    ].sort());
