...
import { ApiWireResponseError } from "../lib/api-wire.js";
...
const KEYS = [
  ...
  "EMAILS_API_HTTP_CONNECT_TIMEOUT",
  "EMAILS_API_HTTP_TIMEOUT",
  "DATABASE_URL",
...
describe("Emails api client resolver", () => {
  beforeEach(() => {
  ...
  test("unset env fails direct api resolution loud", () => {
    expect(() => resolveApiConfig()).toThrow("HASNA_EMAILS_API_URL");
    expect(() => apiStoreFor("domains")).toThrow("HASNA_EMAILS_API_URL");
  });

  test("requires the API URL and a credential", () => {
    expect(() => resolveApiConfig()).toThrow("HASNA_EMAILS_API_KEY");
    process.env["HASNA_EMAILS_API_KEY"] = "test-key";
    resetApiConfigCache();
    expect(() => resolveApiConfig()).toThrow("HASNA_EMAILS_API_URL");
    process.env["HASNA_EMAILS_API_URL"] = "https://emails.example";
    resetApiConfigCache();
    expect(resolveApiConfig()?.baseUrl).toBe("https://emails.example/v1");
  });

  test("EMAILS_CLIENT_ENV_SECRET configures direct api resource resolution", () => {
    installFakeSecrets('{"HASNA_EMAILS_API_URL":"https://emails.example","HASNA_EMAILS_API_KEY":"test-token"}');

    expect(resolveApiConfig()?.baseUrl).toBe("https://emails.example/v1");
    expect(apiStoreFor("domains")).not.toBeNull();
  });
  ...
  test(`a leftover ${LEGACY_SELECTOR}=local is ignored when credentials are present`, () => {
    ...
    process.env[LEGACY_SELECTOR] = "local";
    expect(resolveApiConfig()?.baseUrl).toBe("https://stale-emails.example/v1");
  ...
  test("rejects non-loopback plaintext HTTP", () => {
    ...
    process.env["HASNA_EMAILS_API_KEY"] = "test-key";
    resetApiConfigCache();
    expect(() => resolveApiConfig()).toThrow("must use https");
  });
  ...
  test("transport fails fast and never includes the API key", () => {
    ...
    process.env["EMAILS_API_HTTP_CONNECT_TIMEOUT"] = "1";
    process.env["EMAILS_API_HTTP_TIMEOUT"] = "2";
    resetApiConfigCache();
  ...
  test("curl bridge passes API key and request body through stdin config instead of temp files or argv", () => {
    ...
    const created = apiStoreFor("domains")!.create({ domain: "example.com", note: "line\nbreak" });
    ...
    expect(args).not.toContain("emails-api");
  ...
  test("requireCredential=false never sends an existing environment or vault credential", () => {
    installFakeSecrets(
      `{"${LEGACY_SELECTOR}":"server","HASNA_EMAILS_API_URL":"https://emails.example","HASNA_EMAILS_API_KEY":"vault-api-key-marker","${EMAILS_SESSION_TOKEN_ENV}":"vault-session-marker"}`,
    );
  ...
  test("falls back to the API key after a selected session token needs reauthentication", () => {
    ...
    const rows = apiStoreFor("domains").list();
  ...
  test("does not fall back from a live session with insufficient scope", () => {
    ...
    expect(() => apiStoreFor("domains").list()).toThrow(ApiHttpError);
  ...
  test("root probe rejects malformed JSON without leaking the response body", () => {
    ...
    expect(thrown).toBeInstanceOf(ApiWireResponseError);
  ...
  test("priority-sender-rules get and delete of a missing rule return absence, not a throw", () => {
    ...
    const store = apiStoreFor("priority-sender-rules");
  ...
  test("generic get and delete reject an undeclared 404 contract", () => {
    ...
    const store = apiStoreFor("not-a-resource");
    for (const operation of [
      () => store.get("missing"),
      () => store.del("missing"),
    ]) {
