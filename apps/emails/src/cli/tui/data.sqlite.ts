...
import { getEmailThreading, getThreadMessages, setInboundThreadId } from "../../db/threads.sqlite.js";
import { getLatestActiveProviderId, listProviderSummaries, listProviderNamesByIds } from "../../db/providers.sqlite.js";
import { listDomains } from "../../db/domains.sqlite.js";
import { findAddressesByEmail, getPreferredActiveAddressEmail, listActiveAddressCountsByDomains } from "../../db/addresses.sqlite.js";
...
import { createSentEmailLedger, setSentEmailThreading, storeSentEmailContent } from "../../lib/sent-ledger.sqlite.js";
...
export interface ListMailboxSourcesOptions {
  ...
  search?: string;
  /**
   * Include each source's latest-received timestamp. In API-client configuration this costs an
...
export async function sendComposed(input: ComposeInput, db?: Database): Promise<{ id: string; messageId: string }> {
  ...
  const useMd = !input.html && input.markdown !== false && input.body.trim().length > 0;
  const html = input.html ?? (useMd ? renderMarkdown(input.body) : undefined);
  ...
  const sendOpts = {
    ...
    ...(input.bcc ? { bcc: input.bcc.split(",").map((s) => s.trim()).filter(Boolean) } : {}),
    ...(input.replyToAddress ? { reply_to: input.replyToAddress } : {}),
  ...
  const { sendWithFailover } = await import("../../lib/send.sqlite.js");
