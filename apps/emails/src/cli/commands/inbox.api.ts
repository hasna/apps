...
import { confirmDestructiveAction, handleError, isCliJsonOutput, resolveId } from "../utils.js";
...
// src/cli/tui/data.api.ts already reads to resolve a `--source` ref in this
// mode. Only the INGESTION half (`sync-s3`) is server-owned.
//
// That registry USED to be a second copy inside src/lib/s3-sync.api.ts, which is
  ...
  inboxCmd
    ...
    .action(async (address: string, opts: { from?: string; subject?: string; limit?: string; refresh?: boolean; since?: string }) => {
      try {
        ...
        output(email, formatEmailDetail(email, { backend: ds.backend }));
  ...
  inboxCmd
    ...
    .action(async (opts: { provider?: string; source?: string; folder?: string; address?: string; domain?: string; since?: string; until?: string; limit?: string; offset?: string; search?: string; to?: string; unread?: boolean; read?: boolean; starred?: boolean; archived?: boolean; label?: string; filter?: string }) => {
      try {
        ...
        if (rows.length === 0) {
          // Every command named here is registered when the API client is configured. `emails
  ...
  inboxCmd
    ...
    .action(async (id: string, opts: { keepUnread?: boolean }) => {
      try {
        ...
        output(detail, formatEmailDetail(detail, { backend: ds.backend }));
      } catch (e) {
        handleError(e);
      }
    });
...
function formatEmailDetail(
  ...
  opts: { backend?: MailDataSource["backend"] } = {},
): string {
  const flags = [
  ...
  if (atts.length > 0) {
    ...
    for (const a of atts) {
      const missingLocation = opts.backend === "api"
    ...
    if (opts.backend === "api" && hasFetchableIndexes) {
