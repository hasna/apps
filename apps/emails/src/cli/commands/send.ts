      ...
      try {
        ...
        if (suppressedRecipients.length > 0) {
          ...
          if (opts.dryRun) {
            ...
            console.log(chalk.dim("  A real send would be refused; --force overrides for the local SQLite client only."));
          } else if (!opts.force) {
            handleError(new Error(
            ...
            handleError(new Error(
              `--force cannot override suppression for the API client: the server refuses a send to a `
