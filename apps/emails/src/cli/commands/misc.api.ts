...
// through the local provider pipeline (src/lib/send.sqlite.ts), the batch sender
...
// /v1/scheduled` exists and src/db/scheduled.api.ts is a complete client for
...
export function registerMiscCommands(program: Command, output: (data: unknown, formatted: string) => void): void {
  ...
  // `run_doctor` tool runs, and the same one src/cli/commands/misc.sqlite.ts imports.
  //
  // It no longer probes `/health` and `/ready` — the seam has no readiness operation
  // and reaching those endpoints would need the selector module the seam exists
