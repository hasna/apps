...
//
// This file exists rather than extending `email-log.sqlite.test.ts` because that
...
// coverage at all. The harness below is the one from `inbox.sqlite.test.ts`,
// which passes.
...
import { registerEmailLogCommands } from "./email-log.sqlite.js";

// Cleared by SHAPE, not by name, so this file adds no fresh spelling of the
// selector variable the hygiene guard is retiring (same rule as
// inbox.sqlite.test.ts, from which this harness is taken).
...
describe("local `emails search` covers received mail (db244cd4)", () => {
  ...
  it("returns received AND sent matches together", async () => {
    ...
    // "inbound" while correctly living in the Sent folder (data.sqlite.ts labels
    // the synced_sent branch 'inbound'). In API-client configuration `kind` reports
