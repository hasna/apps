...
import { getActiveProvider, getLatestActiveProviderId, getProvider } from "../../db/providers.sqlite.js";
...
export function registerMiscCommands(program: Command, output: (data: unknown, formatted: string) => void): void {
  ...
  // ─── DOCTOR ───────────────────────────────────────────────────────────────────
  // src/lib/doctor.ts is ONE implementation now, shared with misc.api.ts and the MCP
  ...
  // allowed to become is the one misc.api.ts refuses to register — accepted, documented,
