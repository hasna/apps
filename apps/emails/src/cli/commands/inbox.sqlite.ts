...
import { listProviderNamesByIds } from "../../db/providers.sqlite.js";
...
import { findAddressesByEmail } from "../../db/addresses.sqlite.js";
import { findDomainsByName } from "../../db/domains.sqlite.js";
...
// renders identically in local and API-client configuration.
...
async function runAutoPull(opts: { s3?: boolean; limit?: number }) {
  // Auto-pull is LOCAL S3 ingestion. In API-client configuration the API is the source of truth and
  // each poll re-reads it through the seam, so there is nothing to pull.
  if (resolveMailDataSource().backend !== "sqlite") return { pulled: 0, ok: true, configured: false, reason: "API client mode" };
  ...
  inboxCmd
    ...
    .action(async (emailId: string) => {
      try {
        ...
        const enrichedAddresses = new Map((await enrichAddresses(allAddresses, {
          providerDb: db,
          ownerStore: explainStore,
  ...
  inboxCmd
    ...
    .action(async (opts: { port?: string; provider?: string }) => {
      try {
        ...
        const { createSmtpServer } = await import("../../lib/inbound.sqlite.js");
