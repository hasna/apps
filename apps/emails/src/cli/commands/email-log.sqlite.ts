...
import { getLatestActiveProviderId, getProvider } from "../../db/providers.sqlite.js";
import { getPreferredActiveAddressEmail } from "../../db/addresses.sqlite.js";
...
import { listReplies, listReplySummaries, getReplyCount } from "../../db/inbound.sqlite.js";
...
export function registerEmailLogCommands(program: Command, output: (data: unknown, formatted: string) => void): void {
  ...
  emailCmd
    ...
    .action(async (id: string, opts: ReplyPageOpts) => {
      try {
        const db = getDatabase();
        const { getEmailThreading, getThreadMessages } = await import("../../db/threads.sqlite.js");
        const { getInboundEmail } = await import("../../db/inbound.sqlite.js");
        const { limit, offset } = parseReplyPage(opts);
