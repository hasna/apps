...
import { createAddress, markVerified } from "../../db/addresses.sqlite.js";
...
describe("email show command", () => {
  it("renders stored HTML as readable text", async () => {
    ...
    // the local ledger write lives in `src/lib/sent-ledger.sqlite.ts`, which is what the two
...
describe("webhook listen command", () => {
  ...
  // `src/lib/webhook.sqlite.ts` directly — bypassing the facade — so the consumer swap is exercised
