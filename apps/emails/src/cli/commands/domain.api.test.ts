// API-only: the domain repo routes every read/write to `/v1/domains`, so
...
// the `warm*` schedule commands route reads AND writes to the api HTTP
...
describe("domain CLI — api (api) /v1 routing", () => {
  it("add writes to the api API (not a local provider)", async () => {
    const { data } = await runDomainCommand(["domain", "add", "cloudy.example.com", "--provider", "api", "--send-only"]);
  ...
  it("list reads from the api API", async () => {
    await runDomainCommand(["domain", "add", "one.example.com", "--provider", "api", "--send-only"]);
    await runDomainCommand(["domain", "add", "two.example.com", "--provider", "api", "--send-only"]);
  ...
  it("domains list (plural) reads from the api API instead of blocking", async () => {
    await runDomainCommand(["domain", "add", "alpha.example.com", "--provider", "api", "--send-only"]);
    await runDomainCommand(["domain", "add", "beta.example.com", "--provider", "api", "--send-only"]);
  ...
  it("domains status (no arg) lists via the api API instead of blocking", async () => {
    await runDomainCommand(["domain", "add", "gamma.example.com", "--provider", "api", "--send-only"]);
    const { data } = await runDomainCommand(["domains", "status"]);
    const domains = data as Array<{ domain: string }>;
    expect(domains.map((d) => d.domain)).toEqual(["gamma.example.com"]);
  });

  it("domains status <domain> shows the API record in api", async () => {
    await runDomainCommand(["domain", "add", "delta.example.com", "--provider", "api", "--send-only"]);
  ...
  it("add is idempotent by name against the API client", async () => {
    await runDomainCommand(["domain", "add", "dup.example.com", "--provider", "api", "--send-only"]);
    await runDomainCommand(["domain", "add", "dup.example.com", "--provider", "api", "--send-only"]);
    expect((await serverDomains()).length).toBe(1);
  });

  it("remove deletes from the api API", async () => {
    await runDomainCommand(["domain", "add", "gone.example.com", "--provider", "api", "--send-only"]);
    const remote = await serverDomains();
  ...
  it("resolves an id PREFIX against the api dataset for remove", async () => {
    await runDomainCommand(["domain", "add", "prefix.example.com", "--provider", "api", "--send-only"]);
    const remote = await serverDomains();
    const id = remote[0]!["id"] as string;
    // An 8-char prefix must be resolved by listing the api store — proving
  ...
  it("refuses unshipped domain subcommands without claiming a server implements them", async () => {
    ...
    // CRUD and no route for any of them, so the old "it runs on the api
  ...
  it("domain adopt cannot run in the bare api client without a resolvable provider", async () => {
