...
import { closeApiPool, getApiPool, requireSigningSecret } from "../../server/api/env.js";
import { issueApiKey, listApiKeys, revokeApiKey, rotateToEmailsApiKey } from "../../server/api/keys.js";
...
async function keyStore(): Promise<{ store: ApiKeyStore; signingSecret: string }> {
  ...
  const store = new ApiKeyStore(getApiPool().client);
  await store.ensureSchema();
  return { store, signingSecret };
}

export function registerApiCommands(program: Command, output: (data: unknown, formatted: string) => void): void {
  const api = program.command("server").description("Operate your Emails API deployment");
  registerIdpPrincipalCommands(api, output);
  const key = api.command("key").description("Create, list, and revoke API keys");
  ...
  key.command("create")
    ...
    .action(async (opts: { scope: string[]; ttlDays: string; expiry: boolean; agent?: string }) => {
      try {
        ...
        const minted = await issueApiKey(store, signingSecret, {
          scopes: opts.scope,
  ...
  key.command("list")
    ...
    .action(async () => {
      try {
        ...
        const records = await listApiKeys(store);
  ...
  key.command("revoke <kid>")
    ...
    .action(async (kid: string, opts: { reason: string }) => {
      try {
        const { store } = await keyStore();
        const revoked = await revokeApiKey(store, kid, opts.reason);
