...
test.each(["settle", "late-rejection"] as const)("real Bun timed-out setup %s drains before pool cleanup", async (mode) => {
  const root = await mkdtemp(join(tmpdir(), "emails-setup-fixture-"));
  const home = join(root, "home"), events = join(root, "events.jsonl");
  await mkdir(home);
  const helper = resolve(import.meta.dir, "../../../scripts/serial-integration-fixture.ts");
  const source = `import { afterAll, beforeAll, test } from "bun:test";
import { appendFileSync } from "node:fs";
import { createSerialIntegrationFixture } from ${JSON.stringify(helper)};
const mode = ${JSON.stringify(mode)}, record = (value) => appendFileSync(${JSON.stringify(events)}, JSON.stringify(value) + "\\n");
const fixture = createSerialIntegrationFixture(1000);
const pool = { closed: false };
beforeAll(() => fixture.run(async () => {
  record("started"); await Bun.sleep(200);
  if (pool.closed) throw new Error("Pool closed while setup was still running");
  record("settled");
  if (mode === "late-rejection") throw new Error("owned late setup rejection");
}), 25);
test("body must not run after setup timeout", () => { record("body-ran"); });
afterAll(async () => {
  record("cleanup-started");
  await fixture.drain();
  pool.closed = true; record("closed");
}, 1500);
`;
  try {
    const file = join(root, "owned.test.ts");
    ...
    const result = await new Promise<{ status: number | null; output: string }>((accept, reject) => {
      ...
      child.stderr.on("data", chunk => { output += chunk; });
      child.once("error", error => { clearTimeout(timer); reject(error); });
      child.once("close", status => { clearTimeout(timer); expired ? reject(new Error("Owned Bun setup fixture exceeded process deadline")) : accept({ status, output }); });
    });
    expect(result.status).toBe(1); // Draining cleanup must not turn a timed-out setup into a pass.
    expect(result.output).toContain("hook timed out for this test");
    if (mode === "late-rejection") expect(result.output).toContain("error: owned late setup rejection");
    else expect(result.output).not.toContain("Unhandled error between tests");
    const markers = (await readFile(events, "utf8")).trim().split("\n").map(line => JSON.parse(line));
    expect(markers).toEqual(["started", "cleanup-started", "settled", "closed"]);
    expect(markers).not.toContain("body-ran");
  } finally { await rm(root, { recursive: true, force: true }); }
}, 5000);
