...
async function seedDomain(d: SelfHostedServiceDeps, domain: string, verified: boolean): Promise<void> {
  await d.store.createDomain({ domain, verified, status: "active" });
}

async function provisioningEvents(d: SelfHostedServiceDeps): Promise<Record<string, unknown>[]> {
...
describe("POST /v1/addresses — verified-domain default", () => {
  test("verified domain => address created verified with an automatic system audit", async () => {
    ...
    const res = await handleSelfHostedRequest(d, req(writeToken(), { email: "sender@example.com" }));
  ...
  test("unverified domain => address stays pending and no audit event is recorded", async () => {
    ...
    const res = await handleSelfHostedRequest(d, req(writeToken(), { email: "ops@pending.com" }));
    expect(res.status).toBe(201);
  ...
  test("no domain record => address stays pending", async () => {
    const d = deps();
    const res = await handleSelfHostedRequest(d, req(writeToken(), { email: "nobody@unknown.tld" }));
    expect(res.status).toBe(201);
    const created = ((await res.json()) as { address: { verified: boolean } }).address;
    expect(created.verified).toBe(false);
  });
  ...
  test("the send gate treats the auto-verified address as sendable", async () => {
    ...
    const d = deps();
    await seedDomain(d, "example.com", true);
    const createdRes = await handleSelfHostedRequest(d, req(writeToken(), { email: "ceo@example.com" }));
