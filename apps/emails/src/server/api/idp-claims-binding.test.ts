...
function deps(mappings: Map<string, MappingRow>): ApiServiceDeps {
...
async function resolve(d: ApiServiceDeps, token: string) {
  const url = new URL("http://api.test/v1/messages");
...
describe("GET /v1/tenants/{id} — an IdP principal can read its own tenant", () => {
  it("returns the tenant /v1/me just named instead of 404", async () => {
    ...
    const response = await handleApiRequest(d, get(`/v1/tenants/${TENANT_ID}`, token));
...
describe("send-intent reconciliation names the IdP principal", () => {
  it("records resolvedBy as idp:<sub>, never apikey:unknown", async () => {
    ...
    const response = await handleApiRequest(
      d,
      new Request("http://api.test/v1/messages/send-intents/reconcile", {
        method: "POST",
