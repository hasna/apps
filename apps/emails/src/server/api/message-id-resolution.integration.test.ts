...
// (handleApiRequest) and the REAL scoped store against a real Postgres
...
import { EmailsApiStore, type TenantScopedStore } from "./store.js";
...
describe.skipIf(!pgClient)("store.resolveMessageId (migration 0014 index)", () => {
  it("full uuid returns verbatim; unique prefix resolves; ambiguous -> ambiguous; none -> null", async () => {
    ...
    const store: TenantScopedStore = new EmailsApiStore(pgClient!).forTenant(tenantId);
  ...
  it("is tenant-scoped: a prefix matching only ANOTHER tenant's row -> null", async () => {
    ...
    const storeA: TenantScopedStore = new EmailsApiStore(pgClient!).forTenant(a.tenantId);
    const storeB: TenantScopedStore = new EmailsApiStore(pgClient!).forTenant(b.tenantId);
