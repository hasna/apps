...
describe("the published contract carries the reason, tolerantly", () => {
  const schemas = emailsApiOpenApi.components?.schemas as
  ...
  //   "Api GET /v1/messages returned an invalid successful response:
...
describe("the generated SDK type carries the reason", () => {
  ...
  it("keeps it readable from the DETAIL response too, via headers", () => {
    ...
    const client = readFileSync(join(HERE, "..", "..", "lib", "api-mail-data-source.ts"), "utf8");
