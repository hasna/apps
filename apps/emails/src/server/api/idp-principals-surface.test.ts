...
describe("listing grants", () => {
  test("an admin lists the tenant's grants; a member is refused", async () => {
    const { d, spy } = deps();
    const res = await handleApiRequest(d, req("GET", "/v1/idp-principals", { token: SESSION_TOKENS.admin }));
    ...
    const denied = await handleApiRequest(d, req("GET", "/v1/idp-principals", { token: SESSION_TOKENS.member }));
    expect(denied?.status).toBe(403);
  });
});
