...
import { EmailsApiStore, TenantScopedStore } from "./store.js";
...
import type { ApiSender } from "./sender.js";
import { formatApiSummaries, toApiSummary } from "../../cli/commands/email-log.api.js";
...
function makeDeps(sender: ApiSender): ApiServiceDeps {
  return {
    client: pgClient!,
...
async function call(
  ...
  const res = await handleApiRequest(deps, new Request(`http://svc${path}`, {
    method,
    headers,
...
async function registerSender(deps: ApiServiceDeps, token: string, domain: string, email: string): Promise<void> {
...
function withBrokenFinalization(deps: ApiServiceDeps): void {
...
describe.skipIf(!pgClient)("A: a message the provider ACCEPTED never reads as a failure", () => {
  ...
  it("and the CLI log renders an accepted send as sent, not as an unknown blank", async () => {
    ...
    expect(formatApiSummaries([summary], "log")).toContain("sent");
  });
});
...
describe.skipIf(!pgClient)("B: a message the provider REJECTED never reads as sent", () => {
  ...
  it("and the CLI log renders a rejected send as failed — never blank, never like delivered mail", async () => {
    ...
    const rendered = formatApiSummaries([summary], "log");
...
describe.skipIf(!pgClient)("C: uncertain sends can be found and closed out on evidence", () => {
  async function seedUncertain(slug: string, count: number): Promise<{ deps: ApiServiceDeps; token: string; keys: string[] }> {
    const deps = makeDeps({ provider: "ses", send: async () => { throw new TypeError("fetch failed"); } });
