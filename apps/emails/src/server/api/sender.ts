...
export type ApiSendProvider = "ses" | "resend";
 ...
 *                    existing operators; reported distinctly so nobody
 ...
 * `api_key`        — Resend.
 */
export type ApiSenderCredentialSource =
  | "environment"
  | "ambient_aws_env"
  | "deployment_role"
  | "api_key";
...
export interface ApiSender {
  readonly provider: ApiSendProvider;
  readonly credentialSource?: ApiSenderCredentialSource;
...
/**
 * SES credentials for the api sender come from the SERVER environment
 ...
 * behaviour open-source operators rely on.
 */
...
const SES_CREDENTIAL_SOURCE_LABEL: Record<"provider" | "ambient" | "chain", ApiSenderCredentialSource> = {
  provider: "environment",
  ambient: "ambient_aws_env",
  chain: "deployment_role",
};

function providerRecord(type: ApiSendProvider, env: NodeJS.ProcessEnv): Provider {
...
export function buildApiSender(env: NodeJS.ProcessEnv = process.env): ApiSender {
  const raw = env["EMAILS_SEND_PROVIDER"]?.trim().toLowerCase();
  if (raw !== "ses" && raw !== "resend") {
    throw new Error(
      "Emails api sending requires EMAILS_SEND_PROVIDER=ses or EMAILS_SEND_PROVIDER=resend. " +
