// `/api/v1` alias parity for the API service.
//
// The native client targets `/api/v1/...` + `Authorization: Bearer <key>`, while
// the api server's canonical surface is `/v1/...`. These tests prove that
...
async function assertAliasParity(
  ...
  const canonical = await snapshot(
    await handleApiRequest(deps(opts.configure), req(method, `/v1${suffix}`, opts)),
  );
  const aliased = await snapshot(
    await handleApiRequest(deps(opts.configure), req(method, `/api/v1${suffix}`, opts)),
  );
