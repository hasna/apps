// HTTP request handler for the Emails api service.
//
...
import { emailsApiOpenApi } from "./openapi.js";
import { resolveServerStorageBackend } from "../storage-backend.js";
import { resourceSpecForPath, type ApiResourceSpec } from "./resources.js";
...
import type { ApiKeyAdminStore } from "./keys.js";
...
import { canonicalizeApiPathname } from "../../lib/api-paths.js";
 ...
 */
const MAX_SEND_JSON_BODY_BYTES = requiredSendJsonBodyBytes(API_SEND_ATTACHMENT_LIMITS);
 ...
 */
async function readinessCheck(deps: ApiServiceDeps): Promise<ReadyResult> {
 ...
 * unassign). This is how the api client persists `assignAddressOwner` /
 ...
 */
function requireResourceWriteAuthority(auth: AuthOk, spec: ApiResourceSpec): Response | null {
 ...
 * 1:1 onto existing api handlers. Normalize those segments (AFTER the
 ...
 *
 *   client dialect               canonical api route
 *   ------------------------     ---------------------------
...
export async function handleApiRequest(
  ...
  const canonicalPathname = canonicalizeApiPathname(url.pathname);
  ...
  if (path === "/openapi.json" || path === "/v1/openapi.json") {
    return json(200, { ...emailsApiOpenApi, info: { ...emailsApiOpenApi.info, version: deps.version } });
  }
  ...
  try {
    ...
    if (path === SES_INBOUND_V1_WEBHOOK_PATH || path === RESEND_INBOUND_V1_WEBHOOK_PATH) {
      ...
      return path === SES_INBOUND_V1_WEBHOOK_PATH
        ? await handleApiSesWebhook(webhookDeps, req)
        : await handleApiResendWebhook(webhookDeps, req);
    }
    ...
    if (path === "/v1/messages/send") {
      ...
      if (rawAttachments.length > API_SEND_ATTACHMENT_LIMITS.maxFiles) {
        return json(400, { error: `at most ${API_SEND_ATTACHMENT_LIMITS.maxFiles} inline attachments are allowed` });
      }
      ...
      try {
        ...
        attachments = rawAttachments.map((value, index) => {
          ...
          if (!content || bytes > API_SEND_ATTACHMENT_LIMITS.maxBytesPerFile) {
            throw new Error(
              `attachment ${index} requires base64 content no larger than `
              + humanLimitBytes(API_SEND_ATTACHMENT_LIMITS.maxBytesPerFile),
            );
          }
          if (totalAttachmentBytes > API_SEND_ATTACHMENT_LIMITS.maxTotalBytes) {
            throw new Error(
              "inline attachments may total at most "
              + humanLimitBytes(API_SEND_ATTACHMENT_LIMITS.maxTotalBytes),
            );
          }
      ...
      if (!reserved.created) {
        ...
        if (reserved.record.send_state === "failed") {
          ...
          try {
            ...
            console.error("[emails-api] failed to re-arm a rejected send intent", {
      ...
      try {
        ...
        messageId = await deps.sender.send({
          provider_id: `api-${deps.sender.provider}`,
        ...
        console.error("[emails-api] provider send failed", {
      ...
      try {
        ...
        console.error("[emails-api] ledger finalization failed after provider accept", {
          message_id: claimed.id,
          provider: deps.sender.provider,
          provider_message_id: messageId,
          error: error instanceof Error ? `${error.name}: ${error.message}` : "UnknownError",
    ...
    console.error("[emails-api] request failed", {
      method,
      path,
