// Api-side SES-inbound ingestion worker for the Emails api service.
//
// Runs as a long-lived ECS task alongside the API (`emails-serve
...
// SES→S3 inbound bucket, normalizes it, and writes it to the SAME api
// Postgres `messages` table the /v1 API serves — so NEW inbound mail lands in
// the api automatically, with no per-machine step.
//
// Idempotency / dedup:
...
//     in `message_id` (the local→api history backfill stored the object key
...
//
// Amendment A1 (PURE REMOTE): the worker reads/writes the shared api Postgres
...
import { getApiPool, closeApiPool } from "./env.js";
 ...
 *   HASNA_EMAILS_DATABASE_URL        (required) — api Postgres DSN
 */
export async function runIngestWorker(options: WorkerOptions = {}): Promise<void> {
  ...
  const store = new EmailsApiStore(client);
 ...
 * tooling for bootstrapping or repairing an API deployment; steady-state
