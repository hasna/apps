// Generic /v1 resource registry for the Emails api api service.
//
// The original service handled only domains/addresses/messages by hand. The
// Api clients need the same resource CRUD vocabulary for the remaining
...
// list`, etc. read api data instead of the local SQLite island (the
...
// deliberately absent — the api resources carry only non-secret metadata.
...
export const API_RESOURCES: ApiResourceSpec[] = [
  {
  ...
  // ---- api-only parity resources -----------------------------------
  // Added so an API client covers EVERY resource the deleted local
  ...
  // ---- api-only parity resources (round 2) --------------------------
  // Every resource the api client routes to that had no server endpoint
...
export function resourceSpecForPath(path: string): ApiResourceSpec | undefined {
  return API_RESOURCES.find((r) => r.path === path);
}

/** The primary key a resource is addressed by (the ORDER BY tiebreaker). */
export function resourceKeyColumn(spec: ApiResourceSpec): string {
 ...
 */
export function resourceListOrderBy(spec: ApiResourceSpec): string {
