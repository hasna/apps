...
import { EmailsApiStore, LEGACY_INBOUND_MISSING_PAYLOAD_SCAN_SQL } from "./store.js";
...
describe.skipIf(!pg)("Row-Level Security backstop (Layer 2, migration 0013)", () => {
  ...
  it("legacy-inbound census: the real scan query fails closed with no GUC, and the store sets the GUC per tenant", async () => {
    ...
    try {
      ...
      const store = new EmailsApiStore(probeClient);
  ...
  it("migration 0018 is internally idempotent and keeps explicit USING/WITH CHECK policy", async () => {
    const recovery0018 = emailsApiMigrations().find((m) => m.id === "0018_send_intent_recovery")!;
  ...
  it("migration 0013 is internally idempotent: re-executing its SQL is a clean no-op", async () => {
    ...
    const rls0013 = emailsApiMigrations().find((m) => m.id === "0013_emails_tenancy_rls_and_seal")!;
