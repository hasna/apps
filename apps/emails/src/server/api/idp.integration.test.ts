...
import type { ApiServiceDeps } from "./service.js";
...
beforeAll(async () => {
  ...
  await new MigrationLedger(pg, emailsApiMigrations()).migrate();
  await pg.execute(
...
describe.skipIf(!pg)("migration 0021 — idp_principal_tenants", () => {
  ...
  it("is internally idempotent: re-executing its SQL is a clean no-op", async () => {
    const migration = emailsApiMigrations().find((m) => m.id === "0021_idp_principal_tenants")!;
