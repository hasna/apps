...
//
// So this suite points the client at `handleApiRequest` itself — the same router
 ...
 */
function serviceDeps(): ApiServiceDeps {
  return {
    client: pgClient!,
...
beforeAll(async () => {
  ...
  server = Bun.serve({
    ...
    async fetch(request: Request, bunServer): Promise<Response> {
      const response = await handleApiRequest(deps, request, {
