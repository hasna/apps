...
import { emailsApiMigrations } from "./migrations.js";
...
describe("Emails api service", () => {
  test("GET /health returns 200 with status/version/backend", async () => {
    ...
    const res = await handleApiRequest(deps(), req("GET", "/health"));
  ...
  test("operational probes never expose database error details", async () => {
    ...
    const health = await handleApiRequest(d, req("GET", "/health"));
    const ready = await handleApiRequest(d, req("GET", "/ready"));
  ...
  test("GET /ready requires exact migration ids and checksums", async () => {
    const cases = [
      {
        name: "pending",
        rows: emailsApiMigrations().slice(0, -1).map(({ id, checksum }) => ({ id, checksum })),
        issue: "pendingMigrations",
      },
      {
        name: "checksum drift",
        rows: emailsApiMigrations().map(({ id, checksum }, index) => ({ id, checksum: index === 0 ? "sha256:drift" : checksum })),
        issue: "checksum mismatch",
      },
      {
        name: "unknown newer migration",
        rows: [...emailsApiMigrations().map(({ id, checksum }) => ({ id, checksum })), { id: "9999_future", checksum: "sha256:future" }],
    ...
    for (const scenario of cases) {
      ...
      const res = await handleApiRequest(d, req("GET", "/ready"));
    ...
    d.client.many = async (sql) => sql.includes("schema_migrations")
      ? emailsApiMigrations().map(({ id, checksum }) => ({ id, checksum })) as never[]
      : [];
    const res = await handleApiRequest(d, req("GET", "/ready"));
    expect(res?.status).toBe(200);
  });
  ...
  test("GET /ready accepts the published 0006b compatibility checksum", async () => {
    const d = deps();
    d.client.many = async (sql) => sql.includes("schema_migrations")
      ? emailsApiMigrations().map(({ id, checksum }) => ({
        id,
  ...
  test("GET /version returns the version+backend shape", async () => {
    const previousDb = process.env["HASNA_EMAILS_DATABASE_URL"];
    process.env["HASNA_EMAILS_DATABASE_URL"] = "postgresql://test";
    const res = await handleApiRequest(deps(), req("GET", "/version"));
    const body = await res!.json();
  ...
  test("unknown non-v1 path falls through (null)", async () => {
    const res = await handleApiRequest(deps(), req("GET", "/dashboard"));
    expect(res).toBeNull();
  });

  test("/v1 without a key is rejected 401", async () => {
    const res = await handleApiRequest(deps(), req("GET", "/v1/domains"));
  ...
  test("PATCH /v1/messages/{id} writes body content and rejects unknown fields", async () => {
    ...
    d.store = { forTenant: () => scoped } as unknown as EmailsApiStore;

    const updated = await handleApiRequest(d, req("PATCH", `/v1/messages/${original.id}`, {
      token,
      body: {
        body_text: "after",
        body_html: "<p>after</p>",
    ...
    const refused = await handleApiRequest(d, req("PATCH", `/v1/messages/${original.id}`, {
      token,
  ...
  test("/v1 with a bad-signature key is rejected 401", async () => {
    ...
    const res = await handleApiRequest(deps(), req("GET", "/v1/domains", { token: forged }));
    expect(res?.status).toBe(401);
  });
  ...
  test("read-scoped key can GET but not POST (403 insufficient scope)", async () => {
    ...
    const listRes = await handleApiRequest(d, req("GET", "/v1/domains", { token: readToken }));
    expect(listRes?.status).toBe(200);
    const writeRes = await handleApiRequest(d, req("POST", "/v1/domains", { token: readToken, body: { domain: "x.com" } }));
  ...
  test("wrong-app key is rejected", async () => {
    ...
    const res = await handleApiRequest(deps(), req("GET", "/v1/domains", { token: otherApp }));
    expect(res?.status).toBe(401);
  });
  ...
  test("write-scoped key creates a domain (201) and it appears in the list", async () => {
    ...
    const create = await handleApiRequest(d, req("POST", "/v1/domains", { token: writeToken, body: { domain: "Example.COM" } }));
    expect(create?.status).toBe(201);
    const created = (await create!.json()).domain;
    expect(created.domain).toBe("example.com");
    const list = await handleApiRequest(d, req("GET", "/v1/domains", { token: writeToken }));
  ...
  test("message counts are exposed through the authenticated api API", async () => {
    const d = deps();
    ...
    const res = await handleApiRequest(d, req("GET", "/v1/messages/counts", { token }));
  ...
  test("per-address unread counts are served from the store with limit/offset forwarded", async () => {
    ...
    const token = mintApiKey({ app: "emails", scopes: ["emails:read"], signingSecret: SIGNING_SECRET }).token;
    const res = await handleApiRequest(d, req("GET", "/v1/messages/unread-by-address?limit=2&offset=1", { token }));
  ...
  test("attachment repair is authenticated, dry-run by default, bounded, and resumable by tenant-scoped run id", async () => {
    ...
    const token = mintApiKey({ app: "emails", scopes: ["emails:*"], signingSecret: SIGNING_SECRET }).token;
    const create = await handleApiRequest(d, req("POST", "/v1/attachments/repairs", {
      token,
      body: {
    ...
    const forbidden = await handleApiRequest(d, req("POST", `/v1/attachments/repairs/${repairRunId}/resume`, {
      token: readOnly,
      body: { limit: 1 },
    }));
    expect(forbidden?.status).toBe(403);

    const resume = await handleApiRequest(d, req("POST", `/v1/attachments/repairs/${repairRunId}/resume`, {
      token,
      body: { limit: 1 },
    }));
  ...
  test("message list rejects malformed cursors and unknown folders", async () => {
    ...
    const badFolder = await handleApiRequest(
      deps(),
  ...
  test("message groups and counts forward the domain scope to the store", async () => {
    ...
    const token = mintApiKey({ app: "emails", scopes: ["emails:read"], signingSecret: SIGNING_SECRET }).token;
    const groups = await handleApiRequest(d, req("GET", "/v1/messages/groups?domain=example.com", { token }));
    expect(groups?.status).toBe(200);
    expect((await groups!.json()).inbox).toBe(1);
    const counts = await handleApiRequest(d, req("GET", "/v1/messages/counts", { token }));
  ...
  test("redacts internal failures from 500 responses", async () => {
    ...
    console.error = () => {};
    try {
      const res = await handleApiRequest(d, req("GET", "/v1/domains", { token }));
  ...
  test("rejects oversized JSON bodies before parsing", async () => {
    ...
    const res = await handleApiRequest(deps(), request);
  ...
  test("POST with missing required field returns 400", async () => {
    ...
    const res = await handleApiRequest(deps(), req("POST", "/v1/domains", { token: writeToken, body: {} }));
    expect(res?.status).toBe(400);
  });
});
...
describe("POST /v1/messages/send attachment caps", () => {
  ...
  test("an attachment one byte over the per-file cap is still refused", async () => {
    const res = await handleApiRequest(
      ...
      req("POST", "/v1/messages/send", {
        token: sendToken(),
        body: sendBody([attachmentOf(API_SEND_ATTACHMENT_LIMITS.maxBytesPerFile + 1)]),
      }),
    );
    const body = await res!.json().catch(() => ({}));
  ...
  test("a set over the total cap is still refused BY THE TOTAL CAP", async () => {
    ...
    const perFile = API_SEND_ATTACHMENT_LIMITS.maxBytesPerFile;
    const files = [attachmentOf(perFile, "f0.pdf"), attachmentOf(perFile, "f1.pdf"), attachmentOf(1, "f2.pdf")];
    expect(files.length).toBeLessThanOrEqual(API_SEND_ATTACHMENT_LIMITS.maxFiles);
    const rawTotal = perFile * 2 + 1;
    expect(rawTotal).toBeGreaterThan(API_SEND_ATTACHMENT_LIMITS.maxTotalBytes);
    const payload = JSON.stringify(sendBody(files));
    expect(Buffer.byteLength(payload)).toBeLessThan(
      requiredSendJsonBodyBytes(API_SEND_ATTACHMENT_LIMITS),
    );
  ...
  test("more files than the count cap is still refused", async () => {
    const files = Array.from(
      { length: API_SEND_ATTACHMENT_LIMITS.maxFiles + 1 },
  ...
  test("the send route still refuses a body beyond its own raised budget", async () => {
    ...
    const request = new Request("http://svc/v1/messages/send", {
      ...
      headers: {
        ...
        "x-api-key": sendToken(),
        "content-length": String(requiredSendJsonBodyBytes(API_SEND_ATTACHMENT_LIMITS) + 1),
      },
