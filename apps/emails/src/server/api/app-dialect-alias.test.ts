// Native-client DIALECT compatibility for the API service.
//
...
// as its canonical api route — never a 404 — while the bare canonical
...
function deps(configure?: (d: ApiServiceDeps) => void): ApiServiceDeps {
  const client = fakeClient();
  const d: ApiServiceDeps = {
    client,
...
const stubReads = (d: ApiServiceDeps) => {
...
async function assertRoutesSame(
  ...
  opts: { auth?: Auth; body?: unknown; configure?: (d: ApiServiceDeps) => void } = {},
): Promise<{ status: number | undefined; body: unknown }> {
  const canonicalResult = await snapshot(
    await handleApiRequest(deps(opts.configure), req(canonical.method, canonical.path, opts)),
  );
  const clientResult = await snapshot(
    await handleApiRequest(deps(opts.configure), req(client.method, client.path, opts)),
  );
...
describe("client dialect — auth providers stub", () => {
  ...
  test("the providers probe is unauthenticated (no token required)", async () => {
    const res = await handleApiRequest(deps(), req("GET", "/api/v1/auth/providers"));
