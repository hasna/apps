// Bootstraps and runs the Emails api service (Bun.serve).
//
...
import { getApiPool, requireSigningSecret, API_APP, API_APP_ALIASES } from "./env.js";
...
import { buildApiSender } from "./sender.js";
...
export function buildApiService(version: string): ApiServiceDeps {
  ...
  console.log(
    `[emails-api] idp auth ${idpAuthenticator ? `jwks=${new URL(idpAuthenticator.jwksUrl).host}` : "disabled (no JWKS configured; idp tokens refused)"}`,
  );
  const sender = buildApiSender();
  ...
  console.log(
    `[emails-api] outbound provider=${sender.provider} credentials=${sender.credentialSource ?? "unknown"}`,
  );
  return {
    client,
    store: new EmailsApiStore(client),
    verifier,
    sender,
...
export async function startApiServer(
  ...
  const deps = buildApiService(version);
  ...
  const server = Bun.serve({
    ...
    fetch: async (req, bunServer) => {
      ...
      const response = await handleApiRequest(deps, req, {
  ...
  console.log(`Emails api service listening on http://${hostname}:${server.port}`);
