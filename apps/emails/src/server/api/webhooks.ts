// Api mount of the provider webhook receivers.
//
...
// the API destination: every write below goes through
// `EmailsApiStore.forTenant(...)`, i.e. the operator's own Postgres under
...
import type { EmailsApiStore, MessageInput, TenantScopedStore } from "./store.js";
import {
  receiveResendEvent,
...
export interface ApiWebhookDeps {
...
function env(deps: ApiWebhookDeps): NodeJS.ProcessEnv {
 ...
 */
export class ApiWebhookReceiptLedger implements WebhookReceiptLedger {
  private readonly scopeCache = new Map<string, string[]>();

  constructor(private readonly store: EmailsApiStore) {}
 ...
 */
export function apiInboundSource(
 ...
 */
function apiDeliveryEventSink(
  deps: ApiWebhookDeps,
  ledger: ApiWebhookReceiptLedger,
  provider: "sns" | "resend",
): DeliveryEventSink {
...
function apiResendInboundSink(deps: ApiWebhookDeps, ledger: ApiWebhookReceiptLedger): ResendInboundSink {
...
export async function handleApiSesWebhook(
  deps: ApiWebhookDeps,
  req: Request,
): Promise<Response> {
  ...
  const ledger = new ApiWebhookReceiptLedger(deps.store);
  return receiveSesNotification(req, {
    ledger,
    env: env(deps),
    inboundSource: () => apiInboundSource(env(deps)),
    ingest: apiSesIngest(deps, prefixDomainMappings),
    recordDeliveryEvent: apiDeliveryEventSink(deps, ledger, "sns"),
...
export function handleApiResendWebhook(deps: ApiWebhookDeps, req: Request): Promise<Response> {
  const ledger = new ApiWebhookReceiptLedger(deps.store);
  return receiveResendEvent(req, {
    ...
    storeInbound: apiResendInboundSink(deps, ledger),
    recordDeliveryEvent: apiDeliveryEventSink(deps, ledger, "resend"),
  });
}
