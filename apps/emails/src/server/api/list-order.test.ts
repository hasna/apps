...
describe("resourceListOrderBy", () => {
  ...
  it("appends the primary key to a non-unique clause", () => {
    ...
    expect(resourceListOrderBy(sources as ApiResourceSpec))
  ...
  it("uses the resource's own key column, not a hardcoded `id`", () => {
    ...
    const keyed = API_RESOURCES.find((spec) => spec.idColumn && spec.idColumn !== "id");
    expect(keyed, "fixture: at least one resource has a non-`id` key").toBeDefined();
    const clause = resourceListOrderBy(keyed as ApiResourceSpec);
    expect(orderTerms(clause)).toContain((keyed as ApiResourceSpec).idColumn);
  });

  it("does not duplicate a key the clause already sorts by", () => {
    const spec: ApiResourceSpec = { path: "x", table: "x", orderBy: "id ASC", columns: [] };
