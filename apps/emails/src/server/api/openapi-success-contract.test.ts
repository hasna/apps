...
describe("api OpenAPI success-response schemas", () => {
  it("keeps successes additive except for explicit attachment and redacted-record boundaries", () => {
    ...
    const componentSchemas = emailsApiOpenApi.components?.schemas as
  ...
  it("declares exact public-auth 403 contracts and a route-accurate verified user", () => {
    const globalUser = emailsApiOpenApi.components?.schemas?.User as
      | Schema
      | undefined;
