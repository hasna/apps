...
export async function handleAuthRoutes(
  ...
  try {
    ...
    // decide which sign-in affordances to render. This api service only
    ...
    console.error("[emails-api-auth] request failed", {
      path,
      method,
...
async function handleCreateKey(deps: AuthServiceDeps, req: Request, url: URL): Promise<Response> {
  ...
  let minted;
  try {
    minted = await issueApiKey(deps.keyStore, deps.signingSecret, {
      scopes,
      ttlDays,
      agent: ctx.tenantId,
...
async function handleRevokeKey(deps: AuthServiceDeps, req: Request, url: URL, kid: string): Promise<Response> {
  ...
  const done = await revokeApiKey(deps.keyStore, kid, "revoked by tenant admin");
