...
//
// The api auth surface is restricted to an OPERATOR-DECLARED set of email
...
// deployment on upgrade. `buildApiService` asserts the variable at boot so
...
export const MISSING_ALLOWED_EMAIL_DOMAINS_MESSAGE =
  `${ALLOWED_EMAIL_DOMAINS_ENV} is required: the api auth surface only ` +
