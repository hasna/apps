...
//
// The api unit tests build a fake in-memory query client and drive
// `handleApiRequest`. After multi-tenancy, `resolveRequestContext` maps an
 ...
 * base store because that is what ApiServiceDeps.store expects; the extra
 * data methods are only reached at runtime (tests are not type-checked).
 */
export function selfScopedStore(client: TypedQueryClient): EmailsApiStore {
  const scoped = new EmailsApiStore(client, { allowUnsafeTestTransactions: true }).forTenant(DEFAULT_TENANT_ID);
  (scoped as unknown as { forTenant: () => unknown }).forTenant = () => scoped;
  return scoped as unknown as EmailsApiStore;
}

/** A key store stub — the data-route tests never mint/list/revoke keys. */
export const STUB_KEY_STORE: ApiKeyAdminStore = {
  insertMinted: async () => {},
  list: async () => [],
  revoke: async () => false,
};
