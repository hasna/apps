// Server-side coverage for the api-only PARITY additions:
...
import { AddressProviderNotFoundError, CrossTenantReferenceError, EmailsApiStore } from "./store.js";
...
describe("api parity: new migrations", () => {
  test("0009 + 0010 are registered and append after 0008", () => {
    const ids = emailsApiMigrations().map((m) => m.id);
  ...
  test("released migration ids/checksums are unchanged by the append", () => {
    const released = Object.fromEntries(emailsApiMigrations().map((m) => [m.id, m.checksum]));
  ...
  test("0025 appends provider-aware address uniqueness after 0024", () => {
    const list = emailsApiMigrations();
  ...
  test("0009 seeds the three email agent settings rows and 0010 adds provisioning columns", () => {
    const m = Object.fromEntries(emailsApiMigrations().map((x) => [x.id, x.sql]));
...
describe("api parity: generic resource round-trips", () => {
  test("aliases create -> list -> get -> delete (protected bool round-trips)", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/aliases", {
      token: writeToken(),
    ...
    const list = await handleApiRequest(d, req("GET", "/v1/aliases", { token: readToken() }));
    expect((await list!.json()).items).toHaveLength(1);

    const get = await handleApiRequest(d, req("GET", `/v1/aliases/${row.id}`, { token: readToken() }));
    expect((await get!.json()).target_address).toBe("boss@x.com");

    const del = await handleApiRequest(d, req("DELETE", `/v1/aliases/${row.id}`, { token: writeToken() }));
    expect(del?.status).toBe(200);
    const gone = await handleApiRequest(d, req("GET", `/v1/aliases/${row.id}`, { token: readToken() }));
    expect(gone?.status).toBe(404);
  });

  test("forwarding rule persists the enabled boolean", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/forwarding", {
      token: writeToken(),
  ...
  test("warming schedule keeps target_daily_volume as an integer", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/warming", {
      token: writeToken(),
  ...
  test("triage round-trips priority (int) and confidence (real)", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/triage", {
      token: writeToken(),
  ...
  test("provisioning event stores detail_json as JSON", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/provisioning", {
      token: writeToken(),
  ...
  test("sources store settings_json + provider_snapshot_json objects", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/sources", {
      token: writeToken(),
  ...
  test("events store the metadata object", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/events", {
      token: writeToken(),
  ...
  test("email-agent-runs round-trip json arrays + numeric fields", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/email-agent-runs", {
      token: writeToken(),
      body: {
  ...
  test("email-digests round-trip highlight/action arrays and counts", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/email-digests", {
      token: writeToken(),
      body: {
  ...
  test("read scope may GET but not POST a parity resource", async () => {
    const d = deps();
    const list = await handleApiRequest(d, req("GET", "/v1/warming", { token: readToken() }));
    expect(list?.status).toBe(200);
    const post = await handleApiRequest(d, req("POST", "/v1/warming", { token: readToken(), body: { domain: "x.com", target_daily_volume: 1 } }));
    expect(post?.status).toBe(403);
  });
});

describe("api parity: natural-key email-agents (agent_key)", () => {
  test("create is keyed on agent_key and idempotent; get/update address by agent_key", async () => {
    const d = deps();
    const first = await handleApiRequest(d, req("POST", "/v1/email-agents", {
      token: writeToken(),
    ...
    const again = await handleApiRequest(d, req("POST", "/v1/email-agents", {
      token: writeToken(),
      body: { agent_key: "categorizer", enabled: false },
    }));
    expect(again?.status).toBe(201);
    const list = await handleApiRequest(d, req("GET", "/v1/email-agents", { token: readToken() }));
    expect((await list!.json()).items).toHaveLength(1);

    const get = await handleApiRequest(d, req("GET", "/v1/email-agents/categorizer", { token: readToken() }));
    expect(get?.status).toBe(200);
    expect((await get!.json()).agent_key).toBe("categorizer");

    const patch = await handleApiRequest(d, req("PATCH", "/v1/email-agents/categorizer", {
      token: writeToken(),
...
describe("api parity: mail-views", () => {
  test("GET /v1/messages/threads returns thread rollups (not treated as a message id)", async () => {
    ...
    const res = await handleApiRequest(d, req("GET", "/v1/messages/threads", { token: readToken() }));
  ...
  test("GET /v1/mailboxes returns mailboxes + folder counts", async () => {
    ...
    const res = await handleApiRequest(d, req("GET", "/v1/mailboxes", { token: readToken() }));
    expect(res?.status).toBe(200);
  ...
  test("GET /v1/messages/{id}/raw reconstructs MIME, 404 when missing", async () => {
    ...
    const ok = await handleApiRequest(d, req("GET", "/v1/messages/m1/raw", { token: readToken() }));
    expect(ok?.status).toBe(200);
    expect((await ok!.json()).raw).toContain("From: a@x.com");
    const missing = await handleApiRequest(d, req("GET", "/v1/messages/nope/raw", { token: readToken() }));
    expect(missing?.status).toBe(404);
  });

  test("mail-views require auth", async () => {
    const d = deps();
    expect((await handleApiRequest(d, req("GET", "/v1/mailboxes")))?.status).toBe(401);
    expect((await handleApiRequest(d, req("GET", "/v1/messages/threads")))?.status).toBe(401);
  });
...
describe("api parity: provisioning fields on domains/addresses", () => {
  test("PATCH /v1/domains applies provisioning fields via applyDomainProvisioning", async () => {
    ...
    const res = await handleApiRequest(d, req("PATCH", "/v1/domains/dom1", {
      token: writeToken(),
  ...
  test("PATCH /v1/addresses applies provisioning fields via applyAddressProvisioning", async () => {
    ...
    const res = await handleApiRequest(d, req("PATCH", "/v1/addresses/a1", {
      token: writeToken(),
...
describe("api parity: round-2 generic resources", () => {
  test("group-members create -> list -> get (vars round-trips) -> delete", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/group-members", {
      token: writeToken(),
    ...
    const list = await handleApiRequest(d, req("GET", "/v1/group-members?group_id=g1", { token: readToken() }));
    expect((await list!.json()).items).toHaveLength(1);

    const get = await handleApiRequest(d, req("GET", `/v1/group-members/${row.id}`, { token: readToken() }));
    expect((await get!.json()).email).toBe("a@x.com");

    const del = await handleApiRequest(d, req("DELETE", `/v1/group-members/${row.id}`, { token: writeToken() }));
    expect(del?.status).toBe(200);
  });

  test("sequence-steps keep step_number/delay_hours as integers", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/sequence-steps", {
      token: writeToken(),
  ...
  test("sequence-enrollments round-trip current_step + status, filter by sequence_id", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/sequence-enrollments", {
      token: writeToken(),
      body: {
    ...
    const patch = await handleApiRequest(d, req("PATCH", `/v1/sequence-enrollments/${row.id}`, {
      token: writeToken(),
      body: { status: "cancelled" },
    }));
    expect((await patch!.json()).status).toBe("cancelled");

    const list = await handleApiRequest(d, req("GET", "/v1/sequence-enrollments?sequence_id=s1", { token: readToken() }));
    expect((await list!.json()).items).toHaveLength(1);
  });
  ...
  test("address-ownership-events honor the client-supplied id (create then GET by that id)", async () => {
    ...
    const create = await handleApiRequest(d, req("POST", "/v1/address-ownership-events", {
      token: writeToken(),
      body: {
    ...
    const get = await handleApiRequest(d, req("GET", `/v1/address-ownership-events/${clientId}`, { token: readToken() }));
    expect(get?.status).toBe(200);
  ...
  test("webhook-receipts create + list (append-only ledger)", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/webhook-receipts", {
      token: writeToken(),
    ...
    const list = await handleApiRequest(d, req("GET", "/v1/webhook-receipts", { token: readToken() }));
    expect((await list!.json()).items).toHaveLength(1);
  });

  test("sandbox-emails round-trip raw arrays + pre-serialized attachments/headers", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/sandbox-emails", {
      token: writeToken(),
      body: {
    ...
    const list = await handleApiRequest(d, req("GET", "/v1/sandbox-emails?provider_id=sandbox", { token: readToken() }));
    expect((await list!.json()).items).toHaveLength(1);
  });
});

describe("api parity: address ownership over /v1/addresses", () => {
...
describe("api parity: address provider binding over /v1/addresses", () => {
  test("create persists provider_id, supports omitted providers, and rejects unknown providers", async () => {
    ...
    const response = await handleApiRequest(d, req("POST", "/v1/addresses", {
      token: writeToken(),
  ...
  test("POST maps an unknown provider to a scoped 404 without changing address policy", async () => {
    ...
    const res = await handleApiRequest(d, req("POST", "/v1/addresses", {
      token: writeToken(),
...
describe("api parity: scoped send-key mint/verify routing", () => {
  test("POST /v1/send-keys/mint returns the one-time token + summary (not read as an id)", async () => {
    ...
    const res = await handleApiRequest(d, req("POST", "/v1/send-keys/mint", { token: writeToken(), body: { owner_id: "o1", label: "ci" } }));
  ...
  test("POST /v1/send-keys/mint requires owner_id and write scope", async () => {
    ...
    const missing = await handleApiRequest(d, req("POST", "/v1/send-keys/mint", { token: writeToken(), body: {} }));
    expect(missing?.status).toBe(400);
    const forbidden = await handleApiRequest(d, req("POST", "/v1/send-keys/mint", { token: readToken(), body: { owner_id: "o1" } }));
  ...
  test("POST /v1/send-keys/verify resolves a token and confirms from-address scope", async () => {
    ...
    const valid = await handleApiRequest(d, req("POST", "/v1/send-keys/verify", { token: writeToken(), body: { token: "esk_ok" } }));
    expect(valid?.status).toBe(200);
    expect(await valid!.json()).toMatchObject({ valid: true, authorized: false });

    const owned = await handleApiRequest(d, req("POST", "/v1/send-keys/verify", { token: writeToken(), body: { token: "esk_ok", from: "mine@x.com" } }));
    expect((await owned!.json()).authorized).toBe(true);

    const foreign = await handleApiRequest(d, req("POST", "/v1/send-keys/verify", { token: writeToken(), body: { token: "esk_ok", from: "victim@x.com" } }));
    expect((await foreign!.json()).authorized).toBe(false);

    const bad = await handleApiRequest(d, req("POST", "/v1/send-keys/verify", { token: writeToken(), body: { token: "esk_bad" } }));
    expect(await bad!.json()).toEqual({ valid: false, authorized: false, key: null });

    const noToken = await handleApiRequest(d, req("POST", "/v1/send-keys/verify", { token: writeToken(), body: {} }));
  ...
  test("send-key verify rejects the read scope (it mutates last_used_at)", async () => {
    ...
    const res = await handleApiRequest(d, req("POST", "/v1/send-keys/verify", { token: readToken(), body: { token: "esk_x" } }));
