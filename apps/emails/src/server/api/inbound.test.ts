...
import { API_SEND_ATTACHMENT_LIMITS } from "../../lib/send-attachment-limits.js";
...
// Column order the store inserts with (see EmailsApiStore.INSERT_COLS).
const COLS = [
...
describe("Emails api inbound messages", () => {
  test("migration set includes the inbound schema migration", () => {
    ...
    expect(ids).toContain("0020_attachment_repair_ledger");
    ...
    expect(ids).toContain("0026_mailbox_filters");
    ...
    expect(ids).toContain("0026_priority_sender_rules");
  ...
  test("POST inbound preserves all fields and returns 201", async () => {
    const res = await handleApiRequest(deps(), post(INBOUND));
  ...
  test("inbound is inferred from received_at when direction is omitted", async () => {
    ...
    const res = await handleApiRequest(deps(), post({ ...noDirection, source_id: "x" }));
    const msg = (await res!.json()).message;
    expect(msg.direction).toBe("inbound");
  });
  ...
  test("re-POST with the same source_id is idempotent (upsert, no duplicate)", async () => {
    const d = deps();
    const first = await handleApiRequest(d, post(INBOUND));
    expect(first?.status).toBe(201);
    const second = await handleApiRequest(d, post({ ...INBOUND, is_read: true }));
    expect(second?.status).toBe(200); // updated, not created
    const list = await handleApiRequest(d, req(d, "GET"));
  ...
  test("GET /v1/messages orders by original receipt time (newest first)", async () => {
    const d = deps();
    await handleApiRequest(d, post({ ...INBOUND, source_id: "older", received_at: "2026-06-01T00:00:00.000Z" }));
    await handleApiRequest(d, post({ ...INBOUND, source_id: "newer", received_at: "2026-06-30T00:00:00.000Z" }));
    const list = await handleApiRequest(d, req(d, "GET"));
  ...
  test("GET /v1/messages omits full bodies while detail keeps them API-only", async () => {
    const d = deps();
    const created = await handleApiRequest(d, post({
      ...INBOUND,
      source_id: "lean-list",
    ...
    const detail = await handleApiRequest(d, new Request(
  ...
  test("keeps attachment bytes out of message reads and serves them from the authenticated attachment route", async () => {
    ...
    const attachment = await handleApiRequest(d, new Request(
    ...
    const abbreviated = await handleApiRequest(d, new Request(
  ...
  test("distinguishes absent content/index and rejects oversized or malformed stored bytes", async () => {
    ...
    const get = (index: number, suffix = "") => handleApiRequest(d, new Request(
  ...
  test("preserves a primitive attachment gap without advertising content or shifting later indexes", async () => {
    ...
    const get = (index: number) => handleApiRequest(d, new Request(
  ...
  test("send endpoint invokes the configured provider before persisting", async () => {
    ...
    const res = await handleApiRequest(d, new Request("http://svc/v1/messages/send", {
      method: "POST",
  ...
  it("looks up and cancels send intents without invoking the provider", async () => {
    ...
    const recovery = (operation: "lookup" | "cancel", key: string) => handleApiRequest(
      d,
  ...
  it("rejects a late send after a cancellation tombstone before provider I/O", async () => {
    ...
    const cancelled = await handleApiRequest(d, new Request("http://svc/v1/messages/send-intents/cancel", {
      method: "POST", headers, body: JSON.stringify({ idempotency_key: key }),
    }));
    expect(cancelled?.status).toBe(200);
    const late = await handleApiRequest(d, new Request("http://svc/v1/messages/send", {
      method: "POST",
      headers,
  ...
  it("validates recovery keys without reflecting them or invoking the provider", async () => {
    ...
    for (const idempotencyKey of ["x".repeat(201), "unsafe\nkey", "\uD800"]) {
      const res = await handleApiRequest(d, new Request("http://svc/v1/messages/send-intents/cancel", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": writeToken() },
  ...
  it("returns the same completed intent on retry without sending twice", async () => {
    ...
    const first = await handleApiRequest(d, request());
    const second = await handleApiRequest(d, request());
  ...
  it("refuses to delete a completed intent and preserves its replay fence", async () => {
    ...
    const deletion = await handleApiRequest(d, new Request(
    ...
    const replay = await handleApiRequest(d, request());
  ...
  it("refuses deletion while provider delivery is in flight", async () => {
    ...
    const first = handleApiRequest(d, request());
    await Bun.sleep(1);
    ...
    const concurrentRetry = await handleApiRequest(d, request());
  ...
  it("keeps ordinary inbound messages deletable", async () => {
    const d = deps();
    const created = await handleApiRequest(d, post({ ...INBOUND, source_id: "delete-inbound" }));
  ...
  it("refuses to reuse an idempotency key for a different payload", async () => {
    const d = deps();
    const send = (subject: string) => handleApiRequest(d, new Request("http://svc/v1/messages/send", {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": writeToken() },
  ...
  it("rejects malformed or oversized inline attachments before reserving a send", async () => {
    const d = deps();
    const send = (content: string) => handleApiRequest(d, new Request("http://svc/v1/messages/send", {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": writeToken() },
      body: JSON.stringify({
    ...
    // its own name.
    const overCap = Buffer.alloc(API_SEND_ATTACHMENT_LIMITS.maxBytesPerFile + 1);
  ...
  test("POST still requires from and to", async () => {
    const noFrom = await handleApiRequest(deps(), post({ to: ["a@b.com"] }));
    expect(noFrom?.status).toBe(400);
    const noTo = await handleApiRequest(deps(), post({ from: "a@b.com" }));
    expect(noTo?.status).toBe(400);
  });
});

function req(d: ApiServiceDeps, method: string): Request {
  void d;
  return new Request("http://svc/v1/messages", {
    method,
    headers: { "Content-Type": "application/json", "x-api-key": writeToken() },
