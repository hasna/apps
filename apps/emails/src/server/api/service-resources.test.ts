...
// providers, groups, …) exposed to api clients: routing, auth/scope
...
describe("api service generic resources", () => {
  test("migration 0005 is registered", () => {
    expect(emailsApiMigrations().some((m) => m.id === "0005_mailery_selfhosted_resources")).toBe(true);
    expect(emailsApiMigrations().some((m) => m.id === "0006_emails_rename_bridge")).toBe(true);
    expect(emailsApiMigrations().some((m) => m.id === "0006b_emails_legacy_messages_backfill_prep")).toBe(true);
    expect(emailsApiMigrations().some((m) => m.id === "0007_emails_legacy_messages_backfill")).toBe(true);
    expect(emailsApiMigrations().some((m) => m.id === "0008_emails_legacy_messages_backfill_dedupe")).toBe(true);
  });

  test("preserves the released Mailery migration ids and checksums before the rename bridge", () => {
    const released = Object.fromEntries(emailsApiMigrations().map((migration) => [migration.id, migration.checksum]));
    ...
    const bridge = emailsApiMigrations().find((migration) => migration.id === "0006_emails_rename_bridge");
    ...
    const ids = emailsApiMigrations().map((migration) => migration.id);
  ...
  test("legacy backfill hardening preserves immutable 0007 and repairs around it", () => {
    const migrations = Object.fromEntries(emailsApiMigrations().map((migration) => [migration.id, migration]));
  ...
  test("GET /v1/contacts requires auth (401)", async () => {
    const res = await handleApiRequest(deps(), req("GET", "/v1/contacts"));
    expect(res?.status).toBe(401);
  });

  test("read scope can GET but not POST contacts", async () => {
    const d = deps();
    const list = await handleApiRequest(d, req("GET", "/v1/contacts", { token: readToken() }));
    expect(list?.status).toBe(200);
    expect((await list!.json()).items).toEqual([]);
    const post = await handleApiRequest(d, req("POST", "/v1/contacts", { token: readToken(), body: { email: "a@x.com" } }));
    expect(post?.status).toBe(403);
  });

  test("create -> list -> get -> delete round-trips a contact", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/contacts", { token: writeToken(), body: { email: "a@x.com", name: "A", send_count: 2, suppressed: true } }));
    ...
    const list = await handleApiRequest(d, req("GET", "/v1/contacts", { token: writeToken() }));
    expect((await list!.json()).items).toHaveLength(1);

    const get = await handleApiRequest(d, req("GET", `/v1/contacts/${created.id}`, { token: writeToken() }));
    expect(get?.status).toBe(200);
    expect((await get!.json()).email).toBe("a@x.com");

    const del = await handleApiRequest(d, req("DELETE", `/v1/contacts/${created.id}`, { token: writeToken() }));
    expect(del?.status).toBe(200);
    const get2 = await handleApiRequest(d, req("GET", `/v1/contacts/${created.id}`, { token: writeToken() }));
  ...
  test("generic list filters bind canonical boolean and integer values", async () => {
    ...
    const scoped = new EmailsApiStore(client).forTenant("tenant-1");
  ...
  test("send-keys (hyphenated path) and scheduled resolve and never expose secrets", async () => {
    const d = deps();
    const create = await handleApiRequest(d, req("POST", "/v1/send-keys", { token: writeToken(), body: { owner_id: "o1", prefix: "esk_abc", label: "ci" } }));
    expect(create?.status).toBe(201);
    const row = await create!.json();
    expect(row).not.toHaveProperty("key_hash");
    const sched = await handleApiRequest(d, req("POST", "/v1/scheduled", { token: writeToken(), body: { subject: "hi", to_addresses: ["b@x.com"], status: "pending" } }));
  ...
  test("tenant-scoped Postgres content write updates all three fields under the tenant fence", async () => {
    ...
    const scoped = new EmailsApiStore(client).forTenant("tenant-content");
  ...
  test("unknown resource still 404s", async () => {
    const res = await handleApiRequest(deps(), req("GET", "/v1/nonsense", { token: readToken() }));
