...
const stubKeyStore: ApiKeyStore = {
  insertMinted: async () => {},
  list: async () => [],
  revoke: async () => false,
};
...
function makeDeps(options: {
...
} = {}): { deps: ApiServiceDeps; fetched: string[] } {
  ...
  const deps: ApiServiceDeps = {
    client: pgClient!,
    store: new EmailsApiStore(pgClient!),
...
beforeAll(async () => {
  ...
  await new MigrationLedger(pgClient, emailsApiMigrations()).migrate();
...
describe.skipIf(!pgClient)("SES inbound webhook lands in the operator's Postgres", () => {
  it("stores a signed Received notification and reads it back through the tenant store", async () => {
    ...
    const body = await json(await handleApiRequest(deps, snsPost(snsEnvelope({
      Type: "Notification",
  ...
  it("replaying the same SNS MessageId leaves exactly one message and one receipt", async () => {
    ...
    const first = await json(await handleApiRequest(deps, snsPost(envelope)));
    const second = await json(await handleApiRequest(deps, snsPost(envelope)));
  ...
  it("an invalid SNS signature writes nothing at all", async () => {
    ...
    const response = await handleApiRequest(deps, snsPost(snsEnvelope({
      Type: "Notification",
      Message: JSON.stringify({
        notificationType: "Received",
...
describe.skipIf(!pgClient)("Resend inbound webhook lands in the operator's Postgres", () => {
  it("stores a signed inbound payload and reads it back through the tenant store", async () => {
    ...
    const body = await json(await handleApiRequest(deps, await resendPost({
  ...
  it("replaying the same svix-id leaves exactly one message", async () => {
    ...
    const first = await json(await handleApiRequest(deps, await resendPost(payload, { id: "svix-replay" })));
    const second = await json(await handleApiRequest(deps, await resendPost(payload, { id: "svix-replay" })));
  ...
  it("a re-delivery under a NEW svix-id does not mark an already-read message unread", async () => {
    ...
    const first = await json(await handleApiRequest(deps, await resendPost(payload, { id: "svix-reread-1" })));
    ...
    const second = await json(await handleApiRequest(deps, await resendPost(payload, { id: "svix-reread-2" })));
  ...
  it("a wrongly-signed payload writes nothing", async () => {
    ...
    const { deps } = makeDeps({ resendSecret: RESEND_SECRET });
    const response = await handleApiRequest(deps, await resendPost({
      type: "inbound.email.received",
...
describe.skipIf(!pgClient)("delivery outcomes land in the operator's Postgres", () => {
  it("a signed SES bounce is persisted in the sending tenant's scope and joined to the send", async () => {
    ...
    const sent = await new EmailsApiStore(pgClient!).forTenant(tenantId).createMessage({
  ...
  it("enforces provider-event uniqueness within a tenant but not across tenants", async () => {
    ...
    await new EmailsApiStore(pgClient!).forTenant(tenantA).createResource(EVENTS_SPEC, eventBody);
    await expect(
      new EmailsApiStore(pgClient!).forTenant(tenantA).createResource(EVENTS_SPEC, eventBody),
    ).rejects.toThrow();
    await expect(
      new EmailsApiStore(pgClient!).forTenant(tenantB).createResource(EVENTS_SPEC, eventBody),
