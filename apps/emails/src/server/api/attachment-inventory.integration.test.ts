...
//
// Runs the REAL request pipeline (handleApiRequest) against a real
...
const stubKeyStore: ApiKeyStore = { insertMinted: async () => {}, list: async () => [], revoke: async () => false };
...
async function call(deps: ApiServiceDeps, method: string, path: string, opts: { token?: string; body?: unknown } = {}) {
  const res = await handleApiRequest(deps, reqOf(method, path, opts));
...
function completeAttachmentRepairRuns(deps: ApiServiceDeps): void {
...
async function drainInventory(deps: ApiServiceDeps, token: string, limit: number): Promise<any[]> {
  const items: any[] = [];
...
describe.skipIf(!pgClient)("checkpointed legacy attachment repair ledger", () => {
  ...
  it("enforces tenant-local active and durable quotas and recovers an active slot after completion", async () => {
    ...
    const constrained = new EmailsApiStore(pgClient!, {
    ...
    const fixture = async (
      tenant: { tenantId: string; token: string },
      store: ReturnType<EmailsApiStore["forTenant"]>,
      suffix: string,
    ) => {
      const messageId = await importMsg(deps, tenant.token, {
