...
// ── which identity the api sender signs with ────────────────────────
//
...
describe("buildApiSender — SES credential resolution", () => {
  ...
  it("falls back to the deployment IAM role when nothing explicit is available", () => {
    ...
    try {
      ...
      expect(buildApiSender({ ...base }).credentialSource).toBe("deployment_role");
    } finally {
  ...
  it("reports the identity the ADAPTER actually signs with, not the configured intent", () => {
    ...
    try {
      ...
      expect(buildApiSender({ ...base }).credentialSource).toBe("ambient_aws_env");
      // …and a scoped pair still wins over it.
      expect(buildApiSender({
        ...base,
        EMAILS_SES_ACCESS_KEY_ID: "AKIA-SERVER",
        EMAILS_SES_SECRET_ACCESS_KEY: "server-secret",
  ...
  it("refuses half a scoped pair instead of silently completing it from the ambient chain", () => {
    expect(() => buildApiSender({ ...base, EMAILS_SES_ACCESS_KEY_ID: "AKIA-SERVER" }))
      .toThrow(/must be set together/);
    expect(() => buildApiSender({ ...base, EMAILS_SES_SECRET_ACCESS_KEY: "server-secret" }))
      .toThrow(/must be set together/);
  });
