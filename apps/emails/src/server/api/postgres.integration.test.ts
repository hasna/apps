...
import { resourceSpecForPath, API_RESOURCES } from "./resources.js";
...
/**
 * The writes the api server actually makes, one per drift class, each
...
describe("api Postgres integration", () => {
  it.skipIf(!client)("migrates dirty text legacy rows and enforces durable send idempotency", async () => {
    ...
    const ledger = new MigrationLedger(client!, emailsApiMigrations());
    ...
    const store = new EmailsApiStore(client!).forTenant(DEFAULT_TENANT_ID);
  ...
  it.skipIf(!client)("0025 preserves provider-scoped address identity with tenant isolation", async () => {
    await resetPublicSchema();
    await new MigrationLedger(client!, emailsApiMigrations()).migrate();
    ...
    const root = new EmailsApiStore(client!);
  ...
  it.skipIf(!client)("0022 rejects new non-enum event types without deleting legacy poison", async () => {
    await resetPublicSchema();
    const migrations = emailsApiMigrations();
    ...
    const scoped = new EmailsApiStore(client!).forTenant(DEFAULT_TENANT_ID);
  ...
  it.skipIf(!client)("reconciles the fully-drifted ad-hoc prod schema (checks / FKs / missing cols / NOT NULL / defaults)", async () => {
    ...
    await client!.execute(`
      -- Legacy tables the drifted FKs point at (the api server no longer
    ...
    const rerun = await new MigrationLedger(client!, emailsApiMigrations()).migrate();
  ...
  it.skipIf(!client || !hasProdDump)("migrates the REAL prod schema dump and every server write succeeds", async () => {
    ...
    // CRITICAL: the api server can write every resource against the real
  ...
  it.skipIf(!client)("0012 creates identity tables, backfills tenant_id to the default tenant, and swaps uniques (drift-aware + idempotent)", async () => {
    ...
    const resourceTables = new Set(API_RESOURCES.map((r) => r.table));
    for (const t of IDENTITY_TABLES) {
  ...
  it.skipIf(!client)("0012 dedups pre-existing duplicate data (deterministic survivor + FK reparent) before building the composite uniques", async () => {
    ...
    const mig0012 = emailsApiMigrations().find(
  ...
  it.skipIf(!client)("0016 removes pending, unverified, and stale legacy inbound routes", async () => {
    ...
    const seal = emailsApiMigrations().find(
  ...
  it.skipIf(!client)("captures the cutover fence from PostgreSQL before migration 0017 exists", async () => {
    ...
    const fenceAt = await new EmailsApiStore(client!).captureInboundProvenanceFence();
  ...
  it.skipIf(!client)("proves a pre-fence writer can commit after the cutoff yet remain outside the audit", async () => {
    ...
    const fenceAt = await new EmailsApiStore(coordinatedClient).captureInboundProvenanceFence();
    const committedWriter = await client!.one<{ created_at: Date }>(
      `SELECT created_at FROM messages WHERE id = 'audit-fence-race'`,
    );
    const result = await new EmailsApiStore(client!).auditInboundSourceProvenance({
      since: fenceAt,
      canonicalBucket,
    });
