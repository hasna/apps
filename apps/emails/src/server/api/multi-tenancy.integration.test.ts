...
import { API_APP, API_APP_ALIASES } from "./env.js";
...
function makeDeps(): { deps: ApiServiceDeps; sent: Captured[] } {
  const sent: Captured[] = [];
...
describe.skipIf(!pgClient)("RBAC hardening (adversarial review fixes)", () => {
  async function loginSession(deps: ApiServiceDeps, email: string, password: string): Promise<string> {
    return (await call(deps, "POST", "/v1/auth/login", { body: { email, password } })).body.session_token;
  }
  async function membershipIdByEmail(deps: ApiServiceDeps, session: string, tenantId: string, email: string): Promise<string> {
...
describe.skipIf(!pgClient)("tenant-scoped key issuance (WI-2e)", () => {
  it("an owner mints a tenant key that resolves back to the same tenant", async () => {
    ...
    const depsWithKeys: ApiServiceDeps = { ...deps, keyStore: realKeyStore };
