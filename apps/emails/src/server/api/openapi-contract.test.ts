...
import { API_RESOURCES } from "./resources.js";
...
const paths = emailsApiOpenApi.paths as Record<string, Record<string, Operation>>;
...
describe("api OpenAPI identity and authorization contract", () => {
  it("publishes the runtime global-role vocabulary", () => {
    const userSchema = emailsApiOpenApi.components?.schemas?.User as
  ...
  it("declares both accepted credential transports and explicitly marks public routes", () => {
    expect(emailsApiOpenApi.security).toEqual([{ apiKeyAuth: [] }, { bearerAuth: [] }]);
    expect(emailsApiOpenApi.components?.securitySchemes).toMatchObject({
  ...
  it("formalizes scoped sender authorization on the send operation", () => {
    ...
    const sendError = emailsApiOpenApi.components?.schemas?.SendMessageError as
  ...
  it("publishes body-only send-intent recovery operations", () => {
    ...
    expect(emailsApiOpenApi.components?.schemas?.SendIntentMessage).toMatchObject({
      type: "object",
  ...
  it("publishes a bounded typed attachment-content operation", () => {
    ...
    const schema = emailsApiOpenApi.components?.schemas?.AttachmentContent as
  ...
  it("publishes availability-only inventory metadata and checkpointed repair operations", () => {
    const inventory = emailsApiOpenApi.components?.schemas?.AttachmentInventoryItem as
      | { required?: string[]; properties?: Record<string, unknown> }
      | undefined;
    const batch = emailsApiOpenApi.components?.schemas?.AttachmentBatchMeta as
      | { required?: string[]; properties?: Record<string, unknown> }
      | undefined;
    const message = emailsApiOpenApi.components?.schemas?.AttachmentMeta as
      | { required?: string[]; properties?: Record<string, unknown> }
      | undefined;
    ...
    const summary = emailsApiOpenApi.components?.schemas?.AttachmentRepairSummary;
    for (const field of [
      "would_repair",
      "operator_action",
      "retrying",
  ...
  it("publishes both provider webhook receivers as explicitly public routes", () => {
    ...
    for (const schema of ["SnsNotification", "ResendWebhookEvent", "WebhookReceipt"]) {
      expect(emailsApiOpenApi.components?.schemas?.[schema], schema).toBeDefined();
    }
  });
