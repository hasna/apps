...
// one. A configured database PATH used to be the implicit opt-in; it is now only the
...
// stale EMAILS_SELF_HOSTED_* client aliases are RETIRED: they are read
...
export function planEmailStore(env: NodeJS.ProcessEnv = process.env): StorePlan {
  ...
  const invalidLocalOptIns = invalidEmailsLocalOptInSettings(env);
  if (invalidLocalOptIns.length > 0) {
    throw new StoreConfigurationError(
      `${invalidLocalOptIns.join(" and ")} must be exactly 1 to opt into the on-box Emails store. ` +
        "Unset the invalid selector or set the canonical HASNA_EMAILS_LOCAL=1; no other value selects SQLite.",
      invalidLocalOptIns,
    );
  }
  ...
  // 3. A PATH WITHOUT THE OPT-IN IS REFUSED, BY NAME. This row previously selected
...
export function createConfiguredEmailStore(): EmailStore {
  ...
  switch (plan.store) {
    ...
    case "api":
      ...
      const initial = hostedEmailStoreBinding();
      return createHttpEmailStore({
        baseUrl: plan.baseUrl,
        credential: initial.credential,
        credentialSetting: initial.credentialSetting,
        credentialFallbacks: initial.credentialFallbacks,
        // Long-lived library consumers pick up key rotations on the next request.
        // If the configured authority changes, the transport refuses before fetch
        // rather than sending the newly resolved key to the construction-time host.
        bindingProvider: hostedEmailStoreBinding,
      });
  }
}
 ...
 */
function hostedEmailStoreBinding(): {
  baseUrl: string;
  credential: string;
