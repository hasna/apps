...
A self-run deployment: the operator owns the deployment, provider accounts and data.
...
# `emails provider add --access-key/--secret-key` does not reach a api
...
The api `providers` resource stores non-secret metadata only
(`name`, `type`, `region`, `active`). `emails provider add --access-key …`
against a server therefore **fails with an explicit error** naming
...
the plaintext token once. `emails server key list` never shows tokens or
hashes; `emails server key revoke <kid>` disables a key immediately. The
service rejects signed keys that are absent from its database.

For a rename cutover, run `emails server key rotate`. It creates a new
...
- Api sends require a durable idempotency key. Inline attachments are
  limited to five, 512 KiB each and 768 KiB total. Scheduled sends are not
  supported by the API. Explicit-id bulk mailbox mutations are.
