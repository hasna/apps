---
id: "emails-canonical-source-plan"
title: "Emails canonical client architecture and historical axis-removal program"
type: "engineering-plan"
owner: "chief-of-engineering"
created_at: null
updated_at: "2026-09-02T17:24:43Z"
status: "in-progress"
source_task: "01a05ce7-5296-7113-bb52-31d460953856"
---

# Emails canonical client architecture and historical program

## Current architecture — source reconciliation, 2026-09-02

Configured clients use authenticated HTTPS only. PostgreSQL is server-side storage,
never a client transport. The historical default-SQLite target is superseded: an
unset or invalid client configuration is an error, not a selection of a local store.
This is the operative architecture for new work; the preserved program below records
earlier decisions, measurements and lessons, not current setup instructions.

- [`src/store-resolution.ts`](../src/store-resolution.ts) builds the sole configured
  HTTP store after canonical URL/credential validation. Clients reject database
  settings and retired selectors, including blank values; aliases must agree.
  [`src/lib/client-config.ts`](../src/lib/client-config.ts) separates pure inspection
  from explicit credential-pointer loading. Plain HTTP is only an explicit loopback
  development exception; no implicit client state or database migration is performed.
- [`src/server/storage-backend.ts`](../src/server/storage-backend.ts) requires
  PostgreSQL service configuration. The current service entrypoint does not select
  the legacy SQLite dashboard when database configuration is absent.
- The explicit SQLite adapters, caller-supplied Database overloads, raw storage
  exports and retained implementation arms remain physically present. They are not
  alternative configured client transports. Preserve their existing contracts and
  meaningful tests while completing the migration; do not hide them or describe the
  whole package as already HTTP-only.
- Product repositories and library functions already consume `EmailStore` through
  the configured factory. The seam is not an unconsumed future design. Keep its
  async `Outcome` contract, tenant-at-construction boundary and shared conformance
  coverage for the concrete HTTP and explicit SQLite implementations.

The public OSS service remains operator-configurable. This source plan neither embeds
private deployment topology nor authorizes deployment, live data migration, mail sends,
credential issuance or authentication changes. Existing identity decisions and frozen
compatibility aliases remain governed by their own contracts; deleting a placement
decision is not permission to revoke identities or remove public exports.

## Current delivery gates and unresolved work

This correction is not release acceptance. The dashboard, tracking handlers, public
listeners, persistent TUI preference writes and provider-filtered reports still require
complete canonical capability closure. Supported base outbound reporting and the existing
SES/API send and ingestion-worker workflow remain required; optional SMTP expansion is
not a prerequisite for that existing workflow. Keep every existing function, tool, option,
field, export and meaningful behavior assertion while resolving gaps. Do not turn them
into accepted deferrals, new refusals or waivers.

Typed refusal is the honest representation of a missing capability, not completion of
that capability. A conformance pass can establish that a false capability was truthfully
reported; it does not establish full CLI/MCP/library/API feature parity. Preserve both
directions of the conformance check, uniform case coverage, all operation cases and the
exactly-once send ledger requirements. Never substitute empty arrays, zeros or no-ops.

The historical ten-phase record is retained below. Its two-client target in §2,
configuration-selection/default assumptions in phase3 and two-configured-store completion
criterion in §9 are superseded by the architecture above. Its phase status, line numbers,
corpus sizes, counts and recorded passing commands are historical evidence, not fresh
measurements of this source. No phase is declared complete by this correction.

All existing source, artifact, seam and ratchet gates remain in force. Do not change
ceilings, scan patterns, exclusions, floors, positive controls or CI to make prose pass.
No zero-axis claim is made: an always-API adapter or identifier rename is not structural
removal. The historical eventual guard/export-removal phases are not authorization to
delete a guard or public symbol now. Removal requires the separately reviewed behavioral,
compatibility and breaking-version gates; this documentation patch changes none of them.

Keep the historical §10 verification commands and their evidence intact; run the current
commands on each proposed change and record actual failures/skips separately. The current
package also declares `bun run test:postgres`, backed by the active isolated PostgreSQL
gate; an unavailable service, omitted suite or optional skip is not live-PG proof. Actual
release validation still requires exact-source tests, independent review and artifact
correspondence. The package's version string does not prove an already-published artifact
contains this source.

Existing mail, attachments and legacy directories must not be deleted or automatically
migrated by client configuration. Preserve tenant scoping, explicit authorization mapping,
data ownership and rollback evidence. Follow [AGENTS.md](../AGENTS.md) for current client,
service and fixture setup; do not execute old local-default recipes from the history.

## Historical program record — retained verbatim

The following complete record preserves the `6646cc8` baseline, later dated phase notes,
all ten phases, verification results and anti-vacuity lessons. Imperatives in this archived
record describe the historical program; the current sections above supersede conflicting
architecture/setup claims. The load-bearing spelling restriction remains applicable to
new prose and tests because this file is still in the unchanged scanned corpus.

<details>
<summary>Historical source plan and measurements — not current client setup</summary>
