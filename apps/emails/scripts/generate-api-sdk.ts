#!/usr/bin/env bun
// Regenerates the Emails API client from the serve's OpenAPI document.
//   bun run scripts/generate-api-sdk.ts
// The output (src/api.ts, exported as @hasna/emails/api) is
...
import { emailsApiOpenApi } from "../src/server/api/openapi.js";

const root = join(dirname(fileURLToPath(import.meta.url)), "..");
const generated = generateSdkFromOpenApi(emailsApiOpenApi, {
  className: "EmailsApiClient",
...
for (const [path, pathItem] of Object.entries(emailsApiOpenApi.paths ?? {})) {
...
const allComponents = emailsApiOpenApi.components?.schemas ?? {};
...
const responseContractOutput = `// @generated from src/server/api/openapi.ts by scripts/generate-api-sdk.ts — DO NOT EDIT.
// Compact runtime response contracts. Regenerate with: bun run scripts/generate-api-sdk.ts
...
export const API_RESPONSE_CONTRACTS: readonly ApiResponseContract[] = ${JSON.stringify(responseContracts, null, 2)};

export const API_RESPONSE_COMPONENTS: Readonly<Record<string, unknown>> = ${JSON.stringify(
  responseComponents,
  null,
  2,
)};
...
for (const operation of generated.operations) {
  const pathItem = emailsApiOpenApi.paths?.[operation.path] as
...
const header = `// @generated from src/server/api/openapi.ts by scripts/generate-api-sdk.ts — DO NOT EDIT.
...
`;
const out = join(root, "src", "api.ts");
writeFileSync(out, header + secureClientCode);
const responseContractsOut = join(root, "src", "lib", "api-response-contracts.generated.ts");
