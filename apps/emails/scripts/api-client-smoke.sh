...
if test -n "${EMAILS_CLIENT_ENV_SECRET:-}"; then
  ...
  fail "configure the api client: set EMAILS_CLIENT_ENV_SECRET, or HASNA_EMAILS_API_URL with a credential"
fi
...
smoke_tmp="$(mktemp -d "${TMPDIR:-/tmp}/emails-api-smoke.XXXXXX")"
cleanup() {
  rm -rf -- "$smoke_tmp"
}
