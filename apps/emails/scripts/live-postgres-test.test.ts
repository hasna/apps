...
describe("PostgreSQL evidence must be complete", () => {
  test("requires the exact nonempty server integration inventory", () => {
    ...
    expect(Object.values(MINIMUM_PASS_COUNTS).reduce((a, b) => a + b, 0)).toBe(216);
    expect(Object.values(MINIMUM_PASS_COUNTS).reduce((a, b) => a + b, 0) + Object.keys(OPTIONAL_SKIPS).length).toBe(218);
  });
