...
The public pure-evaluation route is intentionally unauthenticated. It enforces
a 64 KiB request-body ceiling plus event-wide property-key, code-point, and
confusable-classification work budgets before semantic matching. Requests that
would exceed the bounded CPU budget fail closed with `invalid_input`; embedding
applications should still apply their normal edge rate limits.
...
The exported raw `handlePixelsMcpHttpRequest()` does not infer whether an embedding server is remote. Both the API and MCP HTTP handlers reject declared or streamed bodies above 64 KiB before parsing, cancel an overflowing source immediately, and apply a bounded total body-read deadline (10 seconds by default; configurable with `bodyReadTimeoutMs` and capped at 60 seconds). Remote embeddings must provide `authorize`, enforce rate limits at the server or reverse-proxy boundary, and retain the exact-origin policy. `startPixelsMcpHttpServer()` enforces the non-loopback authorization requirement for its own listener.
...
Unicode-confusable classification is generated from the pinned Unicode 17.0.0
UTS #39 `confusables.txt` data. Builds use a committed offline table and never
fetch `latest`. The source is
<https://www.unicode.org/Public/17.0.0/security/confusables.txt>, pinned at
SHA-256 `091c7f82fc39ef208faf8f94d29c244de99254675e09de163160c810d13ef22a`.
The repository's `scripts/unicode/README.md` documents the bounded profile and
reviewed update procedure; `bun run generate:unicode-confusables --refresh`
refuses content that does not match the pinned digest. The derived data is
distributed under the [Unicode License v3](./UNICODE-LICENSE.txt).
