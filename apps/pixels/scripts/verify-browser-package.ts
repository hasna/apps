...
function run(command: string[], env?: Record<string, string>): string {
  const result = Bun.spawnSync(command, { cwd: root, stdout: "pipe", stderr: "pipe", env });
...
try {
  ...
  // This script runs from `check`, which the publish guard triggers through
  // `npm pack --dry-run --json`. npm forwards its config to lifecycle scripts
  // as env vars, so this nested pack inherits the outer dry-run env flag and
  ...
  const packResult = JSON.parse(run([
    ...
    "--pack-destination",
    archiveDirectory,
  ], packEnv)) as Array<{ filename?: unknown }>;
