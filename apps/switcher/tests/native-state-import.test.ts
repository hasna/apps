...
test("dry-run creates nothing; explicit staging keeps original transcript/tool IDs and skips credentials, SQLite, index and runtime locks", () => fixture(async root => {
  ...
  await file(source, "skills/example/SKILL.md", "excluded synthetic skill fixture");
  ...
  expect(plan.files.map(file => file.path).sort()).toEqual([".hasna/instructions/example.md", "sessions/2026/turn.jsonl"]);
  expect(await readdir(root)).toEqual(["legacy"]);
  const summary=nativeStateImportSummary(plan, false);
  expect(summary).toMatchObject({ mode: "dry-run", entries: expect.arrayContaining(["sessions",".hasna/instructions"]), newFiles: 2, cutoverComplete: false });
  ...
  expect((await readdir(state.home)).sort()).toEqual([".hasna", "sessions"]);
