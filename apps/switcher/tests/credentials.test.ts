...
`,{mode:0o700});
    ...
    // The canonical state marker is a validated path, not an operator credential.
    // Keep every other HASNA_/delivery variable excluded from the native child.
    expect(JSON.parse(result.stdout)).toEqual({auth:true,model:"fixture-pro",subagent:"fixture-pro",stateHome:join(await realpath(dir),".claude"),leaked:[]});
