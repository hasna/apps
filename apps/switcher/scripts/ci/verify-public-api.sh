...
request() {
  ...
  if [[ "$remaining" -le 0 ]]; then
    echo "public API verification deadline exceeded" >&2
    exit 1
  fi
  ...
  case "$status" in
    200)
      jq -es --arg version "$version" 'length == 1 and (.[0] | type == "object" and .status == "ready" and .backend == "postgresql" and .version == $version)' "$body" >/dev/null 2>&1 || {
        echo "public readiness contract did not match the deployed version and PostgreSQL backend" >&2; exit 1;
      }
      break
      ;;
    404)
      [[ "$waiting" == true ]] && jq -es 'length == 1 and (.[0] | type == "object" and .error == "unknown_app")' "$body" >/dev/null 2>&1 || {
        echo "public readiness returned an unexpected missing route" >&2; exit 1;
      }
      ;;
    000|502|503|504)
      [[ "$waiting" == true ]] || { echo "direct origin readiness failed" >&2; exit 1; }
      ;;
    *) echo "public readiness returned a terminal status" >&2; exit 1 ;;
  esac
  remaining=$((deadline - SECONDS))
  [[ "$remaining" -gt 0 ]] || { echo "public API verification deadline exceeded" >&2; exit 1; }
  pause="$delay"
...
[[ "$status" == 200 ]] && jq -es --arg version "$version" 'length == 1 and (.[0] | type == "object" and .version == $version)' "$body" >/dev/null 2>&1 || {
  echo "public version does not match the deployed package" >&2; exit 1;
}
...
[[ "$status" == 401 || "$status" == 403 ]] && jq -es 'length == 1 and (.[0] | type == "object" and (.error.code | type == "string" and startswith("auth_")))' "$body" >/dev/null 2>&1 || {
  echo "anonymous Switcher API request was not denied by authentication" >&2; exit 1;
}
