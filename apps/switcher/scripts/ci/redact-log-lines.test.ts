...
describe("ECS log evidence redaction", () => {
  test("redacts credential-shaped values and preserves migration evidence", () => {
    const result = redactLogText([
      "DATABASE_URL=postgresql://service:password@example.test/switcher?sslmode=require",
      "Authorization: Bearer header.payload.signature",
      "HASNA_SWITCHER_API_SIGNING_KEY=top-secret-value",
      "switcher-serve fatal: Applied migration 'switcher:0002_profiles' (checksum 'sha256:abc123') is not recognized.",
    ].join("\n"));
    ...
    expect(result).toContain("switcher:0002_profiles");
