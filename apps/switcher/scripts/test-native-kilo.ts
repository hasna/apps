...
if(!exe)throw new Error('Set SWITCHER_TEST_NATIVE_EXECUTABLE to the installed Kilo executable.');
...
const root=await mkdtemp(join(scratch,'kilo-authority-native-'));const protocol=process.argv[2]??'anthropic-messages';const attack=true;
...
function stream(body:any,tool:boolean,text:string){
 const input={filePath:join(root,phase===2?'protected.txt':'proof.txt')};let events:any[];
...
const upstream=Bun.serve({hostname:'127.0.0.1',port:0,async fetch(req){
 ...
 if(body.model===other)return stream(body,false,'SECOND_MODEL_SELECTED');
...
await writeFile(join(root,'config/kilo/kilo.json'),JSON.stringify({permission:{bash:'deny'},agent:{build:{prompt:'GLOBAL_AGENT_RULE',permission:{edit:'deny'}}}}));
await writeFile(join(root,'kilo.json'),JSON.stringify({permission:{'*':'deny',read:{'*':'deny','proof.txt':'allow',[join(root,'proof.txt')]:'allow',[join(root,'proof.txt').slice(1)]:'allow'}},agent:{build:{prompt:'PROJECT_AGENT_RULE'}}}));
...
const report:any={root,source:cli,protocol,attack,native:exe,minimumVersion:'7.5.15',at:new Date().toISOString()};
try{
 ...
 const common=['launch','kilo','--provider','fixture','--model',model,'--executable',exe,'--timeout','20','--'];
 ...
 { assert.equal(badRequests,0,'hostile provider configuration was used');
  ...
  const switched=await invoke(['launch','kilo','--provider','fixture','--model',other,'--executable',exe,'--timeout','20','--','run','--format','json','--title','Second model','Use the selected second model and answer briefly.'],'second-model');assert.equal(switched.code,0);assert(switched.stdout.includes('SECOND_MODEL_SELECTED'));assert(calls.some(c=>c.model===other),'native launch did not use the selected second catalog model');
  phase=2;await writeFile(join(root,'protected.txt'),protectedProof);await mkdir(join(root,'.kilo/agents'),{recursive:true});
  await writeFile(join(root,'.kilo/agents/limited.md'),`---\nmode: primary\npermission:\n  read:\n    ${JSON.stringify(join(root,'protected.txt'))}: deny\n---\nLIMITED_AGENT_RULE\n`);
  await writeFile(join(root,'kilo.json'),JSON.stringify({permission:{'*':'deny',read:{'*':'deny',[join(root,'protected.txt')]:'allow',[join(root,'proof.txt')]:'allow'}}}));
  const denied=await invoke([...common,'run','--format','json','--agent','limited','--title','Denied fixture','Read protected.txt through the read tool.'],'deny');assert([0,1].includes(denied.code),'native denial must exit normally');assert(denied.stdout.includes('DENIED_FILE_PROOF'));assert(!denied.stdout.includes(protectedProof));assert(calls.some(c=>c.phase===2&&c.denied&&c.limitedRule));assert(!calls.some(c=>c.protectedContent));
  const listed=await invoke([...common,'models'],'models');assert.equal(listed.code,0);const ids=listed.stdout.trim().split('\n').sort();assert.deepEqual(ids,[id+'/'+model,id+'/'+other].sort(),'native catalog must equal the full selected provider catalog');report.catalog={ids,selectedSecondModel:other};report.sameSession=firstSession;
  assert(calls.some(c=>c.phase===0&&c.toolProof));assert(calls.some(c=>c.phase===1&&c.prior&&c.toolProof));assert(calls.every(c=>c.authMatches&&!c.stolenHeader));assert(calls.filter(c=>!c.title&&c.phase<2&&c.model===model).every(c=>c.projectRule&&c.agentRule));report.passed=true;
 }
