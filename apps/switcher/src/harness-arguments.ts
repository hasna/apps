...
const providerKeys = new Set(["model", "model_provider", "provider", "model_providers", "model_catalog_json", "review_model", "agents", "memories"]);
...
function codexProviderOverride(value: string): boolean {
  ...
  catch { return /^(model|model_provider|provider|model_providers|model_catalog_json|review_model|agents|memories)(?:\s*\.|\s*$)/.test(key); }
}
