...
const reserved: Record<HarnessId, readonly string[]> = {
  ...
  // Cline's provider/model, data/config roots, session identity, and
  // approval/configuration switches are owned by the launch profile.
...
const optionalValues: Partial<Record<HarnessId, readonly string[]>> = {
  claude: ["--resume", "--continue"], grok: ["--resume", "--worktree", "--local-workspace"], pi: ["--list-models"], cline: [],
};
