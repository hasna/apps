...
Contracts release preparation is committed in `8f8e888713329fdef4e03ef8098e504bcb759f88`: runtime/package/kit version 1.0.2, optional Secrets peer, independently reviewed candidate bytes, six clean frozen consumer installs and correct CLI links. Full Contracts verification passes 1,561 tests / 15,662 assertions with real PostgreSQL; one hosted-credential test is skipped. Root names/dependency/secrets/manifests checks pass. The latest publishing guard passes Switcher and the other members except Skills' newly added clean-consumer check, which tries to resolve unpublished Contracts 1.0.2 from npm. The hard published-pins check independently requires Contracts 1.0.2 on npm. Publish the independently reviewed dependency within the open PR wave, then run the ordinary registry-consumer checks before merge; no gate exemption is planned. A Fireworks custom deployment-prefix failure was corrected and its focused regression passed. PR, CI, merge and publication remain pending.
