...
The next Switcher patch targets the installed Cline CLI 3.0.61. It writes Cline's version 1 `settings/providers.json` and `settings/models.json` under a Switcher-owned data directory, preserving the complete coding-eligible catalog and selected model. Anthropic Messages uses Cline `anthropic`; OpenAI Chat uses `openai-compatible`; OpenAI Responses uses Cline's built-in `openai-native` routing provider with an explicit `openai-responses` model format so an operator endpoint remains on the Responses route.
