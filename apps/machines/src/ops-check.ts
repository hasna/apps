...
function buildIssue(input: {
  ...
  const taskSuggestion: FleetOpsTaskSuggestion = {
    fingerprint: fp,
    dedupe_key: `machines:ops:${fp}`,
    title: `[machines:ops:${fp}] ${input.severity} ${input.classification}`,
    description: taskDescription(issueBase),
    priority: priorityFor(input.severity),
    tags: ["machines", "ops-check", input.classification, ...(input.tags ?? [])],
  };
...
function normalizeExpectedMachines(values: string[] | undefined): string[] {
...
export function listLocalTmuxPanes(tmuxCommand = process.env["HASNA_MACHINES_TMUX_BIN"] || "tmux"): { panes: FleetOpsTmuxPane[]; error: string | null } {
...
function buildMachineRows(input: {
  ...
  const topologyById = new Map(input.topology.machines.map((machine) => [machine.machine_id, machine]));
  const routeById = new Map(input.routing.routes.map((route) => [route.machine_id, route]));
  return input.health.machines.map((machine) => {
...
function buildIssues(input: {
  ...
  const machineIds = new Set(input.topology.machines.map((machine) => machine.machine_id));
  for (const expected of input.expectedMachines) {
  ...
  for (const machine of input.machines) {
    ...
    if (machine.doctor_status && !["ok", "healthy", "pass"].includes(machine.doctor_status.toLowerCase())) {
      issues.push(buildIssue({
        ...
        recommendation: "Inspect `machines doctor --machine <id> --json` and route remediation through a task.",
        tags: [machine.machine_id],
        maxEvidenceItems: input.maxEvidenceItems,
      }));
    }
...
export function getFleetOpsCheck(options: FleetOpsCheckOptions = {}): FleetOpsCheck {
  ...
  const machines = buildMachineRows({ topology, health, routing });
  ...
  const issues = buildIssues({
    machines,
    expectedMachines: normalizeExpectedMachines(options.expectedMachines),
    topology,
    tmux,
    maxEvidenceItems,
  });
  ...
  const routeBlocked = machines.filter((machine) => !machine.route_ok || machine.status === "blocked").length;
  const heartbeatAttention = machines.filter((machine) => machine.heartbeat !== "online").length;
  const storageAttention = machines.filter((machine) => machine.storage_sync_status && !["ok", "synced", "idle"].includes(machine.storage_sync_status.toLowerCase())).length;
  ...
  return {
    ...
    summary: {
      ...
      ready: machines.filter((machine) => machine.status === "ready").length,
      degraded: machines.filter((machine) => machine.status === "degraded").length,
      blocked: machines.filter((machine) => machine.status === "blocked").length,
      unknown: machines.filter((machine) => machine.status === "unknown").length,
    ...
    artifacts: [
      ...
      { kind: "routing", ref: "machines routing --json", format: "json", private: false },
      { kind: "loop_preflight", ref: "machines loop-preflight --json", format: "json", private: false },
