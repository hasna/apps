...
// HTTP API selected by HASNA_MACHINES_API_URL + HASNA_MACHINES_API_KEY; the
// server storage switch is `sqlite | postgresql` via
// HASNA_MACHINES_DATABASE_URL. Any STORAGE_MODE variable still set is an
...
export function assertNoLegacyStorageMode(env: NodeJS.ProcessEnv = process.env): void {
  ...
  throw new Error(
    `${legacyKey} was removed. Deployment modes no longer exist: delete the storage-mode variable. ` +
      ...
      `On the server, set HASNA_MACHINES_DATABASE_URL to select the postgresql backend, ` +
