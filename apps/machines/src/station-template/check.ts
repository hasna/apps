 ...
 */
function directiveAssignments(content: string, section: string, name: string): string[] {
 ...
 */
function effectiveScalarDirective(content: string, section: string, name: string): string | null {
  let value: string | null = null;
  for (const raw of directiveAssignments(content, section, name)) {
 ...
 */
function effectiveListDirective(content: string, section: string, name: string): string[] {
  let values: string[] = [];
  for (const raw of directiveAssignments(content, section, name)) {
    if (raw.length === 0) {
      values = [];
  ...
  if (effective.unitConventions) {
    ...
    for (const unitDir of unitDirs) {
      ...
      for (const entry of readdirSync(unitDir)) {
        ...
        const intervalRaw = effectiveScalarDirective(content, "Unit", "StartLimitIntervalSec");
        if (intervalRaw === null) {
        ...
        const burstRaw = effectiveScalarDirective(content, "Unit", "StartLimitBurst");
        if (burstRaw === null) {
        ...
        const onFailure = effectiveListDirective(content, "Unit", "OnFailure");
        ...
        if (conventions.requireAbsoluteExecStart) {
          ...
          for (const value of directiveAssignments(content, "Service", "ExecStart")) {
            if (value.length === 0) {
              effectiveExecStarts = [];
            } else {
