  ...
  for (const absence of effective.absences) {
    ...
    if (absence.service) {
      if (!probe) {
        ...
        if (!load.ok || state.length === 0) {
          // systemd could not be QUERIED — no systemd, a container, dbus down.
          // That is "could not look", and the whole point of this item is that
          // "could not look" must never be reported as "absent". Only a real
          // `not-found` from systemd is evidence of absence. (Review finding P2
          // on #58: a service-only absence in an out-of-tree template could
          // otherwise overstate its proof; the shipped tailscale absence also
          // carries path and command probes, so it was never the whole story.)
          unprobed.push(`service ${absence.service} (systemctl gave no LoadState)`);
