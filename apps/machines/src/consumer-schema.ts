  ...
  $defs: {
    cacheability: {
      ...
      properties: {
        ...
        source_authority: { enum: ["machines", "machines", "manifest", "manifest_metadata", "live_topology", "argument", "inferred", "fallback", "unresolved", "mixed", "unknown"] }, // machines is the legacy stored value, accepted on read
    ...
    note_actor_context: {
      ...
      properties: {
        ...
        source: { enum: ["notes", "agent", "sync", "import", "machines", "unknown", "notes", "machines"] }, // legacy open-* values accepted on read
        display_name: { type: "string" },
      },
    },
    note_machine_context: {
...
export function validateMachinesConsumerEnvelope(
  ...
  if (envelope === "contract") {
    ...
    if (!hasObject(value, "actor")) {
      ...
      if (!["notes", "agent", "sync", "import", "machines", "unknown", "notes", "machines"].includes(String(actor.source))) errors.push("actor.source");
    ...
    if (!hasObject(value, "source")) {
      ...
      if (source.authority !== "machines" && source.authority !== "machines") errors.push("source.authority");
