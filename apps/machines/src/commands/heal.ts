// Self-healing network watchdog for fleet machines.
//
...
export function getHealConfigPath(): string {
  return process.env["HASNA_MACHINES_HEAL_CONFIG_PATH"] || join(getDataDir(), "heal-config.json");
}

export function getHealStatePath(): string {
  return process.env["HASNA_MACHINES_HEAL_STATE_PATH"] || join(getDataDir(), "heal-state.json");
}
