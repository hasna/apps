...
- Config is stored in the effective monitor home (`~/.hasna/monitor/config.json` by default, resolved by the in-package resolver to the XDG data home once the store is migrated there or `HASNA_DATA_HOME` is set) — a user-owned location.
- The SQLite database is stored in the effective monitor home (`~/.hasna/monitor/monitor.db` by default, resolved by the in-package resolver).
- `MONITOR_CONFIG_DIR` (or the `HASNA_MONITOR_HOME` alias) may be set to isolate config and SQLite storage in CI,
