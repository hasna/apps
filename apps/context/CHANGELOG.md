...
- Public server/auth behavior: the HTTP surface (`context-serve`) answers `/health`, `/ready` and `/version` publicly; `/api/*` routes require HTTP auth when `CONTEXT_HTTP_TOKEN` / `HASNA_CONTEXT_HTTP_TOKEN` is set or `CONTEXT_REQUIRE_HTTP_AUTH` / `HASNA_CONTEXT_REQUIRE_HTTP_AUTH` is enabled (Bearer or `x-context-token`), with constant-time token comparison. The HTTP surface now also serves its OpenAPI 3.1 contract at `/openapi.json` (public), and `hasna.contract.json` declares it (`openApiPath` / `generatedFrom`) so the package SDK surface is honest and the repo-conformance gate passes.
