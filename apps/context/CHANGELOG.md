...
- Concurrent first-use migrations can no longer lose data: each migration attempt snapshots to its own unique temp file, verifies it, and atomically places it at the canonical path without ever deleting or overwriting a peer's in-flight or already-placed snapshot (a CLI and `context-serve` upgrading at the same moment). Previously a concurrent migration could unlink the other's snapshot and the verification open would create an empty database that passed `integrity_check` and became canonical, leaving committed legacy rows invisible.
