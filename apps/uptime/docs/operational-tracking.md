...
private deployment metadata records that result. Version `0.1.66` is prepared
...
check can pass. `0.1.66` also adds a bounded hosted Postgres monitor API adapter
for `/api/v1/summary` and `/api/v1/monitors*`; non-migrated hosted reads stay
blocked rather than falling back to SQLite when that adapter is active. The
...
## 0.1.66 Runtime Readiness Snapshot
...
| Hosted monitor API | An explicit API adapter can route `/api/v1/summary` and `/api/v1/monitors*` to Postgres monitor rows with workspace scoping, actor/origin/idempotency metadata, audit rows, and tombstones, while ignoring raw status/last-check body fields. | Bounded control-plane plumbing only; `uptime serve`, reports, incidents, results, imports, probes, scheduler loops, reporter delivery, and worker promotion are still blocked until each has authoritative Postgres storage and live evidence. |
