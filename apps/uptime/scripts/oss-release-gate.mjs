...
function workspaceRoot(repositoryRoot) {
  ...
  while (current !== dirname(current)) {
    ...
    if (existsSync(manifestPath)) {
      ...
      const member = join(repositoryRoot).replace(`${join(current)}/`, "");
      if (entries.some((pattern) => pattern === member || member.startsWith(pattern.replace(/\*.*$/, "")))) return current;
    }
...
function installedPackageLicenses(repositoryRoot) {
  ...
  const visit = (directory) => {
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      if (!entry.isDirectory() || (entry.name.startsWith(".") && entry.name !== ".bun")) continue;
  ...
  const workspaceRootPath = workspaceRoot(repositoryRoot);
  if (workspaceRootPath && workspaceRootPath !== repositoryRoot) visit(join(workspaceRootPath, "node_modules"));
  return licenses;
}
  ...
  try {
    ...
    if (existsSync(nodeModules)) {
      ...
      for (const row of thirdPartyNoticeRows(thirdPartyNotices)) {
        ...
        let installedLicense = installedLicenses.get(key);
        if (!installedLicense) {
          for (const [candidate, license] of installedLicenses) {
            if (candidate.startsWith(`${row.name}@`)) {
