...
test("buildPostgresMigrationPlan emits a blocked cloud-store schema with tombstones and RLS", () => {
  ...
  expect(plan.requiredIndexes).toContain("report_runs_schedule_window_idx");
  ...
  expect(plan.migrationStatements.join("\n")).toContain("CREATE UNIQUE INDEX IF NOT EXISTS report_runs_schedule_window_idx");
...
test("Postgres migration table contracts keep workspace, idempotency, and duplicate guards", () => {
  ...
  expect(table("probe_submissions")).toContain("schedule_slot timestamptz NOT NULL");
  expect(table("probe_submissions")).toContain("probe_class text NOT NULL CHECK (probe_class IN ('public', 'private'))");
  expect(table("probe_submissions")).toContain("probe_location text NOT NULL");
  ...
  expect(table("report_runs")).toContain("status text NOT NULL CHECK (status IN ('pending', 'running', 'succeeded', 'failed', 'retry_exhausted'))");
  expect(table("report_runs")).toContain("finished_at timestamptz");
  expect(table("report_runs")).not.toContain("finished_at timestamptz NOT NULL");
  expect(table("report_runs")).toContain("claimed_by_worker_id text");
  expect(table("report_runs")).toContain("fencing_token text");
  expect(table("report_runs")).toContain("lease_expires_at timestamptz");
  expect(table("report_runs")).toContain("version bigint NOT NULL DEFAULT 1");
