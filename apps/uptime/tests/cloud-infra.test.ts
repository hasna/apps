...
test("AWS infra templates use secret refs and keep services scaled down by default", () => {
  ...
  expect(tfvars).toContain('runtime_package_version  = "0.1.24"');
  ...
  expect(variables).toContain("certificate_arn is required when protected_access_mode is alb_https_cert");
