...
const checkRootArgument = process.argv.find((argument) =>
  argument.startsWith("--check-root="));
const outputRoot = checkRootArgument
  ? checkRootArgument.slice("--check-root=".length)
  : join(repositoryRoot, DEPLOYMENT_GENERATED_ARTIFACT_ROOT);
...
if (checkOnly) {
  ...
  if (drift.size > 0) {
    console.error(
      `Deployment generated artifacts are stale: ${[...drift].sort().join(", ")}`,
    );
    process.exitCode = 1;
  } else {
    console.log(
      `Deployment generated artifacts are current (${expectedPaths.length} files).`,
    );
  }
} else {
  ...
  console.log(
    `Wrote ${expectedPaths.length} deployment artifacts to ${DEPLOYMENT_GENERATED_ARTIFACT_ROOT}.`,
  );
}
