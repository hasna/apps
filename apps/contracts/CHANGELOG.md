...
- **Manifest fields.** `scope`, `client` (`transport: "hosted"`, `credentialChain: "contracts"`, `authority`, `localOptIn`, `localStoreModule`, `readProbe`, or `null` for a repo that ships no client) and `serviceSurfaces[].dataAccess` (`hosted` | `server-only` | `local-opt-in`) with per-command `commands[]`. All optional; cross-checks refuse a mismatched `localOptIn`, a client on a `library`, and `local-opt-in` access without a door. Both JSON Schema copies carry the same additions and stay closed.
...
- **Manifest `serving` block (#2103).** An optional `serving: { routeSlug, access, targetClientBase }` in `hasna.contract.json` records where a served app is reachable from a client — the gateway path segment, its credential gate (`public` | `api-key` | `signature`; named explicitly, never defaulted) and the client base URL (absolute https, never ending in `/v1`; on `api.hasna.com` it must be prefixed with its own `routeSlug`). Mirrors the triple the fleet registry carries per hosted app; `clientKeySecretRefFor()` and `gatewayClientBaseFor()` export the derived key ref and default base. Optional and additive; `hosting` is unchanged; a `library` must not declare it. (CONTRACT.md §9.2.)
...
- Keep existing consumers on their declared registry versions until each app
  explicitly adopts this breaking contract; do not substitute the workspace
  package for a pinned pre-1.0 dependency.
...
- 554a5b9: fix: correct the `@hasna/secrets` peer dependency to `0.3.3`, the version that existed on npm when the fix was written. Contracts 0.13.3 shipped with a peer on `@hasna/secrets@0.3.4`, which was not yet published at that time; the unsatisfiable peer made bun 1.3.14's resolver loop forever in a workspace context (the internal-apps CI hang, todos 1555d199) and blocked fresh installs. `@hasna/secrets@0.3.4` has since published (2026-08-21T19:10:51Z) and is now latest, so a future release may widen the peer.
