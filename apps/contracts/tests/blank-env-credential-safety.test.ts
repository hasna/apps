...
describe("server backend: a defined blank DATABASE_URL fails closed", () => {
  const cases: Array<[string, Record<string, string>]> = [
    ...
    ["short alias whitespace", { DEMO_DATABASE_URL: " \t " }],
  ];
  for (const [label, env] of cases) {
    test(`${label} throws instead of silently selecting sqlite`, () => {
      expect(() => resolveServerDataBackend("demo", env)).toThrow(/blank/i);
    });
  }
...
// ---------------------------------------------------------------------------
// Client: a DEFINED blank API URL is misconfigured, never silent local.
// ---------------------------------------------------------------------------

describe("client: a defined blank API URL is misconfigured, never silent local", () => {
  const cases: Array<[string, Record<string, string>]> = [
    ["canonical empty", { HASNA_DEMO_API_URL: "" }],
    ["canonical whitespace", { HASNA_DEMO_API_URL: "   " }],
    ["canonical quoted whitespace", { HASNA_DEMO_API_URL: '"   "' }],
    ["short alias empty", { DEMO_API_URL: "" }],
  ];
  for (const [label, env] of cases) {
    test(`${label} reports misconfigured=true with a warning naming the key`, () => {
      const r = resolveClientTransport("demo", env);
      expect(r.transport).toBe("sqlite");
      expect(r.misconfigured).toBe(true);
      expect(r.warning).toMatch(/blank/i);
    });
  }

  test("a defined blank env URL is not rescued by a valid disk URL", () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", "HASNA_DEMO_API_URL=https://disk.example\nHASNA_DEMO_API_KEY=diskA\n");
    const r = resolveClientTransport("demo", { HOME: home, HASNA_DEMO_API_URL: "" });
    expect(r.misconfigured).toBe(true);
    expect(r.apiUrlSource).toBeNull();
  });

  test("createClientTransport throws on a defined blank env URL", () => {
    expect(() => createClientTransport("demo", { HASNA_DEMO_API_URL: " " })).toThrow(/blank/i);
  });
});
...
describe("client: a disk URL declaration that is blank or malformed is misconfigured, not absent", () => {
  test("blank URL value on disk", () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", 'HASNA_DEMO_API_URL=""\nHASNA_DEMO_API_KEY=diskA\n');
    const r = resolveClientTransport("demo", { HOME: home });
    expect(r.misconfigured).toBe(true);
    expect(r.warning).toContain(home);
  });

  test("unterminated-quote URL value on disk", () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", 'HASNA_DEMO_API_URL="https://demo.example.com\nHASNA_DEMO_API_KEY=diskA\n');
    const r = resolveClientTransport("demo", { HOME: home });
    expect(r.misconfigured).toBe(true);
  });
...
describe("credential files enforce owner-only regular-file no-symlink safety", () => {
  test("a 0644 credential file is refused, never read as absent", () => {
    ...
    const dir = join(home, ".hasna", "cloud");
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, "demo.env"), "HASNA_DEMO_API_KEY=diskA\n");
    expect(() => resolveCredential("demo", { HOME: home })).toThrow(/not safe|mode/i);
  });

  test("04600 is refused despite masking with 0777", () => {
    const home = makeHome();
    const path = writeCloudEnv(home, "demo", "HASNA_DEMO_API_KEY=diskA\n");
  ...
  test("a directory at the credential path is refused", () => {
    ...
    const dir = join(home, ".hasna", "cloud");
    mkdirSync(dir, { recursive: true });
    mkdirSync(join(dir, "demo.env"));
    expect(() => resolveCredential("demo", { HOME: home })).toThrow(/not safe/i);
  });

  test("0600 and 0400 owner-only regular files are accepted", () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", "HASNA_DEMO_API_KEY=d6a\n");
    expect(resolveCredential("demo", { HOME: home })?.apiKey).toBe("d6a");

    const home2 = makeHome();
    const path = writeCloudEnv(home2, "demo", "HASNA_DEMO_API_KEY=d4a\n");
...
describe("a deliberately blank explicit credential is refused, never resolved around", () => {
  test("blank apiKey argument throws even when a disk credential exists", () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", "HASNA_DEMO_API_KEY=diskA\n");
...
describe("authority coherence: a credential never reaches a different authority", () => {
  ...
  function replaceConfigAtomically(home: string, app: string, body: string): void {
    const path = join(home, ".hasna", "cloud", `${app}.env`);
    const tmp = `${path}.tmp`;
  ...
  test("atomic URL+key replacement never sends the new key to the retired origin", async () => {
    const home = makeHome();
    const path = writeCloudEnv(home, "demo", "HASNA_DEMO_API_URL=https://one.example.com\nHASNA_DEMO_API_KEY=key-one\n");
  ...
  test("same-authority key-only rotation heals without rebuilding", async () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", "HASNA_DEMO_API_URL=https://one.example.com\nHASNA_DEMO_API_KEY=key-one\n");
    const { calls, fetchImpl } = captureFetch();
    const client = createClientTransport("demo", { HOME: home }, { fetchImpl });
  ...
  test("env URL plus a disk pair rotated to a different URL fails closed at construction", async () => {
    const home = makeHome();
    writeCloudEnv(home, "demo", "HASNA_DEMO_API_URL=https://two.example.com\nHASNA_DEMO_API_KEY=key-two\n");
    const { calls, fetchImpl } = captureFetch();
    expect(() =>
