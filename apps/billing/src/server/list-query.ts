/**
 * Shared pagination/filter parsing for /v1 list routes (BUILD-SPEC §3.2). When
 * no list-query params are present the raw array is returned so interface parity
 * with the CLI/MCP surfaces holds; when present, a paginated envelope is
 * returned.
 */
...
export function hasListQuery(url: URL): boolean {
  return Array.from(url.searchParams.keys()).some((key) => listQueryKeys.has(key));
}

export function listQueryResponse<T extends object>(
  url: URL,
  items: T[],
...
function parseSort(url: URL, options: ListQueryOptions): { field: string; order: ListSortOrder } {
  ...
  if (!options.allowed_sorts.includes(field)) {
    throw new InvalidListQueryError(`Unsupported sort field: ${field}. Allowed: ${options.allowed_sorts.join(", ")}.`);
  }
