...
# own the fleet naming convention and will not carry a copy of it. (That is a
# statement about authority, not conformance: plenty of registry slugs do not
# match the convention. Repairing those is a project-rename decision, not a
# derivation one.) A project whose channel is not named after its slug says so
# by setting integrations.conversations_channel, which always wins.
#
# Ensure creates the channel but never writes the link back: pinning a derived
...
Hosted-backend readiness contract: `docs/hosted-backend-readiness-contract.md`
...
`projects-serve` is the HTTP surface for `@hasna/projects`. It talks directly
to PostgreSQL with no local cache or sync in the service.
