...
const CONNECTOR_NAME = 'connect-workflow-trigger';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('WorkflowTrigger workflow automation API connector CLI')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.WORKFLOW_TRIGGER_API_KEY = opts.apiKey;
...
function getClient(): Connector {
  ...
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set WORKFLOW_TRIGGER_API_KEY.`);
    process.exit(1);
  }
...
profileCmd
  ...
  .action((name?: string) => {
    ...
    info(`Base URL: ${config.baseUrl || chalk.gray('not set (default)')}`);
  });
...
triggersCmd
  ...
  .action(async (opts) => {
    try {
      ...
      const result = await client.triggers.list(query);
      print(result, getFormat(triggersCmd));
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
triggersCmd
  ...
  .action(async (opts) => {
    try {
      const client = getClient();
      const body = (parseJsonOption(opts.body, '--body') ?? {}) as Record<string, unknown>;
      if (opts.name) body.name = opts.name;
      const result = await client.triggers.create(body);
...
eventsCmd
  ...
  .action(async (opts) => {
    try {
      const client = getClient();
      const query = parseJsonOption(opts.query, '--query');
      const result = await client.events.list(query);
...
const searchCmd = program
  .command('search')
  .description('Search workflow data')
  ...
  .action(async (opts) => {
    try {
      ...
      if (Object.keys(body).length === 0) {
        warn('No search parameters provided; sending empty body');
      }
...
program
  ...
  .requiredOption('--path <path>', 'API path (e.g. /triggers)')
  ...
  .action(async (opts) => {
    try {
      ...
      const result = await client.rawRequest({
        ...
        body: parseJsonOption(opts.body, '--body') as Record<string, unknown> | undefined,
      });
