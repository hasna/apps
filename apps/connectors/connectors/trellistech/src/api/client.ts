...
import { TrellistechApiError, parseApiError } from '../types';

const DEFAULT_BASE_URL = 'https://app.trellistech.com/api/v1';
...
export class TrellistechClient {
  ...
  workspacePath(resource: string): string {
    return `/workspaces/${encodeURIComponent(this.workspaceId)}${resource}`;
  }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof TrellistechApiError)) {
    ...
    throw lastError || new TrellistechApiError('Request failed', lastStatus);
  }
