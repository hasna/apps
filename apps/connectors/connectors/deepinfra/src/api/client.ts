import { DeepInfraApiError, type DeepInfraConfig, type ChatRequest, type ChatResponse, type ModelsResponse } from '../types';
...
export class DeepInfraClient {
  ...
  private async request<T>(path: string, options: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new DeepInfraApiError(message, response.status);
    }
