import type { DigitalOceanConfig, OutputFormat } from '../types';
import { DigitalOceanApiError } from '../types';
...
export class DigitalOceanClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Handle errors - DigitalOcean returns { id: string, message: string }
    if (!response.ok) {
      const errorData = data as { id?: string; message?: string; request_id?: string } | undefined;
      const message = errorData?.message || response.statusText;
      const requestId = errorData?.request_id || response.headers.get('ratelimit-reset') || undefined;
      throw new DigitalOceanApiError(message, response.status, requestId);
    }
