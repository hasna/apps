...
export class TextCortexClient {
  ...
  private readonly baseUrl = 'https://api.textcortex.com/v1';

  constructor(config: TextCortexConfig) {
    if (!config.apiKey) throw new Error('TextCortex AI apiKey is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new TextCortexApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
