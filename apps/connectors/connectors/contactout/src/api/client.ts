import type { ContactOutConfig, OutputFormat } from '../types';
import { ContactOutApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.contactout.com';
...
export class ContactOutClient {
  ...
  /**
   * Make an authenticated request to the ContactOut API
   * Uses token header authentication as per ContactOut API docs
   */
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // ContactOut uses token header for authentication
    ...
    if (!response.ok) {
      ...
      throw new ContactOutApiError(errorMessage, response.status);
    }
