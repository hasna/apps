import type { EdgeConfigPlatformConfig, OutputFormat } from '../types';
...
export class EdgeConfigPlatformClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      const errorData = data as { error?: { code: string; message: string } } | undefined;
      const message = errorData?.error?.message || response.statusText;
      const code = errorData?.error?.code;
      throw new EdgeConfigPlatformApiError(message, response.status, code);
    }
  ...
  async delete<T>(path: string, params?: Record<string, string | number | boolean | undefined>, body?: Record<string, unknown>): Promise<T> {
