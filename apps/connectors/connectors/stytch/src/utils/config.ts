...
import type { StytchEnvironment } from '../types';
...
export function getProjectId(): string | undefined {
  return process.env.STYTCH_PROJECT_ID || loadProfile().projectId;
}
...
export function getSecret(): string | undefined {
  return process.env.STYTCH_SECRET || loadProfile().secret;
}
...
export function getEnvironment(): StytchEnvironment {
  const env = (process.env.STYTCH_ENVIRONMENT || loadProfile().environment || 'live').toLowerCase();
  return env === 'test' ? 'test' : 'live';
}

export function setEnvironment(environment: StytchEnvironment): void {
  const config = loadProfile();
