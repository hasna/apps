...
import { V0ApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.v0.dev/v1';
...
export class V0Client {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new V0ApiError(errorMessage, response.status);
    }
  ...
  async put<T>(path: string, body?: Record<string, unknown> | unknown[] | string | object): Promise<T> {
