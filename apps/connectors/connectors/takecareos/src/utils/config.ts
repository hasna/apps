...
/**
 * Local CLI configuration for the TakeCareOS connector.
 *
 * Credentials are stored per-profile under ~/.hasna/connectors/takecareos/. The
 * TAKECAREOS_API_KEY / TAKECAREOS_BASE_URL environment variables always take
 * precedence over stored profile values.
 */
...
export function getApiKey(): string | undefined {
  return process.env.TAKECAREOS_API_KEY || loadProfile().apiKey;
}
...
export function getBaseUrl(): string | undefined {
  return process.env.TAKECAREOS_BASE_URL || loadProfile().baseUrl;
}
