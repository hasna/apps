import type { ConnectorConfig, WebEngageDataCenter } from '../types';
import { ConnectorApiError, parseApiError } from '../types';

const DC_HOSTS: Record<WebEngageDataCenter, string> = {
  global: 'https://api.webengage.com',
  in: 'https://api.in.webengage.com',
  sa: 'https://api.ksa.webengage.com',
  eug: 'https://api.eug.webengage.com',
};

export function resolveBaseUrl(config: ConnectorConfig): string {
  if (config.baseUrl) {
    return config.baseUrl.replace(/\/$/, '');
  }

  const dc = config.dataCenter || 'global';
  return DC_HOSTS[dc] || DC_HOSTS.global;
}
...
export class ConnectorClient {
  ...
  private readonly licenseCode: string;
  ...
  accountPath(version: 'v1' | 'v2', resource: string): string {
    const normalized = resource.startsWith('/') ? resource : `/${resource}`;
    return `/${version}/accounts/${this.licenseCode}${normalized}`;
  }
