...
import type { OAuth2Config, OAuth2Tokens, WebEngageDataCenter } from '../types';
...
export function getApiKey(): string | undefined {
  return process.env.WEBENGAGE_API_KEY || process.env.WEBENGAGE_TOKEN || loadProfile().apiKey;
}
...
export function getLicenseCode(): string | undefined {
  return process.env.WEBENGAGE_LICENSE_CODE || loadProfile().licenseCode;
}

export function setLicenseCode(licenseCode: string): void {
  const config = loadProfile();
  config.licenseCode = licenseCode;
  saveProfile(config);
}

export function getDataCenter(): WebEngageDataCenter | undefined {
  const env = process.env.WEBENGAGE_DC as WebEngageDataCenter | undefined;
  return env || loadProfile().dataCenter;
}

export function setDataCenter(dataCenter: WebEngageDataCenter): void {
  const config = loadProfile();
...
export function getBaseUrl(): string | undefined {
  return process.env.WEBENGAGE_BASE_URL || loadProfile().baseUrl;
}
...
export function getToken(): string | undefined {
  return process.env.WEBENGAGE_TOKEN || process.env.WEBENGAGE_API_KEY || loadProfile().token || loadProfile().apiKey;
}
