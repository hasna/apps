...
import { VoiceflowApiError, parseApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.voiceflow.com/v1';
...
/**
 * Build the Authorization header value for Voiceflow.
 * Public docs require the raw VF.DM.* key — no Bearer prefix.
 */
...
export class VoiceflowClient {
  ...
  constructor(config: VoiceflowConfig) {
    if (!config.apiKey) {
      throw new Error('Voiceflow API key is required');
    }
  ...
  getAuthHeader(): string {
    return buildAuthHeader(this.apiKey);
  }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof VoiceflowApiError)) {
    ...
    throw lastError || new VoiceflowApiError('Request failed', lastStatus);
  }
