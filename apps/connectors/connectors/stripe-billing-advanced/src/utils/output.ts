...
export function parseJsonBody(input: string, label = 'data'): Record<string, unknown> {
  try {
    ...
    throw new Error(`Invalid JSON for ${label}: ${err instanceof Error ? err.message : String(err)}`);
  }
}
