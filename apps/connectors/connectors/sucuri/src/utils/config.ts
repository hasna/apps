...
export function getApiKey(): string | undefined {
  return process.env.SUCURI_API_KEY || loadProfile().apiKey;
}
...
export function getMonitorDomain(): string | undefined {
  return process.env.SUCURI_MONITOR_DOMAIN || loadProfile().monitorDomain;
}

export function setMonitorDomain(monitorDomain: string): void {
  const config = loadProfile();
  config.monitorDomain = monitorDomain;
  saveProfile(config);
}
