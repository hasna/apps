import type { WiseConfig, OutputFormat } from '../types';
import { WiseApiError } from '../types';

// Wise API base URLs
const DEFAULT_BASE_URL = 'https://api.transferwise.com';
...
export class WiseClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Handle errors
    if (!response.ok) {
      const errorData = data as { errors?: Array<{ code: string; message: string }> };
      throw new WiseApiError(
        errorData?.errors?.[0]?.message || `Wise API error: ${response.status}`,
        response.status,
        errorData?.errors
      );
    }
