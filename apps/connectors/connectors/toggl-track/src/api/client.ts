...
import { TogglTrackApiError, parseApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.track.toggl.com/api/v9';
...
export class TogglTrackClient {
  ...
  constructor(config: TogglTrackConfig) {
    ...
    if (!token) {
      throw new Error('Toggl Track API token is required');
    }
    this.apiToken = token;
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof TogglTrackApiError)) {
    ...
    throw lastError || new TogglTrackApiError('Request failed', lastStatus);
  }
