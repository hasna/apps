...
export class SeamlessAIClient {
  ...
  private readonly baseUrl = 'https://api.seamless.ai/v1';

  constructor(config: SeamlessAIConfig) {
    if (!config.apiKey) throw new Error('Seamless.AI apiKey is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new SeamlessAIApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
