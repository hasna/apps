import type { ResembleConfig, OutputFormat } from '../types';
import { ResembleApiError } from '../types';

const DEFAULT_BASE_URL = 'https://app.resemble.ai/api/v2';
...
export class ResembleClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new ResembleApiError(errorMessage, response.status);
    }
