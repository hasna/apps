import type { VultrConfig, OutputFormat } from '../types';
import { VultrApiError } from '../types';
...
export class VultrClient {
  ...
  /**
   * Make an authenticated request to Vultr API v2
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new VultrApiError(message, response.status, retryAfter);
    }
