import type { WithAiConfig, OutputFormat } from '../types';
import { WithAiApiError, parseApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.withai.co/v1';
...
export class WithAiClient {
  ...
  constructor(config: WithAiConfig) {
    ...
    this.apiKey = key;
    this.baseUrl = config.baseUrl || process.env.WITHAI_BASE_URL || DEFAULT_BASE_URL;
  }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof WithAiApiError)) {
    ...
    throw lastError || new WithAiApiError('Request failed', lastStatus);
  }
...
export { WithAiClient as ConnectorClient };
