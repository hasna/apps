...
import { PatentsViewApiError } from '../types';

const DEFAULT_BASE_URL = 'https://search.patentsview.org/api/v1';
...
export class PatentsViewClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new PatentsViewApiError(errorMessage, response.status);
    }
