...
// SupportBee base URL is company-specific.
// Format: https://<company>.supportbee.com
// Resolve from an explicit baseUrl, or from SUPPORTBEE_SUBDOMAIN (company slug).
function resolveBaseUrl(baseUrl?: string): string {
  if (baseUrl) {
    return baseUrl.replace(/\/+$/, '');
  }
  const subdomain = process.env.SUPPORTBEE_SUBDOMAIN;
  if (subdomain) {
    return `https://${subdomain}.supportbee.com`;
  }
  throw new Error(
    'A base URL is required (e.g. https://your-company.supportbee.com). ' +
      'Set baseUrl, SUPPORTBEE_BASE_URL, or SUPPORTBEE_SUBDOMAIN.'
  );
}
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    this.apiKey = key;
    // SupportBee requires a company-specific base URL
