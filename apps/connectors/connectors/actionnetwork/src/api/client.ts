...
// Action Network API v2 base URL
const DEFAULT_BASE_URL = 'https://actionnetwork.org/api/v2';
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    if (!key) {
      throw new Error('OSDI API token is required');
    }
    this.apiKey = key;
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Action Network uses OSDI-API-Token header for authentication
