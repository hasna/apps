...
export class Connector {
  ...
  static fromEnv(): Connector {
    const apiKey = process.env.TRAY_API_PLATFORM_API_KEY;
    const baseUrl = process.env.TRAY_API_PLATFORM_BASE_URL;

    if (!apiKey) {
      throw new Error('TRAY_API_PLATFORM_API_KEY environment variable is required');
    }
