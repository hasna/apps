import type { ConnectorConfig, ResponseMeta } from '../types';
import { ConnectorApiError, parseApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.wappalyzer.com/v2';
...
export class ConnectorClient {
  ...
  private lastResponseMeta: ResponseMeta = {};
  ...
  constructor(config: ConnectorConfig) {
    ...
    this.apiKey = key;
    this.baseUrl = (config.baseUrl || process.env.WAPPALYZER_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, '');
  }

  getLastResponseMeta(): ResponseMeta {
  ...
  private captureCreditHeaders(headers: Headers): void {
    ...
    this.lastResponseMeta = {
      creditsSpent: spent ? Number(spent) : undefined,
      creditsRemaining: remaining ? Number(remaining) : undefined,
    };
  }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        this.captureCreditHeaders(response.headers);
