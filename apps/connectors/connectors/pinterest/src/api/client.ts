import type { PinterestConfig, OutputFormat } from '../types';
import { PinterestApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.pinterest.com/v5';
...
export class PinterestClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Handle errors
    if (!response.ok) {
      const errorData = data as { message?: string; code?: number };
      const errorMessage = errorData?.message ||
        (typeof data === 'object' && data !== null ? JSON.stringify(data) : String(data || response.statusText));
      throw new PinterestApiError(errorMessage, response.status, errorData?.code);
    }
