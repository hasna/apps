...
export class Connector {
  ...
  static fromEnv(): Connector {
    const apiKey = process.env.TETTRA_API_PLATFORM_API_KEY;
    const baseUrl = process.env.TETTRA_API_PLATFORM_BASE_URL;

    if (!apiKey) {
      throw new Error('TETTRA_API_PLATFORM_API_KEY environment variable is required');
    }
