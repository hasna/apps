...
const CONNECTOR_NAME = 'connect-weaviate-api-platform';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Weaviate API Platform connector CLI - items, events, and vector search')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.WEAVIATE_API_PLATFORM_API_KEY = opts.apiKey;
...
function getClient(): Connector {
  ...
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set WEAVIATE_API_PLATFORM_API_KEY.`);
    process.exit(1);
  }
...
itemsCmd.command('list').description('List items').action(async () => {
  try {
    const client = getClient();
    print(await client.listItems(), getFormat(itemsCmd));
  } catch (err) {
    error(String(err));
    process.exit(1);
  }
...
itemsCmd
  ...
  .requiredOption('-b, --body <json>', 'JSON request body')
  .action(async (opts) => {
    try {
      const client = getClient();
      ...
      print(await client.createItem(body), getFormat(itemsCmd));
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
itemsCmd.command('get <itemId>').description('Get an item by ID').action(async (itemId: string) => {
  try {
    const client = getClient();
    print(await client.getItem(itemId), getFormat(itemsCmd));
  } catch (err) {
    error(String(err));
    process.exit(1);
  }
...
program
  ...
  .requiredOption('-b, --body <json>', 'JSON search request body')
  .action(async (opts) => {
    try {
      const client = getClient();
      const body = parseJsonOption(opts.body);
...
program
  ...
  .requiredOption('-p, --path <path>', 'Request path (e.g. /items)')
  .option('-m, --method <method>', 'HTTP method', 'GET')
  .option('-b, --body <json>', 'JSON request body')
  .option('-q, --query <json>', 'JSON query parameters')
  .action(async (opts) => {
    try {
      const client = getClient();
