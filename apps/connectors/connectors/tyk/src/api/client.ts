import type { TykConfig } from '../types';
import { TykApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.tyk.io/v1';
...
export class TykClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new TykApiError(message, response.status, errorData?.code);
    }
