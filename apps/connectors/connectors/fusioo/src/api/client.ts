...
export class FusiooClient {
  ...
  private readonly baseUrl = 'https://api.fusioo.com/v2';

  constructor(config: FusiooConfig) {
    if (!config.token) throw new Error('Fusioo token is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new FusiooApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
