...
export function getApiToken(): string | undefined {
  return process.env.UNIT_API_TOKEN || loadProfile().apiToken;
}
...
export function getEnvironment(): UnitEnvironment {
  const env = process.env.UNIT_ENVIRONMENT?.toLowerCase();
  if (env === 'production' || env === 'sandbox') return env;
  return loadProfile().environment ?? 'sandbox';
}

export function setEnvironment(environment: UnitEnvironment): void {
  const config = loadProfile();
