...
export const DEFAULT_BASE_URL = 'https://api.prod.speakeasyapi.dev';
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    if (!key) {
      throw new Error('Speakeasy API key is required (x-api-key header)');
    }
    this.apiKey = key;
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    const { method = 'GET', params, body, headers = {}, retries = 3, timeout = 30000, raw = false } = options;
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (raw || contentType.includes('application/octet-stream') || contentType.includes('application/x-yaml')) {
