...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    if (!config.email && !config.domain) {
      throw new Error('Email and domain are required for Agile CRM authentication');
    }
    this.apiKey = key;
    ...
    if (config.baseUrl) {
      ...
      throw new Error('Domain or baseUrl is required');
    }
  }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Agile CRM uses HTTP Basic Auth: email:apiKey
