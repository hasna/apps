import type { BrowseAIConfig } from '../types';
import { BrowseAIApiError } from '../types';
...
export class BrowseAIClient {
  ...
  constructor(config: BrowseAIConfig) {
    if (!config.apiKey) throw new Error('Browse AI apiKey is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new BrowseAIApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
