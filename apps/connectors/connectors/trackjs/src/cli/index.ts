...
function getClient(): Trackjs {
  ...
  return new Trackjs({ apiKey, customerId, baseUrl: getBaseUrl() });
}
...
profileCmd
  ...
  .option('--base-url <url>', 'Override TrackJS API host')
...
configCmd
  ...
  .description('Set API host override')
  .action((url: string) => {
    setBaseUrl(url);
...
configCmd
  ...
  .action(() => {
    ...
    info(`Base URL: ${baseUrl || chalk.gray('default')}`);
  });
