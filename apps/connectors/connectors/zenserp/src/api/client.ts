import type { ZenserpConfig, OutputFormat } from '../types';
import { ZenserpApiError, parseApiError } from '../types';

const DEFAULT_BASE_URL = 'https://app.zenserp.com/api/v2';
...
export class ZenserpClient {
  ...
  constructor(config: ZenserpConfig) {
    if (!config.apiKey) {
      throw new Error('Zenserp API key is required');
    }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof ZenserpApiError)) {
    ...
    throw lastError || new ZenserpApiError('Request failed', lastStatus);
  }
