...
export class HubPlannerClient {
  ...
  private readonly baseUrl = 'https://api.hubplanner.com/v1';

  constructor(config: HubPlannerConfig) {
    if (!config.apiKey) throw new Error('Hub Planner apiKey is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new HubPlannerApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
