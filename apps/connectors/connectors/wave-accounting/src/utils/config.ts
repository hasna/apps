...
const CONNECTOR_NAME = 'wave-accounting';
...
export function getAccessToken(): string | undefined {
  return process.env.WAVE_ACCESS_TOKEN || loadProfile().accessToken;
}
...
export function getBusinessId(): string | undefined {
  return process.env.WAVE_BUSINESS_ID || loadProfile().businessId;
}
...
export function getOAuthConfig(): OAuth2Config | null {
  ...
  const clientId = process.env.WAVE_CLIENT_ID || profile.clientId;
  const clientSecret = process.env.WAVE_CLIENT_SECRET || profile.clientSecret;
...
export function saveOAuthTokens(tokens: OAuth2Tokens): void {
  ...
  if (tokens.businessId) {
    profile.businessId = tokens.businessId;
  }
  saveProfile(profile);
}
