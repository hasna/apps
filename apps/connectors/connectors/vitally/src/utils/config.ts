...
import type { VitallyRegion } from '../types';
...
export function getApiKey(): string | undefined {
  return process.env.VITALLY_API_KEY || loadProfile().apiKey;
}
...
export function getSubdomain(): string | undefined {
  return process.env.VITALLY_SUBDOMAIN || loadProfile().subdomain;
}
...
export function getRegion(): VitallyRegion {
  const envRegion = process.env.VITALLY_REGION?.toLowerCase();
...
export function setRegion(region: VitallyRegion): void {
  const config = loadProfile();
...
export function getBaseUrl(): string | undefined {
  return process.env.VITALLY_BASE_URL || loadProfile().baseUrl;
}
...
export function buildVitallyConfig(): {
  apiKey: string;
  subdomain?: string;
  region?: VitallyRegion;
  baseUrl?: string;
