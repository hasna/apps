...
const DEFAULT_BASE_URL = 'https://api.accelo.com/api/v0';
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    // Build base URL from deployment name or use override
    if (config.baseUrl) {
      this.baseUrl = config.baseUrl;
    } else if (config.deployment) {
      this.baseUrl = `https://${config.deployment}.api.accelo.com/api/v0`;
    } else {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Accelo uses Bearer token authentication
