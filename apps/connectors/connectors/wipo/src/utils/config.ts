...
export function getApiKey(): string | undefined {
  return process.env.WIPO_API_KEY || process.env.WIPO_TOKEN || loadProfile().apiKey;
}
...
export function getHeadless(): boolean {
  const envVal = process.env.WIPO_HEADLESS;
  if (envVal !== undefined) {
    return envVal !== 'false';
  }
  const config = loadProfile();
...
export function getBrowser(): ProfileConfig['browser'] | undefined {
  return (process.env.WIPO_BROWSER as ProfileConfig['browser']) || loadProfile().browser;
}
...
export function getOutputDir(): string | undefined {
  return process.env.WIPO_OUTPUT_DIR || loadProfile().outputDir;
}
