...
const CONNECTOR_NAME = 'connect-vercel-blob-platform';
...
export function getStoreId(): string | undefined {
  return process.env.BLOB_STORE_ID || loadProfile().storeId;
}
...
export function getOidcToken(): string | undefined {
  return process.env.VERCEL_OIDC_TOKEN || loadProfile().oidcToken;
}

export function setOidcToken(oidcToken: string): void {
  const config = loadProfile();
  config.oidcToken = oidcToken;
  saveProfile(config);
}
