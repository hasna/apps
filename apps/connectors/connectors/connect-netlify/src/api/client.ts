import type { NetlifyConfig, OutputFormat } from '../types';
import { NetlifyApiError } from '../types';
...
export class NetlifyClient {
  ...
  /**
   * Make an authenticated request to Netlify API
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Handle errors - Netlify returns { message: string, code?: number }
    if (!response.ok) {
      const errorData = data as { message?: string; code?: number } | undefined;
      const message = errorData?.message || response.statusText;
      const code = errorData?.code;
      throw new NetlifyApiError(message, response.status, code);
    }
