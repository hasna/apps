...
const DEFAULT_BASE_URL = 'https://webapi.mymarketing.co.il/api';
...
export class ConnectorClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // ActiveTrail uses raw token in Authorization header (no Bearer prefix)
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        // Handle 204 No Content and PUT/DELETE with no body
        if (response.status === 204 || (method === 'DELETE' && response.ok) || (method === 'PUT' && response.ok && response.headers.get('content-length') === '0')) {
          return {} as T;
        }
