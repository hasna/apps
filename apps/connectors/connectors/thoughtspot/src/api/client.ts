...
/** URL-encode a path segment (liveboard IDs, names with special chars). */
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    if (!key) {
      throw new Error('ThoughtSpot API key (bearer token) is required');
    }
    if (!config.baseUrl) {
      throw new Error(
        'ThoughtSpot base URL is required (e.g. https://your-instance.thoughtspot.cloud/api/rest/2.0)',
      );
    }
    this.apiKey = key;
