...
// ActiveCampaign base URL is account-specific
// Format: https://<account>.api-us1.com/api/3
const DEFAULT_BASE_URL = 'https://youraccountname.api-us1.com';
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    // ActiveCampaign requires account-specific base URL
    const base = config.baseUrl || DEFAULT_BASE_URL;
    // Ensure /api/3 suffix
    this.baseUrl = base.endsWith('/api/3') ? base : `${base.replace(/\/+$/, '')}/api/3`;
  }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // ActiveCampaign uses Api-Token header
