...
export const API_VERSION = 'v11_24';
...
export interface RequestOptions {
  ...
  retries?: number;
  timeout?: number;
  /** Skip OAuth-Token header (e.g. password grant token exchange) */
  skipAuth?: boolean;
}
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    this.oauthToken =
      config.oauthToken || config.apiKey || config.token || config.accessToken;
  ...
  getApiPrefix(): string {
    return `${this.baseUrl}/rest/${API_VERSION}`;
  }
  ...
  private buildUrl(
    ...
    const url = new URL(`${this.getApiPrefix()}${normalizedPath}`);
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!skipAuth && this.oauthToken) {
      requestHeaders['OAuth-Token'] = this.oauthToken;
    }
  ...
  getTokenPreview(): string {
    ...
    if (this.oauthToken.length > 10) {
      return `${this.oauthToken.substring(0, 6)}...${this.oauthToken.substring(this.oauthToken.length - 4)}`;
    }
    return '***';
  }
}
