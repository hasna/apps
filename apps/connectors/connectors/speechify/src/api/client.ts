import type { SpeechifyConfig, OutputFormat } from '../types';
import { SpeechifyApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.sws.speechify.com/v1';
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (rawResponse) {
      if (!response.ok) {
        ...
        throw new SpeechifyApiError(errorText || response.statusText, response.status);
      }
    ...
    if (!response.ok) {
      ...
      throw new SpeechifyApiError(errorMessage, response.status);
    }
