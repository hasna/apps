...
export const ZOHO_ACCOUNTS_BASES: Record<string, string> = {
...
export function resolveAccountsBaseUrl(dataCenter?: string): string {
  ...
  const baseUrl = ZOHO_ACCOUNTS_BASES[dc];
  if (!baseUrl) {
    throw new Error(`Invalid data center "${dc}". Must be one of: ${Object.keys(ZOHO_ACCOUNTS_BASES).join(', ')}`);
  }
  return baseUrl;
}
...
export async function exchangeCodeForTokens(
  code: string,
  tokenUrl: string = getTokenEndpoint()
