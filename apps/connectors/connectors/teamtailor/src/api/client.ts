import type { TeamtailorConfig } from '../types';
import { TeamtailorApiError, parseApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.teamtailor.com/v1';
// Stable default X-Api-Version. The header is required on every request; callers
// may override it via config/env to pin a specific dated API version.
const DEFAULT_API_VERSION = '20240904';
...
export class TeamtailorClient {
  ...
  constructor(config: TeamtailorConfig) {
    if (!config.apiKey) {
      throw new Error('apiKey is required for Teamtailor API authentication');
    }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        // Parse response body (JSON:API responses use application/vnd.api+json)
        let data: unknown;
        ...
        // Do not retry once we have a parsed API error.
        if (attempt < retries && !(err instanceof TeamtailorApiError)) {
    ...
    throw lastError || new TeamtailorApiError('Request failed', lastStatus);
  }
  ...
  getCredentialPreview(): string {
    if (this.apiKey.length > 10) {
      return `Token: ${this.apiKey.substring(0, 6)}...`;
    }
    return 'Token: ***';
  }
}
