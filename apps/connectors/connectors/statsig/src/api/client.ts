...
import { StatsigApiError, parseApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://statsigapi.net/console/v1';
export const DEFAULT_API_VERSION = '20240601';
...
export class StatsigClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    const requestHeaders: Record<string, string> = {
      ...
      'STATSIG-API-VERSION': DEFAULT_API_VERSION,
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof StatsigApiError)) {
    ...
    throw lastError || new StatsigApiError('Request failed', lastStatus);
  }
