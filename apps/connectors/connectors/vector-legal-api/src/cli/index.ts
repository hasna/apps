...
const CONNECTOR_NAME = 'connect-vector-legal-api';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Vector Legal API connector CLI - legal document management and search')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.VECTOR_LEGAL_API_KEY = opts.apiKey;
      debug('API key set from command line flag');
    }

    if (opts.baseUrl) {
      process.env.VECTOR_LEGAL_API_BASE_URL = opts.baseUrl;
...
function getClient(): Connector {
  const { apiKey, baseUrl } = getConnectorConfig();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set VECTOR_LEGAL_API_KEY.`);
    process.exit(1);
  }
...
const documentsCmd = program.command('documents').description('Manage legal documents');
...
documentsCmd
  ...
  .action(async (opts) => {
    try {
      const client = getClient();
      const result = await client.documents.create(parseJsonBody(opts.body));
...
const eventsCmd = program.command('events').description('List API events');
...
const searchCmd = program.command('search').description('Search documents and content');
...
searchCmd
  ...
  .action(async (opts) => {
    try {
      const client = getClient();
      const result = await client.search.search(parseJsonBody(opts.body));
...
program
  ...
  .action(async (method: string, path: string, opts) => {
    ...
    try {
      ...
      const params = opts.query ? (parseJsonBody(opts.query) as Record<string, string | number | boolean | undefined>) : undefined;
      const body = opts.body ? parseJsonBody(opts.body) : undefined;
      ...
      switch (normalizedMethod) {
        case 'GET':
          ...
          break;
        case 'POST':
          result = await api.post(path, body, params);
          break;
        case 'PUT':
          result = await api.put(path, body, params);
          break;
        case 'PATCH':
          result = await api.patch(path, body, params);
          break;
        case 'DELETE':
