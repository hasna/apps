...
import { UspsApiError, parseApiError } from '../types';

/** Default USPS REST API base (legacy v1 surface; see https://developers.usps.com/ for current APIs). */
export const DEFAULT_BASE_URL = 'https://api.usps.com/v1';
...
export class UspsClient {
  ...
  constructor(config: UspsConfig) {
    ...
    if (!key) {
      throw new Error('USPS API key is required');
    }
    this.apiKey = key;
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof UspsApiError)) {
    ...
    throw lastError || new UspsApiError('Request failed', lastStatus);
  }
