import type { VelumLabsConfig } from '../types';
import { VelumLabsApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.velum-labs.com/v1';
...
export class VelumLabsClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new VelumLabsApiError(errorMessage, response.status);
    }
