...
export function getApiKey(): string | undefined {
  return process.env.AMADEUS_API_KEY || loadProfile().apiKey;
}

export function getApiSecret(): string | undefined {
  return process.env.AMADEUS_API_SECRET || loadProfile().apiSecret;
}

export function getEnvironment(): 'test' | 'production' {
  const env = process.env.AMADEUS_ENVIRONMENT || loadProfile().environment;
  return env === 'production' ? 'production' : 'test';
}

export function setCredentials(apiKey: string, apiSecret: string, environment?: 'test' | 'production'): void {
