...
export function getProjectRef(): string | undefined {
  return process.env.TRIGGER_PROJECT_REF || loadProfile().projectRef;
}

export function setProjectRef(projectRef: string): void {
  const config = loadProfile();
...
export function isPersonalAccessToken(apiKey: string): boolean {
  return apiKey.startsWith('tr_pat_');
}
