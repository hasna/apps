...
const DEFAULT_BASE_URL = 'https://ipgeolocation.abstractapi.com';
...
export class ConnectorClient {
  ...
  private buildUrl(path: string, params?: Record<string, string | number | boolean | undefined>, baseUrl?: string): string {
    const url = new URL(`${baseUrl || this.baseUrl}${path}`);

    // Add api_key as query parameter (Abstract API auth method)
  ...
  /**
   * Make an authenticated request to the Abstract API
   * Authentication is via api_key query parameter
   */
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    const { method = 'GET', params, body, headers = {}, baseUrl, retries = 3, timeout = 30000 } = options;
  ...
  async get<T>(path: string, params?: Record<string, string | number | boolean | undefined>, baseUrl?: string): Promise<T> {
