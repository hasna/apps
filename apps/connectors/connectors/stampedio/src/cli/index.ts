...
function getClient(): Stampedio {
  if (!hasCredentials()) {
    error(`No Stamped.io credentials configured. Run "${CONNECTOR_NAME} config set" or set STAMPEDIO_PUBLIC_KEY / STAMPEDIO_PRIVATE_KEY / STAMPEDIO_STORE_HASH.`);
    process.exit(1);
  }
...
configCmd
  ...
  .option('--public-key <key>', 'Public API key (Basic Auth username)')
  .option('--private-key <key>', 'Private API key (Basic Auth password)')
  .option('--store-hash <hash>', 'Store hash')
...
loyaltyCmd
  ...
  .action((opts, cmd) =>
    run(async () => {
      const result = await getClient().loyalty.awardPoints(opts.email, opts.points, opts.reason);
      success(`Awarded ${opts.points} points to ${opts.email}`);
      print(result, getFormat(cmd));
    })
  );
...
loyaltyCmd
  ...
  .action((opts, cmd) =>
    run(async () => {
      const result = await getClient().loyalty.deductPoints(opts.email, opts.points, opts.reason);
      success(`Deducted ${opts.points} points from ${opts.email}`);
      print(result, getFormat(cmd));
    })
  );
