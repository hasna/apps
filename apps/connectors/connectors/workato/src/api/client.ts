...
import { WorkatoApiError, parseApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://www.workato.com/api';
...
export function validateBaseUrl(baseUrl: string): string {
  ...
  if (!url.startsWith('https://')) {
    throw new Error('Workato base URL must start with https://');
  }
  return url;
}
...
export class WorkatoClient {
  ...
  constructor(config: WorkatoConfig) {
    if (!config.apiToken?.trim()) {
      throw new Error('Workato API token is required');
    }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof WorkatoApiError)) {
    ...
    throw lastError || new WorkatoApiError('Request failed', lastStatus);
  }
