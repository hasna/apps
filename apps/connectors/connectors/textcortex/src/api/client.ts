...
export const DEFAULT_BASE_URL = 'https://api.textcortex.com';
...
export class TextCortexClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      const errorData = data as { message?: string; error?: { message?: string } } | undefined;
      const errorMessage = errorData?.message || errorData?.error?.message || response.statusText;
      throw new TextCortexApiError(errorMessage, response.status, data);
    }
