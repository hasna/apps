import { OctoAIApiError, type OctoAIConfig, type ChatRequest, type ChatResponse, type ModelsResponse } from '../types';

const DEFAULT_BASE_URL = 'https://text.octoai.run/v1';

export class OctoAIClient {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      throw new OctoAIApiError(errorMessage, response.status);
    }
