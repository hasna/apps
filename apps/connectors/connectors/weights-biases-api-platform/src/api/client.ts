...
import { WeightsBiasesApiPlatformError } from '../types';

const DEFAULT_BASE_URL = 'https://api.weightsbiasesapiplatform.com/v1';
...
export class WeightsBiasesApiPlatformClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new WeightsBiasesApiPlatformError(errorMessage, response.status);
    }
