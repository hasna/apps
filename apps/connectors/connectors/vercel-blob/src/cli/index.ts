...
import type { BlobAccessType, OutputFormat } from '../types';
...
program
  .name(CONNECTOR_NAME)
  .description('Vercel Blob connector - object storage via Vercel Blob')
  .version(VERSION)
...
// Blob commands (alumia inventory: list-blobs, create-blob, get-blob, search)
const blobCmd = program.command('blob').description('Vercel Blob operations');

blobCmd
  .command('list')
  .description('List blobs in the store (list-blobs)')
...
blobCmd
  .command('create <pathname> <file>')
  .description('Upload a file to blob storage (create-blob)')
...
blobCmd
  ...
  .description('Download blob content (get-blob)')
  ...
  .action(async (urlOrPathname: string, opts) => {
    try {
      const access = parseAccess(opts.access);
      const client = getClient();
      const result = await client.getBlob(urlOrPathname, access);
      if (!result) {
...
blobCmd
  .command('search')
  .description('Search blobs by prefix with pagination (search)')
  .option('--prefix <prefix>', 'Pathname prefix filter')
  .option('--cursor <cursor>', 'Pagination cursor from a previous search')
  .option('-l, --limit <number>', 'Maximum blobs to return', '100')
  .option('--mode <mode>', 'expanded or folded', 'expanded')
  .action(async (opts) => {
    try {
      const client = getClient();
      const result = await client.searchBlobs({
        prefix: opts.prefix,
        cursor: opts.cursor,
        limit: parseInt(opts.limit, 10),
        mode: opts.mode,
...
blobCmd
  .command('events')
  .description('List blob store events (list-events — not supported on public API)')
  .action(async () => {
    try {
      const client = getClient();
...
program
  ...
  .description('Send a raw HTTP request to the Vercel Blob API base (https://vercel.com/api/blob)')
  .option('--body <json>', 'Request body (JSON string)')
  .option('-H, --header <header>', 'Extra header (key:value), repeatable', (value, prev: string[]) => [...prev, value], [])
