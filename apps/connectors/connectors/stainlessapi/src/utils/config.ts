...
import type { StainlessEnvironment } from '../types';
...
export function getApiKey(): string | undefined {
  return process.env.STAINLESS_API_KEY || loadProfile().apiKey;
}
...
export function getDefaultProject(): string | undefined {
  return process.env.STAINLESS_PROJECT || loadProfile().project;
}
...
export function getEnvironment(): StainlessEnvironment | undefined {
  return loadProfile().environment;
}

export function setEnvironment(environment: StainlessEnvironment): void {
  const config = loadProfile();
