...
const DEFAULT_BASE_URL = 'https://tenor.googleapis.com/v2';
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    if (!key) {
      throw new Error('Tenor API key is required');
    }
    this.apiKey = key;
  ...
  private buildUrl(path: string, params?: Record<string, string | number | boolean | undefined>): string {
    ...
    // Tenor authenticates via the `key` query parameter (not an Authorization header).
    url.searchParams.append('key', this.apiKey);

    // Optional integration identifier used by Tenor for content personalization.
    if (this.clientKey) {
  ...
  /**
   * Make an authenticated request to the Tenor API.
   * Authentication is via the `key` query parameter.
   */
