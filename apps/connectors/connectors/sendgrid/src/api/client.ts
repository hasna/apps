import type { SendGridConfig, OutputFormat, SendGridErrorDetail } from '../types';
import { SendGridApiError } from '../types';
...
export class SendGridClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Handle 202 Accepted (used for mail/send)
    ...
    // Handle errors - SendGrid returns errors array
    if (!response.ok) {
      const errorData = data as { errors?: SendGridErrorDetail[] } | undefined;
      const errors = errorData?.errors;
      const message = errors?.[0]?.message || response.statusText;
      throw new SendGridApiError(message, response.status, errors);
    }
  ...
  async delete<T>(path: string, params?: Record<string, string | number | boolean | undefined>, body?: Record<string, unknown> | object): Promise<T> {
    return this.request<T>(path, { method: 'DELETE', params, body: body as Record<string, unknown> });
  }
