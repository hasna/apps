...
import { getAuthorizationEndpoint, getCallbackTokenEndpoint, getTokenEndpoint, resolveAccountsBaseUrl } from './auth';

describe('Zoho Vault OAuth endpoints', () => {
  ...
  test('uses callback accounts server for token exchange', () => {
    const callbackUrl = new URL('http://localhost:8089/callback?code=ok&accounts-server=https://accounts.zoho.eu');
    expect(getCallbackTokenEndpoint(callbackUrl)).toBe('https://accounts.zoho.eu/oauth/v2/token');
  });

  test('rejects unsupported callback accounts servers', () => {
    const callbackUrl = new URL('http://localhost:8089/callback?code=ok&accounts-server=https://example.test');
    expect(() => getCallbackTokenEndpoint(callbackUrl)).toThrow('Unsupported Zoho accounts server');
  });
});
