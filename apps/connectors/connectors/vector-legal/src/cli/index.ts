...
const CONNECTOR_NAME = 'connect-vector-legal';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Vector Legal connector CLI - legal document platform')
  .version(VERSION)
...
function getClient(): Connector {
  ...
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set VECTOR_LEGAL_API_KEY environment variable.`);
    process.exit(1);
  }
...
documentsCmd
  ...
  .option('--offset <number>', 'Offset results')
  .action(async (opts) => {
    try {
...
documentsCmd
  ...
  .option('--body <json>', 'JSON body for additional fields')
  .action(async (opts) => {
    try {
      const client = getClient();
      ...
      if (opts.body) {
        const extra = JSON.parse(opts.body);
        Object.assign(params, extra);
      }
      const result = await client.documents.create(params);
...
const eventsCmd = program
  .command('events')
  .description('List platform events');
...
searchCmd
  ...
  .option('--limit <number>', 'Limit results')
  .option('--offset <number>', 'Offset results')
  .option('--body <json>', 'JSON body for additional search parameters')
  .action(async (opts) => {
    try {
      const client = getClient();
      ...
      const result = await client.search.search(params as { query: string });
