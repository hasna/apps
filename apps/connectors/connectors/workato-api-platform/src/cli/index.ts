...
const CONNECTOR_NAME = 'connect-workato-api-platform';
const VERSION = '0.0.1';
...
program
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.WORKATO_API_PLATFORM_API_KEY = opts.apiKey;
      debug('API key set from command line flag');
    }

    if (opts.baseUrl) {
      process.env.WORKATO_API_PLATFORM_BASE_URL = opts.baseUrl;
...
function getClient(): Connector {
  ...
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set WORKATO_API_PLATFORM_API_KEY.`);
    process.exit(1);
  }
