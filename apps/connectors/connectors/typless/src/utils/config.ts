...
function hardenPath(path: string, mode: number): void {
  try {
    ...
    // Best-effort hardening. Reads still fail closed when files are absent.
  }
}
...
export function ensureConfigDir(): void {
  ...
  hardenPath(CONFIG_DIR, PRIVATE_DIR_MODE);
...
export function getApiKey(): string | undefined {
  return process.env.TYPLESS_API_KEY || loadProfile().apiKey;
}
...
export function getApiSecret(): string | undefined {
  return process.env.TYPLESS_API_SECRET || loadProfile().apiSecret;
}
...
export function getToken(): string | undefined {
  return process.env.TYPLESS_TOKEN || process.env.TYPLESS_API_KEY || loadProfile().token || loadProfile().apiKey;
}
