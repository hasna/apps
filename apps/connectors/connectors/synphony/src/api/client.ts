...
const DEFAULT_BASE_URL = 'https://api.synphony.ai/v1';
...
/**
 * HTTP client for the Synphony API.
 *
 * Authentication is performed with an `Authorization: Bearer <api_key>` header.
 * The base URL is overridable so the same client can target self-hosted or
 * staging deployments.
 */
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    if (!key) {
      throw new Error('Synphony API key is required');
    }
    this.apiKey = key;
  ...
  /**
   * Make an authenticated request to the Synphony API.
   */
