...
export const DEFAULT_BASE_URL = 'https://api.woodpecker.co/rest';
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof Error && err.name === 'WoodpeckerApiError')) {
    ...
    throw lastError || parseApiError('Request failed', lastStatus);
  }
