...
export const DEFAULT_REST_BASE_URL = 'https://api.stackadapt.com/service/v2';
export const DEFAULT_GRAPHQL_URL = 'https://api.stackadapt.com/graphql';
...
export class ConnectorClient {
  ...
  constructor(config: ConnectorConfig) {
    ...
    this.apiKey = key;
    this.restBaseUrl = config.baseUrl || DEFAULT_REST_BASE_URL;
  ...
  async graphql<T>(payload: { query: string; variables?: Record<string, unknown>; operationName?: string }): Promise<T> {
  ...
  async put<T>(path: string, body?: Record<string, unknown> | object): Promise<T> {
    return this.request<T>(path, { method: 'PUT', body: body as Record<string, unknown> });
  }
