...
const DEFAULT_BASE_URL = 'https://app.accuranker.com/api/v4';
...
export class ConnectorClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // AccuRanker uses Token authentication: Authorization: Token <token>
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        // Handle 202 Accepted (async job creation)
        if (response.status === 202) {
          ...
          return { ...(data as object || {}), _location: location, _status: 202 } as T;
        }
  ...
  async delete<T>(path: string, body?: Record<string, unknown> | object, params?: Record<string, string | number | boolean | undefined>): Promise<T> {
    return this.request<T>(path, { method: 'DELETE', body: body as Record<string, unknown>, params });
  }
