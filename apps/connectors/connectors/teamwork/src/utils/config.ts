...
export interface ProfileConfig {
  // Teamwork API token (used as the Basic auth username)
  apiKey?: string;
  token?: string; // Alias for apiKey
  // Teamwork site name (subdomain of {installation}.teamwork.com)
  installation?: string;
...
export function getInstallation(): string | undefined {
  return process.env.TEAMWORK_INSTALLATION || loadProfile().installation;
}

export function setInstallation(installation: string): void {
  const config = loadProfile();
...
export function getBaseUrl(): string | undefined {
  return process.env.TEAMWORK_BASE_URL || loadProfile().baseUrl;
}
