...
import { EntraIDApiError } from '../types';
...
export class EntraIDClient {
  ...
  constructor(config: EntraIDConfig) {
    if (!config.token) throw new Error('Entra ID token is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new EntraIDApiError(err.error?.message || response.statusText, response.status);
    }
    return data as T;
  }
}
