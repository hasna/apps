import type { TitoConfig } from '../types';
import { TitoApiError, parseApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.tito.io/v3';
...
export class TitoClient {
  ...
  constructor(config: TitoConfig) {
    ...
    if (!token) {
      throw new Error('Tito API token is required');
    }
    this.apiToken = token;
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof TitoApiError)) {
    ...
    throw lastError || new TitoApiError('Request failed', lastStatus);
  }
...
/** @deprecated Use TitoClient */
export { TitoClient as ConnectorClient };
