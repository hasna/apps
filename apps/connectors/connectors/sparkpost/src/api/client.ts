import type { SparkPostConfig, OutputFormat, SparkPostErrorDetail } from '../types';
...
export class SparkPostClient {
  ...
  /**
   * Make an authenticated request to SparkPost API.
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      const errorData = data as { errors?: SparkPostErrorDetail[] } | undefined;
      const errors = errorData?.errors;
      const message = errors?.[0]?.message || response.statusText;
      throw new SparkPostApiError(message, response.status, errors);
    }
