...
const CONNECTOR_NAME = 'connect-3scribe';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('3Scribe transcription API connector CLI')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.THREESCRIBE_API_KEY = opts.apiKey;
...
function getClient(): Connector {
  ...
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set THREESCRIBE_API_KEY environment variable.`);
    process.exit(1);
  }
...
jobsCmd
  ...
  .option('--perpage <number>', 'Results per page')
...
jobsCmd
  .command('get <jobId>')
  .description('Get a transcription job by ID')
...
program
  ...
  .requiredOption('-u, --url <url>', 'URL to the audio/video file')
  .option('-n, --name <name>', 'Job name')
  .option('-l, --language <lang>', 'Language code')
  .action(async (opts) => {
    try {
      const client = getClient();
      const result = await client.jobs.transcribe({
        url: opts.url,
        name: opts.name,
        language: opts.language,
      });
      success('Transcription job submitted!');
