...
describe('TypeformClient', () => {
  const mockConfig = {
    apiToken: 'placeholder-token-for-tests',
  ...
  describe('appendQuery', () => {
    ...
    test('joins array values with commas', () => {
      const path = appendQuery('/forms/abc/responses', { included_response_ids: ['resp1', 'resp2'] });
      expect(path).toContain('included_response_ids=resp1%2Cresp2');
    });
  ...
  describe('request', () => {
    ...
    test('sends Bearer authorization header', async () => {
      ...
      expect(call[1].headers.Authorization).toBe('Bearer placeholder-token-for-tests');
    });
    ...
    test('deleteResponses uses Typeform response ids', async () => {
      const fetchMock = mock(() =>
        Promise.resolve({
          ok: true,
          status: 200,
      ...
      const typeform = new Typeform(mockConfig);
      await typeform.deleteResponses('form/with/slash', ['resp1', 'resp2']);

      const call = fetchMock.mock.calls[0] as unknown as [string, RequestInit];
      expect(call[0]).toBe('https://api.typeform.com/forms/form%2Fwith%2Fslash/responses?included_response_ids=resp1%2Cresp2');
