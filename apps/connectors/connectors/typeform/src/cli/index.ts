...
responsesCmd.command('delete <formId>')
  .description('Delete responses by token')
  .requiredOption('--tokens <csv>', 'Comma-separated response tokens')
  .action(async (formId: string, opts) => {
    try {
      const client = getClient();
      const tokens = opts.tokens.split(',').map((t: string) => t.trim()).filter(Boolean);
      await client.deleteResponses(formId, tokens);
      success(`Deleted ${tokens.length} response(s) from form ${formId}`);
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
