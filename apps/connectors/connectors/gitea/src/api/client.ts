...
import { GiteaApiError } from '../types';
...
export class GiteaClient {
  ...
  constructor(config: GiteaConfig) {
    if (!config.token || !config.url) throw new Error('Gitea token and url are required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new GiteaApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
