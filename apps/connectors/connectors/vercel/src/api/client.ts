import type { VercelConfig, OutputFormat } from '../types';
import { VercelApiError } from '../types';
...
export class VercelClient {
  ...
  private buildUrl(path: string, params?: Record<string, string | number | boolean | undefined>): string {
    ...
    // Add teamId to all requests if set
    if (this.teamId) {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Handle errors - Vercel returns { error: { code, message } }
    if (!response.ok) {
