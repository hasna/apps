...
import { DEFAULT_BASE_URL } from '../utils/config';
...
export class TriggerDevClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      const errorData = data as { error?: string; message?: string; details?: Array<{ message?: string }> } | undefined;
      const detailMessage = errorData?.details?.[0]?.message;
      const message = detailMessage || errorData?.error || errorData?.message || response.statusText;
