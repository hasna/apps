...
const CONNECTOR_NAME = 'connect-tricentis';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Tricentis test automation platform API connector')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.TRICENTIS_API_KEY = opts.apiKey;
...
function getClient(): Tricentis {
  const apiKey = getApiKey();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set TRICENTIS_API_KEY.`);
    process.exit(1);
  }
  return new Tricentis({ apiKey, baseUrl: getBaseUrl() });
}
...
profileCmd
  ...
  .action((name?: string) => {
    ...
    info(`Base URL: ${config.baseUrl || chalk.gray('default (https://api.tricentis.com/v1)')}`);
  });
...
configCmd
  ...
  .action(() => {
    ...
    info(`Base URL: ${getBaseUrl() || chalk.gray('default (https://api.tricentis.com/v1)')}`);
  });
...
const testsCmd = program.command('tests').description('Manage tests');
...
testsCmd
  ...
  .action(async (opts: { limit?: string; offset?: string }) => {
    try {
      ...
      const result = await client.listTests({
...
testsCmd
  ...
  .option('--body <json>', 'Full JSON request body (overrides name/description)')
...
program
  .command('search')
  .description('Search Tricentis resources')
  .requiredOption('-q, --query <query>', 'Search query')
  .option('--limit <n>', 'Maximum results')
  .option('--body <json>', 'Full JSON search body (overrides query/limit)')
  .action(async (opts: { query: string; limit?: string; body?: string }) => {
    try {
      const client = getClient();
      const body = opts.body
        ? JSON.parse(opts.body) as Record<string, unknown>
        : { query: opts.query, ...(opts.limit ? { limit: Number(opts.limit) } : {}) };
  ...
  .action(async (opts: { path: string; method: string; body?: string; query?: string }) => {
    try {
      ...
      const result = await client.rawRequest({
        ...
        params: opts.query ? JSON.parse(opts.query) as Record<string, string | number | boolean | undefined> : undefined,
      });
      print(result, getFormat(program));
    } catch (err) {
      error(String(err));
