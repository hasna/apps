...
const CONNECTOR_NAME = 'connect-uploadcare';
...
export function getPublicKey(): string | undefined {
  return process.env.UPLOADCARE_PUBLIC_KEY || loadProfile().publicKey;
}
...
export function getSecretKey(): string | undefined {
  return process.env.UPLOADCARE_SECRET_KEY || loadProfile().secretKey;
}
...
export function getBaseUrl(): string | undefined {
  return process.env.UPLOADCARE_BASE_URL || loadProfile().baseUrl;
}

export function setCredentials(publicKey: string, secretKey: string): void {
  const config = loadProfile();
  config.publicKey = publicKey;
  config.secretKey = secretKey;
  saveProfile(config);
