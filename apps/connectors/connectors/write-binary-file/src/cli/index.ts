...
const CONNECTOR_NAME = 'connect-write-binary-file';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Write Binary File connector CLI - binary file platform with multi-profile support')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.WRITE_BINARY_FILE_API_KEY = opts.apiKey;
    }
  });
...
function getClient(): WriteBinaryFile {
  const apiKey = getApiKey();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set WRITE_BINARY_FILE_API_KEY.`);
    process.exit(1);
  }
  return new WriteBinaryFile({ apiKey, baseUrl: getBaseUrl() });
}
...
filesCmd
  ...
  .action(async (opts) => {
    try {
      ...
      const result = await client.listFiles(Object.keys(query).length ? query : undefined);
...
filesCmd
  ...
  .action(async (opts) => {
    try {
      ...
      const result = await client.createFile(body);
      print(result, getFormat(filesCmd));
    } catch (err) {
      error(String(err));
      process.exit(1);
...
eventsCmd
  ...
  .action(async (opts) => {
    try {
      ...
      if (opts.limit) query.limit = Number(opts.limit);
      if (opts.offset) query.offset = Number(opts.offset);
      const result = await client.listEvents(Object.keys(query).length ? query : undefined);
...
program
  .command('search')
  .description('Search files and metadata')
  .requiredOption('-b, --body <json>', 'Search request body JSON')
  .action(async (opts) => {
    try {
      const client = getClient();
      const body = parseJsonOption(opts.body, 'body');
...
program
  ...
  .requiredOption('--path <path>', 'API path (e.g. /files)')
