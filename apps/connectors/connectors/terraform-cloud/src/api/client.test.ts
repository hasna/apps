...
describe('TerraformCloud API routes', () => {
  ...
  test('applyRun POST /runs/:id/actions/apply', async () => {
    ...
    expect(recorded[0].method).toBe('POST');
    expect(JSON.parse(recorded[0].body!)).toEqual({ comment: 'ship it' });
  });

  test('applyRun omits request body when no comment is provided', async () => {
    const recorded = installFetch(() => ({}));
    const tfc = new TerraformCloud({ apiToken: 'token' });
    await tfc.applyRun('run-1');
    expect(recorded[0].url).toContain('/api/v2/runs/run-1/actions/apply');
    expect(recorded[0].body).toBeUndefined();
  });

  test('cancelRun POSTs a plain comment body', async () => {
    const recorded = installFetch(() => ({}));
    const tfc = new TerraformCloud({ apiToken: 'token' });
    await tfc.cancelRun('run-1', 'stuck');
    expect(recorded[0].url).toContain('/api/v2/runs/run-1/actions/cancel');
    expect(recorded[0].method).toBe('POST');
    expect(JSON.parse(recorded[0].body!)).toEqual({ comment: 'stuck' });
  });

  test('discardRun POSTs a plain comment body', async () => {
    const recorded = installFetch(() => ({}));
    const tfc = new TerraformCloud({ apiToken: 'token' });
    await tfc.discardRun('run-1', 'not needed');
    expect(recorded[0].url).toContain('/api/v2/runs/run-1/actions/discard');
    expect(recorded[0].method).toBe('POST');
    expect(JSON.parse(recorded[0].body!)).toEqual({ comment: 'not needed' });
  });
  ...
  test('createWorkspaceVar POST /vars with workspace relationship', async () => {
    ...
    expect(recorded[0].url).toContain('/api/v2/workspaces/ws-1/vars');
    const body = JSON.parse(recorded[0].body!);
  ...
  test('createConfigurationVersion POST /workspaces/:id/configuration-versions', async () => {
    const recorded = installFetch(() => ({ data: { type: 'configuration-versions', id: 'cv-1' } }));
    const tfc = new TerraformCloud({ apiToken: 'token' });
    await tfc.createConfigurationVersion('ws-1', { 'auto-queue-runs': true });
    expect(recorded[0].method).toBe('POST');
    expect(recorded[0].url).toContain('/api/v2/workspaces/ws-1/configuration-versions');
    const body = JSON.parse(recorded[0].body!);
    expect(body.data.type).toBe('configuration-versions');
    expect(body.data.attributes['auto-queue-runs']).toBe(true);
    expect(body.data.relationships).toBeUndefined();
  });
  ...
  test('updateVar PATCH /workspaces/:workspace_id/vars/:id', async () => {
    const recorded = installFetch(() => ({ data: { type: 'vars', id: 'var-1' } }));
    const tfc = new TerraformCloud({ apiToken: 'token' });
    await tfc.updateVar('ws-1', 'var-1', { value: 'eu-west-1' });
    expect(recorded[0].method).toBe('PATCH');
    expect(recorded[0].url).toContain('/api/v2/workspaces/ws-1/vars/var-1');
    const body = JSON.parse(recorded[0].body!);
    expect(body.data.id).toBe('var-1');
    expect(body.data.attributes.value).toBe('eu-west-1');
  });

  test('deleteVar DELETE /workspaces/:workspace_id/vars/:id', async () => {
    const recorded = installFetch(() => ({}));
    const tfc = new TerraformCloud({ apiToken: 'token' });
    await tfc.deleteVar('ws-1', 'var-1');
    expect(recorded[0].method).toBe('DELETE');
    expect(recorded[0].url).toContain('/api/v2/workspaces/ws-1/vars/var-1');
  });
});
