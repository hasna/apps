...
export class TerraformCloud {
  ...
  applyRun(runId: string, comment?: string) {
    return this.client.post(
      ...
      comment ? jsonApiBody('comments', { body: comment }) : undefined,
    );
  }
  ...
  createWorkspaceVar(workspaceId: string, attributes: VariableAttributes) {
    return this.client.post<JsonApiDocument<'vars', VariableAttributes>>(
      '/vars',
      jsonApiBody('vars', attributes as unknown as Record<string, unknown>, workspaceRelationship(workspaceId)),
    );
  }

  updateVar(varId: string, attributes: Partial<VariableAttributes>) {
  ...
  deleteVar(varId: string) {
    return this.client.delete(`/vars/${varId}`);
  }
