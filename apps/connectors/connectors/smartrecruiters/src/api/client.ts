...
export class SmartRecruitersClient {
  ...
  constructor(config: SmartRecruitersConfig) {
    if (!config.apiKey) {
      throw new Error('apiKey is required for SmartRecruiters API authentication');
    }
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        // Do not retry API errors we already classified as terminal.
        if (attempt < retries && !(err instanceof SmartRecruitersApiError)) {
    ...
    throw lastError || new SmartRecruitersApiError('Request failed', lastStatus);
  }
