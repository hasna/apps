...
const CONNECTOR_NAME = 'connect-cerebras';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Cerebras connector CLI - Fast AI inference with multi-profile support')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.CEREBRAS_API_KEY = opts.apiKey;
    }
  });
...
function getClient(): Cerebras {
  ...
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set CEREBRAS_API_KEY environment variable.`);
    process.exit(1);
  }
...
const chatCmd = program
  .command('chat')
  .description('Chat with Cerebras models');
