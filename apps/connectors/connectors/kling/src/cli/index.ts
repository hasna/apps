...
const CONNECTOR_NAME = 'connect-kling';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Kling AI connector CLI - AI video generation with multi-profile support')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.KLING_API_KEY = opts.apiKey;
    }
  });
...
function getClient(): Kling {
  const apiKey = getApiKey();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set KLING_API_KEY environment variable.`);
    process.exit(1);
  }
