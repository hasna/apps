...
export class UProcClient {
  ...
  private readonly baseUrl = 'https://api.uproc.io/api/v2';

  constructor(config: UProcConfig) {
    if (!config.apiKey) throw new Error('uProc apiKey is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new UProcApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
