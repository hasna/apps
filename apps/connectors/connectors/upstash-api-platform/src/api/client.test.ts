...
describe('UpstashApiPlatformClient', () => {
  ...
  test('refuses to send Basic auth to non-Upstash base URLs', async () => {
    let called = false;
    globalThis.fetch = (async () => {
      called = true;
    ...
    const client = new UpstashApiPlatformClient(config);
    await expect(client.get('/teams', undefined, { baseUrl: 'https://example.com' })).rejects.toThrow(
      'Refusing to send Upstash credentials to a non-Upstash API host',
    );
