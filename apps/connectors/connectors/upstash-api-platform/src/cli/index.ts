...
program
  ...
  .option('--show-secrets', 'Print token and credential fields in command output')
  .hook('preAction', (thisCommand) => {
    ...
    setCredentialOverride(opts.email, opts.apiKey);
  });
...
function shouldShowSecrets(cmd: Command): boolean {
  return Boolean(cmd.parent?.opts().showSecrets);
}

function printCommandResult(cmd: Command, data: unknown): void {
  print(data, getFormat(cmd), { redactSecrets: !shouldShowSecrets(cmd) });
}
...
teamCmd
  ...
  .action(async () => {
    try {
      ...
      printCommandResult(teamCmd, await client.listTeams());
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
teamCmd
  ...
  .action(async (name: string, opts) => {
    try {
      ...
      printCommandResult(teamCmd, result);
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
teamCmd
  ...
  .action(async (teamId: string) => {
    try {
      const client = getClient();
      printCommandResult(teamCmd, await client.getTeamMembers(teamId));
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
vectorCmd
  ...
  .action(async () => {
    try {
      const client = getClient();
      printCommandResult(vectorCmd, await client.listVectorIndices());
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
vectorCmd
  ...
  .action(async (id: string) => {
    try {
      const client = getClient();
      printCommandResult(vectorCmd, await client.getVectorIndex(id));
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
vectorCmd
  ...
  .action(async (opts) => {
    try {
      ...
      printCommandResult(vectorCmd, result);
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
accountCmd
  ...
  .action(async () => {
    try {
      const client = getClient();
      printCommandResult(accountCmd, await client.listAuditLogs());
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
...
rawCmd
  ...
  .action(async (opts) => {
    try {
      ...
      printCommandResult(rawCmd, result);
    } catch (err) {
      error(String(err));
      process.exit(1);
    }
