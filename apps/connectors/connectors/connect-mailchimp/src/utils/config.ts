...
export function getApiKey(): string | undefined {
  return process.env.MAILCHIMP_API_KEY || loadProfile().apiKey;
}
...
export function getServerPrefix(): string | undefined {
  return process.env.MAILCHIMP_SERVER_PREFIX || loadProfile().serverPrefix;
}

export function setServerPrefix(serverPrefix: string): void {
  const config = loadProfile();
  config.serverPrefix = serverPrefix;
  saveProfile(config);
}
