import type { MiroConfig } from '../types';
...
export class MiroClient {
  ...
  constructor(config: MiroConfig) {
    if (!config.token) throw new Error('Miro token is required');
  ...
  async request<T>(path: string, options: { method?: string; body?: Record<string, unknown>; params?: Record<string, string | number | undefined> } = {}): Promise<T> {
    ...
    if (!response.ok) throw new MiroApiError((data as { message?: string })?.message || response.statusText, response.status);
    return data as T;
  }
}
