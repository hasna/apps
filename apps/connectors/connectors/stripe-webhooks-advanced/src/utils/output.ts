...
export function formatOutput(data: unknown, format: OutputFormat = 'pretty'): string {
  const safeData = redactSensitiveFields(data);
...
function redactSensitiveFields(data: unknown, key?: string): unknown {
  if (key && isSensitiveKey(key) && data !== null && data !== undefined && data !== '') {
    return '[REDACTED]';
  }
  ...
  if (data && typeof data === 'object') {
    return Object.fromEntries(
      Object.entries(data as Record<string, unknown>).map(([entryKey, value]) => [
        entryKey,
        redactSensitiveFields(value, entryKey),
      ]),
    );
  }
