...
export class Connector {
  ...
  static fromEnv(): Connector {
    ...
    const accountId = process.env.STRIPE_WEBHOOKS_ADVANCED_ACCOUNT_ID;
    const baseUrl = process.env.STRIPE_WEBHOOKS_ADVANCED_BASE_URL;

    if (!apiKey) {
      throw new Error('STRIPE_WEBHOOKS_ADVANCED_API_KEY environment variable is required');
    }
    return new Connector({ apiKey, apiSecret, accountId, baseUrl });
  }
