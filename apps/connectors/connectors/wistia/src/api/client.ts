...
import { WistiaApiError, parseApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.wistia.com';
...
export class WistiaClient {
  ...
  constructor(config: WistiaConfig) {
    const token = config.apiToken || config.token || config.apiKey;
    if (!token) {
      throw new Error('Wistia API token is required');
    }
    this.apiToken = token;
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries && !(err instanceof WistiaApiError)) {
    ...
    throw lastError || new WistiaApiError('Request failed', lastStatus);
  }
