...
import { TykApiPlatform } from '../api';
import {
  getApiKey,
  setApiKey,
  getBaseUrl,
...
const CONNECTOR_NAME = 'connect-tyk-api-platform';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Tyk API Platform connector - items, events, search, and raw API access')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.TYK_API_PLATFORM_API_KEY = opts.apiKey;
    }
    if (opts.baseUrl) {
      process.env.TYK_API_PLATFORM_BASE_URL = opts.baseUrl;
    }
  });
...
function getClient(): TykApiPlatform {
  const apiKey = getApiKey();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set TYK_API_PLATFORM_API_KEY.`);
    process.exit(1);
  }
  return new TykApiPlatform({ apiKey, baseUrl: getBaseUrl() });
}
...
const itemCmd = program.command('item').description('Item operations');
...
itemCmd
  ...
  .requiredOption('--data <json>', 'Item JSON payload')
  .action(async (opts) => {
    try {
...
program
  .command('search')
  .description('Search the API platform')
