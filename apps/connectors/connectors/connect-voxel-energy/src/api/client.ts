import type { VoxelEnergyConfig } from '../types';
import { VoxelEnergyApiError } from '../types';

export const DEFAULT_BASE_URL = 'https://api.voxelenergy.com/v1';
...
export class VoxelEnergyClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new VoxelEnergyApiError(errorMessage, response.status);
    }
