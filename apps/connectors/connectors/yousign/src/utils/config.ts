...
export function getApiKey(): string | undefined {
  return process.env.YOUSIGN_API_KEY || loadProfile().apiKey;
}

export function getEnvironment(): YousignEnvironment {
  const env = process.env.YOUSIGN_ENVIRONMENT || loadProfile().environment || 'production';
...
export function setEnvironment(environment: YousignEnvironment): void {
  const config = loadProfile();
