...
export function getAgentApiKey(): string | undefined {
  return process.env.TERMINAL_USE_AGENT_API_KEY || loadProfile().agentApiKey;
}

export function setAgentApiKey(agentApiKey: string): void {
  const config = loadProfile();
  config.agentApiKey = agentApiKey;
  saveProfile(config);
}

export function getBaseUrl(): string | undefined {
  return process.env.TERMINAL_USE_BASE_URL || loadProfile().baseUrl;
}
