...
const CONNECTOR_NAME = 'connect-udio';
const VERSION = '0.0.1';
...
program
  .name(CONNECTOR_NAME)
  .description('Udio AI connector CLI - AI music generation with multi-profile support')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.UDIO_API_KEY = opts.apiKey;
    }
  });
...
function getClient(): Udio {
  const apiKey = getApiKey();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set UDIO_API_KEY environment variable.`);
    process.exit(1);
  }
