...
import { UdioApiError } from '../types';

const DEFAULT_BASE_URL = 'https://api.udio.com/v1';
...
export class UdioClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new UdioApiError(errorMessage, response.status);
    }
  ...
  async createGeneration(params: GenerationRequest): Promise<Generation> {
    return this.post<Generation>('/generations', params);
  }

  async extendGeneration(params: ExtendRequest): Promise<Generation> {
    return this.post<Generation>('/generations/extend', params);
  }

  async getGeneration(generationId: string): Promise<Generation> {
    return this.get<Generation>(`/generations/${generationId}`);
  }

  async listGenerations(limit?: number, offset?: number): Promise<GenerationsListResponse> {
    return this.get<GenerationsListResponse>('/generations', { limit, offset });
  }

  async deleteGeneration(generationId: string): Promise<void> {
    await this.delete(`/generations/${generationId}`);
  }
