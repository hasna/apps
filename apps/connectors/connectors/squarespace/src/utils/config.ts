...
export function createProfile(profile: string, config: ProfileConfig = {}): boolean {
  ...
  writeFileSync(getProfilePath(profile), JSON.stringify(config, null, 2), { mode: 0o600 });
  chmodSync(getProfilePath(profile), 0o600);
  return true;
}
...
export function saveProfile(config: ProfileConfig, profile?: string): void {
  ensureConfigDir();
  const profileName = profile || getCurrentProfile();
  writeFileSync(getProfilePath(profileName), JSON.stringify(config, null, 2), { mode: 0o600 });
  chmodSync(getProfilePath(profileName), 0o600);
}

export function getApiKey(): string | undefined {
  return process.env.SQUARESPACE_API_KEY || loadProfile().apiKey;
}
