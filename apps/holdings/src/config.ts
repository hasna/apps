...
export const ENV_PREFIX = "HOLDINGS";
...
const MODE_KEYS = [`HASNA_${ENV_PREFIX}_STORAGE_MODE`, `${ENV_PREFIX}_STORAGE_MODE`] as const;
  ...
  if (mode === "local" && databaseUrlPresent(env)) {
    throw new Error(
      "Misconfiguration: a DATABASE_URL is present but storage mode resolved to 'local'. " +
        ...
        `Set HASNA_${ENV_PREFIX}_STORAGE_MODE=cloud, or unset the DATABASE_URL.`,
    );
  }
  if (mode === "cloud" && !databaseUrlPresent(env)) {
    console.warn(
      `[holdings] cloud mode needs HASNA_${ENV_PREFIX}_DATABASE_URL (or *_FILE); ` +
...
export function resolveDatabaseUrl(env: Env = process.env): string | undefined {
  ...
  if (filePath) {
    try {
      ...
      throw new Error(`Could not read HASNA_${ENV_PREFIX}_DATABASE_URL_FILE at ${filePath}`);
    }
  }
