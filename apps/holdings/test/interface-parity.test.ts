...
const SCOPED_CRED = "parity-scoped-token";
const UNSCOPED_CRED = "parity-unscoped-token";
...
function cliJson<T>(args: string[], token = SCOPED_CRED): T {
...
function cliError(args: string[], token = SCOPED_CRED): ErrEnvelope {
  try {
    cliJson(args, token);
  } catch (error) {
...
async function rest<T>(method: string, path: string, token = SCOPED_CRED): Promise<{ status: number; data: T }> {
...
beforeEach(() => {
  ...
  process.env[CRED_ENV] = JSON.stringify([
    { id: "scoped", token: SCOPED_CRED, roles: ["holdings_manager"], entity_ids: [entityId] },
    { id: "unscoped", token: UNSCOPED_CRED, roles: ["holdings_manager"] },
  ]);

  app = createApp({ db, bindHost: "127.0.0.1", mode: "local" });
  const scopedPrincipal = authenticateBearer(SCOPED_CRED)!;
...
describe("interface parity", () => {
  ...
  it("deny-by-default: an UNSCOPED non-bypass principal is denied on ALL surfaces", async () => {
    ...
    const unscoped = authenticateBearer(UNSCOPED_CRED)!;
    const unscopedHandlers = captureMcpHandlers(db, unscoped);

    const cliErr = cliError(["asset", "get", "--id", assetId], UNSCOPED_CRED);
    expect(cliErr.code).toBe("PERMISSION_DENIED");

    const restErr = await rest<ErrEnvelope>("GET", `/v1/assets/${assetId}`, UNSCOPED_CRED);
