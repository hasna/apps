...
- `hasna.contract.json` description now states the resolver-based data-home resolution (legacy default until adopted; exact-app and db-path overrides) instead of declaring `~/.hasna/holdings/holdings.db` unconditionally authoritative; `storage.sqlitePath` remains the legacy default path.
