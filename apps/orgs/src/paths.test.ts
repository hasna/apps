...
describe("paths resolver wiring (single resolver in @hasna/contracts, ruling #1668)", () => {
  ...
  test("the resolver data root is the contracts resolver root for this machine", () => {
    const home = isolateHome();
    expect(getResolverDataRoot()).toBe(dataDir({ app: "orgs", home, env: process.env }));
  });

  test("on macOS the resolver (and therefore the effective) root is the orgs data root", () => {
    const home = isolateHome();
    const mac = dataDir({ app: "orgs", home, platform: "darwin", env: process.env });
    expect(mac).toBe(join(home, ".hasna", "orgs"));
  ...
  test("on Linux the resolver root is the XDG data root", () => {
    const home = isolateHome();
    expect(dataDir({ app: "orgs", home, platform: "linux", env: process.env })).toBe(
      join(home, ".local", "share", "hasna", "orgs"),
    );
  });

  test("the effective root is the resolver root", () => {
    const home = isolateHome();
    expect(getDataRoot()).toBe(dataDir({ app: "orgs", home, env: process.env }));
  });
  ...
  test("the pre-ruling legacy root is spelled under the orgs data root", () => {
    const home = isolateHome();
...
describe("exact-app overrides and store layering", () => {
  test("HASNA_ORGS_HOME exact override wins over the resolver root", () => {
    isolateHome();
    ...
    expect(getOrgDataDir()).toBe(resolve(override));
  });

  test("OPEN_ORGS_STORE/OPEN_ORGS_AUDIT file overrides win over the effective data root", () => {
    isolateHome();
    const store = join(tmpdir(), "open-orgs-store.db");
    const audit = join(tmpdir(), "open-orgs-audit.jsonl");
    process.env.OPEN_ORGS_STORE = store;
    process.env.OPEN_ORGS_AUDIT = audit;
    expect(getOrgStorePath()).toBe(store);
    expect(getOrgAuditPath()).toBe(audit);
  });
