...
// Resolve the scanner from the pinned `@hasna/contracts` dependency's package
// entry rather than node_modules/.bin: a workspace-linked dependency does not
// mint a .bin symlink, while the dist/cli entry exists for both registry and
// workspace resolution. Never `bunx` — an unpinned package runner makes the
// release gate depend on whatever the registry serves that day.
const scanner = join(repoRoot, "node_modules", "@hasna", "contracts", "dist", "cli", "index.js");
const workspace = mkdtempSync(join(tmpdir(), "orgs-artifact-scan-"));
