...
export function validateResponseEnvelope(method: string, path: string, body: unknown): void {
  ...
  // Only the event GET routes reserve these names. Other resources may use
  // them as IDs/slugs/names, and event writes still return singular entities.
  const eventQuery = resource === "events" && method === "GET" && (id === "search" || id === "conflicts");
  const collection = method === "GET" && (!id || resource === "availability" || eventQuery);
  const key = method === "DELETE" ? "deleted" : eventQuery && id === "conflicts" ? "conflicts" : pair[collection ? 0 : 1];
  ...
  if (resource === "events" && id && !eventQuery && method === "GET" && !action) {
