...
const nativeSubscriptionBinding = {
  owner_ref: owner,
  provider_account_id: uuid7,
  ...
  canonical_node_id: uuid7,
  node_key_thumbprint: digest,
  node_generation: counter,
  placement_generation: counter,
  auth_generation: counter,
  auth_state_revision: counter,
} as const;
const nativeSubscriptionBindingRequired = Object.keys(nativeSubscriptionBinding);
const capsuleMaintenanceCommand = string({
  enum: [
    "SUSPEND_ACCOUNT",
    "RESUME_ACCOUNT",
    "REVOKE_ACCOUNT",
...
const capsuleMaintenanceRequest = object(
  {
    schema_version: constant("accounts.capsule-maintenance-request/v1"),
    ...nativeSubscriptionBinding,
    target_kind: string({ enum: ["account_record", "native_capsule"] }),
    command: capsuleMaintenanceCommand,
    expected_target_revision: counter,
    zero_live_evidence_digest: digest,
    drain_evidence_digest: digest,
    idempotency_key_digest: digest,
  },
  [
    "schema_version",
    ...nativeSubscriptionBindingRequired,
    "target_kind",
    "command",
...
const capsuleMaintenanceGrant = object(
  {
    ...(capsuleMaintenanceRequest.properties as Readonly<Record<string, Schema>>),
    schema_version: constant("accounts.capsule-maintenance/v1"),
    grant_id: uuid7,
    issuer: reference,
    issuer_incarnation: reference,
    key_id: reference,
  ...
  [
    ...(capsuleMaintenanceRequest.required as readonly string[]),
    "grant_id",
    "issuer",
    "issuer_incarnation",
    "key_id",
...
const schemas = {
  ...
  NativeSubscriptionProbeRequest: object(
    {
      schema_version: constant("accounts.native-subscription-probe-request/v1"),
      command: constant("PROBE_NATIVE"),
      ...nativeSubscriptionBinding,
    },
    ["schema_version", "command", ...nativeSubscriptionBindingRequired],
  ),
  NativeSubscriptionProbeResult: object(
    {
      schema_version: constant("accounts.native-subscription-probe-result/v1"),
      capability_eligible: { type: "boolean" },
      maintenance_ready: { type: "boolean" },
      reason_codes: array(reasonCode, { uniqueItems: true }),
      ...nativeSubscriptionBinding,
      account_revision: counter,
      capsule_revision: counter,
      account_status: string({ enum: ["active", "suspended", "revoked"] }),
      subscription_status: string({ enum: ["active", "paused", "expired", "revoked"] }),
      account_lane_status: string({ enum: ["ready", "draining", "disabled", "retired"] }),
      capsule_status: string({ enum: ["ready", "degraded", "maintenance", "quiescing", "revoked"] }),
      live_lease_count: counter,
      drain_state: string({ enum: ["not_started", "draining", "drained"] }),
      zero_live_evidence_digest: digest,
      drain_evidence_digest: digest,
  ...
  CapsuleMaintenanceRequest: capsuleMaintenanceRequest,
  CapsuleMaintenanceGrant: capsuleMaintenanceGrant,
  CapsuleMaintenanceConsumeCommand: object(
    {
      account_lane_id: uuid7,
      grant: capsuleMaintenanceGrant,
      consume_request: object(
        {
          schema_version: constant("accounts.capsule-maintenance-consume-request/v1"),
          grant_id: uuid7,
          operation_id: uuid7,
          request_digest: digest,
          idempotency_key_digest: digest,
        },
        ["schema_version", "grant_id", "operation_id", "request_digest", "idempotency_key_digest"],
      ),
    },
    ["account_lane_id", "grant", "consume_request"],
  ),
  CapsuleMaintenanceConsumeReceipt: object(
    {
      schema_version: constant("accounts.capsule-maintenance-consume-receipt/v1"),
      grant_id: uuid7,
      operation_id: uuid7,
      issuer: reference,
      issuer_incarnation: reference,
      ...
      auth_generation: counter,
      auth_state_revision: counter,
      grant_request_digest: digest,
      consume_request_digest: digest,
      consumed_at: timestamp,
      signature: string({ minLength: 1, maxLength: 512 }),
    },
    [
      "schema_version", "grant_id", "operation_id", "issuer", "issuer_incarnation",
      "key_id", "audience", "target_kind", "command", "owner_ref", "provider_account_id",
      "subscription_id", "account_lane_id", "auth_capsule_id", "canonical_node_id",
      "node_key_thumbprint", "auth_generation", "auth_state_revision", "grant_request_digest",
      "consume_request_digest", "consumed_at", "signature",
    ],
  ),
  CapabilityUseConsumeRequest: object(
    {
      schema_version: constant("accounts.capability-use-consume-request.v1"),
      schema_digest: constant("sha256:a7cdc1dfbebeaea3bad6a5014cfb5189be40fb010f57161b46437458492cd1bc"),
      consume_request_id: uuid7,
      capability_id: uuid7,
      capability_digest: digest,
      nonce: reference,
      subject: owner,
      ...
      resource_lease_id: uuid7,
      resource_id: reference,
      resource_lifecycle_generation: counter,
      operation_id: uuid7,
      operation_digest: digest,
      operation_execution_epoch: counter,
      sender_key_thumbprint: digest,
      channel_binding_digest: digest,
      canonical_request_digest: digest,
      provider_destination_policy_digest: digest,
      online_receipt_id: uuid7,
      online_receipt_digest: digest,
      model_call_anchor_digest: digest,
      expected_use_count: constant("0"),
      max_uses: constant("1"),
      not_after: timestamp,
      idempotency_key_digest: digest,
    },
    [
      ...
      "capability_digest", "nonce", "subject", "actor_principal", "account_lane_id",
      "capacity_pool_id", "capacity_domain_ref", "credential_family_id", "resource_lease_id",
      "resource_id", "resource_lifecycle_generation", "operation_id", "operation_digest",
  ...
  CapabilityUseConsumeResult: {
    oneOf: [
      object(
        {
          status: string({ enum: ["consumed", "replayed"] }),
          signed_receipt: string({ pattern: "^[A-Za-z0-9_-]+$" }),
        },
        ["status", "signed_receipt"],
      ),
      object(
        { status: string({ enum: ["idempotency_conflict", "conflict", "exhausted"] }) },
        ["status"],
      ),
    ],
  },
...
export const ACCOUNTS_CAPACITY_OPENAPI = Object.freeze({
  ...
  paths: {
    ...
    "/internal/v1/native-subscriptions/probe": {
      post: {
        operationId: "probeNativeSubscription",
        description: "Credential- and network-free PROBE_NATIVE. This route cannot reserve or mutate state.",
        security: security("accounts:read"),
        requestBody: jsonBody("NativeSubscriptionProbeRequest"),
        responses: { "200": successJson(ref("NativeSubscriptionProbeResult")), ...errors },
      },
    },
    "/internal/v1/capsule-maintenance/grants": {
      post: {
        operationId: "issueCapsuleMaintenanceGrant",
        description: "Accounts-only one-use maintenance reservation bound to exact zero-live and drain evidence.",
        security: security("accounts:credentials:request"),
        requestBody: jsonBody("CapsuleMaintenanceRequest"),
        responses: { "200": successJson(ref("CapsuleMaintenanceGrant")), ...errors },
      },
    },
    "/internal/v1/capsule-maintenance/consume": {
      post: {
        operationId: "consumeCapsuleMaintenanceGrant",
        description: "Atomically consumes one Accounts-issued maintenance grant.",
        security: security("accounts:credentials:issue"),
        requestBody: jsonBody("CapsuleMaintenanceConsumeCommand"),
        responses: { "200": successJson(ref("CapsuleMaintenanceConsumeReceipt")), ...errors },
      },
    },
    "/internal/v1/capability-uses/consume": {
      post: {
        operationId: "consumeCapabilityUse",
        description: "Accounts-owned atomic one-use capability ordinal consumption.",
        security: security("accounts:generation:check"),
        requestBody: jsonBody("CapabilityUseConsumeRequest"),
        responses: { "200": successJson(ref("CapabilityUseConsumeResult")), ...errors },
      },
    },
