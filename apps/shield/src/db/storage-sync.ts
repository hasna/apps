import { getDb, getDbPath, type SqliteDatabase } from "./database.js";
...
export const SECURITY_STORAGE_TABLES = STORAGE_TABLES;
...
function getSqliteColumns(db: SqliteDatabase, table: string): Set<string> {
...
function upsertSqlite(db: SqliteDatabase, table: string, rows: Row[]): number {
...
export async function getStoragePg(): Promise<PgAdapterAsync> {
  return new PgAdapterAsync(getStorageConnectionString("security"));
}
...
export function getStorageStatus(db: SqliteDatabase = getDb()): StorageStatus {
