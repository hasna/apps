...
const cliPath = join(repoRoot, 'bin', 'personalnotes.mjs');

async function tempRoot(t) {
  const root = await mkdtemp(join(tmpdir(), 'personalnotes-events-'));
...
test('note.created envelope and webhook channel are exact, minimal, and disabled by default', () => {
  ...
  const channel = notesCreatedWebhookChannel({
    ...
    secretRef: 'env:PERSONALNOTES_WEBHOOK_SIGNING',
  });
  ...
  assert.equal(channel.webhook.secretRef, 'env:PERSONALNOTES_WEBHOOK_SIGNING');
