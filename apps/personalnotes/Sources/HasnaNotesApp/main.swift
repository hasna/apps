...
//        - reads the store at launch and injects `window.__BOOT__ = { notes }` as a
...
final class NotesBridge {
    ...
    /// The `{notes}` boot payload as a JSON string.
    ...
    /// Build a `Note` from a JS message payload. New notes (create) get a fresh UUID and
    /// `agent = Note.appAgent`. Saves preserve the id and (for existing notes) the original
    /// createdAt on disk.
    private func note(from dict: [String: Any], isCreate: Bool) -> Note {
        ...
        // Preserve the existing on-disk createdAt when saving an existing note.
...
final class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKScriptMessageHandler, WKUIDelegate {
    ...
    func applicationDidFinishLaunching(_ note: Notification) {
        ...
        NSLog("HasnaNotes: boot payload bytes=\(boot.utf8.count)")
