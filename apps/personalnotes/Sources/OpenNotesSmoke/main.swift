...
do {
    ...
    // Multi-machine is nuked with no backwards compatibility: no machine frontmatter fields.
    check(!raw.contains("machine"), "frontmatter carries no machine fields")
    // Key order: id, title, labels, status, folder, content format, title metadata,
    // createdAt, updatedAt, author, agent.
    ...
    do {
        ...
        // Multi-machine is nuked: a legacy `machine:` frontmatter field is ignored and
        // dropped when the note is re-serialized.
        if let parsed {
            check(!MarkdownStore.serialize(parsed).contains("machine"),
                  "legacy machine field dropped on re-serialize")
        } else {
            check(false, "legacy note parsed")
        }
    }
