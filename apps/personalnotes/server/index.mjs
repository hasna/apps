#!/usr/bin/env bun
// PersonalNotes self-hosted server — entrypoint.
// Zero-ops run: `bun server/index.mjs` (repo) or `bunx personalnotes-server`.
// Defaults: bind 127.0.0.1:8788, SQLite at ~/.hasna/apps/notes-server/server.db.
// Flags: --port <n> --host [addr] --db <path> --auto-approve --dev
// Env:   PERSONALNOTES_SERVER_PORT|PORT, PERSONALNOTES_SERVER_HOST,
//        PERSONALNOTES_SERVER_DB, PERSONALNOTES_SERVER_URL,
//        PERSONALNOTES_SERVER_AUTO_APPROVE=1, PERSONALNOTES_SERVER_DEV=1,
//        PERSONALNOTES_SERVER_JWT_SECRET

import { openDb } from './db.mjs';
import { createApp, resolveConfig, SERVICE, VERSION } from './app.mjs';

if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(`${SERVICE} v${VERSION} — self-hosted PersonalNotes sync server (personalnotes/v1 dialect)

Usage: bun index.mjs [--port <n>] [--host [addr]] [--db <path>] [--auto-approve] [--dev]

  --port <n>       listen port (default 8788; env PERSONALNOTES_SERVER_PORT or PORT)
