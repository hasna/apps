...
| `machine` | SSH/Tailscale failure, host unreachable, machine not found, remote timeout | check `spark01`/`spark02`/`apple03`; durable reachability abstractions belong in `@hasna/stations` |
...
| `routing` | sessions-query/source mapping failure, no live sessions, route mismatch | fix route execution in `@hasna/dispatch`; source/config gaps belong in `@hasna/todos`, `@hasna/configs`, or `@hasna/stations` |
...
- `@hasna/stations`: durable machine inventory, reachability, SSH/Tailscale route
