...
// `HASNA_ACCOUNTS_API_KEY` are set (and mode is not explicitly selected and
// `ACCOUNTS_HOME` is unset), every registry read/write routes to the cloud
// ApiStore. Explicit API modes fail closed when either value is missing; an
// unset mode defaults to local. Both
 ...
 * and no isolated `ACCOUNTS_HOME`), else LocalStore.
 */
