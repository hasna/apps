...
test("api-mode readiness routes profiles + current through the Store, not the local file", async () => {
  ...
  const apiStore: AccountsStore = {
    ...
    listCurrent: async (): Promise<CurrentEntry[]> => [{ tool: "claude", name: "cloudwork", revision: "1" }],
    listCurrentForLoginRollback: async (): Promise<CurrentEntry[]> => [{ tool: "claude", name: "cloudwork", revision: "1" }],
    ...
    restoreProfileState: async () => cloudProfile,
    useProfile: async () => ({ profile: cloudProfile, toolId: "claude", currentRevision: "1" }),
    restoreCurrent: async () => true,
    currentProfile: async () => cloudProfile,
    resolveTool: async () => claudeTool,
