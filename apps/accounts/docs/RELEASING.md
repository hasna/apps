...
| 2026-08-06 08:00 | 0.2.33 | agent `accounts-husk-reviewer` (station01, dispatched by agent-ceo), ship worker for task `04a350a9` | Fifth direct publish while the npm trusted publisher remains unconfigured — the `publish-staged` OIDC boundary is unchanged from the 0.2.29–0.2.32 rows, so the documented override was used rather than making that control decorative. Ships the #123/#124 fix train: the converge allowlist is the UNION of the active and local registries, and the hook's detached `--ensure-fresh` token exchange is OFF by default (`ACCOUNTS_HOOK_ENSURE_FRESH=1` re-enables). Published from a clean detached worktree at origin/main `a2e2d48` (`git status --porcelain` -> 0 lines); pre-publish adversarial review returned GO on range `npm/accounts/v0.2.32`..`a2e2d48`. Gates: `bun install` rc=0, `build` rc=0, full suite 1496 pass / 1 skip / 2 fail — both failures trace to ONE new test (`src/usage-profiles.test.ts` "usage CLI emits versioned backwards-compatible JSON and safe cross-tool human output") timing out at bun's default 5000ms budget under station load ~6.5, plus the isolation meta-test that re-runs the suite and inherits it; the same file at `--timeout 30000` passes 7/7 rc=0, so the budget failed, not the substance. Tag `npm/accounts/v0.2.33` pre-existed pointing at `0b31779` (#119's prepared-but-never-published 0.2.33) and was retargeted to `a2e2d48` and force-pushed so provenance queries name the shipped tree. Rollout verified against installed bytes: `accounts --version` -> 0.2.33, and the installed `dist/cli.js` carries `allowlistProfiles` (2 hits) and the `ACCOUNTS_HOOK_ENSURE_FRESH` default-OFF guard (3 hits). Declared in git-publishing (msg 674113) before acting, confirmed after (msg 674141). | Configure the npm trusted publisher for organization `hasna`, repository `accounts`, workflow `release.yml`, environment `npm-release`, then return the next release to the attested workflow. Give the CLI-spawning test in `src/usage-profiles.test.ts` an explicit timeout measured under load, so the suite's verdict does not depend on station idle. |
...
| 2026-08-08 00:22 | 0.2.40 | agent Silvanus (station01), rows `3fdfd3f6` / `OPE15-00059` | **Published AND promoted by the workflow. `latest` moved to 0.2.40 with no hand on a dist-tag** — and it took TWO ATTEMPTS, which is the point of the row. Run `31229648811` for annotated tag `npm/accounts/v0.2.40` at `5bd2a69c`. **Attempt 1 reproduced the 0.2.39 shape on a DIFFERENT READ PATH.** Step 16 published at `00:22:11.483Z`; step 17 failed 21s later with `release provenance failed: npm install --ignore-scripts --audit=false --fund=false --save-exact @hasna/accounts@0.2.40 failed with exit 1: npm error code ETARGET / npm error notarget No matching version found for @hasna/accounts@0.2.40`. **The attestation check that stranded 0.2.39 PASSED on this run.** So the post-publish visibility lag is a property of the REGISTRY rather than of the attestations endpoint — 0.2.39 lost the race on attestations, 0.2.40 on the install/packument path, and step 17 verifies both. **Read that as evidence against the widen-the-retry candidate**: a larger budget on the attestation fetch alone would not have saved this run. **Attempt 2 is the first real exercise of the #141 resume path and it worked**, verbatim from job `93031844254` at steps 14 and 16: `@hasna/accounts@0.2.40 is already published as this exact candidate under release-candidate-0.2.40 and latest was not promoted from it; resuming`, then `skipping publish: @hasna/accounts@0.2.40 is already the staged candidate`. Both call sites fired. All 20 steps green; step 19 `promoted @hasna/accounts@0.2.40 to monotonic latest`. The pre-fix behaviour in the identical state is measured, not assumed: run `31225016753` attempt 2 failed at step 14 with `already exists; versions are immutable` and steps 15-20 skipped. **0.2.39 was not repaired and could not be** — retagging to a commit carrying the fix rebuilds the tarball with a different `gitHead`, so `verifyDownloadedTarball` hashes different bytes and `resolvePublicationState` correctly refuses; 0.2.39 stays on the registry reachable only as `release-candidate-0.2.39`, superseded by 0.2.40 which carries #134, #136, #137, #138 and #141. **Operational consequence worth planning around: expect one re-run per release of this package until the lag is addressed, and each re-run re-enters the `npm-release` human gate** (approvals recorded in `git-releases` 680703 and 680739; intent `git-publishing` 680686, confirm 680802). Second data point on the lag: `>21s` here, `>24s` on 0.2.39; neither bounded tightly above, so the 50s cap remains unproven in both directions. |
