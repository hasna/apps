...
| 2026-07-30 20:43 | 0.2.27 | ops agent Titus (station01, npm user andreihasna2) | Ships the #97 credential cross-write fix (`eb2603e8`): `rankedCopies` admitted a directory occupied by a *different* account as a credential source, so `switch-account` wrote the outgoing account's credential into central under the incoming account's uuid, and central — gated `() => true` — could fan an already cross-written entry back out over an account's own correct credential. Every `switch-account` run before this could overwrite a good credential with a foreign one. The provisioning gate from the 0.2.24 and 0.2.26 rows was re-verified live rather than assumed: `gh api repos/hasna/accounts/environments/npm-release/secrets` returns `total_count: 0` and `npm view @hasna/accounts@0.2.26 dist.attestations` is empty, so the trusted publisher has still never signed a release. Published from clean commit `4279923b` on branch `chore/9242cc9d-release-0.2.27` (pushed; remote tip == local HEAD; `git status --porcelain` empty before packing) after `typecheck` rc=0 measured alone on a clean tree, `bun test` 1326 pass / 1 skip / 0 fail rc=0, and `build` rc=0. Both gates were proven capable of failing at that tree, not merely observed green: an injected `TS2322` drove typecheck to rc=2, neutering the source-admission gate failed exactly `CROSS-WRITE GATE: an extraDir occupied by ANOTHER account is not an eligible source`, and reverting `dirAccountUuid` to its first-path-only form failed exactly the two `DEFAULT-DIR SOURCE` tests; each was reverted and the sha left unchanged. Declared in git-publishing (msg 610431) before acting. Nothing pushed to `main`: the version-bump PR is left open for the coordinator to merge. | Same as the 0.2.24 and 0.2.26 rows — provision `NPM_DIST_TAG_TOKEN` + `RELEASE_GITHUB_ADMIN_TOKEN` in the `npm-release` environment and configure the npm trusted publisher, then return the next release to the workflow. Existing cross-written central entries are deliberately **not** remediated by this release; deduplicating them is a separate owner-held decision and the authentic copy among them cannot be identified from the store alone. |
