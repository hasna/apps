...
| `accounts usage` | One cached, cross-tool profile view: Claude session/weekly headroom where measured, readiness proxy otherwise, plus last switch, process occupancy, active/applied state, and conservative launchability. `--tool` filters; `--refresh` explicitly refreshes supported provider usage. See [usage-aware switching](docs/usage-aware-switching.md). |
