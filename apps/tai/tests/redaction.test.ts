...
test("redacts common provider keys and bearer tokens", () => {
  const input = "Authorization: Bearer secret-token sk-1234567890abcdef gsk_1234567890abcdef GITHUB_TOKEN=ghp" + "_secret PASSWORD=hunter2";
  ...
  expect(redactSensitiveText(input)).not.toContain("ghp" + "_secret");
...
test("does not over-redact text that carries no credential", () => {
  ...
  // This repo and the sibling SMS redaction package differ here; see docs/redaction.md.
