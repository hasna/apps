...
note cited `the sibling SMS redaction package` as carrying a `~8.4s/50k` quadratic in
...
| URL userinfo — `scheme://user:<tok>@host` | honest gap | **Live.** `tai` has no userinfo rule at all. `the sibling SMS redaction package` redacts this via `URL_USERINFO_PATTERN`; this is a genuine divergence, not a shared gap. |
...
leaked signature is not. `the sibling SMS redaction package` does **not** mask `SignatureVersion`
...
## Divergence from the sibling SMS redaction package
...
- `tai` leaks bare `Bearer <tok>`; the sibling SMS redaction package does not (it has a dedicated rule).
- ~~the sibling SMS redaction package had a serialized-JSON string path that `tai` never had.~~
...
- `tai` does not redact URL userinfo; the sibling SMS redaction package does.
- **The sibling SMS redaction package is NOT a superset of `tai`, and must not be transplanted.**
  Measured 2026-08-01 with a canary carrying no provider prefix: on a plain
  `Authorization: Digest ... response="<tok>"` header, the sibling SMS redaction package returns
  ...
  because `tai` has a dedicated Digest rule that the sibling SMS redaction package lacks. Across a
  12-cell matrix (4 schemes × JSON nesting depth 0/1/2) the sibling SMS redaction package leaked 8 cells
...
- **THE AXIS THAT MAKES THE COMPARISON LIE IS THE CANARY, NOT THE SHAPE**, and
  it is the reason the sibling SMS redaction package has been read as clean here more than once. Varying
  only the canary on one fixed shape (depth-1 JSON Digest):

  | canary | the sibling SMS redaction package | `tai` (before the fix) |
  |---|---|---|
...
- **Both are quadratic, on different inputs — an earlier version of this file
  ...
  and fast (0.6ms) while the sibling SMS redaction package is quadratic (2.9s, `URL_USERINFO_PATTERN`).
