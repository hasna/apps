      ...
      if (method === "POST" && ex) {
        if (ex[1] === "f_malformed") {
          return Response.json({
            ...
            source_ref: "not-an-open-files-ref",
          });
        }
...
describe("ported read-side MCP tools on the hosted (api) transport", () => {
  ...
  test("extract_file_snapshot refuses mismatched and malformed hosted extraction identities", async () => {
    const { client, close } = await connectedClient();
    try {
      const mismatched = await client.callTool({
        name: "extract_file_snapshot",
        arguments: { id: "f_requested" },
      });
      expect(mismatched.isError).toBe(true);
      expect(callText(mismatched)).toContain("did not match the requested file id");

      const malformed = await client.callTool({
        name: "extract_file_snapshot",
        arguments: { id: "f_malformed" },
      });
      expect(malformed.isError).toBe(true);
      expect(callText(malformed)).toMatch(/Invalid open-files|source revision identity/);
      expect(fake.hits.filter((hit) => hit.path.endsWith("/extract-text")).map((hit) => hit.path)).toEqual([
        "/files/f_requested/extract-text",
        "/files/f_malformed/extract-text",
      ]);
    } finally {
      await close();
    }
