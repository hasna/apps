...
export const STORAGE_CONFIG_PATH = join(homedir(), ".hasna", "files", "storage", "config.json");
...
export function getStorageDatabaseUrlEnvName(): string | undefined {
  if (process.env["HASNA_FILES_DATABASE_URL"]) return "HASNA_FILES_DATABASE_URL";
  if (process.env["FILES_DATABASE_URL"]) return "FILES_DATABASE_URL";
  return undefined;
}

function envConnectionString(): string | undefined {
  return process.env["HASNA_FILES_DATABASE_URL"] ?? process.env["FILES_DATABASE_URL"];
}
...
export function getStorageConfig(): StorageConfig {
  const config: StorageConfig = {
    ...
    rds: {
      ...
      port: 5432,
      username: "",
      password_env: "FILES_DATABASE_PASSWORD",
      ssl: true,
    },
  };
  ...
  const modeOverride = process.env["HASNA_FILES_STORAGE_MODE"] ?? process.env["FILES_STORAGE_MODE"];
...
export function getStorageConnectionString(dbName = "files"): string {
  ...
  if (!host || !username) {
    throw new Error("Storage database is not configured. Set HASNA_FILES_DATABASE_URL or configure ~/.hasna/files/storage/config.json.");
  }
