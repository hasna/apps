...
beforeEach(() => {
  ...
  server = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      if (req.method === "POST" && path === "/files/f_requested/extract-text") {
...
describe("files knowledge resolve on the hosted transport", () => {
  ...
  test("refuses hosted revision refs instead of labeling current bytes as an old revision", async () => {
    const result = await runCli([
      "knowledge", "resolve", "open-files://file/f_know1/revision/rev_old", "--mode", "content", "--json",
    ]);
    expect(result.exitCode).toBe(0);
    const payload = JSON.parse(result.stdout) as { status: string; status_reason: string };
    expect(payload.status).toBe("unsupported");
    expect(payload.status_reason).toContain("refusing to substitute current file bytes");
    expect(hits).toEqual([]);
    expect(databaseFilesUnder(testDir)).toEqual([]);
  });

  test("applies MIME policy before requesting a hosted signed URL", async () => {
    const result = await runCli([
      "knowledge", "resolve", "open-files://file/f_know1", "--mode", "signed_url", "--mime", "application/pdf", "--json",
    ]);
    expect(result.exitCode).toBe(0);
    const payload = JSON.parse(result.stdout) as { status: string };
    expect(payload.status).toBe("unsupported");
...
describe("files knowledge doctor on the hosted transport", () => {
  test("collects refs from GET /v1/files and reports each check", async () => {
    ...
    // Hosted readiness is based on an actual bounded extraction, not MIME inference.
    expect(hits).toEqual(["GET /files", "GET /files/f_know1", "POST /files/f_know1/extract-text"]);
    expect(databaseFilesUnder(testDir)).toEqual([]);
  });
...
describe("files extract-snapshot on the hosted transport", () => {
  ...
  test("refuses a hosted snapshot whose extraction belongs to another file", async () => {
    const result = await runCli(["extract-snapshot", "f_requested", "--json"]);

    expect(result.exitCode).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("did not match the requested file id");
    expect(hits).toEqual(["POST /files/f_requested/extract-text"]);
    expect(databaseFilesUnder(testDir)).toEqual([]);
  });
