 ...
 *
 * A fleet CLI must NEVER silently fall back to the on-box SQLite store when
 * the hosted API env is missing: no `~/.hasna/files/files.db` on first use, no
 * false-green local session, no local mode as a default. Local mode is reachable
 * only through the explicit opt-in (HASNA_FILES_LOCAL_MODE=1 / FILES_LOCAL_MODE=1).
 */
...
/** Spawn env with the hosted keys AND the local opt-in stripped. */
function unconfiguredEnv(dataDir: string): NodeJS.ProcessEnv {
  ...
  return { ...env, HASNA_FILES_DATA_DIR: dataDir, HASNA_FILES_DB_PATH: join(dataDir, "files.db") };
}

describe("files CLI transport gate (fail closed)", () => {
  test("refuses to run a data-plane command without hosted env or a local opt-in", async () => {
  ...
  test("serves the local store when the explicit opt-in is set (alias form)", async () => {
    const dataDir = makeDataDir();
    const env = unconfiguredEnv(dataDir);
    env.FILES_LOCAL_MODE = "1";
