...
The server selects Postgres when `HASNA_<NAME>_DATABASE_URL` is set and
SQLite otherwise. This kit contains **no sync engine and no merge logic**;
the pool is only ever built for the Postgres server backend.
