...
export async function resolveKnowledgeSourceRefViaApi(
    ...
    if (parsed.kind === "file") {
      ...
      if (requestedRevisionId) {
        return bareResolution(
          sourceRef,
          "unsupported",
          "Hosted revision refs require an exact revision-aware byte route; refusing to substitute current file bytes.",
          purpose,
          allowedPurposes,
          mode,
        );
      ...
      if (file.id !== parsed.file_id) {
        return bareResolution(sourceRef, "error", "Hosted file response did not match the requested file id.", purpose, allowedPurposes, mode);
      }
    } else {
      ...
      if (file.source_id !== parsed.source_id || file.path !== parsed.path) {
        return bareResolution(sourceRef, "error", "Hosted file response did not match the requested source path.", purpose, allowedPurposes, mode);
      }
    }
  } catch (error) {
  ...
  try {
    if (mode !== "metadata" && !isKnowledgeMimeAllowed(base.content.mime, base.name ?? "", opts)) {
      return { ...base, status: "unsupported", status_reason: `MIME type is not allowed for ${mode}: ${base.content.mime}` };
    }
    switch (mode) {
      case "metadata":
        return base;
...
async function resolveExtractedTextViaApi(
  ...
  assertHostedExtractionIdentity(extraction, base.file_id!);
  const status = mapExtractionStatus(extraction);
  const resolvedBase = extraction.revision_id && extraction.source_ref
    ? { ...base, source_ref: extraction.source_ref, revision_id: extraction.revision_id }
    : base;
  const content: KnowledgeSourceResolution["content"] = {
    ...
    text_available: extraction.status === "ready" || extraction.status === "too_large" || extraction.status === "empty",
    bytes_read: extraction.bytes_read,
    truncated: extraction.truncated,
    extracted_text_ref: extraction.source_ref ? `${extraction.source_ref}/text` : base.content.extracted_text_ref,
    extraction: {
  ...
  if (!asSnapshot) {
    return { ...resolvedBase, status, status_reason: extraction.status_reason, content, extracted_text: extraction };
  }
...
export async function doctorKnowledgeSourcesViaApi(
  ...
  const checkExtractedText = requireExtractedText || (opts.check_extracted_text ?? false);
...
async function doctorRefViaApi(
  ...
  // Hosted metadata proves MIME capability, not that extraction bytes exist. A
  // hosted check may report ready only after an actual bounded extraction.
  if (opts.require_extracted_text && opts.check_extracted_text && resolvable) {
    ...
    try {
      ...
      assertHostedExtractionIdentity(extraction, resolution.file_id!);
      if (!["ready", "too_large", "empty"].includes(extraction.status)) addIssue(issueCodes, "missing_extracted_text");
    } catch {
...
export function assertHostedExtractionIdentity(extraction: import("../types/index.js").ExtractedTextResult, fileId: string): void {
  if (extraction.file_id !== fileId) throw new Error("Hosted extraction response did not match the requested file id.");
  let parsed: ReturnType<typeof parseOpenFilesSourceRef>;
  try {
    parsed = parseOpenFilesSourceRef(extraction.source_ref);
  } catch {
    throw new Error("Hosted extraction response carried inconsistent source revision identity.");
  }
  if (parsed.kind !== "file" || parsed.file_id !== fileId || parsed.revision_id !== extraction.revision_id) {
    throw new Error("Hosted extraction response carried inconsistent source revision identity.");
  }
}
