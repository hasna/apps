...
export const openApiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      KnowledgeManifestFilters: {
        type: "object",
        additionalProperties: false,
        properties: {
          source_id: { type: "string", minLength: 1, pattern: "\\S" },
          collection_id: { type: "string", minLength: 1, pattern: "\\S" },
          project_id: { type: "string", minLength: 1, pattern: "\\S" },
          tag: { type: "string", minLength: 1, pattern: "\\S" },
          status: { type: "string", enum: ["active", "deleted", "moved", "all"] },
          delta: { type: "boolean" },
          after: { type: "string", pattern: "^(?:\\d{4}-\\d{2}-\\d{2}|\\d{4}-\\d{2}-\\d{2}T(?:[01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d(?:\\.\\d+)?(?:Z|[+-](?:[01]\\d|2[0-3]):[0-5]\\d))$" },
          before: { type: "string", pattern: "^(?:\\d{4}-\\d{2}-\\d{2}|\\d{4}-\\d{2}-\\d{2}T(?:[01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d(?:\\.\\d+)?(?:Z|[+-](?:[01]\\d|2[0-3]):[0-5]\\d))$" },
        },
        required: ["status", "delta"],
      },
      KnowledgeManifestRoot: {
        type: "object",
        additionalProperties: false,
        properties: {
          open_files_root: { type: "string", minLength: 1, pattern: "\\S" },
          source_id: { type: "string", minLength: 1, pattern: "\\S" },
          source_type: { type: "string", enum: ["local", "s3", "google_drive"] },
          evidence_hash: { type: "string", pattern: "^sha256:[a-f0-9]{64}$" },
        },
        required: ["open_files_root", "source_id", "source_type", "evidence_hash"],
      },
      KnowledgeManifestStorage: {
        type: "object",
        additionalProperties: false,
        properties: {
      ...
      KnowledgeManifestReadableExtraction: {
        ...
        properties: {
          ...
          extracted_text_ref: { type: "string", minLength: 1, pattern: "\\S" },
        },
      ...
      KnowledgeManifestUnavailableExtraction: {
        ...
        properties: {
          ...
          status_reason: { type: "string", minLength: 1, pattern: "\\S" },
        },
      ...
      KnowledgeManifestFile: {
        ...
        properties: {
          ...
          source_ref: { type: "string", minLength: 1, pattern: "\\S" },
          revision_ref: { type: "string", minLength: 1, pattern: "\\S" },
          revision_id: { type: "string", minLength: 1, pattern: "\\S" },
          change_cursor: { type: "string", pattern: "^(0|[1-9][0-9]*)$" },
          source_revision_hash: { type: "string", pattern: "^sha256:[a-f0-9]{64}$" },
          file_id: { type: "string", minLength: 1, pattern: "\\S" },
          source_id: { type: "string", minLength: 1, pattern: "\\S" },
          source_type: { type: "string", enum: ["local", "s3", "google_drive"] },
          name: { type: "string" },
          mime: { type: "string" },
          size: { type: "integer", minimum: 0 },
          hash: { type: "string", minLength: 1, pattern: "\\S" },
          status: { type: "string", enum: ["active", "deleted", "moved"] },
          updated_at: { type: "string", format: "date-time" },
          deleted: { type: "boolean" },
          tombstone: { type: "boolean" },
          tags: { type: "array", items: { type: "string" } },
          open_files_root: ref("KnowledgeManifestRoot"),
          storage: ref("KnowledgeManifestStorage"),
          extraction: ref("KnowledgeManifestExtraction"),
          permissions: {
            type: "object",
            additionalProperties: false,
            properties: {
            ...
            required: ["mode", "allowed_purposes"],
          },
          permission_labels: { type: "array", items: { type: "string" } },
        },
        required: [
          "kind", "source_ref", "change_cursor", "source_revision_hash", "file_id", "source_id",
          "source_type", "name", "mime", "size", "status", "updated_at", "deleted", "tags",
          "open_files_root", "storage", "extraction", "permissions", "permission_labels",
        ],
      },
      KnowledgeManifest: {
        type: "object",
        additionalProperties: false,
        properties: {
          filter_contract: { type: "string", enum: ["files.knowledge.manifest.v1"] },
          cursor_contract: { type: "string", enum: ["files.knowledge.manifest.change.v1"] },
          manifest_id: { type: "string", minLength: 1, pattern: "\\S" },
          generated_at: { type: "string", format: "date-time" },
          format: { type: "string", enum: ["json", "jsonl"] },
          filters: ref("KnowledgeManifestFilters"),
          item_count: { type: "integer", minimum: 0 },
          cursor: { type: "string", minLength: 1, pattern: "\\S" },
          next_cursor: { type: "string", minLength: 1, pattern: "\\S" },
          has_more: { type: "boolean" },
          complete: { type: "boolean" },
          delta: { type: "boolean" },
          high_watermark: { type: "string", pattern: "^(0|[1-9][0-9]*)$" },
          delta_cursor: { type: "string", minLength: 1, pattern: "\\S" },
          tombstone_count: { type: "integer", minimum: 0 },
          items: { type: "array", items: ref("KnowledgeManifestFile") },
        },
        required: [
          "filter_contract", "cursor_contract", "manifest_id", "generated_at", "format", "filters",
          "item_count", "has_more", "complete", "delta", "high_watermark", "delta_cursor",
          "tombstone_count", "items",
        ],
      },
      KnowledgeManifestError: {
        ...
        properties: {
          ...
          reason: {
            type: "string",
            enum: [
              "invalid_manifest_cursor", "invalid_manifest_query", "unknown_manifest_query",
              "legacy_sync_version_unavailable", "acl_summary_unavailable",
              "evidence_assets_unavailable", "filtered_delta_unavailable", "tenant_binding_missing", "manifest_store_incompatible",
            ],
          },
        },
  ...
  paths: {
    ...
    "/knowledge/manifest": {
      get: {
        ...
        summary: "Export a tenant-bound, immutable knowledge-source manifest page",
        description:
          "Returns files from a signed, globally monotonic change cursor. The configured Files app base omits /v1; the client appends it exactly once. "
          + "Cursors are signed, tenant-bound, and query-bound. Hosted manifests never disclose station identity, local paths, or object-store coordinates. "
          + "include_acl_summary and include_evidence_assets are explicit refusals; since_sync_version is a refused legacy local cursor. "
          + "Delta reads are unfiltered until membership-exit tombstones are versioned.",
        parameters: [
          { name: "source_id", in: "query", schema: { type: "string", minLength: 1, pattern: "\\S" } },
          { name: "collection_id", in: "query", schema: { type: "string", minLength: 1, pattern: "\\S" } },
          { name: "project_id", in: "query", schema: { type: "string", minLength: 1, pattern: "\\S" } },
          { name: "tag", in: "query", schema: { type: "string", minLength: 1, pattern: "\\S" } },
          ...
          { name: "since_cursor", in: "query", schema: { type: "string", minLength: 1, maxLength: 2048 }, description: "Signed checkpoint cursor from a previous hosted manifest." },
          { name: "since_sync_version", in: "query", deprecated: true, schema: { type: "integer", minimum: 0 }, description: "Always refused on hosted manifests; use since_cursor." },
          { name: "after", in: "query", schema: { type: "string", pattern: "^(?:\\d{4}-\\d{2}-\\d{2}|\\d{4}-\\d{2}-\\d{2}T(?:[01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d(?:\\.\\d+)?(?:Z|[+-](?:[01]\\d|2[0-3]):[0-5]\\d))$" } },
          { name: "before", in: "query", schema: { type: "string", pattern: "^(?:\\d{4}-\\d{2}-\\d{2}|\\d{4}-\\d{2}-\\d{2}T(?:[01]\\d|2[0-3]):[0-5]\\d:[0-5]\\d(?:\\.\\d+)?(?:Z|[+-](?:[01]\\d|2[0-3]):[0-5]\\d))$" } },
          { name: "cursor", in: "query", schema: { type: "string", minLength: 1, maxLength: 2048 }, description: "Signed continuation cursor. Cannot be combined with since_cursor." },
          { name: "limit", in: "query", schema: { type: "integer", minimum: 1, maximum: 1000, default: 100 } },
          { name: "format", in: "query", schema: { type: "string", enum: ["json", "jsonl"], default: "json" }, description: "Requested client rendering; HTTP response remains the typed JSON envelope." },
          { name: "include_acl_summary", in: "query", schema: { type: "boolean" }, description: "true is refused with acl_summary_unavailable." },
          { name: "include_evidence_assets", in: "query", schema: { type: "boolean" }, description: "true is refused with evidence_assets_unavailable." },
        ],
        responses: {
          "200": ok(ref("KnowledgeManifest")),
          "400": ok(ref("KnowledgeManifestError")),
          "403": ok(ref("KnowledgeManifestError")),
          "500": ok(ref("KnowledgeManifestError")),
        },
      },
    },
