      ...
      try {
        if (path === "/evidence/assets" && method === "GET") {
          return json(listFileAssets({
            ...
            provenance_type: url.searchParams.get("provenance_type") ?? undefined,
            provenance_id: url.searchParams.get("provenance_id") ?? undefined,
            provenance_ref: url.searchParams.get("provenance_ref") ?? undefined,
            version: url.searchParams.has("version") ? Number(url.searchParams.get("version")) : undefined,
            classification: url.searchParams.get("classification") ?? undefined,
            retention_policy: url.searchParams.get("retention_policy") ?? undefined,
        ...
        if (path === "/evidence/upload-intents" && method === "POST") {
          ...
          const result = await createEvidenceUploadIntent({
            ...
            provenance_type: optionalString(body.provenance_type),
            provenance_id: optionalString(body.provenance_id),
            provenance_ref: optionalString(body.provenance_ref),
            external_references: Array.isArray(body.external_references)
              ? body.external_references.filter((value): value is string => typeof value === "string")
              : undefined,
