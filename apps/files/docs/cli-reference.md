...
Data-root and home overrides must be absolute; relative or blank values are
ignored rather than resolved against the current working directory.

If explicit local startup finds data at a retired XDG/macOS root but no
database at the implicit canonical root, it exits with
`FILES_STRANDED_XDG_DATA` before creating anything. Move the complete data
directory only after stopping Files processes and backing up both roots. The
non-moving recovery is to set `HASNA_FILES_DATA_DIR` to the retained root. An
offline migration must preserve `files.db`, WAL/SHM state, configuration,
permissions, and the other root contents; the CLI never moves a possibly-live
