...
export function normalizeMcpAuditLimit(limit: number | undefined, full: boolean): number {
  return Math.max(1, Math.min(MCP_LIST_MAX, Math.trunc(limit ?? (full ? 50 : MCP_LIST_DEFAULT))));
}

function mcpPage<T, U>(
  items: T[],
  ...
  return { items: selected.map(map), count: selected.length, total: items.length, limit, cursor, next_cursor: nextCursor, has_more: nextCursor !== null, compact: true, ...(sort ? { sort } : {}), hint };
}
...
export function buildServer(): McpServer {
  ...
  server.tool(
    ...
    async ({ kind, limit, cursor, full }) => {
      ...
      return jsonContent(mcpPage(entries, { limit, cursor }, (entry) => ({ id: entry.id, kind: entry.kind, title: entry.title, subtitle: entry.subtitle ?? null, domain_count: entry.domains.length, tag_count: entry.tags.length, favorite: entry.favorite, updated_at: entry.updated_at }), "Page with cursor; use full=true only for the legacy complete metadata array.", { fields: ["favorite", "title", "id"], directions: ["desc", "asc", "asc"] }));
    }
  );
  ...
  server.tool(
    ...
    async ({ key, limit, cursor, full }) => {
      const effectiveLimit = normalizeMcpAuditLimit(limit, full === true);
      ...
      return jsonContent({ items: pageItems, count: pageItems.length, limit: effectiveLimit, cursor: effectiveCursor, next_cursor: hasMore ? effectiveCursor + pageItems.length : null, has_more: hasMore, compact: true, sort: { fields: ["timestamp", "id"], directions: ["desc", "desc"] }, hint: "Page with cursor; use full=true for the legacy text listing." });
    }
  );
  ...
  server.tool(
    ...
    async ({ type, limit, cursor, full }) => {
      ...
      return jsonContent(mcpPage(users, { limit, cursor }, (user) => ({ id: user.id, name: user.name, type: user.type, last_seen: user.last_seen ?? null }), "Page with cursor; use full=true only for the legacy complete text listing.", { fields: ["type", "name", "id"], directions: ["asc", "asc", "asc"] }));
    }
  );
