...
//
// This is the piece that makes the HOSTED client real. It sits on top of
...
//
// An app's storage resolver selects this client when the client env contract
// resolves to `hosted-http` (API_URL AND API_KEY both set), and falls through
// to the local store otherwise. See `resolveClientTransport` /
...
export type ResolveStorageClientResult =
  ...
  | { transport: "hosted-http"; client: HasnaStorageClient };

/**
 * The one call an app's storage resolver makes. Reads the client env contract
 * for `name`; when it resolves to `hosted-http` (API_URL + API_KEY both set),
 * returns a ready {@link HasnaStorageClient}. Otherwise returns
 * `{ transport: 'local', client: null }` so the app uses its local store.
 * Throws if the hosted config is partial (so callers never silently read the
 * wrong dataset).
 */
export function resolveStorageClient(
  ...
  if (wired.transport === "hosted-http") {
    return { transport: "hosted-http", client: createHasnaStorageClient(name, wired.client) };
  }
