// The @hasna/contracts client types that @hasna/secrets PUBLISHES.
//
...
// the source wrote. When #1720 deleted the vendored copy in
// `src/store/contracts-client/` it moved `import ... from "@hasna/contracts/client"`
// onto the emitted public type entry (`dist/sdk.d.ts` -> `dist/sdk/client.d.ts`
// -> `dist/store/client.d.ts`), and a TS consumer with only this package's
// runtime dependencies installed got 7 x TS2307 "Cannot find module
// '@hasna/contracts/client'". `./sdk` is a documented public surface whose own
// requirement is that it be self-contained, so the fix is to publish the types
// rather than to publish a 20 MB build-time dependency to every consumer or to
// re-open the `secrets -> contracts -> peer @hasna/secrets` cycle.
//
// WHAT THIS IS NOT. It is NOT the vendored resolver that #1720 removed. There
...
// assignments at the seam in `./client.ts` — a shape that drifts fails the
// build, it does not silently publish a lie.
//
...
/**
 * Options `@hasna/secrets` forwards to the shared resolver: tier-1 credential
 * inputs and the Keychain-tier seam.
 *
 * Published, not internal: it is the second parameter of `getStore()` /
 * `getStoreWithResolution()` on the `./storage` export.
 */
