...
export class ApiStore implements Store {
  ...
  async pruneExpired(): Promise<number> {
    // Expiry is metadata, enforced by the server on write (ttl parsing) but not
    // swept for the tenant on read, so the hosted pruning lane must delete the
    // lapsed rows itself — the same sweep LocalStore performs, via the API.
    // Values are never fetched (metadata-only listing).
    const now = Date.now();
    const expired = (await this.listSecretMetadata()).filter(
      (meta) => meta.expires_at !== undefined && meta.expires_at !== null && Date.parse(meta.expires_at) < now,
    );
    let pruned = 0;
    for (const meta of expired) {
      if (await this.deleteSecret(meta.key)) pruned++;
    }
    return pruned;
  }
