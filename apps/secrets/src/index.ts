...
function compactListEnvelope<T, U>(items: T[], flags: Record<string, string>, map: (item: T) => U, noun: string) {
...
switch (command) {
  ...
  case "audit": {
    ...
    if ("json" in flags) {
      const payload = flags.full === "true" ? selected : { items: selected, count: selected.length, limit: requestedLimit, cursor, next_cursor: hasMore ? cursor + selected.length : null, has_more: hasMore, compact: true, hint: "Continue with --cursor; pass --full for the legacy audit array." };
  ...
  case "users": {
    ...
    switch (sub) {
      case "list": {
        ...
        const page = compactListEnvelope(users, flags, (user) => ({ id: user.id, name: user.name, type: user.type, last_seen: user.last_seen ?? null }), "user");
        if ("json" in flags) {
  ...
  case "items": {
    ...
    switch (sub) {
      case "list": {
        ...
        const page = compactListEnvelope(items, flags, (item) => ({ id: item.id, kind: item.kind, title: item.title, subtitle: item.subtitle ?? null, domain_count: item.domains.length, tag_count: item.tags.length, favorite: item.favorite, updated_at: item.updated_at }), "vault item");
        if ("json" in flags) {
