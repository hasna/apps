...
export const REQUIRED_SUITES = Object.freeze({
  "tests/encryption-maintenance.pg.test.ts": 1,
  "tests/lossless-migration.pg.test.ts": 1,
  "tests/prune-expired.pg.test.ts": 1,
});

export function validateTestDatabaseUrl(value: string | undefined): string {
  const refuse = () => { throw new Error("SECRETS_TEST_DATABASE_URL must name the disposable secrets_test user/database on literal 127.0.0.1 with an explicit port and no URL options"); };
  if (!value || /[\s\x00-\x1f\x7f?#]/.test(value)) return refuse();
  let url: URL;
  try { url = new URL(value); } catch { return refuse(); }
  if (!["postgres:", "postgresql:"].includes(url.protocol)
    || url.hostname !== "127.0.0.1" || url.username !== "secrets_test"
    || !["", "secrets_test"].includes(url.password) || url.pathname !== "/secrets_test"
...
export function buildTestEnv(source: NodeJS.ProcessEnv, home: string): Record<string, string> {
  return {
    ...
    SECRETS_TEST_DATABASE_URL: validateTestDatabaseUrl(source.SECRETS_TEST_DATABASE_URL),
  };
}
...
export function discoverSuites(root: string): string[] {
  ...
  const walk = (directory: string) => {
    for (const item of readdirSync(directory, { withFileTypes: true })) {
      ...
      else if (item.name.endsWith(".test.ts") && readFileSync(path, "utf8").includes("process.env.SECRETS_TEST_DATABASE_URL")) paths.push(relative(root, path));
    }
  };
...
export function assertSuiteInventory(actual: string[]): void {
  if (JSON.stringify([...actual].sort()) !== JSON.stringify(Object.keys(REQUIRED_SUITES).sort())) {
    throw new Error("Required Secrets PostgreSQL suite inventory changed; update and review the executed-case census");
  }
}
...
export async function assertFixtureDatabase(value: string): Promise<void> {
  const pool = new Pool({ connectionString: validateTestDatabaseUrl(value), max: 1, connectionTimeoutMillis: 5000 });
  try {
    const { rows } = await pool.query("SELECT current_user AS username,current_database() AS database,rolsuper FROM pg_roles WHERE rolname=current_user");
    const row = rows[0];
    if (!row || row.username !== "secrets_test" || row.database !== "secrets_test" || !row.rolsuper) throw new Error("fixture identity");
  } catch { throw new Error("Secrets PostgreSQL gate requires the isolated secrets_test fixture database and its administrative fixture role"); }
  finally { await pool.end(); }
}

export async function runLivePostgresTests(source = process.env): Promise<void> {
  // Never substitute a production DSN, saved credential, profile, or ambient PG*.
  await assertFixtureDatabase(validateTestDatabaseUrl(source.SECRETS_TEST_DATABASE_URL));
  const root = resolve(import.meta.dir, "..");
  assertSuiteInventory(discoverSuites(root));
  const home = mkdtempSync(join(tmpdir(), "secrets-postgres-gate-"));
  let failed = false;
  try {
    ...
    if (failed) throw new Error("Secrets PostgreSQL gate failed: every required case must execute without skips");
    console.log(`[postgres-gate] PASS: ${Object.keys(REQUIRED_SUITES).length} required suites, zero skips`);
  } finally { rmSync(home, { recursive: true, force: true }); }
}

if (import.meta.main) {
  try { await runLivePostgresTests(); } catch (error) {
    console.error(error instanceof Error ? error.message : "Secrets PostgreSQL gate failed");
    process.exitCode = 1;
  }
}
