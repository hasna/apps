...
//
// This app's explicit mode policy (not libpq opportunistic negotiation):
//   - disable / prefer / allow / absent -> no TLS (explicit ssl: false in pool)
//   - require                   -> encrypt, do NOT verify the server cert
//                                  (rejectUnauthorized: false) — this matches
//                                  what libpq `require` means and is the AWS
//                                  RDS default when no CA bundle is supplied.
//   - verify-ca / verify-full   -> verify BOTH certificate and hostname against a CA
...
export function sslModeFromConnectionString(connectionString: string): SslMode {
  ...
  for (const name of params.keys()) {
    if (/^ssl/i.test(name) || name.toLowerCase() === "uselibpqcompat") {
      if (!["sslmode", "ssl"].includes(name) || params.getAll(name).length !== 1) {
        throw new Error("Unsupported or duplicate PostgreSQL TLS parameter.");
      }
    }
  }
  if (params.has("sslmode") && params.has("ssl")) throw new Error("Conflicting PostgreSQL TLS parameters.");
  if ((params.has("sslmode") && !params.get("sslmode")?.trim()) || (params.has("ssl") && !params.get("ssl")?.trim())) {
    throw new Error("Blank PostgreSQL TLS parameter.");
  }
  ...
  if (ssl && !["0", "false", "no", "off"].includes(ssl)) throw new Error("Unsupported PostgreSQL TLS boolean.");
...
export function resolveTlsConfig(
  ...
  if (mode === "disable" || mode === "prefer") {
    // No opportunistic negotiation is implemented. The pool explicitly supplies
    // false so pg cannot inherit a different policy from PGSSLMODE.
    return undefined;
  }
