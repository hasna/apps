...
describe("ApiStore", () => {
  ...
  test("rejects email-gated options client-side (local-mode-only feature)", () => {
    // The guard fires synchronously in the non-async ApiStore method, before
    // any network call — fail-fast, so no request can leak a local-only option.
    const store = new ApiStore(makeFakeV1());
    expect(() => store.uploadFile("/tmp/a.txt", { requireEmail: true })).toThrow(
      /email-gated links.*only available in local mode/,
    );
    expect(() => store.uploadFile("/tmp/a.txt", { allowedEmails: ["a@b.c"] })).toThrow(
      /email-gated links.*only available in local mode/,
    );
  });

  test("rejects custom baseUrl options client-side", () => {
    const store = new ApiStore(makeFakeV1());
    // Both ApiStore upload methods are non-async wrappers, so the guard throws
    // synchronously — fail-fast before any network call.
    expect(() => store.uploadFile("/tmp/a.txt", { baseUrl: "http://x:3459" })).toThrow(
      /internal.*only available in local mode/,
    );
    expect(() =>
      store.uploadStream({} as NodeJS.ReadableStream, "a.txt", undefined, { baseUrl: "http://x" }),
    ).toThrow(/internal.*only available in local mode/);
  });
