...
- 257280d: Resolve credentials through the `@hasna/contracts` client chain (hasna/apps#1720).
  ...
  - The app's own env chain in `src/core/client-config.ts`: the hand-rolled
    ...
    client database-URL and DB*PATH rejections, and the DEPRECATED legacy
    ...
    key store. The unprefixed `ATTACHMENTS*\*` spellings survive only as the
...
- f17669f: Resolver validation fixes (#1720): a credential-chain refusal from any data
...
- bb6eb74: `attachments link <id> --regenerate` returns HTTP 500 "Could not load credentials from any providers" when the host's only working AWS route is a named profile (e.g. `hasna-xyz-infra`): the S3 client fell back to the SDK default credential chain, which cannot select a non-default profile, while `aws s3 presign --profile <name>` works.
  ...
  - **Deploy dependency:** the server-side HTTP 500 clears only after the deployment supplies the credentials — set `ATTACHMENTS_S3_PROFILE` (or a task role the SDK default chain can assume) on the `attachments-serve` deployment, with the named profile present in the deployment's shared credentials/config files. Deploy-side credential remediation is tracked separately as task 33be4780.
...
- 2b87a81: Hermeticize the attachments test suites (21a04472): tests stash ambient API/todos keys, the server test harness is split out of the test file, and the MCP server test is isolated. Test-only change — no runtime code delta in this release.
