...
# the hosted API.
#
# The client backend is selected by the API URL + key pair (see
# core/cloud-v1.ts:resolveStorageClient), so with those exported the CLI/MCP tests
# silently ran against the real service and 8 test files failed for environmental
