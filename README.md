...
.github/workflows/     CI: ci.yml (gates, test-suites, build-test, verify-generated, publish-guard) + release-app.yml (the generic OIDC npm release lane; supersedes the per-package release.yml / release-todos.yml shapes once every member is bound — they are not yet decommissioned), deploy-projects.yml, deploy-skills.yml, deploy-todos.yml, recordings-macos.yml, blacksmith-testbox.yml
```
